...
| 2026-06-15 | **round-20 배치 적재 하네스 신설 + 라이브 적재 전환** — 사용자 `/harness:harness`("매 적재 백업·270 상품 한 건씩=과한 프로세스·시간소모→스크립트 배치·컨텍스트 최소화"). **선행 라이브 적재 전환**: DB 미적재 원칙 해제(사용자 승인)·출력소재 종이비 GAP 7행 첫 실제 COMMIT(8→7정정·NULLS DISTINCT 멱등 함정→NOT EXISTS 가드·백업 bak_papergap8·R1~R7 게이트 GO)·적용일 시계열 구조(apply_ymd 단가행 분기 금지=이중계상·2026-06-01 유지 UPSERT)·출력소재(IMPORT) 전 상품군 반전(종이 외 이미 적재·실 GAP=디지털 종이)·plate 76% 오적재 별트랙→[[dbmap-live-load-transition-260615]]. **배치 하네스(harness Phase 3~6)**: 신규 스킬 `dbm-batch-load`+scripts 4종(classify[상태×동형 클래스 분류]·gen_batch_upsert[멱등 NOT EXISTS NULL-safe·apply_ymd 고정]·verify_batch.sql[집계 전수]·apply_batch.sh[DRY-RUN/commit/baseline]·작동 검증). **배치 단위=동형 클래스**(동일 옵션구성×가격계산방식·시트 아님)·**상태 분류**(ready/pending/unlisted)·**선결 게이트**(엔티티/속성 값 먼저 제대로 등록)·**백업=위험변경만+git baseline CSV**(매 적재 백업 폐기)·**자동화=DRY-RUN+집계까지·COMMIT은 GO 후 인간 승인**. dbm-load-builder Round-20 배치 모드·dbm-validator 집계 검증 모드·오케스트레이터 round-20. | `.claude/skills/dbm-batch-load/`{SKILL·scripts·references}·`dbm-load-builder.md`·`dbm-validator.md`·`huni-dbmap-orchestrator/SKILL.md`·`30_load-test-prep/`·`09_load/_exec_papergap8_260615/`·CLAUDE.md §7·메모리 | 사용자(`/harness:harness` 적재 효율화·배치·백업 정책) |
| 2026-06-15 | **round-19 가격 폐루프 closeout — 가격 3종 큐 제안본 3종 + 독립 게이트 GO** — 사용자 "가격 폐루프 닫기"(엽서·상품권·명함 ①UI·②차원환원 PASS·③가격만 막힘→닫으면 완전 견적가능 GO). **Phase A 가격표 재대조**(권위 260527 직접 열람·비파괴): MATGROUP GO(명함 시트가 소재 동일가 "/"명시=3,500군 3소재/3,800군 2소재)·CUT_PERF_1H6 조정(가격표 타공 "단가0원"/"합가" 별블록→라이브 CUT_PERF=0원 placeholder→D-1b서 제외)·WIRE-3 GO(명함박=종이포함 완제품가 F57·042=공용 박가공비 신설=종이 이중계상 방지). **Phase B D-1b 정정 제안본**(ⓒ-2 신규 PRICE_TYPE.03·그룹① **13 comp**=CREASE/PERF/VARTEXT/VARIMG/CORNER_ROUND[CUT_PERF·CORNER_RIGHT 0원 제외·CORNER_ROUND 진성 누진 총액형 포함]·멱등 UPDATE·base_code .03 충돌0·엔진 .03 규칙 명세·엽서 C3 104,330/상품권 C2 48,073 재검증·1정정 엽서+상품권 동시정상화 최고ROI). **Phase C WIRE 배선 제안본**(명함 공식분리 ⓑ=PRF_NAMECARD_PREMIUM/COAT 신설+바인딩·031박 FOIL 배선·033 MATGROUP 대표가복제[mat_cd 074백모조220/081아트250/091스노우250/082아트300/092스노우300 라이브 실측 해소]·042 박 공용 COMP_FOIL_AREA_* 신설). **dbm-validator 독립 게이트 V1~V6 전건 PASS·실결함0**(생성자≠검증자, 13 comp 카운트·종이/동판 귀속 표현 정정 2건 반영). 미해소 컨펌 5(C-D1b·D1b-06=webadmin Phase11·WIRE-1=round-2·WIRE-3b=실무진·GAP-2=ddl-proposer). DB 미적재(엔진 evaluate_price 미구현=실청구 위험0·실 COMMIT/DDL=인간 승인). | `28_price-arbitration/_price-queue-closeout/{phase-a-pricetable-recheck,phase-b-d1b-remediation,phase-c-wire-remediation,_gate/phase-bc-validation-gate}`·`29_readiness/_rtm.md`·CLAUDE.md §7·메모리 | 사용자(가격 폐루프 닫기·완전 견적가능 GO 준비) |---
