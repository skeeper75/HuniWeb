...
**변경이력:** 최신: 2026-07-01(2세션) **스캐너 정밀화+가격표 재확인+엽서북 단품정합 확정(결함 23→17)** — ① round9 LEGIT_UNUSED 스캐너 분리(게이트 '미배선이 정답' 판정 comp=addon/superseded 별도 카운트·결함 은닉 아님·종료척도 false-positive 차단 해소) 23→19. ② ★가격표(260527) 재확인[HARD 교훈·사용자 지적]: CONFIRM 큐가 가격표 미확인으로 '호스트 모름' 남발→커팅타공41=완칼타공 호스트(헤더택/벽걸이캘린더)·아크릴34=미러3T 통용단가·명함포토카드81=포토카드대량·포스터사인228=PET거치대 addon·폼보드A1·코롯토/지비츠 단가 전부 가격표에 존재→CONFIRM 큐 7→5 축소. ③ round10 완칼타공1H6/2H6=손님선택 합가 addon(PET거치대 동형)→격리 19→17. ④ ★★엽서북30p 라이브 재진단(오진단 3회 정정): opt_cd 설계→'셋트 내지공식 부재'→**최종 evaluate_set_price(L920)가 셋트 완제품 공식을 set_selections로 호출=통째 단가표 계산(22,000/23,000 가격표 일치)·094 셋트 해제 불요·데이터만 COMMIT**(design-pcb30p-fix 유효). 0원=set_selections 미전달 호출오류. [HARD] 배선 설계 전 라이브 sim_meta(is_set·page_rule)+evaluate_set_price 경로 먼저 확인. 커밋 f47967a·7cdad06·58d1561. 남은 17=고아9(엽서북 데이터COMMIT준비·미러3T/포토카드/코롯토 §18·폼보드/캘린더/접지 CONFIRM)+dead7(신상아크릴)+deleted1(082 §23). 직전: 배선 서브트랙 신설+종단 COMMIT 2건(박명함037·화이트명함040). → `_workspace/_foundation/batch/wiring/HANDOFF.md`·[[formula-components-wiring-subtrack-260701]]
