...
description: >
  후니프린팅 기초코드 거버넌스 하네스의 4-way 진단 방법론 스킬. 권위 정답 사전을 기준으로 라이브 t_*(자재/사이즈/도수/
  인쇄옵션/공정/기초코드/카테고리)를 ①권위 ②라이브 ③역공학 ④경쟁사 4-way로 대조해 오염/누락/오매칭/고아를 전수
  진단하는 절차를 제공한다. 결함 분류(현재값 vs 정답·원인유형 ⓐv03/ⓑvessel-gap/ⓒdata-gap/ⓓ코드도메인·라우팅 신규/
  교정/축이동), 라이브 읽기전용 psql 실측 패턴, dbmap round-13/22 진단 인용·정합(중복 재진단 금지·stale 경계),
  자재 오염(.08/.09/.10)·카테고리 고아 진단 패턴을 정의한다. '기초코드 진단', '4-way 대조', '자재 오염 진단',
  '카테고리 고아 진단', '오매핑 진단', '결함 보드', '라우팅 분류' 작업 시 반드시 이 스킬을 사용. 정답 사전은
  hbg-authority-curation, 등록 명세는 hbg-registration-spec이 담당하므로 그 작업에는 트리거하지 않는다.
---
