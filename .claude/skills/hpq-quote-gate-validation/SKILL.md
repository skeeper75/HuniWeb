...
description: >-
  생성측 결함 보드와 권위 골든값을 독립 재실측하고 라이브 가격엔진(evaluate_price)을 호출/재구현해
  견적을 재계산해 P1~P7 게이트로 GO/NO-GO를 내는 냉철한 검증 방법론 스킬(가격계산 검증 하네스). P1
  엔진계약 충실성(재구현 동치 입증)·P2 골든 재현성(재계산 final_price=엑셀 골든·오차 0)·P3 차원매핑
  정확성(use_dims↔단가행↔권위 3원·생성측 매트릭스 독립 재실측)·P4 불필요분 실재성(판별차원없음·동시매칭·
  고아·중복 재현쿼리 재실행)·P5 옵션/템플릿/제약/공정 정합(BUNDLE·JSONLogic·dim_vals·트리거 무결성)·P6
  사이즈 무중복(siz_cd 중복·축혼동·비규격)·P7 생성-검증 독립성(생성자 주장 비신뢰·라이브 직접 재실측·
  dodge-hunt·날조 적발). 라이브 evaluate_price 직접 호출(Django shell) 우선·재구현은 P1 동치 입증 후만
  신뢰·라이브 시뮬레이터/뷰어 화면값 3원 대조·하나라도 FAIL이면 NO-GO·정직한 CONDITIONAL 허용·거짓 GO
  금지·돈 크리티컬 신중·실 교정은 인간 승인 후 dbmap 위임. 'P1 P7', '가격 게이트', '독립 재계산', '견적
  검증', '시뮬레이터 검증', 'GO NO-GO', '냉철한 평가', '골든 재현', '게이트 다시' 작업 시 반드시 이 스킬을
  사용. 가격사슬 생성 검사는 hpq-price-chain-inspection, 옵션/제약 생성 검사는 hpq-option-constraint-mapping이
  담당하므로 그 생성 작업에는 트리거하지 않는다(본 스킬은 검증 전용).
---
