-- dosu-load-draft.sql — 디지털인쇄 흑백 도수 적재 초안 (DRYRUN · DB 미적재)
-- 산출: hpe-engine-designer 2026-06-29 · 권위 verbatim(디지털인쇄비 시트) · 날조 0
-- ★[HARD] 실 COMMIT 금지 — 인간 승인 후 dbmap(dbm-load-execution) 위임. 이 파일은 BEGIN…ROLLBACK 검증 전용.
-- 권고 모델 = C안(proc_cd로 도수 인코딩 · 별색 패턴 정합 · 신규 comp 0 · 신규 proc_cd 1개).
-- 차원 매핑: 흑백 = COMP_PRINT_DIGITAL_S1 + proc_cd=PROC_000150(신규 흑백디지털)
--           × plt_siz_cd(SIZ_000499 국4절 / SIZ_000077 3절) × print_opt_cd(POPT_000001 단 / POPT_000002 양) × min_qty.
-- clr_cd=CLR_000002(흑백·표시/감사용·엔진 매칭 비사용) · prc_typ=PRICE_TYPE.01(칼라 행과 동일 단가형).

BEGIN;  -- ★DRYRUN: 끝에서 ROLLBACK. 절대 COMMIT 금지(인간 승인 전).

-- ============================================================
-- 1) 신규 proc_cd 신설 (흑백디지털) — search-before-mint: MAX+1
--    부모 = PROC_000001(인쇄 직속·칼라 PROC_000004와 형제 → proc_grp:PROC_000001 직속 매칭 보장)
--    ★컨펌큐 C-1: 부모 PROC_000001(권고) vs PROC_000004(자식·손자 매칭 검증 필요)
-- ============================================================
INSERT INTO t_proc_processes (proc_cd, proc_nm, upr_proc_cd, disp_seq, use_yn, note, reg_dt, del_yn)
SELECT 'PROC_000150', '디지털인쇄(흑백)', 'PROC_000001', 6, 'Y',
       '흑백(1도) 디지털인쇄 도수. 칼라 PROC_000004와 형제. 단가=가격표 디지털인쇄비 흑백열 verbatim.', now(), 'N'
WHERE NOT EXISTS (SELECT 1 FROM t_proc_processes WHERE proc_cd='PROC_000150');

-- ============================================================
-- 2) 흑백 단가행 212행 INSERT (권위 verbatim · 멱등 NOT EXISTS 가드)
--    멱등키 = (comp_cd, proc_cd, plt_siz_cd, print_opt_cd, min_qty, apply_ymd)
-- ============================================================
...
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1, 3000,
       '디지털인쇄 출력비(국4절)/흑백(1도)/단면 출력매수 1장 이상', now()
WHERE NOT EXISTS (
  SELECT 1 FROM t_prc_component_prices
  WHERE comp_cd='COMP_PRINT_DIGITAL_S1' AND proc_cd='PROC_000150'
    AND plt_siz_cd='SIZ_000499' AND print_opt_cd='POPT_000001' AND min_qty=1 AND apply_ymd='2026-06-01');
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1, 4000,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 2, 2000,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 2, 2900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 3, 1600,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 3, 2500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 4, 1400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 4, 2200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 5, 1200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 5, 2000,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 6, 900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 6, 1900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 7, 800,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 7, 1700,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 8, 700,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 8, 1500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 9, 600,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 9, 1300,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 10, 500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 10, 1200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 15, 450,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 15, 900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 20, 400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 20, 800,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 25, 350,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 25, 700,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 30, 300,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 30, 600,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 35, 280,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 35, 560,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 40, 250,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 40, 500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 45, 250,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 45, 500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 50, 250,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 50, 500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 60, 200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 60, 400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 70, 200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 70, 400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 80, 200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 80, 400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 90, 200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 90, 400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 100, 200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 100, 400,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 150, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 150, 280,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 200, 130,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 200, 260,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 250, 120,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 250, 240,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 300, 110,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 300, 220,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 350, 100,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 350, 200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 400, 90,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 400, 180,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 450, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 450, 160,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 500, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 500, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 600, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 600, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 700, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 700, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 800, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 800, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 900, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 900, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1000, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1000, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1200, 70,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1200, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1400, 65,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1400, 130,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1600, 65,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1600, 130,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1800, 60,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1800, 120,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 2000, 60,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 2000, 120,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 2500, 55,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 2500, 110,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 3000, 55,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 3000, 110,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 3500, 50,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 3500, 100,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 4000, 50,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 4000, 100,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 4500, 45,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 4500, 90,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 5000, 45,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 5000, 90,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 6000, 40,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 6000, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 7000, 40,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 7000, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 8000, 40,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 8000, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 9000, 40,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 9000, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 10000, 40,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 10000, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000001', 'CLR_000002', 1000000, 40,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000499', 'POPT_000002', 'CLR_000002', 1000000, 80,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1, 3500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1, 5000,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 2, 2500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 2, 2900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 3, 2000,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 3, 2500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 4, 1800,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 4, 2200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 5, 1200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 5, 2000,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 6, 900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 6, 1900,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 7, 800,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 7, 1700,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 8, 700,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 8, 1500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 9, 600,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 9, 1300,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 10, 500,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 10, 1200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 15, 610,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 15, 1200,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 20, 540,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 20, 1080,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 25, 470,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 25, 950,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 30, 410,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 30, 810,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 35, 380,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 35, 760,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 40, 340,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 40, 680,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 45, 340,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 45, 680,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 50, 340,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 50, 680,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 60, 270,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 60, 540,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 70, 270,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 70, 540,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 80, 270,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 80, 540,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 90, 270,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 90, 540,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 100, 270,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 100, 540,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 150, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 150, 380,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 200, 180,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 200, 350,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 250, 160,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 250, 330,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 300, 150,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 300, 300,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 350, 140,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 350, 270,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 400, 120,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 400, 250,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 450, 110,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 450, 220,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 500, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 500, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 600, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 600, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 700, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 700, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 800, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 800, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 900, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 900, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1000, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1000, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1200, 95,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1200, 190,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1400, 88,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1400, 180,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1600, 88,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1600, 180,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1800, 81,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1800, 160,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 2000, 81,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 2000, 160,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 2500, 75,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 2500, 150,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 3000, 75,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 3000, 150,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 3500, 68,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 3500, 135,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 4000, 68,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 4000, 135,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 4500, 61,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 4500, 122,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 5000, 61,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 5000, 122,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 6000, 54,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 6000, 108,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 7000, 54,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 7000, 108,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 8000, 54,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 8000, 108,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 9000, 54,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 9000, 108,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 10000, 54,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 10000, 108,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000001', 'CLR_000002', 1000000, 54,
...
INSERT INTO t_prc_component_prices
  (comp_cd, apply_ymd, proc_cd, plt_siz_cd, print_opt_cd, clr_cd, min_qty, unit_price, note, reg_dt)
SELECT 'COMP_PRINT_DIGITAL_S1', '2026-06-01', 'PROC_000150', 'SIZ_000077', 'POPT_000002', 'CLR_000002', 1000000, 108,
...
-- ============================================================
-- 3) 상품 도수 옵션그룹/아이템 (예시 1상품 — 프리미엄엽서 PRD_000016)
--    실제는 흑백 판매 상품마다 반복(§5.4 영향 ~15상품·실무진 컨펌 C-4 후 범위 확정).
--    ★ option_items.ref_dim_cd=proc_cd 주입 연결이 라이브 작동하는지 시뮬레이터 선결(C-3).
--      미연결 시 proc_sels 경로(시뮬레이터 procs)로 폴백.
-- ============================================================
-- (스켈레톤 — opt_grp_cd/opt_cd 채번은 dbmap 적재 시 MAX+1로 확정. 여기선 구조만.)
...
--   VALUES ('PRD_000016', '<NEW_OPT_GRP>', '도수', 'SEL_TYPE.01', 1, 1, 'Y', 0, 'Y', '인쇄도수 택1(흑백/칼라). ref proc_cd.', now(), 'N');
-- INSERT INTO t_prd_product_option_items (prd_cd, opt_cd, item_seq, ref_dim_cd, ref_key1, use_yn, reg_dt, del_yn)
--   VALUES ('PRD_000016', '<NEW_OPT>', 1, 'proc_cd', 'PROC_000004', 'Y', now(), 'N'),  -- 칼라(dflt)
--          ('PRD_000016', '<NEW_OPT>', 2, 'proc_cd', 'PROC_000150', 'Y', now(), 'N');  -- 흑백
...
SELECT count(*) AS bw_rows FROM t_prc_component_prices
 WHERE comp_cd='COMP_PRINT_DIGITAL_S1' AND proc_cd='PROC_000150';
-- 칼라 행 무변경(=212 기대):
SELECT count(*) AS color_rows FROM t_prc_component_prices
 WHERE comp_cd='COMP_PRINT_DIGITAL_S1' AND proc_cd='PROC_000004';
-- 골든 셀 샘플(국4절 단면 흑백 qty1=3000 기대):
