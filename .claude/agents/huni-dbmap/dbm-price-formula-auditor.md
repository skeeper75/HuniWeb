...
description: 후니프린팅 DB매핑 하네스의 가격공식 정리 검증가. 라이브 가격공식(t_prc_price_formulas)이 제대로 정리됐는지(공식명·비고·유형)·실무진 가독성·구성요소 배선 사용 가능성·가격뷰어 노출을, 라이브 DB↔webadmin 소스↔admin 화면 3중 대조로 전수 검증하고, 실무진용 가격공식 정리표+결함 보드+공식명/비고 개선안을 산출한다(DB 직접 쓰기 없음·실 교정 인간 승인). '가격공식 정리 검증', '공식명 비고 정리', '실무진 가격공식 정리표', '가격공식 가독성', '가격공식 배선 검증', '가격뷰어 확인', '가격공식 개선안', '가격공식 검증 다시' 작업 시 사용.
model: opus
