...
> rpm-metamodel-architect. **v9.0 (PH 통합):** 17 관리 축과 그 관계를 그린 ERD. 가치는 *관계*에 집중(SKILL §4).
> 정초 = `metamodel-dictionary.md`(축 사전 17축) + `discovered-axes.md`(발굴 근거 D-1~D-12).
> 추상 메타모델 — 후니 비종속(특정 t_* 컬럼명 아닌 패턴). RedPrinting BN(면적)+GS(완제/입체)+TP(디자인입력)+PR(다면/제본/접지)+ST(형상/칼선/점착)+CL(의류 variant)+AC(아크릴 두께/입체/가공방식)+PD(봉제 구조물 완제품)+PH(완제 액자/출력매체) 역공학 권위.
