...
> **[SUPERSEDE 2026-06-08-C → `silsa-option-layer-v2.md`]** 본 문서 **§3/§4/§7(옵션→엔티티 매핑)은 v2가 supersede**한다. v1은 가공/추가 옵션을 **공정(.04)-only 또는 셋트(.07)** 로 매핑했으나(반쪽 매핑 결함), 사용자 HARD 모델은 한 옵션=자재 의미+공정 의미이므로 v2가 각 옵션을 **자재(.03)+공정(.04) BUNDLE**(각목·끈=material, 타공=아일렛 자재+타공 공정 등)으로 분해한다. v1의 **§2 sel_typ·§2 사이즈 이산매트릭스(R8)·§7.1 가격위임·§5 R-GAKMOK 골격은 보존**(v2도 승계). 적재 CSV 권위 = `load_silsa_v2/`(v1 `load_silsa/`는 supersede 이전 본).
>
> **상태/이력** 작성 2026-06-07 · **정정 2026-06-08(live-admin-groundtruth LV-1~5 정합)** · **재정합 2026-06-08-B(가격표 B26 + 도메인·경쟁사 리서치 + 사용자 HARD 확정)** · **v2 supersede 2026-06-08-C(자재+공정 BUNDLE)** · `dbm-option-mapper` 산출 · round-6 파일럿 #2(silsa, master map §6 후보 2). DB 미적재(실 INSERT/코드행/DDL = 인간 승인).
