...
description: 후니프린팅 상품마스터 전 상품군(시트) × 라이브 t_* 엔티티/속성/관계를 3차원 입체 커버리지 매트릭스로 조망하는 검증 방법론 스킬. 시트별 종단(깊이)이 아니라 전 상품군에 걸친 횡단(너비)으로, 각 상품에 필요한 요소가 라이브 DB에 빠짐없이 적재됐는지와 무엇이 미적재인지를 한 판에서 입증한다. 상품군별 필요요소 도출(엑셀 권위)·라이브 DB 적재 실측(읽기전용 psql)·라이브 admin product-viewer 3원 대조(gstack browse)·엔티티 관계(FK/polymorphic) 무결성·미적재 갭 보드 정직 분류·재현 스크립트 패턴을 제공한다. DB 직접 적재는 하지 않는다. '입체 커버리지', '커버리지 매트릭스', '전 상품군 조망', '전체 시트 매핑 검증', '상품마스터 전수 검증', '미적재 조망', '필요요소 도출', '시트별 엔티티 매트릭스', '엔티티 관계 무결성', '3원 대조', 'admin 대조 검증', 'round-7', '커버리지 검증 다시', '매트릭스 재실행' 작업 시 반드시 이 스킬을 사용. 단일 상품군 깊이 매핑(L1)은 dbm-mapping/dbm-price-formula, 이미 적재된 DB↔엑셀 단건 정합은 dbm-mapping-audit, CPQ 옵션 레이어 설계는 dbm-cpq-option-mapping이 담당하므로 그 작업에는 트리거하지 않는다.
---
