...
## 캘린더 섹션 (2026-06-22 · 9번째 종단)

### C-1. 역공학 커버리지 = 부재 (캘린더 직접 증거 없음)

- **widget_monitor PRICE 캡처**: 캘린더 미관측 — `grep -i calendar|캘린더|달력 raw/widget_monitor/` 결과 RedPrinting 카탈로그 JSON에 상품명만 존재, PRICE 응답 캡처 0건. RedPrinting은 캘린더를 같은 위젯 가격 API로 노출하지 않음(에디터형 디자인 상품).
- **rpmeta 카테고리**: 캘린더 대응 카테고리 부재(13 샘플 AC/BN/CL/FS/GS/NC/OT/PD/PH/PO/PR/ST/TP에 캘린더 없음). 캘린더는 디지털인쇄 도메인 산물로 RP 메타모델 샘플 밖.
- 함의: 캘린더 가격 설계는 **상품마스터+가격표+라이브 t_prc_* 내부 권위만으로 충분**(역공학 흡수 후보 불요). 디지털인쇄 PRF_DGP 패밀리(이미 검증된 후니 내부 자산) 직계 재사용이 흡수 경로.

### C-2. 경쟁사 구성요소 축 후보 (간접·갭헌팅)

- 와우프레스/RedPrinting 캘린더 가격 = (관측 범위 밖) 일반 인쇄물 패턴 추정: 인쇄비(수량×페이지) + 제본/링 + 거치대 add-on = **후니 calc-draft 원자합산형과 동형**. 신규 축 발굴 없음.
- ★흡수 결론: **신규 가격축 0** — 캘린더 구성요소(인쇄·용지·제본·가공·페이지수)는 전부 후니 기존 t_prc_* 그릇으로 표현됨. naming/codes 유입 금지(역공학 후보 채택 없음).

`확신도: 높음(캘린더 역공학 증거 부재 자체가 내부권위 충분의 방증)`
