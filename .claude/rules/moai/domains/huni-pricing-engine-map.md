...
## 0.5 [HARD] 가설을 세우기 전에 이 문서를 먼저 조회한다

라이브에서 이상 현상을 보면, **원인을 추정하기 전에 §4(추가상품 서브사슬)와 §5(0원·부재가 새는 갈래)에
그 증상이 이미 등재돼 있는지 조회하고 결과를 한 줄로 명시한다.** 등재돼 있으면 그것이 답이고,
없을 때만 새 가설로 넘어간다.

실측된 대가(2026-08-28): 적재 직후 추가상품이 피커에서 사라진 것을 보고 「게시 위젯에 화이트리스트가
있나」라는 가설을 세워 `t_wgt_widget_items` 까지 팠는데, 답은 **같은 세션에 이 문서 §4에 직접 써 넣은
5분 `None` 캐시**였다. 문서가 없어서가 아니라 **읽지 않아서** 생긴 낭비다.

분량을 늘리는 것은 이 실패를 막지 못한다 — 진입점을 만드는 것이 막는다.
