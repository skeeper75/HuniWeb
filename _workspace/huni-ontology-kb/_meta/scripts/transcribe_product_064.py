...
# -*- coding: utf-8 -*-
"""
전사 보강 스크립트 — PRD_000064 소량자유형스티커 (D-9·pack-sticker §4·[HARD] LLM 손전사 금지).

064는 052(반칼 자유형)와 형제(자유형스티커 CAT_000309·완제품가 룩업 PRF_STK_FIXED)이나 다음이 다르다:
  - use_yn=N(미출시·063과 함께 비활성·pack §1.1)
  - 소형 사이즈 7종(50x70~94x94·소형반칼)·자재는 PARENT 코드(153/084/242/155/156·052는 자식코드 재키잉)
  - 커팅 공정=PROC_000054(반칼·052는 PROC_000122로 이관·064는 구 코드 그대로 활성)
  - CPQ 옵션 레이어 전면 부재(option_groups/options/option_items 0행·BATCH-6·GAP-ST-6)
  - 코팅=자재(155/156)만·라미공정(014/015) 없음 → 052보다 단일 표현(이중가산 위험 낮음)
  - 판형 파일사양 7행 del_yn=Y(06-30 정리)→SIZ_000521(46전지) 단일 활성

이 스크립트는 사이즈·자재·공정·도수·판형(활성+삭제)·수량 + 가격구성요소(COMP_STK_PRINT use_dims·행수요약)
+ 연당가 대조(064 소재가 260702 substantive 연당가 변경 대상인가) + CPQ 부재 확인을 결정론 전사한다.
기존 transcribe_product_052.py 등 미수정(새 파일 원칙). 공유 노드(카테고리·자재·공정·판형·공식·구성요소·
043/036 사이즈)는 피어 companion/axis에 이미 실재 → 재사용(재-mint 없음·053 halfcut-clear 패턴).

사용:  python3 transcribe_product_064.py            # stdout markdown 블록
       python3 transcribe_product_064.py --json     # cache/transcribed-064-260703.json 갱신
...
STAMP = "2026-07-03"
PRD = "PRD_000064"
SCRIPT = "transcribe_product_064.py"

# 260702 diff의 substantive 연당가 변경 소재(pack §4-A) = 투명/홀로/크라프트/투명후지.
# 064 자재(유포153/비코팅084/미색242/무광155/유광156)는 이 목록 밖 → 전행 NO(clean).
...
def _san(v):
    """마크다운 표 셀 보호 — 파이프/개행 치환(라이브 note에 '|' 실재 → 표 컬럼 깨짐 방지)."""
...
def build():
    ...
    plt = [r for r in plt_all if r["del_yn"] != "Y"]
    plt_del = [r for r in plt_all if r["del_yn"] == "Y"]
    ...
    oitems = [r for r in rd("t_prd_product_option_items.csv") if r["prd_cd"] == PRD and r["del_yn"] != "Y"]

    # 셋트/추가상품/제약/묶음수 (부재 확인)
    sets = [r for r in rd("t_prd_product_sets.csv") if r.get("prd_cd") == PRD or r.get("sub_prd_cd") == PRD]
    addons = [r for r in rd("t_prd_product_addons.csv") if r["prd_cd"] == PRD]
    cons = [r for r in rd("t_prd_product_constraints.csv") if r["prd_cd"] == PRD]
    bql = [r for r in rd("t_prd_product_bundle_qtys.csv") if r["prd_cd"] == PRD]
    ...
    # COMP_STK_PRINT 단가행 행수 요약 (064 활성 siz × 활성 mat) — 값 나열 아님·행수 집계(D-22 접기)
    ...
    combos_expected = len(active_sizes) * len(active_mats)
    combos_present = len(grid_summary)
    ...
    return {
        ...
        "sizes": [{"siz_cd": r["siz_cd"], "siz_nm": siz.get(r["siz_cd"], {}).get("siz_nm", "?"),
                   ...
                   "note": siz.get(r["siz_cd"], {}).get("note", ""),
        ...
        "materials": [{"mat_cd": r["mat_cd"], "mat_nm": mat.get(r["mat_cd"], {}).get("mat_nm", "?"),
                       ...
                       "note": mat.get(r["mat_cd"], {}).get("note", ""),
        ...
        "plate_deleted": [{"siz_cd": r["siz_cd"], "output_paper_typ_cd": r["output_paper_typ_cd"],
                           "output_file_typ": r["output_file_typ"], "note": r["note"], "del_dt": r["del_dt"]}
                          for r in plt_del],
        ...
        "price_grid_coverage": {"expected": combos_expected, "present": combos_present},
        "cpq_layer": {"option_groups": len(og), "options": len(opts), "option_items": len(oitems)},
        "relations_absent": {"sets": len(sets), "addons": len(addons),
                             "constraints": len(cons), "bundle_qtys": len(bql)},
...
def md(d):
    ...
    o += ["| siz_cd | 라벨 | 작업(work mm) | 재단(cut mm) | dflt | disp | master_del_yn | note(라이브) |",
          "|---|---|---|---|---|---|---|---|"]
    for v in d["sizes"]:
        o.append(f'| {v["siz_cd"]} | {v["siz_nm"]} | {v["work"]} | {v["cut"]} | {v["dflt"]} | {v["disp"]} | {v["master_del_yn"]} | {_san(v["note"])} |')
    o.append("")

    tb("t_prd_product_materials+t_mat_materials (del_yn=N)")
    o += ["| mat_cd | 자재명 | mat_typ | 상위 | 평량(g) | dflt | disp | usage | note(라이브) |",
          "|---|---|---|---|---|---|---|---|---|"]
    for v in d["materials"]:
        o.append(f'| {v["mat_cd"]} | {v["mat_nm"]} | {v["mat_typ_cd"]} | {v["upr_mat_cd"]} | {v["weight"]} | {v["dflt"]} | {v["disp"]} | {v["usage"]} | {_san(v["note"])} |')
    o.append("")
    ...
    tb("t_prd_product_plate_sizes 활성 (del_yn=N)")
    o += ["| siz_cd | output_paper_typ_cd | dflt_plt | note |", "|---|---|---|---|"]
    for v in d["plate"]:
        o.append(f'| {v["siz_cd"]} | {v["output_paper_typ_cd"]} | {v["dflt_plt"]} | {_san(v["note"])} |')
    o.append("")

    tb("t_prd_product_plate_sizes 삭제분 (del_yn=Y·파일사양 정리 이력·2026-06-30)")
    o += ["| siz_cd | output_paper_typ_cd | output_file_typ | note | del_dt |", "|---|---|---|---|---|"]
    for v in d["plate_deleted"]:
        o.append(f'| {v["siz_cd"]} | {v["output_paper_typ_cd"]} | {v["output_file_typ"]} | {_san(v["note"])} | {v["del_dt"]} |')
    o.append("")
    ...
    cov = d["price_grid_coverage"]
    tb(f"t_prc_component_prices COMP_STK_PRINT 행수요약 (064 활성 siz×mat={cov['present']}/{cov['expected']} 조합·값 나열 아님·D-22 접기)")
    ...
    c = d["cpq_layer"]
    r = d["relations_absent"]
    tb("CPQ 레이어 + 셋트/추가상품/제약/묶음수 행수 (부재 확인·064)")
    o += ["| 레이어 | 라이브 행수 |", "|---|---|",
          f'| option_groups | {c["option_groups"]} |',
          f'| options | {c["options"]} |',
          f'| option_items | {c["option_items"]} |',
          f'| product_sets | {r["sets"]} |',
          f'| product_addons | {r["addons"]} |',
          f'| product_constraints | {r["constraints"]} |',
          f'| bundle_qtys | {r["bundle_qtys"]} |', ""]

    tb("연당가 대조 (064 활성 자재가 260702 substantive 연당가 변경 대상인가·pack §4-A)")
...
if __name__ == "__main__":
    d = build()
    if "--json" in sys.argv:
        p = os.path.join(os.path.dirname(__file__), "cache", "transcribed-064-260703.json")
