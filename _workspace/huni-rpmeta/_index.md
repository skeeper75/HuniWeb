...
> 실행 이력: ① BN(현수막류) 파일럿 6+1상품. ② GS(굿즈/잡화) 확장 12상품. ③ TP(디자인템플릿) 확장 대표3+20횡단. ④ PR(인쇄물·책자) 확장 대표3+53횡단. ⑤ ST(스티커) 확장 대표3+33횡단. ⑥ CL(의류) 확장 대표3+27횡단. ⑦ AC(아크릴·키링·코롯토·명찰·등신대) 확장 대표3+17횡단. ⑧ PD(스툴·슬리퍼·강아지계단·봉제 구조물) 전수 3상품. ⑨ PH(포토보드·액자·사진인화·포토북·포토굿즈) 확장 대표3+27횡단(★§0.5 client-render 재캡처로 거치/마운팅 블로커 해소). ⑩ FS(패브릭·봉제 완제 직물 굿즈) 확장 대표5+16횡단(★타일링 신규 fragment 발굴·신규축 선별 모드). ⑪ NC([옵셋] 명함·카드·쿠폰·포토카드) 라이브 대표3(★선별 모드 핵심 프로브 — "인쇄방식 옵셋 vs 디지털"이 distinct #18 관리축인가 → **1차 예측 부결[흡수]**). ⑬ PO(포맥스·폼보드·등신대·피켓) 라이브 대표5+전7횡단(★선별 모드 — "경질 기재 마운팅[합지 vs 직접출력]·자립 구조[등신대/피켓]가 distinct #18인가" → **1차 예측 부결[흡수]·관측 기반**·옵션 SSR 완전노출). 대표 샘플링(답습 전수수집 아님). **메타모델 = 17축(13 카테고리·distinct: TP #16·ST #17·나머지 0·PH 재포화 9번째·FS 재포화 10번째·NC 재포화 11번째·PO 재포화 13번째·★FS 타일링 #18 후보 1건 판정보류→아키텍트/갭·★NC 인쇄방식 #18 후보 1차 부결[전용 슬롯 부재·KB 결함 부재·기존 #11가격모델/기초코드 흡수]·★PO 기재마운팅/자립구조 #18 후보 1차 부결[제작방식=자재#1+공정#2·자립=형상#17+부속물#8·PH 거치 부결 동형·옵션 SSR 노출로 관측 기반 격상]).**
...
## 커버리지 맵 — OT(상자·패키징, 7상품·★전개도/dieline #18 프로브)

| pdtCode | 상품 | 구조 다양성 | 축 수 | 캡처 | 자재 | 도수 | size 프리셋(작업=전개도) | 후가공 | 출처 |
|---------|------|------------|-------|------|------|------|------------------------|--------|------|
| OTPKCAK | 케이크상자 | 박스(뚜껑·3D 130X105X80) | 6 | /detail SSR | BV/350 | 단면 | 495X280·553X296·694X336 | 없음 | `[live:detail]` 풀 |
| OTPKFLT | 납작상자 | 박스(날개) | 6 | /detail SSR | BV/350 | 단면 | 324X331·377X335·384X431 | 없음 | `[live:detail]` 풀 |
| OTPKHMN | 반달상자 | 박스(반달 날개) | 6 | /detail SSR | BV/350 | 단면 | 195X245·285X377·390X470 | 없음 | `[live:detail]` 풀 |
| OTPKENV | 봉투상자 | 박스(뚜껑·커스텀) | 6 | /detail SSR | BV/350 | 단면 | 435X332·597X446 | 없음 | `[live:detail]` 풀 |
| OTPKARP | 답례품상자 | 박스(소형) | 6 | /detail SSR | BV/350 | 단면 | 425X272 | 없음 | `[live:detail]` 풀 |
| OTPOCLP | 클래퍼 | 비박스(평면 손피켓) | 7 | /detail SSR | 아트지/백색모조·300 | 양면 | 710X350(칼틀) | ★코팅(무광/유광) | `[live:detail]` 풀 |
| OTCPHOL | 에어홀더 | 비박스(컵홀더) | — | 미캡처(추정) | — | — | — | — | `unobserved` |

> ★OT는 `/product/item/OT/{code}`=소개 랜딩(옵션 위젯 없음·infoCall 0)이고, 실제 옵션 위젯은 **`/{code}/detail` 레거시 SSR select 폼**(NC Vue infoCall과 다른 구식). 박스 옵션 모델=평면 인쇄물과 **동일 슬롯**(용지·평량·도수·size·건수·수량). 박스 dieline 전개도는 `makers.redprinting.net/v1/templates/{code}` koi 에디터 디자인 자산(#16 TemplateAsset). 5상품 라이브 추출 성공(주문 0건·DOM read-only·"장바구니/주문하기" 클릭 안 함). 모집단=catalog category=OT 7상품.
> **★OT 발굴 — 전개도/dieline #18 1차 예측: 부결(흡수).** 박스/패키징이라는 *평면 인쇄물에 없던 입체/전개 차원*조차 17축 무손실 흡수. 핵심 증거 = `/detail`이 **"제품사이즈(3D 130X105 높이80) / 재단사이즈(485X270) / 작업사이즈(495X280)" 3종 병기**하나 *선택 옵션은 size 프리셋(작업=전개도 평면) 1개뿐*·3D는 파생 표시값. **OT-1 전개도/dieline=사이즈#13(작업치수)+공정#2(도무송 칼틀·오시 접지·spec kw "칼선/오시")로 분배 흡수**(ST 칼선=공정#2 facet 판정 동형·전개도=칼틀의 박스 인스턴스) · **OT-2 박스 형태(케이크/납작/반달/봉투/뚜껑/날개)=size 프리셋 라벨+pdtCode 분기**(PR/TP 카드형 형상=사이즈 1:1 동형·ST `shape_info` 1:多 분리 슬롯 *부재*) · **OT-3 3D 입체치수=사이즈#13 파생 표시값**(선택축 아님) · **OT-4 ★풀칠/접합/입체 조립=흡수 불요**("납작하게 접힌 상태 배송·고객이 조립"=RP는 평면까지만 생산·조립=고객 수작업→#14 형태가공[RP가 입체 *생성*]의 정반대) · **OT-5 클래퍼 코팅=공정#2 후가공(인쇄비31,100+후가공10,600 분리 합산)·용지→코팅 캐스케이드(#5)**. **★승격 기준 둘 다 불충족(ST 형상#17 대조): ① 전용 슬롯 라이브 부재(전개도/접지/3D 전용 슬롯 0·전부 size 프리셋+표시텍스트 융합·ST shape_info 같은 분리 슬롯 없음) ② 후니 KB "어느 축에도 못 담음" 결함 부재(사이즈=재단/작업치수·공정=도무송/오시·카테고리=상품군이 무왜곡 흡수)** → 전개도/dieline #18 **부결**. NC 인쇄방식#18 부결과 동형·ST 형상#17 승격과 정반대. **★12번째 카테고리로 17축 재포화(distinct 0)·박스라는 입체/전개 차원조차 흡수 — 선별 모드 핵심 프로브 통과.** ★단 validator 검증 nuance 3: (O-1) dieline 에디터 템플릿 내부 구조(접지선/풀칠탭 좌표)가 #16 TemplateAsset로 충분한지 vs "구조 dieline" sub-type 필요한지 `unobserved`(`makers.../templates/{code}` 스키마 실측) · (O-2) "재단사이즈 vs 작업사이즈" 2치수를 후니 size축 한 프리셋에 담는 표현력(여백 10mm·t_siz 이중등록 동형·data-gap 가능·vessel-gap 아님) · (O-3) 3D 제품치수가 size 프리셋 부가 컬럼인가 미저장 파생인가.
...
## 커버리지 맵 — PO(포맥스·폼보드·등신대·피켓) (상품 → 축 → 출처)

> ⑬ PO 확장 대표5+전7횡단. ★선별 모드 프로브 = "경질 기재 마운팅(합지 vs 직접출력)·자립 구조(등신대/피켓)가 distinct #18 관리축인가". **★PO는 PH/AC와 달리 옵션 SSR 완전노출(레거시 real_item)→distinct 판정이 관측 기반.**

| pdtCode | 상품명 | 구조 다양성 | 축 수 | 기재(자재) | 사이즈 | 인쇄면 | 형상/재단 | 부속물(자립) | 출처 |
|---------|--------|------------|-------|-----------|--------|--------|-----------|-------------|------|
| POMXPRT | 포맥스 직접출력 | 직접출력/기본 | 8 | 포맥스 4(3T/5T/8T/10T) | 1000X1000+직접입력(면적) | 단면/양면 | 정사이즈/모양재단 | 거치대3+와이어3 | `[live:SSR-legacy]` |
| POMXHAP | 포맥스 합지(파인아트) | 합지/검정variant | 9 | 포맥스+검정포맥스 7 | 동 | 단면/양면 | 정사이즈/모양재단 | 거치대+와이어 | `[live:SSR-legacy]` |
| POFMPRT | 폼보드 직접출력 | 기재 대비 | 8 | 폼보드 3(3T/5T/10T) | 동 | 단면/양면 | 정사이즈/모양재단 | 거치대+와이어 | `[live:SSR-legacy]` |
| POFMPCK | 폼보드 피켓 | 자립=내재손잡이 | 6 | 폼보드 5T(1 고정) | 직접입력만 | 단면/양면 | 정사이즈/모양재단 | **★없음**(손잡이 내재) | `[live:SSR-legacy]` |
| POSTPRT | 등신대(포/폼) 직접출력 | 자립=거치대add-on | 8 | 포맥스4+폼보드3(7) | 동 | 단면 고정 | 모양재단(인물컷) | **거치대700/1200/1500**+와이어 | `[live:SSR-legacy]` |
| (POFMHAP/POSTHAP 횡단) | 폼보드 합지·등신대 합지 | 합지 검정variant | 9 | +검정 variant | 동 | — | — | 거치대+와이어 | `[live:SSR-legacy]` |

**PO 핵심: distinct #18 = NO(전부 17축 흡수, 관측 기반·13번째 재포화).**
- 제작방식(합지/직접출력) = 자재#1 variant(검정 기재) + 공정#2(라미)로 분해 — 합지만 코팅 활성(제작방식→후가공 캐스케이드 실증). PH 프레임재질·AC 소재variant 분기 동형.
- 자립구조(등신대 거치대/피켓 손잡이) = 형상#17(모양재단 컷아웃) + 부속물#8(거치대 add-on SKU=GS/GSSBMTL) 분해. **★PH 거치(탁상용/벽걸이) 옵션 캐스케이드 부결 동형 — 단 PO는 옵션 SSR 완전노출+거치대 명시 부자재 SKU라 "관측 기반 부결"로 격상**(PH는 client-render·판정불가였음).
- 타일링(분할출력)=공정#2(BN 동형)·기재두께=자재#1 facet(AC 동형)·사이즈=면적연속차원(BN 동형).

## 다음 단계 — PO 추가분 (rpm-gap-analyst 주목 — ★전부 흡수·일부 data-gap/경계)
36. **★제작방식(합지/직접출력) 그릇 — 공정#2 facet vs 별 enum 차원(PO-1)** — RP는 pdtCode로 가르나 후니에 "제작방식(직접출력/합지/UV직접)" 명시 차원이 있는지 gap 분석. 1차 예측=공정#2(라미) 흡수. BN 인쇄방식·ST UV/DTF/후지 분기와 동근. distinct 부결·흡수 경로 검증만.
37. **★등신대 거치대 = add-on이 독립 부자재 SKU(GS/GSSBMTL) 참조하는 구조(PO-2)** — 부속물#8 add-on이 옵션값 복제인가 독립 SKU 참조(template#/완제 결합)인가. BN 아일렛·AC 받침 동형. "add-on이 독립 SKU 참조" 정규화 그릇 검증.
38. **★PO 사이즈 = 면적 연속차원(PO-4)** — size=1000X1000 고정 1개+직접입력=사실상 비규격 면적축. dbmap 면적매트릭스(siz_width/siz_height 구간) 동형. 이산 siz_cd 아님. number4_sel(1배~10배)=면적 가격계수 경계(PO-3·real_price 가격엔진 huni-widget 영역).
