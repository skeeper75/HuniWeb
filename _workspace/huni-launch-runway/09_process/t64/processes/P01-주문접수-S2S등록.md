...
|---|---|---|---|---|---|---|
| 1 | 결제 성공 직후 `order/register` 호출 | huni-mall | 김동학 | `STD-ORD-030` · `T4-1` *(cross t63)* | 미착수·부분 | **부르는 쪽 0건** — `huni-mall/src/**` 에서 `order/register`·`orderRegister` grep 0건. `/api/w/v1` 호출은 `huni-mall/src/lib/printly/huni.ts:9` 의 requote 프록시 하나뿐 |
...
> **t63 대조(260919).** t63 이 같은 다리를 `STD-ORD-030`(B7 · 김동학 · 미착수 · 「주문 생성 직후 후니 주문등록 S2S 호출」)으로 원장화했고 **호출측을 찾지 못했다**고 보고했다. 이 카드의 실측(§4)과 **일치한다.** 행은 t63 이 갖고, t64 P01 은 그 빈 다리에서 시작한다.
