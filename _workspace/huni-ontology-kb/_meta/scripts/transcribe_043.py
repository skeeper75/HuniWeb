...
# -*- coding: utf-8 -*-
"""
전사 보강 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — PRD_000043 인쇄배경지(OPP봉투타입).
...
사용:  python3 transcribe_043.py           # stdout에 markdown 블록(사이즈 전사표 + 멤버십 요약)
       python3 transcribe_043.py --json    # cache/transcribed-043-260703.json 갱신
...
def build():
    ...
    return {
        "meta": {"snapshot": SNAP_ID, "captured_at": STAMP, "prd_cd": PRD,
                 "generated_by": "transcribe_043.py"},
...
def md_sizes(d):
    out = [f"<!-- transcribed-by: _meta/scripts/transcribe_043.py from {SNAP_ID} t_siz_sizes (PRD_000043 sizes) @ {STAMP} -->",
...
def md_membership(d):
    p = d["product"]
    out = [f"<!-- transcribed-by: _meta/scripts/transcribe_043.py from {SNAP_ID} t_prd_product_* (PRD_000043) @ {STAMP} -->",
           "| 축 | 라이브 멤버십 |", "|---|---|",
...
if __name__ == "__main__":
    d = build()
    if "--json" in sys.argv:
        p = os.path.join(os.path.dirname(__file__), "cache", "transcribed-043-260703.json")
        with open(p, "w", encoding="utf-8") as f:
            json.dump(d, f, ensure_ascii=False, indent=2, sort_keys=True)
        print("wrote", p)
    else:
        print("### 사이즈 전사표 (043 전용)\n")
        print(md_sizes(d))
        print("\n### 라이브 축 멤버십 (043)\n")
