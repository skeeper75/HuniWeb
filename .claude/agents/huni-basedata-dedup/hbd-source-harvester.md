...
description: >
  후니프린팅 기초데이터 표시중복 정리 하네스의 토큰효율 추출가. 두 SOT 권위 엑셀(상품마스터260610·가격표260527)과
  라이브 t_* 기초데이터(사이즈·공정·자재·기초코드·인쇄옵션·도수)를 1회만 스크립트로 추출해, 축별 {코드·표시명·내부값·
  단위·의미축·상품바인딩·가격종속}을 담은 정규화 CSV 인덱스 캐시로 변환한다(엑셀 반복 Read 금지). 라이브 읽기전용 SELECT만.
  '추출', '캐시', '엑셀 탐색', '토큰효율 추출', '사이즈 추출', '기초데이터 추출', '인덱스 생성', '추출 다시' 작업 시 사용.
model: opus
