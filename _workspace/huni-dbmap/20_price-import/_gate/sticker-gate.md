...
> **재게이트 2026-06-13(2차)**: F1·F2 BLOCKER 보정 후 P1·P5 재검 → **둘 다 PASS, 종합 GO**. 상세는 "재게이트(2차)" 절. 1차 NO-GO 본문은 이력으로 보존.
> **재게이트 2026-06-13(3차)**: 사용자 피드백("소재별로 나눠야") → 소재 7종 분해 재빌드 후 P3·P5 재검 + mat_cd 실재 확인 → **전부 PASS, 종합 GO 유지**. 상세는 맨 아래 "재게이트(3차)" 절.
...
**최종 한 줄 평결(2차): GO** — F1·F2 보정 검증 완료. (3차 재게이트로 갱신됨, 아래 참조)

---

## 재게이트 (3차) — 소재 7종 분해 후 P3·P5 재검 + mat_cd 실재 · 2026-06-13

> **계기** 사용자 피드백("유포·비코팅·미색·투명·홀로그램을 소재별로 나눠야") → `sticker-material-axis.md` 신설 + `sticker-import.xlsx` `4_component_prices` 소재축 재빌드(가격표 3그룹 → 개별 소재 7종, M-2=(a) 코팅 자재 variant 확정).
> **범위** P3(무손실)·P5(동시매칭)만 재검 + mat_cd 라이브 실재 추가확인(생성자 요청). P1(컬럼 무변경)·P4·P6은 2차 GO 보존. F3(타투 base 2000)은 Q-STK-1 컨펌 대기 = 차단 아님.
> **방법** 라이브 `t_mat_materials` 10코드 실측 + openpyxl로 원본 그룹단가 ↔ 소재행 unit_price round-trip(B01 1512셀·B02·B03·B04 전수) + main 1052행/blocked 528행 자연키 중복 재측정.

### mat_cd 라이브 실재 — **PASS (10/10 실재)**

`t_mat_materials` 실측: `MAT_000153`유포·`MAT_000084`비코팅·`MAT_000242`미색·`MAT_000155`무광코팅·`MAT_000156`유광코팅·`MAT_000162`투명·`MAT_000163`홀로그램스티커 + B02/B04 `MAT_000165`유포지+엠보코팅·B05 `MAT_000167`타투전용지 모두 실재. 초판 오매핑 `MAT_000170`=투명데드롱스티커(합판도무송 전용)도 실재 확인 → **material-axis.md의 "170=합판전용 오매핑" 정정 타당**(투명=162가 정답). 발명 코드 0.

### P3 분해 무손실 — **PASS**

원본 그룹단가 ↔ 소재행 round-trip(독립 재계산, 원본 셀 좌표 직접 대조):

| 블록 | 검증 셀 | mismatch | 비고 |
|------|---------|----------|------|
| B01 매트릭스(6siz×7mat×36qty) | **1512** | **0** | 그룹단가 정확 복제 — `유포/비코팅/미색`열 6000 → 153·084·242 각 6000 / `무광/유광`열 7000 → 155·156 각 7000 / `투명/홀로그램`열 7000 → 162·163 각 7000 ✅ |
| B02 낱장(A4/A3/A2×6) | 18 | 0 | mat 165, 원본 47-52행 일치 |
| B03 투명완칼 CLEAR(A4/A3/A2×6) | 18 | 0 | mat 162, 원본 58-63행 일치 |
| B04 대형(400x600×6) | 6 | 0 | mat 165, 원본 69-74행 일치 |
| B05 타투 | 1 | 0 | mat 167, min3=4000 |
| B06 팩 | 1 | 0 | 4000 (mat NULL=Q-STK-6 미확정·정직 gap) |

값 왜곡 **0**, 소재 오복제 **0**. 같은 그룹 내 N개 소재는 동일 단가 균등 복제(엔진이 소재로 매칭하므로 정당 — 같은 가격이라도 소재는 별 행). **행 수 정합**: main 1052 = B01-mapped(4siz×7mat×36=1008)+B02(18)+B03(18)+B04(6)+타투(1)+팩(1). blocked 528 = B01-unmapped(100*148·90*110 2siz×7mat×36=1008... 504×... = 252+252)+B4(12)+B3(12). main 1052 + blocked 528 = **1580**(원본 719 논리셀 × 소재다중도 확장·드롭 0). unit_price NULL: main 0·blocked 0.

### P5 동시매칭 0 — **PASS**

independent 재측정:
- **main 1052행**: 자연키 `(comp,apply,siz,clr,**mat**,proc,opt,coat,bdl,min)` 중복 **0**. mat_cd가 키에 포함되어 소재별 행 분리 → 1차 NULL-siz 충돌·소재 collapse 모두 해소. NULL siz_cd **0**, NULL unit_price **0**. mat 분포 균등(153/084/242/155/156/163 각 144·162=162[B03 18 추가]·165=24·167=1·NULL=1[팩]).
- **NULL mat_cd 1행** = COMP_STK_PACK(Q-STK-6 소재 미확정). 자연키 중복 미유발(siz/bdl/min 고유). 정직 gap·차단 아님.
- **blocked 528행**: round-2 BLOCKED 4사이즈(100*148·90*110·B4·B3)만, siz_cd 전부 NULL(적재 안 함). B4/B3 mat = 165(B02)·162(B03) 정합. unit_price 보존.

라이브 `ux_t_prc_comp_prices_nat_key`(10컬럼 UNIQUE) 위반 0. Phase11 "동시매칭=데이터 오류" 0.

### 재게이트 평결(3차)

| 게이트 | 2차 | 3차 재검 | 증거 |
|--------|-----|---------|------|
| mat_cd 실재 | — | **PASS** | 10/10 라이브 실측·발명0·170 오매핑 정정 타당 |
| P3 무손실 | PASS | **PASS** | B01 1512셀+B02/B03/B04 round-trip mismatch 0·소재 균등복제 정당·1580 무손실 |
| P5 동시매칭 0 | PASS | **PASS** | main 1052 중복0(mat_cd 키 분리)·blocked 528 round-2 정합 |
| P1 컬럼 | PASS | 보존 | 4_component_prices 컬럼 무변경 |
| P4/P6 | PASS | 보존 | 단가/합가 판별·엔진 시뮬 미변경 |

잔여 컨펌(차단 아님): Q-STK-1 타투 base 2000 · Q-STK-5 투명전용지 코드 · Q-STK-6 스티커팩 소재 · M-2 코팅 자재(a)/공정(b) 최종(현 (a) 채택) · 비코팅084·미색242 MAT_TYPE.01→.11 오적재(round-13 트랙).

---

**최종 한 줄 평결(3차): GO — 소재 7종 분해 검증 완료. mat_cd 10/10 라이브 실재(170 오매핑 정정 타당)·P3(B01 1512셀+B02~B04 round-trip mismatch 0·소재 균등복제 정당·1580 무손실)·P5(main 1052행 mat_cd 키 분리로 중복0·blocked 528 round-2 정합) 모두 PASS. main 1052행 webadmin 복붙·적재 가능, blocked 528행 siz 선적재 후 별 트랙, 잔여 컨펌(타투 base·투명지코드·팩소재·코팅축)은 차단 아님.**
