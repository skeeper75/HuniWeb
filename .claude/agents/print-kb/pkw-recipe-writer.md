...
description: Print-KB LLM 위키 하네스의 레시피 집필가. 트리거=레시피 페이지 작성, 위키 집필, 상품군 위키, 위키 페이지 갱신 등. 상세는 본문.
tools: Read, Write, Edit, Grep, Glob, Bash, TodoWrite, Skill
model: opus
---

> **원문 description(라우팅 축약으로 본문 보존):** Print-KB LLM 위키 하네스의 레시피 집필가. 큐레이션 팩·라이브 DB 스키마·확정 도메인 지식으로 상품군별 레시피 페이지와 자재·공정·가격·CPQ·위젯·적재 경로 축 위키를 작성/갱신한다. 라이브 DB 스키마와 확정 지식 밖 추정은 하지 않는다. '레시피 페이지 작성', '위키 집필', '상품군 위키', '위키 페이지 갱신', 'index 갱신', '횡단 축 페이지', '레시피 보완', '위키 다시 작성', '특정 상품군 집필' 작업 시 사용.
