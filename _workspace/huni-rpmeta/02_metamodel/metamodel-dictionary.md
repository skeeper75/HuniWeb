...
> - **★v13.0 (PO 통합·현재):** + 포맥스/폼보드·등신대·피켓(PO·경질 기재에 사진/그래픽 출력한 홍보용 POP·7상품·합지/직접출력 제작방식 분기) 대표5+7횡단 `[live:SSR-legacy]`(7상품 전부 SSR 완전노출·옵션 select/아이콘/부자재 토글 실측·주문/POST 0). BN·GS·TP·PR·ST·CL·AC·PD·PH·FS·NC·OT·PO **13 상품군 증거**로 검증. **★PO distinct 신축 0종 — 17축 재포화(PR·CL·AC·PD·PH·FS·NC·OT 패턴 반복·★기재마운팅(제작방식)·자립구조 #18 둘 다 부결).** PO=**선별 모드 13번째 프로브** — 제작방식(합지 vs 직접출력)·자립 구조(등신대 자립 컷아웃·피켓 손잡이·거치대 add-on)라는 강력한 #18 후보 2건 보유. 4 fragment(PO-1~PO-4) 적대 판정 후 전부 facet/data-gap. **★PO 결정적 우월점 = PH(client-render)·AC(SSR-negative)와 달리 7상품 전부 SSR 완전노출 → 관측 기반 양방향 부결(추정 0·1회 GET 실측).** ★directive 최대 관전 1(기재마운팅=제작방식 합지/직접출력이 #18인가): **(PO-1)=인쇄방식레시피#12(D-7) 라미 인스턴스 + 자재#1 검정 variant + 공정#2 코팅 게이팅** — 한 상품 내 "제작방식 select" 부재(pdtCode 분기)·합지=종이 인쇄→보드 라미네이트 공정·검정포맥스/검정폼보드=자재#1 색/면 variant·코팅 가용성(HAP=True/PRT=False)=#12가 후가공 게이팅. **★결정적 — 제작방식=인쇄방식의 일종 → 이미 #12로 등재됨(NC 인쇄방식 #18 부결과 정확히 동형·중복 신축 금지)·#12의 5번째 인코딩**(BN 자재facet·ST/PR pdtCode·CL 상품내옵션·NC item_gbn 토큰·PO 제작방식 pdtCode+paper variant). ★directive 최대 관전 2(자립구조=등신대 거치대/피켓이 #18인가): **(PO-2)=형상#17(모양재단 CUT_ZUN_ZDFRM)+부속물#8(거치대 CDL_DFT add-on)** — 거치대=`opt_use_yn` 선택형 부자재 토글(700/1200/1500mm)이면서 **별 상품 `GS/GSSBMTL/detail/54` 독립 부자재 SKU**(D-1 부속물 가장 명시적 인스턴스·후니 addl_product 정확 매핑)·피켓=거치대/와이어 부자재 *없음*(손잡이 내재·자립방식=형상+부속물 유무로 분해)·PH 거치 부결 동형. **★ST 형상(#17)과 정반대로 #18 부결(HARD 양방향) — 제작방식=①전용 슬롯 부재(pdtCode enum·새 슬롯 0)+②KB 결함 부재(=#12로 이미 1급) 둘 다 불충족(NC 동형)·자립구조=①거치대 슬롯 OBSERVED(CDL_DFT)+②KB 결함 부재(형상#17·부속물#8이 담음) → ②불충족(PH 거치 동형).** ★PO-3 number4_sel(1배~10배)=수량#10/가격기여역할#11(면적계수) 경계 data-gap(real_price 엔진 실측 unobserved·huni-widget 영역). ★PO-4 사이즈 직접입력(비규격 면적)=사이즈#13 면적 연속차원(가로×세로 구간·BN/실사 면적매트릭스 siz_width/siz_height·이산 siz_cd 아님·data-gap). ★dbmap 정합: rpmeta "제작방식=#12·자립=형상#17+부속물#8·면적=사이즈#13 면적축·신축 #18 부결" = 메모리 `dbmap-print-method-not-absolute-axis`(인쇄방식≠절대축·시트가 1차 단위)·`dbmap-area-matrix-wh-dimension`(면적매트릭스) — 같은 결론 다른 렌즈·상호 보강(PO 합지가 별 가격엔진 없이 real_price 공유=제작방식이 시트 속성). 총 **17축 유지**. 13번째 카테고리(경질 기재 POP·등신대·피켓·directive 최대 관전 2건)가 distinct 0 = *가장 강력한 기재마운팅/자립구조 후보 2건*조차 무손실 흡수(관측 기반 양방향 부결). PO가 더한 것 = #12 5번째 인코딩(제작방식)·자재#1 검정 variant+기재×두께×색 합성·부속물#8 "거치대 add-on이 독립 부자재 SKU 참조" 강화·형상#17 모양재단 PO 인스턴스·사이즈#13 면적 연속차원·공정#2 타일링(BN/FS 동형).
> - **★v12.0 (OT 통합):** + 상자/패키징(OT·케이크/납작/반달/봉투/답례품상자 + 클래퍼 비박스 대비) 대표5박스+1비박스 `[live:detail]`(SSR select·infoCall 0·주문/POST 0). BN·GS·TP·PR·ST·CL·AC·PD·PH·FS·NC·OT **12 상품군 증거**로 검증. **★OT distinct 신축 0종 — 17축 재포화(PR·CL·AC·PD·PH·FS·NC 패턴 반복·★전개도/dieline #18 부결).** OT=**선별 모드 핵심 프로브** — 박스/패키징은 *평면 인쇄물에 전무했던 입체(3D 가로×세로×높이)·전개(평면 net/dieline)·접지(칼틀+오시)·접합* 차원군을 가진 **첫 상품군**(가장 강력한 입체/전개 #18 후보). 4 fragment(O-1~O-4) 적대 판정 후 전부 facet/data-gap. **★결정적 — 박스 옵션 모델은 평면 인쇄물(PR/NC)과 paper/paper_sub/sodu/size/number *100% 동일 슬롯***(`[live:detail]` 5박스 일치·전개도/접지/3D 전용 관리 슬롯 0건) → 전개도=사이즈#13(작업치수=전개도 평면)+공정#2(도무송 칼틀·오시 접지선)+카테고리#7(박스형태=pdtCode 분기) 무손실 분배. ★directive 최강 적대 프로브(전개도가 distinct #18인가) 적대 판정 = **부결.** **★ST 형상(#17)과 정반대 — 형상은 ① 전용 슬롯(shape_info·사이즈와 분리) 실재 + ② 후니 KB G-SK-2 "어느 축에도 없음" 결함 둘 다 충족(승격), 전개도 #18은 ① 전용 슬롯 부재(100% 동일 base-data 슬롯·새 슬롯 0) + ② KB 결함 부재(작업/재단치수=size#13+plate_size·도무송/오시=공정#2가 왜곡 없이 담음) 둘 다 불충족 → 부결.** ★PH-2 거치·FS-1 타일링(둘 다 ①OBSERVED·②불충족)보다 **OT는 ①조차 불충족**(전용 슬롯 자체 부재)=*더 약한 후보의 더 깨끗한 부결*. ★O-3 결정적 분기(재단/작업 2치수=distinct vs data-gap): 박스 재단(485×270)/작업(495×280) 2치수는 후니가 *이미 2 별개 1급 축*(`entity-semantic-model.md:22` size=재단치수·`:27` plate_size=작업/전지 판형[임포지션·판걸이])으로 분리 모델링한 두 차원에 매핑 = **사이즈 2치수 표현력 data-gap**(NC 이산tier·PD-4·FS-1·PH-2 data-gap 동형·vessel-gap 아님). ★O-1 3D 제품치수=size 미저장 파생(앱 계산·`dbmap-compute-in-app-db-stores-lookup` 동형·선택 옵션 아님). ★O-2 dieline 에디터 템플릿=#16 TemplateAsset(D-11·"구조 dieline" sub-type 여부=응답 스키마 unobserved·validator 실측). **★핵심 비대칭(HARD): 형태가공#14(GS/PD)=RP가 평면→입체 *생성* ↔ 박스=평면(접지 평면지)까지만 RP 생산·입체 조립은 고객 수작업("납작하게 접힌 상태 배송")=#14의 *정반대*** → 박스 접합/조립은 흡수처조차 불요(RP 생산 범위 밖·형태가공#14가 "본체 생성"이지 "모든 입체화" 아님을 음의 사례로 확정). ★dbmap 정합: rpmeta "전개도=size#13+plate_size 2축 data-gap·신축 #18 부결" = 메모리 `dbmap-platesize-is-output-paper`·G-GP-2 "size(재단)↔plate_size(작업) 이미 별개 t_* 축" — 같은 결론 다른 렌즈·상호 보강(박스가 두 축 분리 설계를 입체물에서 역검증). 총 **17축 유지**. 12번째 카테고리(상자/패키징·평면 인쇄물에 전무했던 입체/전개 차원)가 distinct 0 = *가장 강력한 입체/전개 distinct 후보*조차 무손실 흡수(①전용 슬롯조차 부재한 가장 깨끗한 부결). OT가 더한 것 = 사이즈#13+plate_size 2축 분리 재확인(O-3·박스가 입체물에서 역검증)·공정#2 도무송/오시=ST 칼선/PR 접지의 박스 인스턴스·형태가공#14 비대칭 음의 사례·#16 dieline 자산·3D 미저장 파생 패턴.
> - **★v11.0 (NC 통합):** + 옵셋 명함·카드·쿠폰·포토카드(NC·전부 옵셋 인쇄방식) 대표3+디지털 BC 동형 대비. BN·GS·TP·PR·ST·CL·AC·PD·PH·FS·NC **11 상품군 증거**로 검증. **★NC distinct 신축 0종 — 17축 재포화(PR·CL·AC·PD·PH·FS 패턴 반복·★인쇄방식(옵셋 vs 디지털) #18 부결).** NC=**선별 모드 핵심 프로브** — 옵셋 명함(NCDFDFT/FLD/PHO)이 디지털 명함(BC·BCSPDFT)과 *동일 상품군이되 인쇄방식만 옵셋*인 자연 실험. 옵셋은 **별도 가격엔진(offset2023_price)·자재종속 이산 부수 tier(MTRL_CD×PRN_CNT 100~500·자유입력 불가)·옵셋전용 자재 pool(RXWMO220)**까지 가진 *가장 강력한 인쇄방식 distinct #18 후보*. 5 fragment(N-1~N-5) 적대 판정 후 전부 facet/관계 간선. **★결정적 — 인쇄방식은 *이미 메타모델 #12(D-7 인쇄방식/생산 레시피·조건부 distinct)로 v2.0(BN)에 등재됨* → 신축 #18은 #12 중복.** NC가 발견한 옵셋 차이는 새 #18 축이 아니라 **기존 #12를 토큰 레벨(`offset2023_*`)로 재확인 + #11 가격모델 라우팅(offset2023_price) + #10 수량 이산 tier·#5 제약 분산**으로 전부 흡수. ★directive 최강 적대 프로브(인쇄방식이 distinct #18인가) 적대 판정 = **부결.** **★ST 형상(#17)과 정반대 — 형상은 ① 전용 슬롯(shape_info·사이즈와 분리) 실재 + ② 후니 KB G-SK-2 "어느 축에도 없음" 결함 둘 다 충족(승격), 인쇄방식 #18은 ① 전용 슬롯 부재(item_gbn/price_gbn=enum 토큰·옵셋 NC와 디지털 BC가 base-data 슬롯 100% 동형 공유·새 슬롯 0) + ② KB 결함 부재(인쇄방식=#12로 이미 1급·process-recipe §1 PROC_000002~6·왜곡 없이 담음) 둘 다 불충족 → 부결.** ★N-2 결정적 분기: 옵셋 이산 부수 tier(자재종속 허용부수)는 인쇄방식 축 증거가 아니라 **수량#10 이산 슬롯+제약#5(자재→허용부수)+가격#11(부수 tier=단가구간) 표현력 data-gap**(연속 increment만 가정한 후니가 이산 tier 미적재·PD-4·FS-1 data-gap 동형·vessel-gap 아님). ★dbmap 정합: rpmeta "인쇄방식=#12 게이팅 축·신축 #18 부결" = 메모리 `dbmap-print-method-not-absolute-axis` "인쇄방식≠절대 최상위 축·시트가 1차 단위" — 같은 결론 다른 렌즈(메타모델 추상화 vs 적재/이해 단위)·상호 보강. 총 **17축 유지**. 11번째 카테고리(옵셋 명함/카드)가 distinct 0 = *가장 강력한 인쇄방식 distinct 후보*조차 무손실 흡수(가장 깨끗한 부결·신후보가 아니라 기존 #12 토큰 재확인). NC가 더한 것 = #12 인쇄방식 4번째 인코딩(BN 자재facet/ST·PR pdtCode/CL 상품내옵션/**NC item_gbn·price_gbn 토큰 bundle**)·#11 offset2023_price 라우팅 추가·#10 이산 tier 모드 data-gap 명시·#12 자재풀 게이팅(P-7) 재확인.
> - **★v10.0 (FS 통합):** + 패브릭·봉제 완제 직물 굿즈(FS·면직물에 풀프린팅 후 재단·봉제·마감하는 직물 굿즈·코스터/포스터(현수막형)/쿠션/파우치/에코백/테이블보/스카프/스크런치) 대표5+16횡단. BN·GS·TP·PR·ST·CL·AC·PD·PH·FS **10 상품군 증거**로 검증. **★FS distinct 신축 0종 — 17축 재포화(PR·CL·AC·PD·PH 패턴 반복·★타일링 TILL_WH_GBN #18 부결).** FS reverse가 "타일링(반복패턴 인코딩)=distinct #18 후보"를 유일 후보로 제기(BN/GS/TP/PR/ST/CL/AC/PD/PH 전 9 카테고리 어디에도 없던 `TILL_WH_GBN` 전용 라디오 슬롯·5상품 전수)했으나 **공정#2(인쇄 배치/조판) facet으로 무손실 흡수.** ★directive 1순위 적대 판정(타일링이 distinct #18인가): **(타일링)=공정#2 인쇄 배치 파라미터(#9 종속)** — 직물 풀프린팅의 "디자인 패턴 세로/가로 반복(tiling/repeat) 배치"는 *인쇄 면배치/조판(imposition) 공정 파라미터*이지 별 관리 축 아님. **★ST 형상(#17)·PH 거치(부결)와 같은 HARD 양방향 기준 적용 — ① 전용 슬롯 라이브 실재(TILL_WH_GBN 명시 라디오·5상품 OBSERVED=충족) + ② 후니 KB가 기존 축으로 무왜곡 흡수 불가(결함)** → **②가 불충족**: 후니 KB `plate_size`(#6·`entity-semantic-model.md:27` "작업/전지 판형·임포지션·판걸이·돔보")가 "작업물을 원판에 어떻게 배치하나(조판)"를 *이미 1급 모델링*, 타일링=그 배치 차원의 고객 선택값(세로/가로 반복)이 공정#2 인쇄 배치 파라미터(#9·접지 FLD_DFT·오시 줄수 동형)로 *왜곡 없이 담김*. 후니 KB에 "패턴 반복/배치 어느 축에도 없음" 같은 결함 명시 **없음**(plate_size/공정 파라미터가 담음) → ②불충족 → distinct 0. **★주의(경계 HARD): 타일링≠판걸이수**(판걸이수=앱 계산·DB 미저장·메모리 `dbmap-compute-in-app-db-stores-lookup`) — 판걸이수는 *생산측 면배치 계산 결과*, 타일링은 *고객측 디자인 반복 배치 선택 입력*. 둘 다 plate_size/조판 도메인의 facet이나 타일링은 고객 입력 파라미터(공정#2 배치 param), 판걸이수는 그 입력+사이즈로 앱이 계산하는 파생값(DB 미저장). 타일링은 입력이므로 공정 파라미터#9로 등재(판걸이수는 등재 금지). 나머지 FS fragment(FS-2~8) 전부 facet: **(FS-2 방향 PAPER_WH W/H)=사이즈#13 방향 facet** · **(FS-3 면사 수 PXFBW0NN)=자재#1 PTT 직물 차원+WGT 다의(면사 수=평량)·CL oz·PD 원단·AC mm 합류** · **(FS-4 별색 SID_FBR 6색×3농도)=공정#2 별색 family**(CL Pantone 1124 축소판·직물 날염 제한 도메인=기초코드#6 색 enum·round-22 별색=공정) · **(FS-5 마감봉제 SEW_FBR·제품가공 PDT_WRK)=공정#2/본체형태가공#14 family**(PD SEW_LTR·GS PDT_WRK 동형·라벨/끈/포켓/자석 FBR-접미=부속물#8 BUNDLE) · **(FS-6 솜/끈/자석 완제 부자재)=자재#1 sub_mtrl+부속물#8**(일부 옵션노출=PD-4 고정부속과 분기·선택형 부자재) · **(FS-7 가격모델 real_price/real_calc_price)=가격#11 라우팅**(완제 봉제 굿즈도 면적/실계산·PD tmpl과 분기) · **(FS-8 PCS 상세 enum/단가)=infoCall unobserved·축 판정 무영향**. RP가 타일링을 공정 배치 라디오로 둠 = data-gap(후니가 직물 타일링 취급 시 공정#2 인쇄 배치 파라미터에 적재)이지 vessel-gap(축 부재) 아님(갭분석가). 총 **17축 유지**. 10번째 카테고리(직물 풀프린팅+봉제 완제 굿즈·가장 이질적)가 distinct 0 = 모델 재포화 재확인 — 유일 신규 후보 타일링조차 무손실 흡수. ★FS의 진짜 기여 = ① 자재#1에 "면직물·면사 수(綿絲 count)" 평량 단위 입증(지종×평량 외 직물 차원) ② 공정#2에 "마감봉제(edge finish: 오버로크/말아박기/벨크로)" family 멤버 + "인쇄 타일링(반복 배치)" 파라미터 입증 ③ 가격#11 라우팅이 완제 봉제 굿즈도 real_calc_price(면적/실계산).
> - **★v9.0 (PH 통합):** + 포토보드·액자·사진인화·포토북·포토굿즈(PH·사진을 어떤 물성으로 출력하느냐로 묶인 출력매체 카테고리·5개 이질 상품군) 대표3+30횡단. BN·GS·TP·PR·ST·CL·AC·PD·PH **9 상품군 증거**로 검증. **★PH distinct 신축 0종 — 17축 재포화(PR·CL·AC·PD 패턴 반복·★완제 액자 그릇/마운팅 PH-1·PH-2 #18 부결).** PH reverse가 1차 예측한 "distinct 0(facet)·미확정(SSR-negative 블로커)"를 **§0.5 client-render 재캡처(gstack browse 2026-06-17)가 블로커 해소 후 적대 판정으로 비준** — 가장 distinct로 *보이던* 완제 액자(인쇄물 + 별도 프레임 2-파트 조립)조차 17축 무손실 흡수. ★directive 최대 관전(완제 프레임·마운팅/거치·전면 보호재=distinct #18인가) 적대 판정: **(PH-1 완제 프레임)=완제SKU#4(거치+마감+사이즈 인코딩)+자재#1 프레임재질 variant+생산형태#15**(AC 두께/소재 variant·GS 완제 본체 SKU G-1 동형·프레임재질 pdtCode 분기=GS 코스터 G-2 동류) · **(PH-2 거치 RESOLVED OBSERVED)=옵션#3 캐스케이드 상위 차원**(§0.5 탁상용/벽걸이 버튼 토글·거치→마감→완제SKU사이즈→수량·전면재=자재#1 디아섹 내재·후면받침=부속물#8 미관측) · **(PH-3 인화지×마감)=자재#1 surface-finish**(§0.5 "인화용지(반광-러스터)/유광" 합성·ST S-4/AC A-2 동형) · **(PH-4 set 단위 600매/4·5sheets)=수량#10 set 배수+완제SKU#4 base_quant**(GS 텀블러 동형) · **(PH-5 머그·화분)=카테고리#7 다중분류**(GS 코드접두≠본질 동형) · **(PH-6 형태 일반/정사각/파노라마)=사이즈#13 비율 프리셋 흡수·형상#17 부결**(§0.5 OBSERVED·1:1·ST 1:多 미충족·PD-3 동형). ★PH-1/PH-2가 ST 형상(#17)과 정반대로 부결된 결정적 근거(HARD 기준 양방향) = **형상은 ① 전용 슬롯(shape_info) 실재 + ② 후니 KB G-SK-2 "어느 축에도 없음" 결함 둘 다 충족(승격), PH 완제 그릇/마운팅은 ① 거치 전용 슬롯 §0.5 OBSERVED(미싱데이터 해소)되었으나 ② 후니 KB 결함 명시 없음**(완제SKU#4·옵션#3·자재#1·사이즈#13이 왜곡 없이 담음) → ②가 불충족 → distinct 0. **★§0.5 재캡처가 블로커를 OBSERVED로 해소했음에도 거치가 옵션 캐스케이드 + 완제 SKU variant로 구현됨이 실측 → facet 결론이 미싱데이터 해소 후에도 강화**(추정 아닌 관측 기반 부결). RP가 완제 프레임/거치를 완제 SKU combobox + 옵션 캐스케이드로 둠 = *data-gap(완제 SKU/거치 캐스케이드 그릇 미적재)이지 vessel-gap(축 부재) 아님*(PD-4 data-gap·AC A-3 PASS 동형·갭분석가). 총 **17축 유지**. 9번째 카테고리(완제 액자/출력매체)가 distinct 0 = directive 최대 관전(완제 그릇/마운팅/거치)도 무손실 흡수.
> - **★v8.0 (PD 통합):** + 스툴·슬리퍼·강아지계단(PD·봉제 구조물/3D 조립 완제품) 3상품 전수. BN·GS·TP·PR·ST·CL·AC·PD **8 상품군 증거**로 검증. **★PD distinct 신축 0종 — 17축 재포화(PR·CL·AC 패턴 반복·★완제 구조물 내재BOM PD-4 #18 부결).** PD reverse가 1차 예측한 "distinct 0(8번째 재포화)"를 적대 판정으로 비준 — 가장 이질적 *봉제 구조물 완제품*조차 17축 무손실 흡수. ★directive 최대 관전(조립·구조·3D폼·완제 내재BOM=distinct #18인가) 적대 판정: **(PD-1 봉제/제품가공)=본체 형태가공#14(GS D-10) "봉제(SEW_LTR 레더재봉)" family 신규 멤버**(PDWRSLP PDT_WRK=GS 동일 코드·기존 PCS 슬롯 인코딩) · **(PD-2 직물/PU 원단)=자재#1 PTT 차원**(면10수/슬리퍼원단/PU폴리우레탄·AC 아크릴·CL 의류원단 동형·비종이 자재 이미 관측·면10수=번수 평량 다형성) · **(PD-3 단수/형상)=사이즈#13 프리셋 흡수**(계단 2단=495×320 1:1·스툴 원형↔305×305 1개=ST 형상 1:多 미충족=구조 distinct 부결 결정적 증거) · **(★PD-4 완제 구조물 내재BOM[다리/받침/솜/지퍼/논슬립])=부속물#8(고정 ESN=Y 완제 부속)+자재#1 usage(솜/지퍼)+생산형태#15(C 완제품 governing)+가격#11(tmpl) 분산 facet·#18 부결** · **(PD-5 모양커팅/추가부자재 enum)=공정#2/부속물#8·infoCall unobserved·축 판정 무영향).** ★PD-4가 ST 형상(#17)과 정반대로 부결된 결정적 근거 = **형상은 후니 KB G-SK-2 "형상 어느 축에도 없음" 결함이 distinct 강제, 완제 내재BOM은 후니 KB가 addl_product(#9)·자재 usage·생산방식 A/B/C를 이미 1급 모델링(결함 없음·왜곡 없이 담음)** = PD가 던진 질문(완제 내재BOM을 옵션과 분리해 어디 두는가)의 답=네 기존 축 분담. RP가 내재BOM을 마케팅 카피로만 둠 = *data-gap(부속물/생산BOM 그릇 미적재)이지 vessel-gap(축 부재) 아님*(갭분석가). 총 **17축 유지**. 8번째 카테고리(봉제 구조물 완제품)가 distinct 0 = 가장 이질적 카테고리도 무손실 흡수.
> - **★v7.0 (AC 통합):** + 아크릴·키링·코롯토·명찰·등신대(AC) 대표3+17횡단. BN·GS·TP·PR·ST·CL·AC **7 상품군 증거**로 검증. **★AC distinct 신축 0종 — 17축 재포화(PR·CL 패턴 반복·신규 강후보 A-8 부결).** AC reverse가 "가공방식 그룹핑 슬롯(GRP_OPTION_CD/production_method)=distinct #18 강후보"를 강하게 제기했으나 **9 fragment(A-1~A-9) 전부 기존 17축 facet/family/cascade/정책으로 무손실 흡수.** ★directive 4 관전 적대 판정: **① 두께(3T/5T/8T)=자재#1 WGT facet**(WGT 슬롯 다의성 평량/두께·[huni-ref] 후니가 투명3T/1.5T를 mat_cd 차원 통합 1.5T=3T×0.8 동형) **② 소재variant(글리터/거울/자개/홀로그램/렌티큘러/파스텔/유색)=자재#1 surface-finish 합성 facet**(ST S-4 점착/내후 동형·거울 별공식=#11 라우팅 not 자재분류) **③ 입체/스탠드=분산 facet**(받침=부속물#8 평면본체 유지=생산형태#15 아님·본체생성 아님=형태가공#14 아님·코롯토 두께블록=자재#1·양면=옵션#3·입체조형=공정#2) **④ ★가공방식 그룹핑(일반/라미)=공정#2(라미)+자재#1 합성(라미 결과)+옵션#3 cascade(가공방식→자재 subset 게이팅) facet·#18 부결.** ★A-8이 ST 형상(#17)과 정반대로 부결된 결정적 근거 = **형상은 후니 KB G-SK-2 "형상 어느 축에도 없음" 결함이 distinct 강제, 가공방식은 기존 축이 왜곡 없이 담음**(라미=공정 멤버 이미 수용·KB 결함 없음). 나머지 A-4(부착물=부속물#8+공정#2 부착·고리 KR/CN/CR ST 공유=단일 부자재 마스터)·A-5(인쇄면+화이트=옵션#3+공정#2+제약#5·ST S-7)·A-6(3 가격엔진=가격#11 acrylic2025 라우팅)·A-7(명찰 PET+합지=자재#1+공정#2·G-1/CL 라벨 융합)·A-9(ACTPKEY=#16 TemplateAsset·T-A 이중의미). 총 **17축 유지**. 7번째 카테고리(아크릴 두께/입체/가공방식)가 distinct 0 = 가장 강한 새 후보 A-8조차 무손실 흡수.
...
>
> 축 총 **17개 = 7 정적 축(자재·공정·옵션·템플릿·제약·기초코드·카테고리) + 4 관계/동역학 축(부속물·공정파라미터·수량모델·제약논리) + 2 횡단 축(가격기여역할·인쇄방식레시피) + GS 신축 2(생산형태·본체 형태가공) + TP 신축 1(디자인 입력 채널) + ST 신축 1(형상 #17)**. 발굴 근거=`discovered-axes.md`. **PR distinct 0(4군 포화)→ST distinct 1(형상·5군 포화 붕괴)→CL distinct 0(6군 재포화)→AC distinct 0(7군 재포화·신규 강후보 A-8 가공방식 그룹핑 무손실 흡수)→PD distinct 0(8군 재포화·봉제 구조물 완제품·신규 강후보 PD-4 완제 내재BOM 무손실 흡수)→PH distinct 0(9군 재포화·완제 액자/출력매체·신규 강후보 PH-1/PH-2 완제 그릇/마운팅 무손실 흡수·★§0.5 거치 OBSERVED 블로커 해소 후에도 옵션 캐스케이드 구현)→FS distinct 0(10군 재포화·직물 풀프린팅+봉제 완제 굿즈·유일 신규 후보 타일링 TILL_WH_GBN 무손실 흡수·공정#2 인쇄 배치 파라미터)→NC distinct 0(11군 재포화·옵셋 명함/카드·★최강 #18 후보 인쇄방식(옵셋 vs 디지털)이 이미 #12 D-7로 등재·별도 가격엔진 offset2023_price=#11·이산 부수 tier=#10/#5/#11 분산 data-gap·신축 #18 중복 부결)→OT distinct 0(12군 재포화·상자/패키징·★평면 인쇄물에 전무했던 입체/전개 차원을 가진 첫 상품군의 #18 후보 전개도/dieline이 사이즈#13(작업치수)+공정#2(도무송 칼틀·오시 접지)+카테고리#7(박스형태 분기)로 무손실 분배·3D 제품치수=size 미저장 파생·재단/작업 2치수=size#13+plate_size 2축 data-gap·dieline 에디터 템플릿=#16 TemplateAsset·①전용 슬롯조차 부재한 가장 깨끗한 부결·형태가공#14 음의 사례)=모델은 카테고리 증거에 정직(포화·진화·재포화 전부 증거 강제).**
>
...
- **★FS 확장 — 면직물 PTT + 면사 수(綿絲 count) WGT 다의성 + 솜 충전재 sub_mtrl (v10.0·FS-3/FS-6):**
  - **★면직물 = 자재#1 PTT 직물 차원(비종이 자재·별 축 아님):** FS 본체 자재(`paper` select·`[live:SSR]` 실측)가 면10수/20수/40수/60수 화이트(MTRL_CD=PXFBW0NN·FB=fabric·W=white·NN=수)로 분기 — 종이(지종×평량 g/m²)가 아닌 *면직물(cotton)*. 비종이 자재(AC 아크릴·CL 의류원단·PD 직물/PU)와 합류한 자재#1 PTT 차원 값 확장. 색은 화이트 1종(인쇄로 컬러 구현·CL 54색 스와치와 대조=직물은 풀프린팅 매체).
  - **★면사 수 = WGT 슬롯 다의성(평량의 직물판·별 축 아님):** 면사 수(綿絲 count·10/20/40/60수)가 직물의 *굵기/질감 차원*으로 종이 평량(g/m²)·아크릴 두께(mm·AC A-1)·CL oz와 같은 WGT 슬롯 다의 사용 — **WGT 슬롯이 평량(종이 g)·두께(아크릴 mm)·oz(의류)·번수(직물 면사 수)를 도메인별 의미로 다의 인코딩**(PD 면10수=번수 평량 다형성 합류). 자재 합성코드 WGT 차원의 정당한 다형성(별 "면사 축" 거부). entity-semantic-model #2 "두께=자재 식별자" 원칙의 직물판 — `measure_type`(평량/두께/oz/번수) 구분 vessel 검토(갭분석가·AC A-1과 합류).
  - **★솜 충전재(SUB_MTR_SUB_SELECT TN001 사각쿠션솜) = 자재#1 sub_mtrl + 부속물#8 (FS-6):** 쿠션(FSCUDFT) 솜=완제 쿠션 충전 부자재(`[live:SSR]` icon_txt "쿠션가공"·선택안함 가능=옵션 노출). PD-4 완제 내재BOM(솜/지퍼=자재 sub_mtrl)·AC 받침(부속물#8) 동형 — 단 *옵션 노출*(선택형 부자재·view_yn=Y·PD-4 고정 ESN=Y와 분기). 솜=충전 자재 sub_mtrl(usage), 끈/자석=부속물#8(아래·#8 FS 확장). **★도메인 경계:** 직물 굿즈 본체 자재(면직물)와 완제 부자재(솜/끈/자석)를 분리 — 본체=PTT 자재, 부자재=sub_mtrl/부속물#8.
  - **도메인 정합:** 후니 자재모델(`entity-semantic-model.md:22` material = 종이/소재/부속 + usage_cd + 두께)에 *직물 물성 차원(면사 수·신축성·짜임) 부재* = round-22 굿즈 본체소재 부재 결함과 연결(FS-3). FS가 드러낸 자재#1 직물 물성 갭(GS 본체소재·PD 원단·AC surface-finish와 합류·갭분석가).
...
- **★ST 확장 — 칼선 2메커니즘 + 재단입자 family (v5.0·S-2/S-3) ★directive #1/#2 — facet 확정:**
  ...
  - **★전개도/dieline(박스 작업사이즈) = 사이즈#13(작업치수) + 공정#2(도무송 칼틀·오시 접지) + 카테고리#7(박스형태 분기) 분배 흡수 (OT·v12.0·O-1~O-3·★전개도 #18 부결·재포화):** 박스(OTPKCAK/FLT/HMN/ENV/ARP)는 *평면 인쇄물에 전무했던 입체(3D)·전개(평면 net)·접지(칼틀+오시)·접합* 차원을 가졌으나 옵션 모델이 평면 인쇄물(PR/NC)과 **paper/paper_sub/sodu/size/number 100% 동일 슬롯**(`[live:detail]` 5박스 일치·전용 관리 슬롯 0건). 전개도=칼틀의 박스 인스턴스(ST 칼선=공정#2 facet 동형): ① 작업사이즈(전개도 펼친 평면 칼틀 495×280)=사이즈#13 프리셋("케익상자(소)") + plate_size(작업판형) ② 도무송 칼틀·오시 접지선=공정#2 멤버(ST 칼선·PR 접지의 박스판) ③ 박스형태(케이크/납작/반달/봉투)=카테고리#7 상품 pdtCode 분기(별 옵션축 아님·size 프리셋 라벨 융합). **★ST 형상(#17)과 정반대로 #18 부결 — ① 전용 슬롯 부재(100% 동일 base-data 슬롯) + ② KB 결함 부재(작업/재단=size#13+plate_size·도무송/오시=공정#2 왜곡 없이 담음) 둘 다 불충족(PH-2 거치·FS-1 타일링은 ①OBSERVED였으나 OT는 ①조차 부재=가장 깨끗한 부결).** ★O-3 재단/작업 2치수(재단 485×270 vs 작업 495×280·여백 10mm)=후니가 *이미 2 별개 1급 축*(`entity-semantic-model.md:22` size=재단치수·`:27` plate_size=작업/전지 판형[임포지션·판걸이])으로 분리 모델링한 두 차원에 매핑=**사이즈 2치수 표현력 data-gap**(NC 이산tier·PD-4·FS-1·PH-2 동형·vessel-gap 아님). ★O-1 3D 제품치수(130×105 높이80)=size 미저장 파생(앱 계산·전개도 접기 비선형 변환·`dbmap-compute-in-app-db-stores-lookup` 동형·선택 옵션 아님). ★O-2 dieline 에디터 템플릿=#16 TemplateAsset(D-11·"구조 dieline" sub-type 여부=응답 스키마 unobserved·validator 실측). **★핵심 비대칭(HARD): 형태가공#14(GS/PD)=RP가 평면→입체 *생성* ↔ 박스=평면(접지 평면지)까지만 RP 생산·입체 조립은 고객 수작업("납작하게 접힌 상태 배송")=#14의 *정반대*** → 박스 접합/조립은 흡수처조차 불요(RP 생산 범위 밖·형태가공#14가 "본체 생성"이지 "모든 입체화"는 아님을 박스가 음의 사례로 확정).
...
- **★FS 확장 — 인쇄 타일링(반복 배치) 파라미터 + 마감봉제 family + 현수막 행잉/봉 가공 + 제품가공 (v10.0·FS-1/FS-5/FS-7) ★directive 1순위 — 타일링 facet 확정:**
  - **★★타일링(TILL_WH_GBN) = 공정#2 인쇄 배치 파라미터(#9 종속·★distinct #18 부결):** `TILL_WH_GBN` 라디오(TIL_NON 없음/TIL_HGH 세로/TIL_WDT 가로·`[live:SSR]` 5상품 전수 실측)가 직물 풀프린팅의 *디자인 패턴 세로/가로 반복(tiling/repeat) 배치* 선택. **BN/GS/TP/PR/ST/CL/AC/PD/PH 전 9 카테고리 어디에도 없던 전용 라디오 슬롯**이라 유일한 distinct #18 후보로 제기됐으나 — **공정#2 인쇄 배치/조판(imposition) 파라미터로 무손실 흡수.** ★ST 형상(#17)·PH 거치 부결과 같은 HARD 양방향 기준: ① 전용 슬롯 라이브 실재(TILL_WH_GBN 명시 라디오·5상품 OBSERVED=**충족**) + ② 후니 KB가 기존 축으로 무왜곡 흡수 불가(결함)=**불충족**. 후니 KB `plate_size`(#6·`entity-semantic-model.md:27` "작업/전지 판형·**임포지션·판걸이**·돔보")가 "작업물을 원판에 어떻게 배치하나(조판/임포지션)"를 *이미 1급 모델링*·`pdf-domain-knowledge.md:146` "조판(임포지션)=작업물을 원판에 판걸이 개수만큼 앉히는 행위" 정합 → 타일링=그 배치 차원의 고객 선택값(세로/가로 반복)이 *인쇄 배치 공정 파라미터(#9·접지 FLD_DFT 면분할·오시 줄수 동형)*로 왜곡 없이 담김. 후니 KB에 "패턴 반복/배치 어느 축에도 없음" 같은 결함 명시 **없음**(ST 형상 G-SK-2 같은 결함과 정반대) → ②불충족 → distinct 0.
  - **★★타일링 ≠ 판걸이수 (경계 HARD·핵심 구분):** 타일링은 *고객측 디자인 반복 배치 선택 입력*(없음/세로/가로 라디오)이고, **판걸이수(imposition count)=앱 계산(DB 미저장)**(`pdf-domain-knowledge.md:149` "판걸이=원판당 앉히는 작업물 개수"·메모리 `dbmap-compute-in-app-db-stores-lookup` "판걸이수=앱 계산 DB미저장"). 둘 다 plate_size/조판 도메인의 facet이나 — 타일링=*입력 파라미터*(공정#2 인쇄 배치 param·공정 파라미터#9로 등재), 판걸이수=그 입력+사이즈로 *앱이 계산하는 파생값*(DB 미저장·등재 금지). 임포지션은 N개 *다른* 작업물을 한 원판에 앉힘(다도안 판걸이), 타일링은 한 디자인을 *반복* 배치 = 직물 풀프린팅 특유의 면 채움 방식. **★메타모델 정답: 타일링=공정 파라미터#9(인쇄 배치 입력값)·판걸이수=앱계산(DB 미저장)** — 타일링을 별 "반복 배치 축"으로 1급화 금지(공정 배치 파라미터·접지 면분할/오시 줄수와 동근).
  - **★마감봉제(SEW_FBR_SUB_RADIO) = 공정#2 마감봉제 family (FS-5·PD SEW_LTR 동형):** 가장자리 마감 4종(RNDFT 기본/오버로크·RNRIN 리본/말아박기·RNSML 작은마감·RNVEL 벨크로·`[live:SSR]` icon_txt "얇은오버로크2mm/두꺼운오버로크4mm/말아박기1cm") = 직물 *가장자리 마감(edge finish)* 봉제. PD SEW_LTR(레더재봉)·CL 봉제와 동형 공정#2 봉제 family — FS는 edge finish 특화 멤버(오버로크/말아박기/벨크로) 신규 등재. 제품가공(PDT_WRK·쿠션/에코백/파우치/스크런치 가공)=본체 형태가공#14 family(GS PDT_WRK·PD 동형·아래 #14 FS 확장).
  - **★현수막 행잉/봉 가공 (FS-7·BN family 상속):** 패브릭 포스터(FSSQPST)=*천 현수막* → icon_txt "행잉가공·테두리가공·봉-일반형·봉-고리형·작은고리·벨크로·hooker_selectall"가 BN 현수막 행잉/봉/고리/거치 후가공과 **완전 동형**(공정#2 현수막 마감 family 상속 + 부속물#8 봉/고리). 즉 "패브릭 포스터"는 FS 카테고리이면서 BN 현수막 가공축을 그대로 상속(카테고리⊥공정 family).
  - **★재단/포장 멤버 (FS):** CUT_ZUN(재단/정사이즈재단/규격모양재단)=공정#2 재단 + 사이즈 cascade·PAK_POL(폴리백 개별포장)=공정#2 포장. ST/GS/PD 동형 멤버.
...
- **★PD 확장 — 고정(미노출·ESN=Y) 완제 부속 + 완제 구조물 내재BOM facet (v8.0·PD-4) ★directive 핵심:**
  - **★"옵션 미노출 고정 완제 부속" = 부속물#8의 facet(별 축 아님):** 봉제 구조물 완제품(스툴/슬리퍼/강아지계단)의 내재BOM 부품 — 다리(소나무 원목)·발받침·미끄럼방지 패드·바닥 논슬립(실리콘 미끄럼방지 원단)·받침 구조 = *본체와 분리된 완제 부속 부품*이나 **주문 옵션에 미노출**(RP 고정 제조 사양·`[live:SSR-marketing]` 마케팅 카피에만 등장·고객 미선택). = 부속물#8(addl_product)의 *고정(필수·고객 미선택) facet* — AC 등신대 받침(ESN_YN=Y 필수 부속물·A-3)·BN 거치대(D-1)와 동형 부속물이되 *옵션 노출 없이 본체 정체에 고정*. "옵션 미노출"은 부속물의 *필수성/노출 차원*(ESN=Y + view_yn=N)이지 새 관리 축이 아님.
  - **★완제 구조물 내재BOM = 분산 facet(부속물#8 + 자재#1 usage + 생산형태#15 + 가격#11·#18 부결):** 다리/받침/논슬립 패드=부속물#8(고정 부속)·솜 충전/지퍼=자재#1 sub_mtrl usage + 공정#2(봉제 consumes·FLX_ZIP 패턴)·"옵션 미노출 고정 제조" 성격=생산형태#15(C 완제품 governing·내재 BOM은 완제품 정체에 묶인 고정 레시피)·완제 단가=가격#11(tmpl_price 내재 BOM 포함 개당단가). 네 기존 축이 *왜곡 없이* 담음 → distinct "완제 구조물 내재BOM 축 #18" 거부.
  - **★ST 형상(#17)과 정반대 — 후니 KB 결함 유무가 분기:** 형상(#17)은 후니 KB G-SK-2 "형상 enum 어느 축에도 없음" 결함이 distinct 강제. 완제 내재BOM은 후니 KB가 **addl_product(#9·`entity-semantic-model.md:30` 완제 부속: 거치대·우드봉·볼체인)·자재 usage_cd(#2·`:23`)·생산방식 A/B/C(#15·`:113`)를 이미 1급 모델링**(결함 명시 없음) → 기존 축이 왜곡 없이 담음 = facet. **★[중요·갭분석가] RP가 내재BOM을 마케팅 카피로만 둠 = data-gap(부속물#8/생산BOM 그릇에 적재해야 할 다리/받침/논슬립·솜/지퍼를 미적재)이지 vessel-gap(축 부재) 아님.** 후니가 완제 구조물 취급 시 부속물#8(다리/받침/논슬립=고정 ESN=Y·view_yn=N)·자재 usage(솜/지퍼 consumes)에 적재 = 기존 그릇 채우기(새 그릇 불요).
  - **★FS 확장 — 직물 굿즈 부자재(끈/자석/라벨/포켓) 부속물#8 + 선택형(옵션 노출) facet (v10.0·FS-6):** 에코백(FSBGECO) 손잡이 끈(LIN_PRT)·자석 잠금(WRK_MTR·AC WRK_MTR 동형)·라벨(LAB_FBR)·내부 포켓(POC_FBR)이 *직물 굿즈 본체와 분리된 완제 부속/부자재* — 끈/자석=부속물#8(BN 거치대·AC 고리 동형)·라벨/포켓=자재(라벨지·포켓 직물)+공정#2(부착 봉제) BUNDLE(AC/ST/PD SUB_MTR 동형). **★PD-4 고정 부속(ESN=Y·view_yn=N)과 분기 — FS 직물 굿즈 부자재는 *옵션 노출*(고객 선택 가능·솜 선택안함·끈 커스텀)** = 부속물 *선택성/노출 차원*(ESN=N·view_yn=Y). 즉 부속물#8이 (a)고정 부속(PD-4 다리/논슬립·AC 받침 ESN=Y) (b)선택형 부자재(FS 끈/자석/솜·view_yn=Y) 두 노출 모드를 가짐(필수성×노출 facet). FBR-접미 슬롯(LAB_FBR/LIN_PRT/POC_FBR/WRK_MTR)=패브릭 전용 부자재 슬롯이나 의미상 자재+공정 BUNDLE로 분해(새 "조립축" 없음).
  - **★[정정·PD-M1 D-PD-1] 슬리퍼 밑창색(검정/흰색) = SUB_MTR 부자재 variant(부속물#8/자재 sub_mtrl) — 별색(공정#2)과 구분:** PDWRSLP 밑창색이 reverse 초판에서 `six_clr`(별색)로 오귀속됐으나 실제는 `SUB_MTR` sub-radio(`SLB*`/`SLW*` 밑창색×사이즈 12-variant·PD-M1 검증 적발). 밑창=슬리퍼 본체에 결합되는 *부자재(sole) variant*(고무/EVA 밑창의 제조 색×사이즈 SKU) = 부속물#8(AC 받침·ST SUB_MTR 동형) 또는 자재 sub_mtrl(솜/지퍼 PD-4 동류) facet. **★별색(six_clr·공정#2·인쇄 잉크)과 명확 구분** — 밑창색은 부자재 소재 색이지 별색 아님(round-22 "별색=공정" 경계). PD-4 내재BOM과 달리 *옵션 노출*(SUB_MTR sub-radio·view_yn=Y·고객 선택). distinct 0 결론 불변(밑창색이 별색이든 부자재든 둘 다 기존 축 facet·facet 귀속 정확성만 정정). (상세 = `_resolved-fragments.md` PD-6.)
...
- **★FS 확장 — 인쇄 타일링(반복 배치) 파라미터 (v10.0·FS-1·★directive 1순위 distinct #18 부결의 귀속처):** 직물 풀프린팅 타일링(TILL_WH_GBN: 없음/세로/가로)이 *인쇄 배치(조판/imposition) 공정에 종속된 매개변수 슬롯* — 접지방식(FLD_DFT)·오시 줄수(0~3)·UV 변형이 공정 파라미터인 것과 동형(인쇄/면가공 공정 입력값). `param_type`=반복배치(없음/세로/가로)·value_domain=TIL_NON/TIL_HGH/TIL_WDT. **★타일링이 별 "반복 배치 축 #18"이 아니라 공정 파라미터#9인 결정적 근거:** ① 후니 plate_size(#6 임포지션/판걸이/조판)가 인쇄 배치를 이미 1급 모델링(KB 결함 없음·ST 형상 G-SK-2와 정반대) ② 타일링=*입력* 파라미터(고객 디자인 반복 선택)이지 판걸이수(=앱계산 파생값·DB미저장) 아님 — 파라미터 축은 입력 매개변수만 담음(판걸이수 등재 금지 경계와 정합). 가격영향(반복=인쇄 면적/방식 변화)은 공정과 결합 기여(#11)·unobserved(infoCall 후행). **도메인 경계(HARD): 타일링(입력 파라미터)≠판걸이수(앱계산 파생).**
...
- **★NC 확장 — 부수(PRN_CNT) 연속 vs 이산 tier 두 모드 + 이산 tier data-gap (v11.0·N-2·★결정적 분기):** 같은 `pdt_prn_cnt_info`/`pdt_exp_prn_cnt_info` 슬롯이 인쇄방식별로 정반대로 채워짐 — **디지털(BC)=연속 increment**(`FIR_CNT:100·INC_CNT:100·INC_STEP:10·MIN_PRN_CNT:1`·exp_prn_cnt=null) vs **옵셋(NC)=자재종속 이산 tier**(`pdt_exp_prn_cnt_info` 10행=MTRL_CD×PRN_CNT 매트릭스·RXSNO250→{100,200,300,400,500}·자유입력 불가). → 수량모델#10이 *연속·이산 두 모드*를 가져야 함을 입증. **★directive 결정적 분기(이산 tier=distinct vs data-gap): data-gap(vessel-gap 아님).** 옵셋 이산 tier는 한 슬롯에 3 의미 합성(자재→허용부수→단가): ① 허용 부수 집합(자재→{100~500})=수량#10 이산 슬롯+제약#5(자재가 허용부수 제약·match) ② 부수 tier=단가구간=가격#11(round-1 t_dsc_*·tiered_price 동근·NCCDPHO "대량"=이 tier) ③ 자재종속=#12 인쇄방식이 부수 이산화 게이팅(옵셋 판/대수 임포지션 경제성). **인쇄방식 #18 축 증거가 아니라** *수량축#10 이산 모드 표현력 + 제약#5 자재→허용부수 매트릭스 + 가격#11 부수구간 단가*의 미적재(세 기존 축 표현력 data-gap·PD-4 내재BOM·FS-1 타일링 data-gap 동일 계열·축 충분·데이터/표현력 미적재). 갭분석가: 후니 수량축#10이 이산 tier를 수용하는지(연속 vs 이산 모드)·제약#5가 자재→허용부수를 담는지 라이브 확인. ★박/형압 명함(NCDFFOI·[Coming soon])은 미출시 unobserved — 박/형압이 어떤 옵션축(공정?별SKU?)으로 들어올지 출시 후 재캡처.
...
- **★NC 확장 — offset2023_price 전용 가격엔진 라우팅 (v11.0·N-3·directive #18 후보):** 옵셋 명함(NC)이 `price_gbn=offset2023_price`(디지털 BC=`digital_price`·현수막=`real_price` 면적매트릭스)로 *세 번째 가격엔진 토큰* 추가 — 옵셋 자재×부수 룩업(이산 단가구간·NCCDPHO "대량"=부수 tier 단가). pricing_model enum이 기존 7종(면적 real_price·digital원자합산·tmpl·vTmpl·tiered·book2025·acrylic2025)에 **offset2023(옵셋 자재×부수 룩업·이산 단가구간)** 추가 = ST die-cut/판/정가(S-6)·AC acrylic2025(A-6)·GS tmpl/vTmpl(G-7)와 같은 "인쇄방식/소재/형태별 전용 가격엔진(세대)" 패턴 — **신축 불요**(라우팅 키=price_gbn=인쇄방식 종속). **★directive 최강 적대 프로브 부결의 핵심:** 별도 가격엔진 존재가 인쇄방식 distinct #18을 *만들지 않음* — price_gbn=가격#11 라우팅 enum 값일 뿐(전용 관리 슬롯 아님). 가격엔진 선택을 인쇄방식이 *결정*(offset2023_item↔offset2023_price 동반)하나, 이는 #12 인쇄방식이 #11 가격엔진을 게이팅하는 *관계 간선*(이미 #12 게이팅 lifecycle·후니 product_price_formulas 상품↔공식 바인딩이 흡수)이지 새 축 아님. **검증 필요분: offset2023_price 산정식(자재×부수 단가표)·이산 tier↔단가 매핑 → unobserved(주문플로 회피)·dbmap 가격 트랙·갭분석가(round-1 t_dsc_* 수량구간 할인 동근 확인).**
...
- **★NC 확장 — 인쇄방식 토큰 인코딩(`item_gbn`/`price_gbn`) = 4번째 인코딩·★최강 #18 후보 부결 (v11.0·N-1/N-4):** 옵셋 명함(NC)이 디지털 명함(BC)과 *동일 base-data 스키마(`pdt_mtrl/size/dosu/prn_cnt/pcs_info`) 100% 동형 공유*하되 인쇄방식 차이가 **① `item_gbn=offset2023_item`/`price_gbn=offset2023_price` 토큰(디지털=`digital_*`) ② 수량 의미(연속 vs 이산 tier) ③ 별도 가격엔진(offset2023_price)** 으로만 나타남 — **새 스키마 축(슬롯) 추가 0**(reverse 차이표 7행 전부 "같은 슬롯, 값만 다름"). **즉 #12 인쇄방식 축의 4번째 인코딩**: BN=자재 facet(수성/라텍스 MTRL_CD)·ST·PR=pdtCode prefix(윤전/UV/DTF)·CL=상품내 옵션(PTP_*)·**NC=item_gbn/price_gbn 토큰 bundle**. **★directive 최강 적대 프로브(인쇄방식이 distinct #18인가) 부결** — 인쇄방식은 *이미 #12(D-7)로 조건부 등재*되어 신축 #18은 중복: ① 전용 슬롯 부재(item_gbn=enum discriminator·CL clothes2025 동형·명제 #19) ② KB 결함 부재(#12로 이미 1급·process-recipe §1·왜곡 없이 담음). **ST 형상(#17·①②둘 다 충족=승격)과 정반대로 ①②둘 다 불충족=부결**(가장 깨끗한 부결·신후보가 아니라 기존 축 토큰 재확인). **★자재풀 게이팅(N-4·P-7 재확인):** 옵셋전용 자재 pool(RXWMO220 모조지·3종 vs 디지털 5종)이 PR 윤전→YWM pool(P-7) 동형 = 인쇄방식이 가능 자재 부분집합 게이팅. **★dbmap 정합:** rpmeta "인쇄방식=#12 게이팅 축·신축 #18 부결" = 메모리 `dbmap-print-method-not-absolute-axis` "인쇄방식≠절대 최상위 축·시트가 1차 단위" — 같은 결론 다른 렌즈(메타모델 추상화 vs 적재/이해 단위)·상호 보강. **★N-5 접지 관리축 분기:** NCDFFLD 접지(2단/3단)는 *사이즈 SKU 16종으로 베이크*(자유사이즈 차단)+오시(OSI_DFT) 공정 cascade — PR 리플렛 FLD_DFT 독립 옵션축과 *같은 개념 상품군별 다른 관리축*(명함=치수 프리셋 강해 SKU 흡수·리플렛=면 분할 본질이라 옵션축·둘 다 기존 사이즈#13/공정#2).
...
- **★PD 확장 — 봉제(SEW_LTR) family 멤버 + 봉제 구조물 완제품 횡단 (v8.0·PD-1):**
  - **★봉제(SEW_LTR 레더재봉) = 형태가공#14의 새 family 멤버:** PDCHSTL 스툴·PDSRPPY 강아지계단의 `SEW_LTR_CHK`(레더재봉)·PDWRSLP 슬리퍼의 `PDT_WRK_CHK`(제품가공)이 평면 원단/인쇄물을 *봉제·조립해 입체 완제 구조물 본체를 생성*(`[live:SSR]` icon_txt 실측). GS 형태가공(PDT_WRK 파우치가공·FLX_ZIP 지퍼)과 **동일 lifecycle**(본체 형태 자체를 생성·없으면 본체 부재). PDWRSLP `PDT_WRK`는 GS와 *동일 코드*(조립 마감)·PDCHSTL/PDSRPPY `SEW_LTR`은 #14에 **"봉제(sewing)" family 멤버 신규 등재**(assembly_type=봉제). 기존 PCS 슬롯에 인코딩(별 "조립 축" 없음) → PD-1 = #14 family 확장이지 distinct #18 아님.
  - **★봉제 구조물 완제품 = #14 횡단 입증:** GS(파우치/마이크텍 굿즈)에서 PD(스툴/슬리퍼/강아지계단 봉제 구조물)로 #14가 횡단 확장 — 형태가공이 굿즈에 국한되지 않고 봉제 구조물 완제품 전반의 본체 생성 lifecycle을 담음. 솜 충전·지퍼=자재#1 sub_mtrl consumes(FLX_ZIP 지퍼 패턴 동형). **도메인 정합:** entity-semantic 생산방식 C 완제품 + 후니 굿즈 BOM "평면→입체 조립".

- **★FS 확장 — 제품가공(PDT_WRK) 직물 굿즈 봉제 완제 family 횡단 (v10.0·FS-5):** FS 직물 굿즈(쿠션/에코백/파우치/스크런치)의 `PDT_WRK`(제품가공)가 *평면 직물을 봉제·재단해 입체 완제 굿즈 본체 생성* — **★icon_txt가 상품마다 다른 완제 가공명**(쿠션가공/에코백가공/스트링파우치가공/스크런치가공)이나 *동일 PDT_WRK 슬롯에 인코딩*(GS PDT_WRK 파우치가공·PD PDWRSLP 슬리퍼조립과 같은 코드·다른 상품 인스턴스). = #14 형태가공 family(GS D-10·PD 봉제)에 직물 굿즈 봉제 완제 멤버 추가. **★공정 유형 enum vs 공정 인스턴스 (FS-5 모호 해소):** PDT_WRK가 상품마다 다른 icon_txt를 가지나 DB상 *동일 PCS_COD(PDT_WRK)·상품별 라벨/단가 인스턴스* — 후니 공정모델은 "동일 형태가공 슬롯(PDT_WRK)·상품별 가공 인스턴스(라벨/단가/공정 레시피)"로 관리(공정 유형=PDT_WRK, 인스턴스=상품별 봉제 레시피). GS/PD/FS 횡단으로 #14가 직물 굿즈까지 확장 입증(별 "제품별 가공 축" 거부·동일 슬롯).
...
26. **★인쇄 타일링(반복 배치)은 공정의 인쇄 배치 파라미터다 (FS·v10.0·FS-1·#18 부결·재포화)** — 직물 풀프린팅 타일링(TILL_WH_GBN: 없음/세로/가로)이 BN/GS/TP/PR/ST/CL/AC/PD/PH 전 9 카테고리 어디에도 없던 *전용 라디오 슬롯*(distinct #18 유일 후보)이나 **공정#2 인쇄 배치(조판/imposition) 파라미터(#9 종속)로 무손실 흡수.** ★ST 형상(#17)과 정반대로 부결 — 형상은 ① 전용 슬롯(shape_info) 실재 + ② 후니 KB G-SK-2 "형상 어느 축에도 없음" 결함 *둘 다* 충족(승격), 타일링은 ① TILL_WH_GBN 전용 슬롯 OBSERVED(충족)이나 ② 후니 KB `plate_size`(#6 임포지션/판걸이/조판)가 인쇄 배치를 *이미 1급 모델링*(KB 결함 명시 없음·타일링=배치 차원의 고객 선택값이 공정 배치 파라미터로 왜곡 없이 담김)이라 ②불충족 → distinct 0. **★경계(HARD): 타일링≠판걸이수** — 타일링=*고객측 디자인 반복 배치 입력 파라미터*(공정#9 등재), 판걸이수=*앱 계산 파생값*(DB 미저장·등재 금지·`dbmap-compute-in-app-db-stores-lookup`). 임포지션=N개 다른 작업물을 한 원판에(다도안 판걸이), 타일링=한 디자인 반복 배치(직물 면 채움). **★RP가 타일링을 공정 배치 라디오로 둠 = data-gap(후니가 직물 타일링 취급 시 공정#2 인쇄 배치 파라미터에 적재)이지 vessel-gap(축 부재) 아님**(갭분석가). **10번째 카테고리(직물 풀프린팅+봉제 완제 굿즈)가 distinct 0 = 17축 재포화(PR·CL·AC·PD·PH 패턴 반복)** — 유일 신규 후보 타일링조차 무손실 흡수. 면직물(면사 수)=자재#1 PTT/WGT 다의(CL oz·PD 번수·AC mm 동형)·마감봉제(오버로크/말아박기/벨크로)=공정#2 봉제 family·제품가공=형태가공#14 family·끈/자석/솜=부속물#8/자재 sub_mtrl(선택형 노출=PD-4 고정 ESN=Y와 분기)·현수막 가공=BN family 상속·별색(6색×3농도)=공정#2 별색(CL Pantone 축소)·가격모델(real_calc_price)=가격#11 라우팅.
25. **★완제 구조물 내재BOM은 부속물+자재usage+생산형태+가격의 분산 facet이다 (PD·v8.0·PD-4·#18 부결·재포화)** — 봉제 구조물 완제품(스툴/슬리퍼/강아지계단)의 내재BOM(다리/받침/솜/지퍼/논슬립)이 *옵션 슬롯에 미노출·고정 제조 사양*(`[live:SSR-marketing]` 카피에만)이나 네 기존 축으로 무손실 분해: ① 완제 부속 부품(다리/받침/논슬립)=부속물#8의 *고정(ESN=Y·view_yn=N) 완제 부속* facet(AC 등신대 받침·BN 거치대 동형) ② 솜/지퍼=자재#1 sub_mtrl usage + 공정#2(봉제 consumes) ③ "옵션 미노출 고정 제조" 성격=생산형태#15(C 완제품 governing·내재 BOM은 완제품 정체 고정 레시피) ④ 완제 단가=가격#11(tmpl_price). **★ST 형상(#17)과 정반대로 부결** — 형상은 후니 KB G-SK-2 "형상 어느 축에도 없음" 결함이 distinct 강제, 완제 내재BOM은 후니 KB가 addl_product(#9·`:30`)·자재 usage(#2·`:23`)·생산방식 A/B/C(#15·`:113`)를 *이미 1급 모델링*(결함 명시 없음·왜곡 없이 담음)이라 facet. **★RP가 내재BOM을 마케팅 카피로만 둠 = data-gap(부속물#8/생산BOM 그릇 미적재)이지 vessel-gap(축 부재) 아님**(갭분석가: 후니 완제 구조물 취급 시 부속물#8·자재 usage에 적재=기존 그릇 채우기). **8번째 카테고리가 distinct 0 = 17축 재포화(PR·CL·AC 패턴 반복)** — directive 최대 관전(조립/구조/3D폼/완제 내재BOM) 무손실 흡수. 봉제/제품가공(SEW_LTR/PDT_WRK)=본체 형태가공#14(GS D-10) 봉제 family 신규 멤버(별 조립축 아님)·직물/PU 원단=자재#1 PTT(AC/CL 동형)·단수(2/3단)·스툴형상=사이즈#13 프리셋 흡수(2단=495×320 1:1·원형↔305×305 1개=ST 형상 1:多 미충족=구조 distinct 부결 결정적 증거).

> **도메인 사실 준수 체크(HARD):** 별색=공정(D-2/#2) · 본체색=자재 CLR(D-2) · 판걸이수=앱계산 DB미저장(D-4) · UV변형=공정파라미터 not print_side(#2) · size≠plate(#13) · **형상=별 분류축(#17 ST 1:多)·1:1이면 사이즈 흡수(BN/GS/TP/PR)** · **생산형태⊥카테고리(#15)** · **완제 본체=자재 facet not 신축(#1)** · **두께=자재(#1 GS 장패드 4T)** · **디자인 입력채널⊥본체옵션(#16·가격0)** · **PRT_WHT=공정 not 도수/자재(#2·T-E·S-7)** · **템플릿 자산≠완제SKU(#4/#16 분리·T-A)** · **표지/내지=usage_cd 슬롯 not 신축(#1·P-2)** · **접지 면수=파생값 not 차원(#2·P-1)** · **page_rule=내지 페이지(수량#10 슬롯+별 엔티티·P-3)** · **digital_price≠면적매트릭스(같은 좌표·다른 엔진·#11·P-6)** · **반칼=PROC_054/완칼=PROC_053/스티커완칼=PROC_055=공정 멤버 not 별 재단입자 축(#2·S-3)** · **점착/내후=자재 합성 차원 not 별 자재계열(#1·S-4)** · **의류 본체색=자재 CLR not 별색(#1·C-2)** · **의류 size×color matrix=사이즈#13×색상 Cartesian+셀가용성#5 not 별 의류variant축(#1/#5·C-2/C-3·#18 부결)** · **item_gbn=구현 discriminator not 관리축(정책패턴·C-2/C-5)** · **의류 Pantone=별색 공정 not 자재/도수(#2·C-7)** · **인쇄위치=공정 멀티슬롯 not 별 축(#2·C-4)** · **GBN=사이즈 하위속성 not 별 분류축(#13/#6·C-8)** · **두께=자재 WGT 차원 not 별 두께축([huni-ref] mat_cd 통합·#1·A-1)** · **소재 surface-finish(글리터/거울/자개)=자재 합성 차원 not 별 자재계열(#1·A-2·ST S-4 동형)** · **입체/스탠드=분산 facet[받침=부속물#8·두께=자재·양면=옵션·조형=공정] not 별 3D축·생산형태#15/형태가공#14 아님(A-3)** · **가공방식 그룹핑(일반/라미)=공정#2(라미)+자재#1 합성+옵션#3 cascade not 별 그룹핑 슬롯 축(#18 부결·A-8)** · **거울 별 가격공식=#11 라우팅 not 자재분류(A-2/A-6)** · **명찰 본체=PET 자재 not 상품명 아크릴·합지=공정(#1/#2·A-7·G-1 동형)** · **ACTPKEY=#16 TemplateAsset not 완제SKU#4(A-9·T-A)** · **완제 구조물 내재BOM(다리/받침/솜/지퍼/논슬립)=부속물#8(고정 ESN=Y)+자재#1 usage+생산형태#15+가격#11 분산 not 별 완제BOM축(#18 부결·PD-4)** · **봉제/제품가공(SEW_LTR/PDT_WRK)=본체 형태가공#14 family 멤버 not 별 조립축(PD-1·GS D-10)** · **단수(2/3단)·스툴형상=사이즈#13 프리셋 흡수 not 별 구조/형상축(2단=495×320 1:1·원형↔305×305 1개=ST 형상 1:多 미충족·PD-3)** · **직물/PU 원단=자재#1 PTT 차원 not 별 비종이 자재축(면10수=번수 평량 다형성·PD-2)** · **★타일링(반복 배치 없음/세로/가로)=공정#2 인쇄 배치 파라미터#9 not 별 반복배치축(#18 부결·FS-1)·타일링(입력)≠판걸이수(앱계산 파생·DB미저장)** · **면직물/면사 수=자재#1 PTT/WGT 다의 차원 not 별 직물축(면사 수=번수 평량·CL oz·AC mm·PD 번수 동형·FS-3)** · **마감봉제(오버로크/말아박기/벨크로)=공정#2 봉제 family·제품가공(PDT_WRK)=형태가공#14 family not 별 직물가공축(FS-5)** · **직물 굿즈 부자재(끈/자석/솜·옵션 노출)=부속물#8/자재 sub_mtrl 선택형 facet not 별 조립축(PD-4 고정 ESN=Y와 노출 차원만 분기·FS-6)** · **FS 별색(6색×3농도)=공정#2 별색 family not 자재/도수(CL Pantone 축소 도메인·FS-4)**. 전부 entity-semantic-model L3(C-9 생산방식·usage_cd 7종·page_rule·G-SK-2 형상갭·addl_product 부속물·variant 분해)·process-recipe-tree L2(접지/제본/인쇄방식 레시피)·pdf-domain-knowledge(반칼/완칼/도무송 공정·Case2 스티커 레시피) 정합.
...
> **★PD 통합 과잉일반화 거부 기록 (v8.0·distinct 0·재포화·★완제 구조물 내재BOM PD-4 #18 부결):** ㉼ "완제 구조물 내재BOM 축(#18)"(PD-4) = **★distinct 부결 — 분산 facet**(부속물#8 고정 부속[다리/받침/논슬립] + 자재#1 usage[솜/지퍼] + 생산형태#15[C 완제품 governing] + 가격#11[tmpl]). 후니 KB가 addl_product·자재 usage·생산방식 A/B/C를 이미 1급 모델링(결함 없음)·RP 마케팅 카피=data-gap 아닌 vessel-gap. ㉽ "조립/봉제 축"(PD-1) = 본체 형태가공#14(GS D-10)의 "봉제(SEW_LTR 레더재봉)" family 신규 멤버(PDWRSLP PDT_WRK=GS 동일 코드·기존 PCS 슬롯·별 조립축 아님). ㉾ "직물/PU 원단 자재 축"(PD-2) = 자재#1 PTT 차원(면10수/슬리퍼원단/PU·AC 아크릴·CL 의류원단 동형·비종이 자재 이미 관측). ⓐ "단수/구조 축·형상 축"(PD-3) = 사이즈#13 프리셋 흡수(계단 2단=495×320 1:1·스툴 원형↔305×305 1개=ST 형상 1:多 미충족=구조 distinct 부결 결정적 증거). ⓑ "모양커팅/추가부자재 enum 축"(PD-5) = 공정#2(THO_CUT)/부속물#8(SUB_MTR)·infoCall unobserved·축 판정 무영향. **distinct 승급 0종 — 5 fragment 전부 facet/family/cascade. 8번째 카테고리(봉제 구조물 완제품)가 distinct 0 = 17축 재포화(PR·CL·AC 패턴 반복·모델 안정성 재확인). ★PD-4(완제 내재BOM·directive 최대 관전 강후보)는 침묵선택 거부하고 "별 완제 BOM 축 #18 vs 네 기존 축 분산 facet" 트레이드오프 펼친 뒤 facet. 역방향 오류(distinct를 facet으로 숨김) 점검: "옵션 미노출 고정 부품 명세"가 유일 잔여 후보였으나 부속물#8의 고정(ESN=Y·view_yn=N) 완제 부속 facet(AC 등신대 받침 동형)으로 무손실 흡수·★ST 형상#17과 정반대(형상=KB G-SK-2 결함이 distinct 강제·완제 내재BOM=후니 KB addl_product/usage/생산방식 이미 모델링[결함 없음])·숨김 아님.**
