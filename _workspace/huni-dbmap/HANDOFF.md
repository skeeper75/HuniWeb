...
## 한 줄 현황 (round-11 — 2026-06-10·이번 세션·최신)
**상품마스터 컬럼 도메인 의미 + 상품 BOM + raw/webadmin 적재명세 → 매핑정보 하네스 확장 + 디지털인쇄 파일럿 GO(DB 미적재).** `/harness:harness`로 신규 에이전트 2(`dbm-domain-researcher` 컬럼의미+상품BOM·`dbm-loadspec-extractor` webadmin 적재명세)·스킬 2(`dbm-column-domain`·`dbm-loadspec-extract`)·오케스트레이터 round-11 추가. **디지털인쇄 7상품·44컬럼·1026행** 5산출(`15_domain-spec/digital-print/` + `_loadspec/`): ① column-dictionary(44컬럼 의미축·도메인의미·목표 t_*·확정도 **빈칸0**) ② product-bom(7상품 자재/공정 BOM·공통 백본) ③ loadspec(webadmin `BaseAdmin` 제너릭·`BASE_CODE_GROUP` 14·상품인라인6·두 적재 surface·file:line) ④ mapping-info(44컬럼→t_* 통합·FK 적재순서) ⑤ domain-research-notes(컨펌5). **핵심 도출: 별색≠도수(C18~22→`t_proc_processes`)·판수=앱 임포지션 계산(C6 DB 미저장)·출력판형≠재단≠작업(C10/C5/C8 3축)·자재=parent+usage_cd(낱장 C단일)·박색(C37)/파일명약어(C11)/폴더(C13) 귀속 컨펌.** 07_domain 의미축(L2 공정레시피·L3 9속성) 재사용(재유도0). 커밋 65c3028. 권위 [[dbmap-column-domain-loadspec-round11]]. **이는 round-9 schema-design-intent-map(#1)의 엑셀 측 입력 절반 — t_* 측 설계의도와 결합하면 매핑=도출.** **사용자 선택: 다음 = 나머지 10시트 확대(아래 시작점 #0).**
...
0. **[사용자 선택·최우선] round-11 나머지 10시트 확대** — 디지털인쇄 파일럿 패턴(시트별 4산출 `15_domain-spec/<family>/`: column-dictionary 44+컬럼·product-bom·mapping-info·domain-research-notes / loadspec은 `_loadspec/` 공유)으로 **스티커→책자→포토북→캘린더→디자인캘린더→실사→아크릴→문구→굿즈파우치→상품악세사리** 순차. 스킬 `dbm-column-domain`+`dbm-loadspec-extract`, 에이전트 `dbm-domain-researcher`+`dbm-loadspec-extractor`(round-11 생성·**다음 세션부터 로드됨**, 단 round-10 교훈처럼 인라인 한국어 진행 가능). **시트별 오모델 주의**: 굿즈파우치 size→option 재분류([[dbmap-change-tracking-round10]])·실사 가격=포스터사인 면적매트릭스([[dbmap-silsa-price-via-poster-sign]])·아크릴 두께 variant=material·책자/포토북 제본·반제품(B 셋트). 07_domain 의미축 재유도 금지(인용만). **디지털인쇄 컨펌 5건**(박칼라 C37 공정param vs 포일자재·파일명약어 C11/폴더 C13 견적 귀속·블리드 C7 도출·별색 핑크/금/은 proc_cd 정합·건수 C26 단위)은 라이브 대조/사용자 확인으로 해소(`digital-print/domain-research-notes.md`).
