...
## 3. PR-2 — 레이플랫 PROC_000025 소프트삭제 (AX-6 해소·확정)

| 명세 단위 | 내용 |
|-----------|------|
| **현재값** | PROC_000025 **레이플랫제본** del_yn='N'·use_yn='Y'(활성). 권위 사전 = 미운영 마스터(Q10). |
| **★라이브 실측 (2026-06-18 read-only SELECT)** | `SELECT count(*) FROM t_prd_product_processes WHERE proc_cd='PROC_000025'` → **연결 상품 0건**. [HARD] "연결 있으면 보존 우선" 조건 = 충족 안 함(연결 0) → 미운영 SEED 확정·소프트삭제 가능. |
| **대상 t_* + 코드값** | `t_proc_processes` PROC_000025(레이플랫제본). |
| **라우팅 (확정)** | **소프트삭제** — `del_yn='Y'`(+`del_dt`=now()). **use_yn 아님**(조회 차단 권위는 del_yn·[[dbmap-del-yn-soft-delete-authority]]). round-22 카테고리 소프트삭제 패턴과 동형. |
| **멱등 가드 [HARD]** | `WHERE del_yn='N'`(이미 'Y'면 skip)·del_yn·use_yn 둘 다 추적. |
| **적재경로** | catalog Django `tprocprocesses__change`(del_yn 논리삭제·`admin.py` logical_delete) 또는 dbmap `dbm-load-execution` 멱등 UPDATE. |
| **영향분석** | 연결 상품 0 → BOM/가격사슬 파손 0(삭제 안전). family head 아님(자식 공정 영향 0). 롤백 = del_yn='N' 원복(+del_dt NULL). 라이브 del_yn='Y'는 TRUNCATE 재적재 시 휘발 가능 → 근본 = 경로 Y(v03 미운영 행 제거). |
| **컨펌** | **AX-6 해소**(연결 0 실측 + 사용자 미운영 확정·2026-06-18). 재제안 금지. |
...
### 6.1 컨펌 확정 (2026-06-18 사용자·재제안 금지)

| ID | 확정 결과 | 반영 |
|----|-----------|------|
| **AX-6** ✅해소 | 레이플랫 PROC_000025 = **미운영(소프트삭제)** 확정. 라이브 연결 0 실측 → del_yn='Y'(+del_dt)·use_yn 아님·멱등 가드 WHERE del_yn='N' | §3 PR-2(소프트삭제 확정) |

### 6.2 잔여 컨펌 (escalate)

| ID | 막힌 결정 | 평이한 한국어 질문 |
|----|-----------|--------------------|
| **AX-5** [B4·이관 범위] | 공정 파라미터(줄수·조각수·UV 변형) 선택값을 **기존 dtl_opt 칸에 채우는 범위**(어느 옵션 항목에 어느 값) — 신규 칸 신설 아님 | "오시 2줄, 반칼 4조각, 아크릴 풀빼다 같은 '공정 세부 선택값'을 담는 칸은 이미 있습니다(봉제 옵션에서 쓰는 중). 다만 대부분 비어 있어요. 어느 상품·옵션부터 이 값을 채울지만 정하면 됩니다(새 칸 만들 필요 없음)." |

> **escalate:** **AX-6 해소**(레이플랫 소프트삭제 확정·연결 0). AX-5 = 이관 범위 결정만(신규 컬럼 ALTER 철회·DDL 인간 승인 불요·dbmap 적재 트랙). 공정 축 잔여 escalate = AX-5 1건(범위 결정·data-gap 채움).
...
| **레이플랫 PROC_000025 소프트삭제(PR-2)** | **1행** | 0 | **AX-6 해소·확정**(del_yn='Y'·연결 0) |
| 마스터 교정 | **0** | 0 | 마스터 오염 0(🟢) |

> **★공정 축 신규 mint = 0**(B4·ref_param_json 컬럼 신설 철회). 실 조치 = 기존 dtl_opt에 param 선택값 채움(data-gap) + **레이플랫 PROC_000025 소프트삭제 1행(AX-6 확정·연결 0)**. 신규 공정 코드행/교정 0. dtl_opt가 인쇄옵션 PO-1의 목적지(기존 그릇·선결은 option_items 행 적재).
...
- **컨펌 확정:** **AX-6 해소** — 레이플랫 PROC_000025 소프트삭제 확정(라이브 연결 0 실측·del_yn='Y'·use_yn 아님·멱등 가드). **잔여 escalate = AX-5 1건**(dtl_opt 이관 범위·DDL 불요·dbmap 적재 트랙). 평이한 한국어 질문 §6.2.
