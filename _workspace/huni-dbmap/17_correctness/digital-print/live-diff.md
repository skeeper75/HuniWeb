...
| 카테고리 | CAT_000295 상품권(upr=NULL orphan) | 〃 | 판매 카테고리 트리 연결 | 🟡 orphan(F-ID-3) |
...
SELECT cat_cd,cat_nm,COALESCE(upr_cat_cd,'-'),cat_lvl FROM t_cat_categories WHERE cat_cd='CAT_000295';
```
...
| 카테고리 | CAT_000296 배경지(upr=NULL orphan) | 〃 | CAT_000012 포장 하위 | 🔴 오분류(F-ID-1/F-ID-3) |
...
SELECT c.cat_cd, cc.cat_nm FROM t_prd_product_categories c JOIN t_cat_categories cc ON c.cat_cd=cc.cat_cd WHERE c.prd_cd='PRD_000043';  -- CAT_000296 배경지
SELECT cat_cd,cat_nm,COALESCE(upr_cat_cd,'-') FROM t_cat_categories WHERE cat_cd='CAT_000296';  -- upr=NULL (orphan)
...
> **배경지 핵심 진단:** size·도수·plate·가격은 CORRECT지만, **정체(포장 세트)를 구성하는 (1) 전용 커팅 형상 (2) 봉투/케이스 세트 (3) 포장 카테고리 위상** 3가지가 전면 미적재/오분류. 일반 인쇄물로 봤다면 보이지 않을 결함 — C-ID 정체 선행이 잡아냄.
...
| 카테고리 | **CAT_000296 배경지(upr=NULL orphan)** | CAT_000012 포장(라벨/포장스티커 CAT_000283) | 🔴 오분류 |
