...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-10 | **round-11 확대 #2 — 책자 시트 컬럼 도메인 + 상품 BOM + 적재명세** — 사용자 "책자로 넘어가자"(round-11 확대 #2). 스티커 패턴으로 **책자(11상품·43컬럼·1010행, URL 가이드행 R85 제외)** 5산출(`15_domain-spec/booklet/` 4 + `_loadspec/` delta, 510줄·확정도 빈칸0): ① column-dictionary(43컬럼) ② product-bom(11상품) ③ mapping-info(43컬럼→t_*) ④ domain-research-notes(컨펌 BK-1~6) ⑤ loadspec 책자 delta. **책자는 디지털인쇄/스티커와 구조 근본 차이(내지/표지 이중 블록·반제품·제본). 핵심 도출: ⓐ 생산방식 3구조 정확 매핑(A통합=중철/무선/PUR/트윈링·B셋트=하드커버3종+레더링바인더 표지 반제품+면지·떡제본=엽서북/떡메모지), 자재 권위=parent+usage_cd 항상 ⓑ page_rule 신규 축 활성(디지털/스티커 0행)·제본별 차등 07_domain 예측 실측일치(중철 4~28 증가4=4배수·무선 24~300·떡메모지 빈값=page 무의미) ⓒ 제본(C31)=PROC_000017 자식 7종+GRP-BOOK-제본 택일(레이플랫 enum 부재=후니 미운영 가설 정합) ⓓ usage 슬롯 분리(.01내지/.02표지/.03면지/.05투명커버)·면지=D-29 자재 ⓔ 떡제본 묶음수=QTY_UNIT.03 권·page 아님 ⓕ 책등 두께가 size(A4 두께A 31mm)+D링 mm 양쪽 인코딩.** **오모델 충돌 0**(스티커 G-SK-2 같은 문제 없음). 07_domain 제본 KB(entity-semantic §4/§5·db-structure PROC_000017 8자식·D-26박/D-29면지) 재사용(재유도0). DB 미적재·컨펌 BK-1~6(BK-1 박칼라·BK-6 메타는 디지털/스티커와 일괄 후보)·다음=포토북(확대 #3) | _workspace/huni-dbmap/15_domain-spec/{booklet(4),_loadspec(책자 delta)}·HANDOFF·메모리1갱신·CLAUDE.md, 커밋 본커밋 | 사용자("책자로 넘어가자") — round-11 확대 |
