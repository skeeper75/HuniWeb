...
-- 멱등 실증(롤백전용): mc-02-namecard_premium 본문 2회 적용 후 ROLLBACK. 게이트 RAISE 시 즉시 abort(비0 종료).
BEGIN;
\echo '=== mc-02-namecard_premium PASS 1 ==='
...
BEGIN
  ...
  SELECT count(*) INTO v_canon_pre  FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_PREMIUM';
  SELECT count(*) INTO v_member_price FROM t_prc_component_prices WHERE comp_cd IN ('COMP_NAMECARD_PREMIUM_S1_MGA', 'COMP_NAMECARD_PREMIUM_S1_MGB', 'COMP_NAMECARD_PREMIUM_S2_MGA', 'COMP_NAMECARD_PREMIUM_S2_MGB');
  IF v_canon_pre>0 AND v_member_price>0 THEN
    RAISE EXCEPTION '게이트① 혼재중단: 정본 COMP_NAMECARD_PREMIUM 단가행 %건 + 멤버 단가행 %건 동시존재(반쯤적용 의심).', v_canon_pre, v_member_price;
  END IF;
...
-- [2] 단가행 이관: 멤버 comp_cd → 정본 (분리축 값은 행에 이미 충전·verbatim 불변). 기대 28행(재실행 0=멱등)
UPDATE t_prc_component_prices SET comp_cd='COMP_NAMECARD_PREMIUM', upd_dt=now() WHERE comp_cd IN ('COMP_NAMECARD_PREMIUM_S1_MGA', 'COMP_NAMECARD_PREMIUM_S1_MGB', 'COMP_NAMECARD_PREMIUM_S2_MGA', 'COMP_NAMECARD_PREMIUM_S2_MGB');
...
BEGIN
  ...
  SELECT count(*) INTO v_member_fc FROM t_prc_formula_components WHERE comp_cd IN ('COMP_NAMECARD_PREMIUM_S1_MGA', 'COMP_NAMECARD_PREMIUM_S1_MGB', 'COMP_NAMECARD_PREMIUM_S2_MGA', 'COMP_NAMECARD_PREMIUM_S2_MGB');
  IF v_member_fc<>0 THEN RAISE EXCEPTION '게이트② 멤버 fc참조 %건(0 기대) → 중단.', v_member_fc; END IF;
  -- 구조적 골든 등가①: 정본 단가행수 == 이관 기대(28)
  SELECT count(*) INTO v_canon_rows FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_PREMIUM';
  IF v_canon_rows<>28 THEN RAISE EXCEPTION '정본 단가행수 %건(기대 28) → 중단.', v_canon_rows; END IF;
  -- 멤버 잔존 단가행 0
  SELECT count(*) INTO v_member_price FROM t_prc_component_prices WHERE comp_cd IN ('COMP_NAMECARD_PREMIUM_S1_MGA', 'COMP_NAMECARD_PREMIUM_S1_MGB', 'COMP_NAMECARD_PREMIUM_S2_MGA', 'COMP_NAMECARD_PREMIUM_S2_MGB');
  ...
  SELECT count(*)-count(DISTINCT (apply_ymd,siz_cd,plt_siz_cd,clr_cd,mat_cd,proc_cd,opt_cd,print_opt_cd,coat_side_cnt,bdl_qty,siz_width,siz_height,min_qty,COALESCE(dim_vals,'{}'::jsonb)::text)) INTO v_dup
    FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_PREMIUM';
  IF v_dup<>0 THEN RAISE EXCEPTION '정본 내부 nat_key 중복 %건(0 기대) → 중단.', v_dup; END IF;
END $post$;

\echo '=== mc-02-namecard_premium PASS 2 (idempotency) ==='
SET LOCAL statement_timeout = '120s';
...
\echo '=== mc-02-namecard_premium 2PASS 통과 → 멱등·게이트OK ==='
ROLLBACK;
