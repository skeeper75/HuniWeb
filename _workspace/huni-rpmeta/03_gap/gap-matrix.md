...
> - **v12.0 (OT 통합) ★17축 재포화·12번째(박스/패키징·전개도 #18 부결·신규 vessel-gap 0):** OT(케이크/납작/반달/봉투/답례품상자·클래퍼·에어홀더)가 **distinct 신축 0건(★전개도/dieline #18 부결) + facet 강화 5항(O-1~O-4·F4)**. OT reverse 1차 예측("#18 부결·12번째 재포화·전부 data-gap/unobserved")을 적대 판정으로 비준 — *평면 인쇄물에 없던 입체/전개 차원*을 가진 첫 상품군조차 17축 무손실 흡수. 갭의 핵심 = **박스 facet(3D 제품치수·전개도/dieline 2치수·도무송칼틀/오시접지·dieline 에디터템플릿·OTPKENV 커스텀라벨)을 후니가 같은 표현력으로 담는가 + 신규 vessel-gap이 진짜 필요한가**. **신규 축 카운트 변동 0 — 17축 유지.** OT 추가 = **§XXIII(OT facet 5항 PASS/WEAK/GAP·라이브 2026-06-20 실측)·§XXIV(OT 종합 카운트)**. BN·GS·TP·PR·ST·CL·AC·PD·PH·FS·NC 판정 **보존**. **★실측 핵심(라이브 information_schema 직접 SELECT 2026-06-20): OT facet 5건 중 신규 vessel-gap 0 — ① O-3 전개도/dieline 2치수(재단 485×270 / 작업 495×280·여백 10mm)=PASS(★`t_siz_sizes` 단일 행에 `work_width/work_height`+`cut_width/cut_height`+`margin_top/bot/lft/rgt` 4종 모두 보유 — 박스 2치수를 *한 siz_cd 프리셋*이 무손실로 담음·라이브 실데이터 65행이 work≠cut 2치수 분리 등록 입증[SIZ_000007 A5: work150×212/cut148×210/margin1] → 박스 2치수=data-gap 아님·완전 표현·plate_size 그릇 동원 불요·t_siz 이중등록 패턴은 박스에 불필요한 우회) ② O-1 3D 제품치수(130×105 높이80)=PASS(파생 표시값·선택축 아님·`t_siz_sizes`에 height 컬럼 부재이나 3D는 작업사이즈[전개도 평면]에서 비선형 파생[접기 변환]이라 *미저장 파생*이 정답·메모리 `dbmap-compute-in-app-db-stores-lookup` 판수=앱계산 동형·별 메타데이터 그릇 부재가 결함 아님) ③ O-3 도무송칼틀/오시접지=PASS(★`t_proc_processes` 오시 `PROC_000029/090`·접지 family `PROC_000056~074`[2단~8단·롤·병풍·미싱·오시접지]·형압 `PROC_000050`·타공 `PROC_000079/092` 라이브 실재 — 박스 칼틀=ST 칼선 family 동형·자유칼선[도무송] 전용 row는 ST와 동일하게 부재[완칼+모양파라미터 환원·V-12 게이팅 영역]) ④ O-2 dieline 에디터템플릿=data-gap/unobserved(★#16 TemplateAsset[D-11·V-11]로 충분·`t_prd_templates`[tmpl_cd·base_prd_cd·tags jsonb] 그릇 보유·박스 dieline 접지선 좌표는 makers 에디터 응답 스키마 `unobserved`·"구조 dieline" sub-type 불요로 1차 판정·별 vessel 신설 미요) ⑤ F4 OTPKENV 커스텀라벨=PASS(카테고리#7 운영 라벨·`t_prd_products` prd_typ_cd/tags 흡수·별 축 아님). 신규 vessel-gap 0·전부 기존 §I~§XXII 판정(#13·#2·#16·#7)에 흡수·V-11·V-12 신규 mint 불변.**
