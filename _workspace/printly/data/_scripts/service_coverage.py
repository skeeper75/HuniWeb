...
# ★정렬 확장 조사 결과(260705·라이브 DB 실측): 후니 매칭 확인분 = 양벤더 승격.
EXTEND_MATCH = {
    "팬시제품": "후니 대응 有 → 양벤더(반칼팬시스티커062/063·떡메모지097/098·메모패드179 — 스티커/굿즈 인접·정렬엣지 추가 필요)",
    "포토/액자": "후니 대응 有 → 양벤더(포토북100~107·포토카드024/025·우드액자131·레더액자132·CD앨범204 — 정렬엣지 추가 필요)",
}
# 정렬 미완 상품군의 성격(정직): 확정 와우단독 vs 애매
UNALIGNED_NOTE = {
    "카페용품": "확정 와우 단독(후니 미대응·POP 실증)", "선거홍보물": "확정 와우 단독(선거 특화·캠페인버튼만 A-15 부분)",
    "와우기획상품": "확정 와우 단독(와우 기획전)", "시스템상품": "확정 와우 단독(와우 플랫폼 상품)",
    "서식류": "★확정 와우 단독(후니 영수증/NCR 미대응·실측)",
    "디지털인쇄": "미정렬(인쇄'방식' 축·후니 전반 디지털 처리·전용 상품군 아님)",
}
...
def main():
    ...
    for cat, items in sorted(groups.items(), key=lambda kv: -len(kv[1])):
        ...
        elif cat in EXTEND_MATCH:
            assign = "후니 기본(양벤더·정렬 확장)"
            match = EXTEND_MATCH[cat]
        else:
            assign = "와우 단독/미정렬"
            match = UNALIGNED_NOTE.get(cat, "후니 미정렬")
        rows.append((cat, len(items), rep, match, assign))

    aligned = sum(1 for r in rows if r[4].startswith("후니"))
    ...
    md += ["",
           "## 요약",
           f"- **양벤더 가능(후니 정렬) {aligned}군**: 후니 기본 배정·와우 전환 가능. 배정표(sourcing_table)로 가격·능력 대조.",
           "  - 정식 정렬(family-alignment 16쌍) + ★정렬 확장 승격 2(팬시제품·포토/액자·후니 대응 실측 — 정렬엣지 추가 필요).",
           f"- **와우 단독/미정렬 {wowonly}군**: 확정 와우 단독 5(카페용품·선거홍보물·와우기획·시스템·**서식류**[후니 영수증/NCR 미대응 실측]) + 미정렬 1(디지털인쇄=인쇄'방식'·전용 상품군 아님).",
           "- **다음**: ①팬시·포토액자 same_family_as 엣지 추가(정식 양벤더화) ②상품군별 와우 대표 가격 fetch(배정표 24군 확장) ③후니 스펙 정규화·쿠폰 재앵커.",
