...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-14 | **Tier A 35상품 CPQ 옵션 레이어 적재 설계·검증 GO (round-6 확대)** — 사용자 "Tier A 37상품 전체 적재". 기초완전 Tier A 37 중 기적재(현수막138·합판066 stub) 제외 **실질 35상품**에 CPQ 옵션 레이어(option_groups→options→option_items→constraints→templates) 설계·검증·멱등 DRY-RUN. 옵션 구조(L1 시트) 같은 **4 덩어리** 분해 → 각 `dbm-option-mapper` 설계 → `dbm-validator` 독립 교차검증(생성자≠검증자). **집계: INSERTABLE 1,026행(groups122·options460·option_items437·constraints7) / BLOCKED 17 / L1 LINK 3 분리** — 전 덩어리 트리거 `fn_chk_opt_item_ref` 전건 통과·2-pass 멱등 delta 0·ROLLBACK 후 영구변경 0·COMMIT 0. 덩어리: 디지털인쇄14(577/6 GO)·스티커4(55/0 GO·066보강)·책자4(312/0 GO)·면적형13(82+L1LINK3/11 GO·prn=0). **[HARD] disp_seq=L1 옵션성 컬럼 등장순서**(엽서 파일럿 누락 보강 전 상품 적용·적재 3원칙 ⒜)·FK위상 기초→옵션→템플릿→제약(silsa)·option_item=기적재 차원행 포인터(차원0행=BLOCKED 분리). 교훈: ①추출 스냅샷 stale·라이브 권위(postcard BLOCKED 종이/후가공 실적재→INSERTABLE 승격) ②형제 상품 대조 과소적재 검출(029 종이13→14) ③opt_id↔print_side 상품마다 다름 ④**L1/L2 경계 엄수**(면적형139 끈 차원 product-link를 L2 트랜잭션에 묶은 것 적발→`_l1_link_preload.sql` 분리) ⑤생성자≠검증자(검증이 디지털1행 과소적재·면적형 경계위반 적발). **✅ GO분 라이브 COMMIT 완료(사용자 "GO분 안전 적재"·백업→DRY-RUN→commit 4패키지→검증→멱등2회차): 라이브 증분 1,026행(og+122·opt+460·item+437·cst+7)·공정 고아0·35상품 전부 적재·멱등 재실행 신규0. 백업 `_exec_tierA_backup_260614/`·기록 `_APPLIED-260614.md`.** 잔여 미적재(인간 승인)=BLOCKED 17·L1 LINK 3·테스트더미 정리(016/025/066)·GAP DDL. | `_workspace/huni-dbmap/10_configurator/tierA/`(설계 4·_SUMMARY·_APPLIED)·`09_load/_exec_tierA_{digitalprint,sticker,booklet,areaform,backup_260614}/`·`03_validation/cpq-tierA-*-gate.md`·HANDOFF·메모리·CLAUDE.md | 사용자(Tier A 37상품 전체 적재 → GO분 COMMIT) |
| 2026-06-14 | **컨펌대기 71건 정리(14 BATCH 상태판·원칙미정0) + 스티커 코팅 예외(자재유지) + 카테고리 잔존고아 12상품 라이브 COMMIT + 적재 3원칙 + 구성완전성 스캔** — 사용자 "핸드오프 확인 후 진행". ① 71건=14 BATCH × Wave-0(D-1~5) × 2026-06-14 적용 차감 → 상태판 `_CONFIRM-STATUS-260614`·14 BATCH 전부 방향확정·**원칙미정 0건**. ② 스티커 코팅 CONFLICT 결정(사용자): **스티커 예외=코팅 전체 자재유지**(L1·라이브·가격표 3중·Q9는 스티커만 비적용). ③ **카테고리 잔존고아 명함10·상품권2=12상품 동명 정상노드 재연결 라이브 COMMIT**(`category-orphan-final/`·`dbm-validator` 8/8 GO·멱등2회차0·상품권 Q-ID-B/W-05 실측 해소·누적118상품). 부자재2(타이벡북커버·이미지피켓)=구성미흡 체크 후순위. ④ **[HARD·사용자 적재 3원칙→메모리]**: 컬럼순서=옵션 `disp_seq` 권위(엽서 미흡·보강)·기초데이터→옵션→템플릿→제약 FK위상(silsa 실증)·구성완전 먼저 미흡 체크 후순위. ⑤ **구성완전성 스캔**(`25_load-readiness-scan/`·247상품×17테이블)=Tier A 완전37(CPQ 1순위·opt_grp만 빔)·B 65(가격결손46)·C 미흡54. bun 설치(블로커 해소). 커밋 704f1a1·b63a429·1adff5a. | `_workspace/huni-dbmap/23_remediation-apply/{_CONFIRM-STATUS-260614,category-orphan-final,sticker}`·`25_load-readiness-scan/`·HANDOFF·메모리·CLAUDE.md | 사용자(핸드오프 확인 후 컨펌대기 정리·진행) |
| 2026-06-14 | **round-13 라이브 정정 매니페스트 — 전 11 상품시트 2단 양식 산출 + 즉시적용분 라이브 COMMIT** — 사용자 "260610 기준 라이브 정정 매니페스트 진행" + 6대 품질기준 명시(①기존추출 대비 개선점 ②인쇄도메인/경쟁사 기초데이터 검증 ③매니페스트 비전문가 실무진 이해 ④적재 용어 실무진 가독 ⑤webadmin `sql/`+`table-spec_260610.html` 매핑정합·option/template/constraint 반영 ⑥정규화 오염방지+등록일시 변경추적). 11시트 각 `dbm-correctness-auditor`(병렬 팬아웃)→`dbm-validator`(K1~K6+6대기준 독립게이트·전건 GO·뒤집힘0). 산출 `23_remediation-apply/<slug>/`(실무진 정리표 `manifest-worksheet`+멱등 `apply.sql`+`rollback`+`normalization-guard` 3중 화이트리스트+`cpq-plan`)·게이트 `17_correctness/_gate/`·적용기록 `_APPLIED-*.md`. **즉시적용 라이브 COMMIT(백업→DRY-RUN→COMMIT→검증→멱등2회차 전건): 카테고리 고아 정상화 106상품(전역고아 main=Y 잔존14·BATCH-1 거의완성·비어있는 정상노드 1:1 재연결 search-before-mint·PK복합 INSERT+고아 `main_cat_yn`=N 강등)+자재정정(원단7 .08→.05·점착지3 .01→.11)+MES5·editor4·무광공정·PUR필수·종이용도·자재논리정리·빈옵션정리.** 권위추가 `docs/huni/table-spec_260610.html`(코드 한글도메인)·`raw/webadmin/sql/`(FK/CHECK). 타임스탬프 `reg_dt`/`upd_dt`+note "정정 2026-06-14:X→Y". 컨펌대기 ~71건(size→option·코팅·세트·박색·형상 적재처·미싱제본·장수·가격사슬)=다음 세션. 비파괴(즉시적용분만 COMMIT·신규적재/CPQ 보류). | `_workspace/huni-dbmap/23_remediation-apply/`(11시트 신규)·`24_master-extract-260610`·`17_correctness/_gate`·HANDOFF·메모리·CLAUDE.md | 사용자(260610 기준 라이브 정정 매니페스트 진행) |---
