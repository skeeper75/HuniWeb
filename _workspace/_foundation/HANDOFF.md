...
## 다음 시작점 (한 줄·★인간 승인 대기)
**★밴드총액 .01 과대청구 교정 — dryrun 준비완료·인간 승인 후 COMMIT.** 시뮬레이터 실증=투명명함 100매=1,350,000(정답 13,500·100배). 교정=25 component `.01→.02`(바인딩 12 긴급+미바인딩 13 예방)·dryrun ROLLBACK 검증 완료. **승인 시 `remediation/bandtotal-prctyp-fix-COMMIT.sql` 실행→시뮬레이터 전수 재실증**(투명명함 13,500 등). 검증필요 4(떡메·엽서북·미니배너·미니보드)=라이브 확인 결과 **per-unit(.01 정상·버그 아님)** 종결. 진단·SQL=`remediation/FINDING-bandtotal-x-qty-overcharge.md`·`bandtotal-prctyp-*`. 그 다음=L1 합가 가족 라이브 교차검증 계속.
