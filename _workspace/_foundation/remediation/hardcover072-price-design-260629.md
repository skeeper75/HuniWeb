...
>
> ★★ 모델 정정(2026-06-29·사용자 directive "이전 사이트에서 별도/합산 확인"): 이전 사이트
> pcode=40(하드커버무선) 실측 결과 — **표지종이를 손님이 고르지만(맞춤표지·표지종이 220g/250g/
> 300g/몽블랑240g) 가격은 표지종이와 무관(3종 전부 동일가 41,800원)**. 즉 **표지+제본 = 합산
> 고정가(수량 밴드별)·표지종이=사양선택(가격 무관)**. → 사용자 말씀대로 **합산이 맞고, 표지를
> 역산 분리할 필요 없었음**(아래 표지 역산 분리 설계는 불필요·정정 대상). 정정 모델 =
> 표지+제본 합산 1 comp(밴드별·표지종이 무관) + 내지(페이지 기반·§18). 합산값=라이브 p02 밴드
> (또는 가격표 combined)·표지종이 220g/250g/300g는 COMP_PAPER에 존재하나 표지가격엔 미반영.
>
> ★스코프(구·정정 전): 이 문서는 **표지(전용지) 단가 comp + 세트 합산 공식 + 부모 바인딩**의 가격 종단 설계다.
