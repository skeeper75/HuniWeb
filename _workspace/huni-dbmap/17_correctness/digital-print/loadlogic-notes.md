...
### L-H [오적재 — 상품이 잉여 고아 노드에 연결] 🟡 Med (F-GATE-1 정정)
- **무엇(정정):** 정상 포장 하위 노드는 **이미 라이브 실재**한다 — `CAT_000273 인쇄배경지(OPP봉투타입)`·`CAT_000274 인쇄배경지(투명케이스타입)`·`CAT_000275 인쇄헤더택`·`CAT_000283 라벨/포장스티커`가 전부 `upr_cat_cd=CAT_000012 포장`·lvl2. 결함은 노드 부재가 아니라, 배경지/상품권 상품이 정상 노드 대신 **별개 잉여 고아 노드(`CAT_000296 배경지`·`CAT_000295 상품권`, upr_cat_cd=NULL·lvl3)에 연결**된 것.
- **라이브 재측정(읽기전용):** `SELECT cat_cd,cat_nm,COALESCE(upr_cat_cd,'NULL'),cat_lvl FROM t_cat_categories WHERE cat_cd IN ('CAT_000012','CAT_000273','CAT_000274','CAT_000295','CAT_000296')` → 012 포장(NULL,1)·273(012,2)·274(012,2)·295 상품권(NULL,3)·296 배경지(NULL,3). 상품 연결(`t_prd_product_categories` JOIN): 043/044/045/046→CAT_000296·041/042→CAT_000295(전부 고아).
- **적재경로 근본원인:** `01_카테고리` 시트가 정상 포장 트리(273/274/275/283)와 **별개로** 디지털인쇄 시트 `구분` 라벨에서 파생된 편의 그룹(배경지/상품권)을 중복 카테고리로 정의, 상위코드를 비운 채(NULL) 적재. `11_상품별카테고리`(load_rel_categories load_master.py:282-291)가 상품을 그 고아 노드에 연결. load_master는 시트 그대로 충실 적재 → 잉여 고아 노드 + 오연결.
- **함의:** 정상 노드(273/274/275/283)를 두고 잉여 고아 296/295에 묶임 = 판매 카테고리 위상 오연결. 교정 = **정상 노드로 상품 재연결(search-before-mint, 기존 행 재사용)** + 잉여 고아 296/295 논리정리. 296의 upr를 012로 UPDATE는 차선(잉여 노드 잔존). mapping-final C1 비고("시트 편의 그룹≠판매 카테고리")의 실증.

> **L-G/L-H 종합:** 배경지 정체(포장 세트)를 모른 채 칼럼만 보면 봉투 세트·카테고리 오연결이 보이지 않는다(round-11/12가 일반 인쇄물로 봐서 놓침). C-ID 선행이 두 적재경로 결함(L-G addon 파싱범위·L-H 잉여 고아 노드 오연결)을 드러냄 = round-13 방법론(정체 선행 + 라이브=교정대상)의 입증.
