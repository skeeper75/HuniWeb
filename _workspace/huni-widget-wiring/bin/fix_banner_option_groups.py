"""현수막 옵션그룹 가격배선 — 통합형 가격구성요소 3종 신설 + 공식 배선.
...
SPECS = [
    ("COMP_BANNER_ADDON", "일반현수막 부자재", "PRC_COMPONENT_TYPE.06",
     "OPT_000004", "PRF_POSTER_BANNER_N", "PRD_000138",
     # 권위 M246 「추가없음」 0 은 옵션 OPV_000012 가 논리삭제 상태라 제외한다
     # (미선택 = 0 이므로 그릇이 필요 없다).
     [("OPV_000013", "큐방(4개)", 3000, "N247"),
      ("OPV_000014", "끈(4개)", 4000, "N248"),
      ("OPV_000015", "각목(900mm이하)+끈(4개)", 4000, "N249"),
      ("OPV_000016", "각목(900mm 초과)+끈(4개)", 8000, "N250")],
    ...
    ("COMP_BANNER_MESH_OPTION", "메쉬현수막 가공옵션", "PRC_COMPONENT_TYPE.04",
     "OPT_000285", "PRF_POSTER_BANNER_M", "PRD_000139",
     [("OPV_001000", "재단만", 0, "K267"),
      ("OPV_001001", "아일렛(4구)", 3000, "K268"),
      ("OPV_001002", "아일렛(6구)", 4000, "K269"),
      ("OPV_001003", "아일렛(8구)", 5000, "K270")],
     ["COMP_POSTEROPT_BANNER_MESH_PROC_PUNCH"]),

    ("COMP_BANNER_MESH_ADDON", "메쉬현수막 부자재", "PRC_COMPONENT_TYPE.06",
     "OPT_000023", "PRF_POSTER_BANNER_M", "PRD_000139",
     [("OPV_000425", "큐방(4개)", 3000, "N268"),
      ("OPV_000426", "끈(4개)", 4000, "N269")],
     ["COMP_POSTEROPT_BANNER_MESH_ADD_QBANG_4",
      "COMP_POSTEROPT_BANNER_MESH_ADD_STRING_4"]),
]

BACKUP = os.path.join(ROOT, "_workspace", "huni-widget-wiring", "out", "backup",
                      "pre-banner-option-groups-260827.txt")
...
def report(tag):
    ...
    allok = True
    for prd_cd, nm in (("PRD_000138", "일반현수막"), ("PRD_000139", "메쉬현수막")):
        rows = wiring(prd_cd)
...
def main():
    ...
    print("=" * 78)
    print(f"현수막 옵션그룹 가격배선 — {'COMMIT(실적재)' if commit else 'DRY-RUN(강제 롤백)'}")
    print("=" * 78)
    print("\n[배선 전]")
    report("before")
