...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-10 | **round-11(상품마스터 컬럼 도메인 의미 + 상품 BOM + 적재명세) 하네스 확장 + 디지털인쇄 파일럿** — 사용자 `/harness:harness`(각 컬럼 의미를 인쇄 도메인 리서치로 애매모호 0까지 확정 + 상품별 자재/공정 리서치 + raw/webadmin 코드 기반 적재명세로 데이터 매핑 정보 정리). **하네스 확장**: 신규 에이전트 2(`dbm-domain-researcher` 컬럼 도메인 의미+상품 BOM·`dbm-loadspec-extractor` webadmin 적재명세) + 신규 스킬 2(`dbm-column-domain`·`dbm-loadspec-extract`) + 오케스트레이터 round-11(Rounds·Team·설명·트리거). **디지털인쇄 파일럿(7상품·44컬럼·1026행)** 5산출: ① column-dictionary(44컬럼 의미축·도메인의미·목표 t_*·확정도 **빈칸0**) ② product-bom(7상품 자재/공정 BOM·공통 백본) ③ loadspec(`BaseAdmin` 제너릭 적재·`BASE_CODE_GROUP` 14 코드그룹·상품 인라인 6·두 적재 surface·file:line) ④ mapping-info(44컬럼→t_* 통합·FK 적재순서) ⑤ domain-research-notes(컨펌 5). **핵심 도출: 별색≠도수(C18~22 화이트/클리어/핑크/금/은→`t_proc_processes`)·판수=앱 임포지션 계산(C6 DB 미저장)·출력판형≠재단≠작업(C10/C5/C8 3축 분리)·자재=parent+usage_cd(낱장 C단일)·박색(C37)/파일명약어(C11)/폴더(C13) 귀속 컨펌**. round-9 `schema-design-intent-map`의 엑셀 측 입력 절반 충족·07_domain 의미축(L2 공정레시피·L3 9속성) 재사용(재유도0). DB 미적재(분석/정리 전용)·나머지 10시트 확대·컨펌 해소는 인간/다음 | .claude/agents/huni-dbmap/{dbm-domain-researcher·dbm-loadspec-extractor}(신규)·.claude/skills/{dbm-column-domain·dbm-loadspec-extract}(신규)·huni-dbmap-orchestrator(round-11)·_workspace/huni-dbmap/15_domain-spec/{digital-print(4),_loadspec}·메모리1신규·CLAUDE.md, 커밋 본커밋 | 사용자(`/harness:harness`) — 컬럼 도메인 의미·상품 자재/공정·적재명세 정리 |
| 2026-06-10 | **round-10(버전 변경 추적 + 델타 적용) 하네스 신설 + 260527→260610 실행·GO** — 사용자 `/harness:harness`(신규 상품마스터 260610 변경분 적용 + 추적성 + 라이브 t_* 매핑 구조적 접근). **하네스 확장**: 신규 스킬 `dbm-change-tracking`(키 기반 3-way diff[baseline/new/라이브]·변경 매니페스트·멱등 델타 UPSERT·V1~V8 게이트·references/diff-engine.md) + 신규 에이전트 `dbm-change-tracker` + 오케스트레이터 round-10(rounds·역할·파이프라인 6단·결정게이트·시나리오·트리거). **핵심 설계: 적용 델타 ≠ (new − baseline) — 라이브 DB가 상태 권위**(baseline=의도·live=현실). REMOVED=논리삭제 제안(hard-delete 금지)·신규 코드값=선적재 제안·COMMIT/DDL=인간 승인. **260527→260610 실행(V1~V8 8/8 PASS·GO)**: 상품 추가/삭제/재명명 **0**, MODIFIED 527셀이지만 **자동 UPSERT 0·전부 ESCALATE(526)/GAP(1)**[①굿즈파우치 448=`사이즈(필수)`→`상품(옵션)` 재분류(224쌍·58상품) ②스티커 37=커팅클리어 ③아크릴 41(LOW)=아크릴미니파츠 변형행−16(−24행 정체) ④실사 1=범례]. **dbm-validator 독립**: diff 스크립트 byte-identical 재실행·라이브 read-only 실측(레더라벨 SIZ_000492/494/496·option_items 18=silsa)·527셀 dodge-hunt(진짜 in-place 스칼라=실사 범례 1건뿐=정당 GAP) → escalate-everything이 정직한 평결 확증. 교훈: 버전변경=데이터모델 의도전환(size→option)일 수 있음·기계적 size 삭제 금지(가격사슬, schema-design-intent-first)·ID타입드리프트(int↔float)/부동소수 노이즈 정규화 필수·신규 에이전트 생성세션 미로드(general-purpose 대행). DB 미적재(델타 COMMIT·CPQ L2·논리삭제=인간 승인) | .claude/skills/dbm-change-tracking(신규)·.claude/agents/huni-dbmap/dbm-change-tracker(신규)·huni-dbmap-orchestrator(round-10)·_workspace/huni-dbmap/{14_change-tracking/260527-to-260610,03_validation/change-tracking-gate}·메모리1신규·CLAUDE.md, 커밋 본커밋 | 사용자(`/harness:harness`) — 상품마스터 버전 변경 추적·델타 적용·구조적 접근 |
| 2026-06-09 | **round-9 CPQ 옵션레이어 실 적재 개시 — 코드전략 비준 + silsa 43행 라이브 COMMIT + OTC 인벤토리 + 방법론 전환** — ①**코드 식별자 전략 D1~D5 비준**(순차 surrogate PK·이름(prd_cd+_nm)기반 멱등[신규 DDL 0]·separator `_` 통일[CPQ 하이픈 폐기]·코드생성 별도트랙, expert-backend 독립검토 CONFIRM-WITH-ADJ→`00_schema/code-identifier-strategy.md`) ②**silsa(PRD_000138) CPQ 43행 라이브 COMMIT**(자재mint4[큐방·각목×2·봉제사]+공정mint1[열재단]+링크7+그룹2+옵션11+옵션항목18 — **option_items 전역 0→18 = CPQ 레이어 라이브 최초 성립**, COMMIT 후 재-dryrun delta 0 멱등실증)+**각목 방향(세로/가로) 재모델·R-GAKMOK 제약 폐기**(비규격이라 siz집합/숫자var 부적합→파생옵션으로 제약 소멸·각목 자재 1개 통합) ③**전 11시트 OTC(option/template/constraint) 인벤토리**+[CONFIRM] 6건 확정(C-1 볼체인/리필잉크=TEMPLATE·C-2 잉크색≠도수·C-3 면지/바인더링=자재+조건부공정·C-4 우드거치대 가공/추가상품 분기·C-5 50장1권=제본+묶음수·C-6 굿즈 색+사이즈 한덩어리, 경쟁사 리서치 동반) ④엽서(PRD_000016) 적재본 빌드(29행·DRY-RUN GO·미COMMIT). **[HARD·사용자 방법론 전환] 기계적 매핑 금지 → 스키마 설계의도 + 삼중 바인딩(① UI componentType ② 생산 자재/공정 BOM·MES ③ 가격엔진) 선행.** 카드봉투 색=siz 오모델(PRD_000004 SIZ가 "화이트/블랙") 적발 → 적재 전 스키마 WHY를 RedPrinting 역공학 참조로 정립해야 잘못된 t_* 매핑 방지. 다음세션 최우선=`schema-design-intent-map.md`(미작성). 운영교훈: 에이전트 위임이 "완료"만 반환→산출 직접 읽어 회수·인라인 한국어 선호. | _workspace/huni-dbmap/{00_schema/code-identifier-strategy,09_load/(_exec_silsa_cpq·_exec_silsa_cpq_remodel·_exec_postcard_cpq),03_validation/silsa-cpq-load-gate,10_configurator/(all-sheets-otc-extract·option-vs-template-guide·cpq-confirm-research-c1-c4),HANDOFF}·메모리3(code-identifier-strategy·schema-design-intent-first 신규·option-material-process-bundle 갱신)·CLAUDE.md, 커밋 본커밋 | 사용자(CPQ 실적재·코드전략·방법론 전환 directive) |
---
