...
## 9. 스티커(이산규격×소재×수량·형상=칼틀 비가격축) 가격계산 모델 — 종단 보강

> 상세 흡수 분석 = `absorption-candidates-sticker.md`(C-S1~C-S8). 여기선 모델 골격만.
> 출처 `[red:ST]` = `_workspace/huni-rpmeta/categories/ST/{reverse,summary}.md`(2026-06-17 STTHUSR/STCUXXX/STPADPN 풀 infoCall+priceCall·shape_info 5형상·THO_DFT/THO_GRA 칼선 2메커니즘·CUT_DFT 재단입자·점착소재 26종·인쇄방식 4계열·disable 227·#17 형상 distinct) · `[huni:stk]` = `_workspace/huni-dbmap/33_silsa-price-quote/{sticker-3axis-design,bankal-shapes-resolution}.md`(가격표 260527 스티커 7블록 3축·라이브 t_prc_*·**형상=칼틀=가격축 아님**) · `[wow:probe]` = WowPress 라이브(2026-06-20·`goods/list?category=sticker`·`goods/view` 404·미관측).

### 9.1 레드프린팅 스티커 = die-cut/판/자유정가 3엔진 분기 `[red:ST]`
- **가격엔진 3종**: die-cut(자유형/반칼·자유좌표+칼틀)=`digital_price` / 판·DTF·UV(고정규격·장단위)=`vTmpl_price` / 자유형정가(STPADIY)=`tmpl_price`. 형태가 엔진을 가름.
- **사이즈**: ① 자유좌표(CUT_WDT/HGH·자유형) ② 프리셋 칼틀이 사이즈 겸함(CL001~010 원형·RC001~025 라운드·SQ 사각) ③ 판=고정규격(140x200·A4).
- **★형상(shape_info)·칼선(THO_DFT/THO_GRA)·재단입자(CUT_DFT 반칼DFXXX/완칼DFITM)** = 1급 옵션 슬롯·가격 PCS로 전달되나 형상 자체는 가격 인자 아님(칼틀이 좌표 전달).
- **소재**: 자재 합성코드(지종 PTT × 평량 WGT × 점착성)·26종 한 상품 enum + 점착특화 상품(리무버블/옥외/저온/자석).
- **인쇄방식**: pdtCode prefix 분기(STTH일반/STPAU UV/STPAD DTF/STBP후지)·자재·도수노출·화이트강제·가격엔진 동반결정(PR §0.4 동형).
- **수량**: die-cut prn_cnt 100단위·이중수량(ordCnt 디자인수 × printCnt 수량)·판은 장 단위. disable 227건(소재→후가공·전 상품군 중 최다).

### 9.2 와우프레스 스티커 = 미관측 `[wow:probe]`
WowPress 라이브 `goods/list?category=sticker`·`goods/view` 전부 HTTP 404(스티커 옵션/가격 페이지 미해소). 옛 캡처에 스티커 명목이 거론되나 옵션·가격 실측분 부재. 스티커 가격화 방식 **보강 불가**(정직 기록·실사/아크릴 동일).

### 9.3 후니 스티커 가격표(이산 siz_cd × mat_cd × 수량 + 세트형 .02) — 권위 `[huni:stk]`
| 블록 | 내용 | 공식유형 | 후니 그릇 |
|------|------|---------|----------|
| B01 반칼 | 124x186/A4/A3/90x190/100x148/90x110 × 7소재 × 36수량단 | **단가형(.01)** | `PRF_STK_FIXED`·`COMP_STK_PRINT` use_dims `[siz_cd,mat_cd,min_qty]` |
| B02/B03 낱장(일반/투명) | A4/B4/A3/B3/A2 × 6수량단 | 단가형(.01) | 동일 comp(siz_cd 추가·B4/B3 미적재) |
| B04 대형 | 400x600 | 단가형(.01) | 동일 comp |
| B05 타투 | 3장당 4000(+기본가2000) | **합가형(.02)** | `COMP_STK_TATTOO`·bdl_qty=3(미적재) |
| B06 팩 | 54장 1세트 4000 | **합가형(.02)** | `COMP_STK_PACK`·bdl_qty=54(★라이브 .01 오적재) |
| 형상(원형/사각/팬시) | (가격 차원 없음) | — | 가격축 아님(상품정체·칼틀·proc_cd) |

### 9.4 한 줄 결론
레드프린팅 스티커 가격 골격(*형태별 3엔진[die-cut digital_price 좌표+칼틀 / 판·DTF vTmpl_price 규격템플릿 / 자유형정가 tmpl_price] × 사이즈(자유좌표 or 프리셋칼틀) × 소재(점착 합성자재) × 수량 × 인쇄방식(상품분기)*)은 후니 스티커 가격표(이산 siz_cd × mat_cd × min_qty 수량구간 + 세트형 .02 합가형)와 라이브 `PRF_STK_FIXED`/`COMP_STK_PRINT`가 **동형**으로 담는다. ★**형상·칼선·재단입자는 RedPrinting이 1급 옵션 슬롯으로 두나 후니 가격표는 가격 차원으로 두지 않는다(같은 사이즈/소재=같은가)** — **option-axis ≠ price-axis**(가격축 신설 부결·rpmeta ST 형상 #17 distinct는 옵션 그릇 vessel-gap이지 가격 vessel-gap 아님). ★사이즈=**이산 siz_cd**(면적매트릭스 부결·아크릴/실사와 결정적 분기)·소재=mat_cd 7종(라이브 3 collapse+170 오매핑 보강)·수량=min_qty/bdl_qty가 핵심 흡수 원칙(전부 기존 그릇·신규 축 0). ★돈크리티컬 2건: B06 팩 prc_typ .01 오적재(→.02·54배 왜곡 회피)·반칼/완칼 단가 다른데 siz_cd 공유(오청구·siz 분리) = 디지털 ×qty·아크릴 .02 동일 클래스. ★실질 흡수 = 점착소재 mat_cd 전개 + disable 227 제약(round-6 CPQ·최다) + 형태별 frm_cd 분기(엔진 코드 분기 부결·과분화). WowPress 미관측(보강 불가). 6종단 누적 결론(신규 가격축 0·후니 그릇 우월·data/배선-gap) 정합.
