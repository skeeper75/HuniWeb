...
| 10 | 작업 사이즈 입력·자동 도출(도련 포함) | webadmin | 서희항 | `STD-OPT-016` | 부분 | raw/webadmin/webadmin/catalog/static/catalog/widget_renderer.js:3921-3930 |
...
| `STD-OPT-012` | 별색(spot color) 선택 | 별색은 공정(PROCSEL |
| `STD-OPT-013` | 화이트/UV white 언더베이스 선택 | 화이트인쇄 공정(PROCSEL)이 뒷면별색과 함께 언더베이스 개념으로 다뤄지는 것을 확인했으나 전용 모델은 없다. |
| `STD-OPT-016` | 작업 사이즈 입력·자동 도출(도련 포함) | nsReasons가 범위 위반 사유를 계산하나 재단→작업(도련) 자동 도출 공식 자체의 코드 위치는 특정하지 못해 부분으로 남긴다. |
...
| 서희항 | 별색(spot color) 선택 | 코드는 있는데 원장 상태가 「작동」다 |
| 서희항 | 화이트/UV white 언더베이스 선택 | 코드는 있는데 원장 상태가 「작동」다 |
