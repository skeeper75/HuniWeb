# -*- coding: utf-8 -*-
"""t62 검증 — 산출물을 원천에서 독립적으로 다시 읽어 게이트를 판정한다.

build.py 의 메모리 구조를 쓰지 않는다. 디스크에 쓰인 산출물과 원천(canon·원장)만 읽는다.
"""
import csv
import json
import os
...
L1S = {'회원·인증', '마이페이지', '프로모션', 'B2B', '운영자·회원CS'}
KINDS = {'가', '나', '다', '라'}
...
def prefix_of(i):
    return i.rsplit('-', 1)[0] if re.match(r'^STD-.+-\d+$', i) else None
...
def main():
    ...
    gate('V3', '범위 밖·신설 행 0 — 735행 원장에 새 행을 섞지 않았다',
         not outside, '범위 밖 %d %s' % (len(outside), outside))
    ...
    for f in mds:
        t = open(os.path.join(pdir, f), encoding='utf-8').read()
        n = t.count('```mermaid')
        has_seq = 'sequenceDiagram' in t
        has_flow = 'flowchart' in t
        if n != 2 or not has_seq or not has_flow:
            bad.append((f, n, has_seq, has_flow))
    ...
    gate('V9', 'L1 대분류 5종 고정', l1s <= L1S, '관측 %s' % sorted(l1s))
    ...
    gate('V10', 'gaps 4종 분류 · 프로세스 실재', not bad_kind and not bad_pid,
         '종류 %d · 미지 프로세스 %d · 총 %d건' % (len(bad_kind), len(bad_pid), len(gaps)))

    # V11 — index.html 에 모든 프로세스가 실려 있다
    miss = sorted(p for p in pids if '"%s"' % p not in idx)
    ...
    # V13 — 「없다」 단정 금지: 산출물 본문에서 단정 표현을 쓰지 않았는지
    bad = []
    for f in mds:
        t = open(os.path.join(pdir, f), encoding='utf-8').read()
        for m in re.finditer(r'(코드가 없다|API 가 없다|어디에도 없다(?! —))', t):
            bad.append((f, m.group(0)))
    gate('V13', '「없다」 단정 금지 — 확인 범위를 밝히지 않은 단정 표현 0',
         not bad, '적발 %d %s' % (len(bad), bad[:3]))
