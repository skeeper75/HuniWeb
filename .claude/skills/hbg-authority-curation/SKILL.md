...
description: >
  후니프린팅 기초코드 거버넌스 하네스의 권위 큐레이션 방법론 스킬. 6축(자재/사이즈/도수/인쇄옵션/공정/기초코드)의 정답 기준을
  권위 원천에서 추출해 축별 정답 사전(authority dictionary) 작성. 권위 순서[HARD] ①상품마스터+가격표 ②라이브 t_* 설계의도
  ③인쇄 도메인+rpmeta 역공학 ④경쟁사(갭헌팅). 추정 0(가설+출처+컨펌ID)·갭헌팅 vs 권위 덮어쓰기 금지. 트리거: 권위 정답 사전,
  축별 정답 기준, 권위 큐레이션, 상품마스터 권위 추출, 갭헌팅 보강, 권위 충돌 정리. 라이브 진단은 hbg-basecode-diagnosis,
  등록 명세는 hbg-registration-spec이 담당.
---
