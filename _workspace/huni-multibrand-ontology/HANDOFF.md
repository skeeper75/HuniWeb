...
**Phase 1~5 + 구조 리서치 + tykimos 추출 + 질의 시뮬레이터 명세까지 완료.** 다음 세션 시작점 후보:
1. **질의 결합 시뮬레이터 P1 파일럿** — 명세(`00_research/structure/query-simulator-spec.md`) 기반. ★결정=온톨로지 스튜디오 행위(질의→순회→답+하이라이트)+우리 경량 그래프+nl-query-paths. P1=Q1(고급명함 후니vs와우) 종단 실행(용어해석→경로순회→evaluate_price 호출→답+시각화). 재사용=nl-query-paths×3·product_viewer/Cytoscape(§29). 선행 G-SIM-1=nl-query-paths를 실행 쿼리로 형식화(개선-2 CQ 테이블화와 결합).
2. **intent(용도) 원자 분해 파일럿** — 키스톤(AliCoCo). ★tykimos 차용=근거 N+ 승격 임계 규칙 함께 적용. 속성 먼저(D1)·과분해 금지·시뮬 용어해석과 동시 진행 권장(G-SIM-2).
3. **아키텍처 게이트(MB7) 통과 + 개체/관계 승격** — 라우팅 E22~E25·RT-1~6, 상위개체 E18~E21 정식 등재는 **인간 승인** 후. 결정=독립 후 통합 확정.
4. **구조 권고 게이트 검증** — 구조 리서치·tykimos 추출 권고를 mbo-verify-gate로 아직 미검증(라우팅 층만 검증됨).
