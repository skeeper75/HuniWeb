...
description: >
  후니프린팅 DB 데이터 매핑 하네스 오케스트레이터. Railway railway DB(PostgreSQL 18.4, 44테이블=t_* 도메인 34
  + Django 10, CPQ 옵션/템플릿/제약 레이어 포함)를 읽기전용 시트화하고, 상품마스터·인쇄상품 가격표 엑셀을 t_*에
  매핑(설계서 + 적재 CSV)한다. 7인 팀(dbm-schema-analyst / dbm-excel-analyst / dbm-mapping-designer /
  dbm-validator / dbm-load-builder / dbm-ddl-proposer / dbm-option-mapper)으로 구조·엑셀 병렬 분석 → 매핑 설계 →
  교차검증. 라운드별 메서드 스킬로 라우팅: 구간할인=dbm-mapping, 가격공식=dbm-price-formula, 적재준비/실행=
  dbm-load-readiness/dbm-load-execution, CPQ 옵션=dbm-cpq-option-mapping, 커버리지=dbm-coverage-matrix,
  변경추적=dbm-change-tracking, 6축 교정=dbm-axis-staged-load, 카테고리 MAP=dbm-category-audit/mapping. 실제
  INSERT·DDL·COMMIT은 인간 승인. 트리거: 'DB 매핑', 'DB 구조 파악', '테이블 시트화', '엑셀 데이터 매핑',
  '구간할인 매핑', '가격표 매핑', '상품마스터 매핑', 'Railway DB', '적재 CSV', '매핑 검증', '적재 준비',
  '적재 SQL', 'DDL 제안', 'CPQ 옵션 매핑', '커버리지 매트릭스', '변경 추적', '6축 교정', '카테고리 매핑',
  'DB매핑 하네스 실행/재실행/업데이트', '특정 테이블만 매핑'. 단순 질문은 직접 응답.
---
...
| round-18 | **가격엔진 실증검토** (적재된 가격 데이터의 가격사슬 완전성 전수 실측 — [정정 2026-06-18] 라이브 `evaluate_price`는 실재·구현됨[`pricing.py` @line247·§13/§15 실호출 검증]·"엔진 미구현" 전제 STALE → 골든 재계산은 §13/§15 실호출에 양보·이 트랙은 ① 가격사슬 완전성 라이브 실측[소스 바인딩·공식→`formula_components` 배선→`price_components`→`component_prices` 단가행·`t_dsc` 수량구간 할인 연결·등급]에 집중. 보조로 사슬 단절 클래스 진단용 재계산[라이브 `pricing.py` 코드+11-CONTEXT 보조·prcx01 STALE 인용금지·단가형/합가형·NULL 와일드카드·동시매칭·수량구간·시계열·할인 순차곱] + 가격표 엑셀 known값↔가격뷰어 수치 대조 + 가격뷰어 정합) | 아크릴·문구·굿즈/파우치 → `26_price-engine-verify/<family>/`{chain-completeness·recompute.py·recompute-cases·expected-vs-computed·defects} (PE1~PE6 게이트, 적재 아님·읽기전용) | `dbm-price-engine-verify` | ACTIVE |
...
| round-24 | **MAP 카테고리 맵 검증 + 카테고리-상품 매핑** (상품마스터 MAP 시트=고객 카테고리 IA[12 1차 × ▶︎하위 × 상품 × →별칭]를 **1단계** 검증[MAP 12 1차 ↔ 라이브 `t_cat_categories` L1 활성 12 1:1 정합·244 상품 출시상태 권위=원천(상품마스터+가격표) 분석으로 QUOTABLE 판정] + **2단계** 카테고리-상품 매핑 명세[레이어A MAP IA→`t_cat_categories` 노드 매핑·search-before-mint·round-22 ⑥ 고아노드 정합 / 레이어B prd_cd→cat_cd 귀속 junction `t_prd_product_categories`(prd_cd,cat_cd) PK·다중분류·main_cat_yn 단일·별칭 교차참조]. 생성≠검증: validator V1~V6[PK충돌·main 단일성·del='Y' 귀속·날조·신규 mint·substring 과매칭 가드]. ★[HARD] partial 매칭은 라이브 prd_nm 1:1 검증 후 승인) | 상품마스터 MAP 시트 → `35_category-map/`{01~07·_gate·_live·_meta·scripts} (V1~V6 게이트·DB 미적재·실 적재 인간 승인) | `dbm-category-audit`(1단계) · `dbm-category-mapping`(2단계) | ACTIVE |
...
| `dbm-price-engine-verifier` | **round-18 only**: 가격엔진 실증검토 — [정정 2026-06-18] 라이브 `evaluate_price` 실재·구현(`pricing.py` @line247·§13/§15 실호출 검증·"엔진 미구현" 전제 STALE) → 상품군(아크릴·문구·굿즈/파우치)별 ① 가격사슬 완전성 라이브 실측(소스 바인딩·공식→배선→단가행·`t_dsc` 할인 연결·등급)에 집중·골든 재계산은 §13/§15 양보 ② (보조) 사슬 단절 클래스 진단용 계산기(`recompute.py`, 라이브 `pricing.py`+11-CONTEXT 보조·prcx01 STALE 인용금지·단가/합가·NULL·동시매칭·수량구간·시계열·할인 순차곱) ③ 가격표 known값↔가격뷰어 수치 대조 ④ 가격뷰어 정합. round-16/17 산출 재사용. 비파괴(읽기전용) | **dbm-price-engine-verify** |
...
| `dbm-category-auditor` | **round-24 1단계만**: 상품마스터 MAP 시트(고객 카테고리 IA) 검증 — MAP 엔트리 × 데이터시트 실상품 × 라이브 DB(`t_prd_products`·`t_cat_categories`·option_items·component_prices) 3원 대조 → 매칭 매트릭스·미매칭 양방향 보드(MAP-only/Sheet-only)·출시상태 분류. MAP 12 1차 ↔ 라이브 L1 활성 12 정합 입증. 라이브 읽기전용 SELECT만(파괴적 쓰기 0·COMMIT 없음) | **dbm-category-audit** |
| `dbm-category-mapper` | **round-24 2단계만**: 카테고리-상품 매핑 명세 — 레이어A MAP IA→`t_cat_categories` 노드 매핑(search-before-mint·round-22 ⑥ 고아노드 정합) + 레이어B prd_cd→cat_cd 귀속(junction `t_prd_product_categories` PK·다중분류·별칭 교차참조·main_cat_yn 단일). FK 위상(카테고리 선적재→상품 귀속)·채번 MAX+1·separator '_'. 매핑 명세+적재순서+영향분석까지(COMMIT 없음·실 적재 인간 승인) | **dbm-category-mapping** |
...
**[프레임] round-2/16/17은 가격을 매핑·그릇·정적정리(공식이 정리·배선됐는가)했다. round-18은 *가격사슬이 끝까지 연결됐는가*를 전수 실측한다. [정정 2026-06-18] 라이브 `evaluate_price` 엔진은 실재·구현됐다(`pricing.py` @line247·§13/§15 실호출 검증)·"엔진 미구현" 전제는 STALE → 골든 재계산(얼마인가)은 §13 huni-price-quote/§15 huni-quote-verify의 evaluate_price 실호출에 양보(중복 회피)하고, 이 트랙은 사슬 완전성 실측에 집중한다(사슬 단절 클래스 진단에 한해 라이브 `pricing.py` 코드+11-CONTEXT 보조로 재계산·prcx01 STALE 인용금지).** 생성(`dbm-price-engine-verifier`)·게이트(`dbm-validator` PE1~PE6) 분리. 둘 다 `dbm-price-engine-verify` 스킬 로드. 산출 루트 = `26_price-engine-verify/<family>/`. round-9/10 교훈으로 인라인 한국어 진행 선호(재계산 read-only).

**Phase 0 — 입력 확인**: 계산 권위(라이브 `raw/webadmin/webadmin/catalog/pricing.py` `evaluate_price` @line247·1순위 · `raw/webadmin/.planning/phases/11-price-engine-simulator/11-CONTEXT.md` 보조 · **`docs/prcx01-pricing-model.md`는 STALE·인용 금지**) · 가격표 엑셀(`docs/huni/..._가격표_260527.xlsx`) · round-16 `20_price-import/`·round-17 `21_price-formula-audit/` 존재 점검. 라이브 `.env.local RAILWAY_DB_*`(읽기전용)·`HUNI_ADMIN_*`(가격뷰어 gstack).
...
- **Round-18 flow (price-engine-verify)**: "가격엔진 실증" / "가격계산 검증" / "round-18" / "가격공식 매핑 실증" / "아크릴/문구/굿즈 가격 검증" → Phase 0 권위(라이브 `pricing.py`·11-CONTEXT·prcx01 STALE 인용금지)·가격표·round-16/17 확인 → verifier 가격사슬 완전성 실측(소스·배선·단가행·할인·가격뷰어 3원)에 집중·골든 재계산은 §13/§15 양보 → (보조) 사슬 단절 클래스 진단용 `recompute.py`로 재계산 → 가격표 known값 대조 + 결함 보드 → validator PE1~PE6 독립 게이트(대표 1건 손계산 재현) → lead가 사슬 단절·재계산 불일치·계산불가 + 단절 교정 에스컬레이션. [정정] 엔진 실재 전제(`pricing.py` @line247·§13/§15 실호출 검증)·읽기전용·인라인 한국어 선호. 산출 루트 `26_price-engine-verify/`.
