...
- **088 레더링바인더 = 유일 진짜 결함(PRICE=0)**: 부모공식 바인딩 전무 + 구성원(표지+면지4) 전부 무공식 + **가격 낼 내지 member 부재**(082/077과 결정적 차이·빈 바인더). 데이터 dryrun만으로 교정 불가 — 실무진 제본방식(HOLD)+권위단가+cover_mult ×2 C트랙 확인 후 §23 SOP로 구성.
  - **★권위(상품마스터260610) 실측: 088은 '(보류중)' 상품**(빈 바인더·표지 레더·D링 31/42/56mm·O링/D링·면지4·**내지 없음**). D링 제본비 권위 부재=날조 불가. 082 하드커버트윈링과 다른 제본방식(D링).
  - **★★교정 완료(2026-07-02·실무진 답변 후 라이브 COMMIT)**: 실무진 결정=**제본/바인더비=077 레더하드커버 통합단가 동형**(COMP_HC_MUSEON_COVERBIND 표지+제본 통합·100부 7,969)·**D링 두께(31/42/56mm) 무관 단일가**. 088=빈 바인더(내지없음)→COVERBIND 하나만 배선(전용공식 PRF_LEATHER_RINGBINDER_SET mint·comp/단가행 재사용·신규단가행0). **0원→시뮬 검증 1부 34,100·100부 796,900(7,969/부·077 tier 동형)**·use_yn N→Y 재노출. fix=`_foundation/batch/wiring/088-build-260702-fix.sql`·undo 보유. ★잔여=레더 프리미엄(+3,900·077과 동일 COVERBIND use_dims=[min_qty]한계·엔진 C트랙)·물리 바인더 공정(3구타공 PROC_128 등) 생산정보 미등록(가격무관). **19/19 셋트 전부 PRICE≠0 달성.**
