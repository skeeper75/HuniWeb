...
>
> 가격 API는 동일 엔드포인트지만 `price_gbn`으로 서버측 가격 룰 엔진을 분기 — 이는 **Red 위젯의 동작 방식(참고)**이다. `price_gbn` 값/분기 체계는 Red 고유 속성이며, 후니가 이 키나 분기 구조를 채택해야 한다는 의미가 아니다. 후니는 후니 자체 가격체계로 분기하면 되고, 위젯은 어느 쪽이든 불투명 키를 echo만 한다.
...
> 아래는 **Red 위젯이 이렇게 동작한다**는 관찰(참고)이다. Red의 가격 구조(`ORD_INFO`+`PCS_INFO`/3단 워터폴/`price_gbn`)를 후니가 그대로 채택해야 한다는 처방이 아니다. 후니는 후니 자체 가격 공식·API를 갖는다. 위젯에 영향이 있는 유일한 불변식은 ①번(서버 권위 + 클라이언트 무계산)뿐이며, ②~④는 Red 동작 설명일 뿐 후니 가격 정책을 구속하지 않는다.

1. **서버 권위 모델 (위젯 불변식)**: Red 위젯은 가격을 절대 계산하지 않는다. 옵션 변경 → 디바운스 → API 호출 → 응답 표시. 이 모델은 후니에서도 유지된다(위젯은 옵션을 보내고 후니 가격 API 응답을 표시할 뿐). 단 응답을 만드는 가격 공식은 후니 고유이며, Red→후니 가격값 비교/정합은 하지 않는다.
2. **공정별 분해 표시**(Red의 result[]·result_log): Red는 공정별 단가를 분해 표시한다(참고). 후니가 가격 투명성을 차별점으로 가져갈지는 후니 결정이며, 정규화 계약은 분해 행(`lines`)을 선택적으로 수용할 수 있도록만 둔다.
3. **3단 가격 워터폴**(정가/할인가/몰가) + 부가세 별산 + 배송비: 이는 Red 서버의 결제금액 산정 방식(참고)이다. 후니는 후니 자체 할인·금액 산정 정책을 가지며, 정규화 계약은 최종 `finalPrice`/`vat`/`shipping`만 받으므로 후니가 어떤 방식으로 산정하든 무관하다.
4. 가격 룰 자체(수량구간 단가 테이블)는 RedPrinting 서버 내부 — 역산 불가·불필요. 후니 가격 마스터·공식도 후니 서버 내부이며, 위젯은 어느 쪽이든 결과만 표시한다.
