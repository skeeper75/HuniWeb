...
> **하네스** hbg Phase 1 · 후속 확장용 스캐폴드. **작성** 2026-06-18 · **확장 완료** 2026-06-18(2차 회차).
> 1순위(자재·카테고리)=`axis-authority-material.md`·`axis-authority-category.md`.
>
> ✅ **본 스캐폴드 4축은 2차 회차에서 정답 사전으로 확장 완료** — 아래 각 축 표는 이제
> 전용 사전 파일로 채워졌다. 본 문서는 **요약 인덱스**로 보존(전용 사전이 권위):
> - ② 사이즈 → **`axis-authority-size.md`**
> - ③ 도수 → **`axis-authority-color.md`**
> - (인쇄옵션) → **`axis-authority-printoption.md`**
> - ⑤ 공정 → **`axis-authority-process.md`**
> 4축 충돌/모호/빈칸은 `_authority-conflict-board.md` 2차 회차 append(A2/B2/C2/D.2) 참조.
>
> **권위 순서·확정도 범례는 자재/카테고리 사전과 동일.** 아래 헤더 요약은 전용 사전의 §0 정의와 동일.
