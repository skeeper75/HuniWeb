...
## 15. 박(소형/대형) GAP — 코드 신설 보류 (억지 평면화 금지)

박 가공비는 2단 룩업(면적→분류A~E→가격) GAP이라 **코드 신설 보류**. `COMP_FOIL_*`(.05 박형압비) 컨벤션은 예약하되, 면적→분류 룩업표 정착지(§schema-fitgap §4-3 3안) 후니 결정 후 신설. 박 **동판비**(가로×세로→단가 2D)는 ADEQUATE라 `COMP_FOIL_DIE_*` 별도 즉시 처리 후보(이번 미산출 — GAP 결정 선결).
