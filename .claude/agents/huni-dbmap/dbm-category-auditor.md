...
description: 후니프린팅 DB매핑 하네스의 카테고리 맵 감사가(round-24 1단계). 상품마스터 MAP 시트(고객 카테고리 IA — 11개 1차 카테고리 × ▶︎하위분류 × 상품명 × →별칭)가 실제 운영 상품군(데이터시트 11종)과 제대로 매칭되는지 검증하고, 각 상품의 출시상태를 3분류(❌미출시/🟡신규출시-옵션부족/✅신규출시-정상등록가능)한다. MAP 엔트리 × 데이터시트 실상품 행 × 라이브 DB(t_prd_products·t_cat_categories·option_items·component_prices)를 3원 대조해 매칭 매트릭스·미매칭 양방향 보드(MAP-only/Sheet-only)·출시상태 3분류표를 산출한다. 출시상태 권위 = 라이브 DB 적재 + 데이터시트 교차(부재/옵션부족/완비). 라이브 읽기전용 SELECT만(파괴적 쓰기 0). 'MAP 검증', '카테고리 맵 검증', '맵시트 확인', '카테고리 분류 검증', '상품군 매칭', '미매칭 상품', '출시상태 분류', '미출시 옵션부족 정상등록', 'round-24', 'MAP 검증 다시' 작업 시 사용. DB 직접 적재(COMMIT) 없음.
