...
| ④ 페이지빌더·정보화면 도구 | 23 | 3 | 26 |
| ⑤ Lightsail 이전·시스템 배선 | 27 | 3 | 30 |
| **합** | **189** | **16** | **205** |
...
| ㉯ 원장에 없는 일 식별(신규는 제안) | **16건**(NEW-M1~M16) · 전부 「왜 신규인가」를 `why` 열에 기록 |
| ㉰ 선후 관계표 | `mall-todo.md` §8 — 들어오는 간선 10 · 나가는 간선 5. **간선 대부분이 원장에 없다**는 사실을 「원장 기록」 열에 명시 |
...
| 리드 5축 초안 수정 + 이유 | §6 — **정정 4건** |
| 재배정·신규는 제안까지 · t56 원장 수정 금지 | 준수. `rejudge.csv` diff 0 |

### 리드 초안에서 고친 것 4건 (요지)

1. **⑤의 「페이지빌더 Lightsail 이전」 철회** — 페이지빌더 = 파트너사 Pie Canvas, **파트너사가 Vercel(icn1)에
   호스팅**(`_workspace/huni-page-compose/01_recon/pie-canvas-model.md:20`) · REST 미노출(`:97`). 옮길 수 있는
   물건이 아니다. 실제 일은 **발행본 DB 목적지 대응**이고 ④로 옮겼다(NEW-M12).
2. **`STD-ADC` 는 7이 아니라 6** — `STD-ADC-014` 는 t56 이 재배정→서희항 판정.
3. **초안 접두 10개 합(163) ≠ 남은 일(189)** — 빠진 26행을 성격대로 배정.
4. **`STD-SYS` 25행은 한 축이 아니다** — 7행이 ①③④의 일. `STD-SYS-045`(서버키)는 **②의 선행**이다.
...
입력 rejudge.csv = 735행 / 김동학·남은일 = 189행 / 출력 = 205행
  ...
  ('4.페이지빌더·정보화면 도구', '신규제안'): 3
  ('4.페이지빌더·정보화면 도구', '원장'): 23
  ('5.Lightsail 이전·시스템배선', '신규제안'): 3
...
PASS G3 — 전체 205행 = 원장 189 + 신규 16 · 축 5개 1:34 2:46 3:69 4:26 5:30
...
6. **라이브 접속 0** — `provided` 23행 evidence 의 셀러어드민 URL 은 t56 이 9/16 에 본 것이고
   내가 재확인한 것이 아니다.
7. 작업량·기간 0. **행수는 일의 크기가 아니다.**

---

## 7. 리드 회신용 한 줄

t61 완료 — `WT-mall-role-todo` · 산출 `08_system-screen/t61/`(mall-todo.md · axis-rows.csv 205행 =
원장 189 + 신규 16 · huni-mall-decisions.md · verify.py 7게이트 전부 PASS).
분모는 **189**(카드 187 과 2행 차 · 원인 §2). 리드 5축 초안 **정정 4건**(최대: ⑤의 페이지빌더
Lightsail 이전은 성립하지 않는다 — 파트너사 Vercel 호스팅). 샵바이 최신 명세 9개 직접 대조 완료 —
삭제 3경로 전부 huni-mall 호출 0건(회귀 없음), 원장 판정 전제 변경 4건.
