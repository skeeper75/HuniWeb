...
**변경이력:** 최신: 2026-07-04(6) 판형 오배선 2차 전수교정(스티커/문구/봉투)+원인분석·재발방지 — 1차에서 합판도무송26 완제품 as-plate를 "고정가 무관"으로 임의제외한 오류 교정. 정의[HARD]=판형=impos_yn='Y' OR 판걸이수 시트·완제품 as-plate=결함(가격영향 무관). 13상품 COMMIT(합판도무송/타투→스티커판형 SIZ_521·낱장자유형 A4 garbage제거·대형/문구셋트7/봉투 판형불요 제거)·전상품 잔존결함0. ★상시게이트 `_foundation/batch/plate_wiring_integrity_check.sql`(전상품 전수·결함0=정합)+원인분석 문서. [[plate-size-authority-pangeori-sheet-260704]]. 직전: 2026-07-04(5) 판형 1차 교정(국4절 SIZ_499·3절 SIZ_475·14상품). 직전: 2026-07-04(4) 굿즈 addon 배선(볼체인8색·grand 11,000) + 수량구간 할인 이미 완비 검증(NO-OP). 직전: 2026-07-04(3) 굿즈 OPTION-same-price 7상품 형상/방향 CPQ(레더/우드/규조토코스터·슬로건·말랑키링·광목·린넨에코백·가격동일이라 공식없이 CPQ만·옵션그룹7/아이템17/siz9·webadmin PASS·src=PRODUCT_PRICE). ★[HARD] 폰케이스류=준비단계(상품명만·구성요소/가격 없음·작업금지·[[goods-phonecase-prep-stage-not-workable-260704]]). 다음=addon·수량구간 할인 배선(money). 직전: 2026-07-04(2) 굿즈 variant 32상품 GB-2 전파 완주 — 라이브 COMMIT 2배치(전부 webadmin simulate 검증·undo/백업). ①clean 28(파우치15·거울2·키링3·클립보드2·기타6·siz재사용22/신규45·단가행67·CPQ그룹27+아이템65·자재delink43)②특수 4(194 워터북보틀·198 피크닉매트·217 만년스탬프·226 아크릴쉐이커[TBD공식 은퇴+rebind·인쇄면 siz화+글리터 무가CPQ]). ★교정 3건: search-before-mint(기존 정확 치수 siz 재사용·중복방지[HARD])·186/187 동일siz 공유·가격상이→상품별 구성요소(공유COMP는 siz당 단일가·라이브/webadmin 무충돌 입증)·OPT_REF_DIM.03(자재) 옵션은 ref_key2=usage_cd 필수. 다음=폰케이스 등록(미등록 40기종)·OPTION-same-price 계열 CPQ·addon/수량구간 배선. [[goods-variant-formula-fixed-price-model-260704]]. 직전: 2026-07-04 미적재 2시트 가격 적재+굿즈 variant 공식식 파일럿(240·공유 PRF/COMP_GOODS_FIXED_SIZ mint) → `_workspace/huni-dbmap/CHANGELOG.md`
...
**변경이력:** 최신: 2026-07-03 **레더/하드커버 셋트 계열 면지 통합 재설계 4셋트(072→077→082→088) 라이브 COMMIT 완주**(§34 적재 거버넌스 O-1이 계열 체계결함 적발→라우팅) — 각 면지=별도 빈멤버 3~4개→면지 1멤버로 통합·나머지 색멤버 은퇴·면지자재(MAT_382/383/384[+385 인쇄면지])+면지옵션 부모→면지멤버 이관·색/인쇄 택1=면지멤버 용지 드롭다운(기본 화이트)·신규 mint 0·물리 DELETE 0·순서[HARD] 자재→옵션→부모은퇴(fn_chk_opt_item_ref). 각 codex DISAGREE 0·S1~S8 GO·골든 무손상 실측(072/077/088=34,100/159,100/796,900·082=30,184/151,844/818,438·면지 공식0행+직접단가0행 둘 다·component_prices 0행·셋트공식 mat 미종속)·webadmin 실화면 PASS(제외0·PRICE≠0·색 드롭다운·빈멤버 미노출)·undo 각 보유(`06_load/leather-hardcover-membrane-{072,077,082,088}-undo.sql`). ★082/088=면지 4택1(인쇄면지 MAT_385 포함 기여0)·USAGE.07 자재 불가침(082 링·088 D링 이중가드 배제·활성 보존). ★088=내지없음·088-redesign-260702(표지 9,000·SSABARI·pending)와 완전 직교(순서 무관·미터치). 목표 모델=사용자 directive(셋트 표지+내지+면지1멤버·색 내부 택1). ★내지 min/max=구성원개수(db_comment) 페이지수 오등록이나 작동 미물림(memberMulti=+추가버튼·페이지=semi_role.01+page_rule)→미변경·§23-inner. 후속 D-1(구성원 옵션그룹 UI 렌더)·D-2(내지 페이지가격)·D-3(인쇄면지 인쇄비). [[set-membrane-1member-taku1-target-model-260703]]. 직전: 2026-07-02(밤) **088 Q2 해소(레더 링바인더 A4=9,000·출력소재관리 0702=제3문서·사용자 전달)→재설계 확정→codex 13/13 합의→게이트 S1~S8 전부 GO — ★적재 승인 대기(COMMIT 미실행)** — 모델=[표지 소재+인쇄 9,000/부(member 089·mint 2)]+[싸바리 제본비(COMP_BIND_SSABARI@PROC_000098 고아→부모 배선·COVERBIND 배선 폐기)]·골든 1부39,000/10부290,000/100부1,800,000 실엔진 오차0·이중합산0·인접 072/077/023/024 무영향·표지 작업사이즈 3종(611/622/636×374·D링 31/42/56)=생산메타(가격축 아님)·적재본=`03_design/088-redesign-260702/apply.sql`(8행)+undo·게이트=`05_gate/088-redesign/`·다음=승인→load-executor COMMIT→webadmin 실화면(제외0·PRICE≠0)·잔존=cover_mult ×2(C트랙)·9,000 가격표 IMPORT 역반영 큐. 직전: 2026-07-02(오후) **엽서북 시뮬레이터 수렴 + 전 상품 수량 진단·교정 + 권위 260702 채택** — ① 094 구성원 빈 껍데기 13행 COMMIT(사이즈·자재·도수·판형)→실화면 충전·판걸이 정상 ② 화면 0원 유일 원인=셋트 UI siz_cd 미전파 코드 1점 확정(menu-first: 데이터-only 후보 6개 전부 ROLLBACK 실측 NO-GO)·`DEV-REQUEST-set-sim-sizcd-260702.md`(백필 원천=내지[HARD]·094/097/100 동시 해소) ③ 부모 all-in=권위 정합 판정(468셀 verbatim 전수 일치·자식 분리 금지) ④ 수량 전수 진단·교정 COMMIT(094 min2·097 min6·065 단가행 54→1·사이즈 수량규칙 49행·016 max 2행)→(상품×사이즈) 신규 TRAP 0 ⑤ 잔여 큐=미니 2상품 수량축 컨펌·박 6=제약규칙(§31)·역전지도 4,071(배지 근거) ⑥ 권위 260702 diff 채택(33+32셀·★스티커 소재 연당가 변경=High 재적재 후속). 직전: 2026-07-02(후속) 088 실무진 답변 반영(제본=싸바리 6밴드 verbatim 일치·배선만 잔여·Q2 레더 A4 소재+인쇄비 CONFLICT 재질문 `03_design/088-Q2-재질문-260702.md`) → `_workspace/huni-set-product/HANDOFF.md`·`CHANGELOG.md`
...
**변경이력:** 최신: 2026-07-01 **가격구성요소 진단·교정 + fn_calc_pansu 개발 수정요청서** — ① **디지털인쇄 base 공정 PROC_000004 미바인딩→인쇄비 영구0 18건 COMMIT**(016만 mand보유·위젯공정=product_processes·채점은 base_print_proc 코드주입해 갭은닉·필수공정 자동선택. 16건+019투명[흰토너008+CMYK004 공존]+023모양) ② **COMP_CUT_FULL_DIECUT 단가형×판수 이중적용 과대청구→.03 고정 COMMIT**(023 8.04M→120K·046 1.35M→50K) ③ 가변component(027/028/029) 정상 확인=직전 "다음시작점(가변)" **오진단 정정**(dim_vals 개수:N 정상·개수 detail 전달시 작동) ④ 썬캡051 진단 정정(비종이X→**출력용지규격/판걸이수 매칭 미반영**·권위 3절 330×540·판수1·국4절 오적재·기하 fn_calc_pansu=0=견적불가) ⑤ **fn_calc_pansu 개발 수정요청서**(`DEV-REQUEST-fn-calc-pansu-260701.md`+재현 `pansu-repro-260701.csv` 27케이스·불일치15=저11/과4·±7~80%·투명자재종속→키=(prd_cd,siz_cd)·lookup우선+폴백) ⑥ 판형 권위 전수감사(국4절22 정상·3절 라인 미완성). **★후속(2026-07-01): 위젯 설계(§6 컨버전트랙 신설)→가격연결 silent-0 진단(sim_meta 뷰어재현 `contribution_sim_scan.py`=진짜 F층·정적 proc-coverage는 과적발)→책자표지288/290/292 base proc COMMIT→썬캡 판형 재확인**(권위=판걸이수시트 pangeori-l1.csv·330×540·판수1·★권위값 기하계산도 0=lookup유일해법 확증·DEV§8/§8.1). **★★후속2(2026-07-01·판형/fn_calc_pansu 해소+webadmin 전수확인): t_siz_pansu 신설(2인자 시그니처 유지·pricing.py 무수정·lookup우선→기하폴백)+11건 판걸이수 forward교정 COMMIT(fn 11/11·폴백 회귀안전)·webadmin 가격시뮬레이터 실화면 전수확인(엽서8·인쇄배경지9·라벨택24 라이브 반영)·썬캡3절 SIZ_000535(330×540) 신규등록 COMMIT(기하판수=1·lookup불요·직전 "기하0" 오판정정)·스티커LOAD_ENG0 15=가짜(고정가by-siz)·투명019=잔여C트랙(prd_cd 인자=pricing.py). ★썬캡 3절 완전 이관 COMMIT(견적0→31,530원·webadmin 제외0): 상품↔판형 연결=t_prd_product_plate_sizes·fn_best_plate 자동선택·단가행 90개 복사+PROC_000004 필수공정. ★도메인 규칙 12항 정본(SOT) 신설=`_workspace/_foundation/HARNESS-DOMAIN-RULES-260701.md`+CLAUDE.md §1 [HARD]. 미마무리=투명019 자재종속(C트랙)·썬캡 용지 3절단가 재적재·3절 나머지3상품(112/049/030)·역방향4·HOLD_BASIS32·완칼골든.** 여정=`_workspace/_foundation/SESSION-JOURNEY-260701.md`. → `_workspace/huni-price-table-integrity/HANDOFF.md`·[[pansu-authority-fn-calc-pansu-260628]]·[[harness-domain-rules-12-260701]]·[[silent-zero-final-simscan-260701]]
...
**변경이력:** 최신: 2026-07-02(배선 서브트랙 7세션·전수 수렴) **데이터로 닫을 수 있는 배선 결함 0 달성** — 전수 재측정(wiring round21 결함10·셋트19 §23정합·F층 120상품 HIGH13→표적실측으로 진짜 2상품만 확정) → 교정 5건 COMMIT(①COMP_BIND_HC_TWINRING 삭제오염 복원 ②110 엽서캘린더 PROC_000004 누락→1,057→16,357원 ③020 화이트인쇄 인쇄옵션 0건→SPOT 발현 ④고아 3 은퇴 use_yn=N ⑤포맥스 5mm BOARD 동형확장 8500/13000/10000/16000 verbatim) · codex high 교차 7/8 AGREE·포맥스 리스크 reconcile(폼보드 기채택 특성)·사용자 전체승인 · 3중검증(dryrun V1~V7·postverify ALL PASS·webadmin 실화면 110/020/130 제외0) · **round22 = 결함 6 전부 아크릴 *_TBD(실무진 BLOCKED)·고아0·삭제오염0**. 잔여=실무진 답변(TBD 6+088 질문서)·컨펌큐(포맥스 A1/3mm 권위충돌·교차조합 위젯제약·028) → `_workspace/_foundation/batch/wiring/HANDOFF.md`·[[formula-components-wiring-subtrack-260701]]. 직전: 6세션 CONFIRM 12→0·지그재그 라벨스왑·벽걸이 이중권위 통일
...
**변경이력:** 최신: 2026-07-03(사상기반 재검증) 가격엔진 사상(pricing.py) 기준 "가격영향 제약" 전 상품 재검증 — ★핵심: 크기(siz_width/height=이하상한 ceiling+ERR_ABOVE_MAX)·수량(min_qty=이상하한) 구간차원은 엔진이 **이미 처리**→제약 불필요(정합검증만)·자유치수/박크기 27상품 "제약불필요"로 정정(C-9 폼빌더 수치범위 개발 가격정당성 소멸)·박크기 min/max(foil_w/foil_h)=엔진 미참조=위젯 주문가드(§6). 3분류=제약신설 6확정+11AMBIG(폼보드·포맥스보드 등록됨/소량전단지·접지리플렛·와이드접지리플렛 코팅×두께 §7대기/오시미싱·박최소수량·커팅사이즈 실무진대기) / 정합검증만 21전수PASS(크기범위 불일치0) / 엔진외 17. 실무진 질문서 통합 작성(`_meta/실무진-확인질문서-260703.md`). 결론=새 가격영향 제약 사실상 없음·위젯 선택↔생산=§6 배선. 직전: 2026-07-02(재실행) 컨펌 큐 처리 — 디지털 34상품 "제약 MISSING" 해체(진짜 신규=047 코팅×두께 1건)·wave-3 047 COMMIT+실화면 PASS(C-10 금지형 전환)·아크릴 5 미출시 확정 → `_workspace/huni-constraint-rules/CHANGELOG.md`
...
**변경이력:** 최신: 2026-07-04 굿즈파우치봉투103(SB-2/3/4) 완성 GO(누적 250상품)=문구/굿즈 그룹(순번5) 전체 완료. 단품 103(고정가룩업59·NEITHER-gap42·공식3·미출시use_yn=N 10)·그래프 노드1361·하드0·멱등·게이트 14시나리오 가격오차0(고정가룩업 59/59 라이브 verbatim)·NEITHER-gap 정직·O1~O7 GO. ★교훈: **O5 코드 spec §151 정렬+가격 gap 화이트리스트**(build_graph.py·비가격 gap[sewing/오염/empty-shell] O5 우회 차단·graph-build-spec v1.0.5)·**고정가룩업**=직접 unit_price·공식없음→gap-goods-fixed-lookup-no-formula(가격 gap·값 props 기재)·**live-snapshot 20260702 노후**(병행 세션 굿즈 적재 중→신규 라이브 SELECT 재프라이싱·H-1 드리프트 33 정정)·색상값≠substrate. 다음=아크릴26(별도 pack-acrylic). 직전: 문구셋트25 GO(누적147·sparse 정직·구성원 slug=자기 prd_cd) → `_workspace/huni-ontology-kb/_meta/CHANGELOG.md`
...
**변경이력:** 최신: 2026-07-03 첫 종단 실행(파일럿 디지털인쇄+072 하드커버 셋트)·오차단 0·NO-GO 0 — ① 디지털 KEEP63/EXTEND21·기준정보 잉여중복 0(옵션↔기준정보 문자적 공존=CPQ 필수)·codex가 Claude 놓친 실결함 적발(042 스타드림 종이 옵션 ref→굿즈 비종이 자재 오배선=잘못된 금액)·§7 재배선 9작업 승인 인계 ② 072 셋트 옵션 NO-OP(전건 KEEP·evaluate_set_price PRICE≠0) ③ ★O-1 격상=계열 전체 면지 구조 결함(072/077/082/088 면지 빈멤버 13개→면지 1멤버 내부 택1 목표 모델·내지 min/max=구성원개수 db_comment 확정, 24~300 페이지수 오등록이나 load-bearing)→§23 재설계 착수(DEV-REQUEST 보류) ④ Phase4 NO-OP → `_workspace/huni-load-governance/_meta/CHANGELOG.md`
