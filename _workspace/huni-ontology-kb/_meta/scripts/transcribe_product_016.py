...
# -*- coding: utf-8 -*-
"""
전사 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — product-016-premium-postcard 전용.

기존 transcribe_snapshot.py(공통 축)를 수정하지 않고, 프리미엄엽서(PRD_000016)의
상품별 차원·BOM·연결 수치를 라이브 스냅샷에서 결정론적으로 전사한다.
- 사이즈(+사이즈별 수량규칙) · 자재 BOM(활성) · 공정 라우트(mand/opt) · 판형 · 추가상품(템플릿)
- 옵션그룹 · 제약(데모) · 가격공식 바인딩
노드 본문의 표는 이 스크립트가 뽑은 것을 그대로 옮긴 것(사람 손전사 아님).

사용:  python3 transcribe_product_016.py           # stdout markdown 블록
...
def marker(tbl):
    return f"<!-- transcribed-by: _meta/scripts/transcribe_product_016.py from {SNAP_ID} {tbl} PRD_000016 @ {STAMP} -->"
...
def materials():
    ...
    out = [marker("t_prd_product_materials+t_mat_materials (활성 del_yn≠Y)"),
           ...
           "", "| mat_cd | 자재명 | usage_cd |", "|---|---|---|"]
    for r in rows:
        out.append(f'| {r["mat_cd"]} | {mm.get(r["mat_cd"], {}).get("mat_nm", "?")} | {blank(r.get("usage_cd"))} |')
    return "\n".join(out)
...
if __name__ == "__main__":
    for title, fn in [("사이즈+수량규칙", sizes), ("수량 규칙(상품/묶음)", qty),
                      ...
                      ("옵션그룹", optgroups), ("제약(데모)", constraints),
