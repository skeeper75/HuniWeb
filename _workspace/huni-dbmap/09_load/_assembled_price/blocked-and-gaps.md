...
## §A 차단(blocked) — 후니 siz 등록 대기 (component_prices, 1,817행)

> **siz 교정 통합(2026-06-06, dbm-load-builder)**: GUK4 870 + GP원형35mm 10 = **880행**이
> 기존 라이브 siz로 **해소(RESOLVED)** → 차단 2,697 → **1,817**. 권위:
> `02_mapping/price-siz-mapping-inspection.md §1-1/§1-4`(데이터 입증·search-before-mint) +
> 라이브 read-only SELECT(SIZ_000499/SIZ_000422 실존 확증). 발명 0·기존 siz 재사용.
...
### §A-0 RESOLVED(880행) — 기존 라이브 siz로 해소, 차단→적재 승격

| 군(placeholder) | → 교정 siz_cd | 행수 | 증거(라이브 실존, del_yn=N) |
|----------------|---------------|-----:|----------------------------|
| `SIZ_PENDING_GUK4` | **`SIZ_000499`** (316x467) | **870** | 국전(636x939)/4 표준 정합·impos_yn=Y(조판=출력판형)·OUTPUT_PAPER_TYPE.01(국전계열) plate 유일행. 870행 distinct siz=1 → 단일 공유 출력판형 데이터 입증 |
| `SIZ_PENDING_GP_원형35mm` | **`SIZ_000422`** (원형35x35) | **10** | siz_nm 정확 일치(round-4 G9가 적발한 매칭) |
| **소계 RESOLVED** | — | **880** | FK `fk_prc_comp_prices_siz_cd` PASS. 무발명/search-before-mint. |
...
### §A-1 잔여 차단(1,817행) — 후니 모델링 결정/등록 선결

| 군(placeholder 접두) | 차단 행수 | distinct siz | 내용 | 정확한 해소 조건 |
|---------------------|----------|-------------|------|----------------|
| `SIZ_PENDING_3JEOL` | 304 | 1 | 3절 출력판형 | 후니가 3절 출력판형을 `t_siz_sizes`에 siz_cd 1종 등록(SIZ_000077/SIZ_000475 후보, 엑셀 cut치수 1건이면 확정 — 인스펙션 §1-2) |
...
> [정직 표기] 3절은 완성품 규격(siz_cd 본의)이 아니라 **출력판형**이나, round-2가 후니 스키마상 siz_cd 재사용으로 표현 가능하다고 확정(HANDOFF §3). 면적군(POSTER·ACRYL)은 "좌표 siz_cd 방식"(가로×세로 조합=siz_cd, 사용자 확정)이며, 면적함수 도입 시 등록 규모(262종)가 달라질 수 있어 후니 결정 여지를 병기.

### 차단/교정 행 프로비넌스
- 원본 `02_mapping/load_price/t_prc_component_prices.csv`(4,805행) 중 `siz_cd LIKE 'SIZ_PENDING%'` = 2,697행.
  - 그 중 **880행**(GUK4 870 + GP원형35mm 10) → siz_cd 1:1 치환(`SIZ_PENDING_GUK4→SIZ_000499`, `SIZ_PENDING_GP_원형35mm→SIZ_000422`) → 적재본 `04` 로 **승격**. (다른 차원 컬럼 불변·무발명. 교정 산출 `02_mapping/load_price/_corrected_siz/t_prc_component_prices__guk4_corrected.csv`, 실행 SQL `09_load/_exec_price/04_prc_component_prices.sql` 의 `-- src: … siz:<from>-><to>` 주석으로 행별 추적.)
  - 잔여 **1,817행** = 본 §A-1, 적재본에서 **제외** 유지.
- 적재 04 = 2,108(즉시) + 880(교정) = **2,988**. 적재 2,988 + 차단 1,817 = 원본 4,805(정합).
...
| siz 교정 적재가능 (GUK4 870 + GP35mm 10) | 880 | `_exec_price/04_prc_component_prices.sql`(승격) |
| **적재가능 소계** | **2,988** | (2,108 + 880) |
| 잔여 차단(후니 siz 등록 대기) | 1,817 | 본 문서 §A-1 (적재본 제외) |
| **component_prices 소계** | **4,805** | (2,988 + 1,817, 원본 정합) |
...
2. **siz 교정 완료(880행, 비차단)**: 국4절(GUK4)→`SIZ_000499` 870 + GP원형35mm→`SIZ_000422` 10 — 기존 라이브 siz 재사용으로 **이미 적재 승격**(후니 등록 불요). 잔여 cosmetic: `SIZ_000499` siz_nm을 `국4절(316x467)`로 보정할지(운영 명명, 비차단).
3. **siz 등록 잔여 6군(1,817행)**: 3절·STK·POSTER·ACRYL·GP(원형35mm 외)·ENV.
