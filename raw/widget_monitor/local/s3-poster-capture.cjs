...
async function run() {
  ...
  const serialized = redact(JSON.stringify(result, null, 2));
  fs.writeFileSync(path.join(OUT, `s3_${PRODUCT}.json`), serialized);
  fs.writeFileSync(path.join(RAW, `s3_${PRODUCT}.json`), serialized);
