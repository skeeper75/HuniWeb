...
① 스티커 상품 판형 교체: `t_prd_product_plate_sizes` 스티커 12행(반칼 10상품+스티커팩+타투)은 살아있으나(`del_yn='Y'` 아님 — 최초 조회가 LIMIT 20이라 삭제행만 보였던 리드측 오류였음), 판형이 `SIZ_000521`(330x470)로 잘못 등록돼 A6·100x140이 9판(권위 8판)으로 환산된다. A6 반칼 유포 1,000장 기준 현재 등록값 638,400원 vs 권위 712,500원(과소). 조치 = 「복구」가 아니라 **`SIZ_000521` → 국4절 `SIZ_000499` 로 교체**. 대형 사이즈(400x600 등)는 3절 판형 신규 등록 필요.
