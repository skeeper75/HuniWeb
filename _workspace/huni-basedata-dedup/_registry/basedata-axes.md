...
| `t_proc_processes` | 공정(인쇄/후가공) | proc_nm | proc_cd | 102 / 102 / 101 | **13** | N (단가행 proc_cd 없음) | **HIGH** |
| `t_cat_categories` | 카테고리(고객 IA·트리) | cat_nm | cat_cd | 326 / 315 / — | **11** | N | **HIGH** |
...
### 우선순위 근거
- **HIGH**: 표시중복 sniff가 많고(11~13) **가격 비종속** → 정리해도 단가행 영향 없어 가장 안전. `t_proc_processes`(UV×3, 반칼/봉제/부착/오시/완칼/미싱/핑크 ×2 등), `t_cat_categories`(부자재/명함/특수스티커/하드커버책자/레더에코백/탁상형캘린더/엽서북 등).
