...
## ★전상품 배치 진단 스캐너 = 구축·실행 완료 (260707)
- **도구**(`batch-scan/`·재사용): scan_body.js(sim-meta 자동기본선택→/simulate·top-level await)·gen_scan.py(슬라이스)·run_all.sh(배치·백그라운드)·analyze.py(버킷분류). 읽기전용.
- **260개 스캔**: 정상162·기성15(견적제외)·반제품37(셋트경유)·**진짜결함 17**(real-defects.csv).
- **결함 17 원인분류**: siz_cd불일치 2(모양명함 SIZ_000147vs008·낱장투명스티커)·책자차원 5(중철/무선/PUR/트윈링/하드커버링=**스캔한계 의심**·내지페이지/제본 미선택·무선책자는 실제 정상)·미적재셀 6(봉투·스티커·폼보드·미니배너·화이트인쇄엽서·포토카드·아크릴키링)·공식복합 2(미니보드스탠딩·머그컵=공식0).
- **교훈**: 옵션공정(박·코팅·가변·귀돌이) 미선택 제외는 정상(오탐)·**진짜신호=최종가0/base구성요소 제외**. 책자·배너는 깊은선택 필요→스캐너 기본선택 개선 여지.
- **다음(수정 루프)**: 상품별 원인 실측→권위 확인→webadmin 교정→재스캔. siz_cd불일치는 §17 dedup 얽힘(어느 코드 정본). 책자 5는 깊은선택 재검증 우선(결함 아닐 것).
