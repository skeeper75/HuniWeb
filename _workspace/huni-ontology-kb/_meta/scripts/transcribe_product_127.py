...
# -*- coding: utf-8 -*-
"""
전사 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — 타이벡프린트 PRD_000127 전용.

transcribe_product_126.py(레더아트프린트·실사 면적매트릭스) 패턴을 계승. 타이벡 특성 차이:
  - ★자재 2개: MAT_000187(타이벡·MAT_TYPE.05·마스터 활성) + MAT_000188(타이벡소프트·MAT_TYPE.05·
    마스터 del_yn=Y 2026-06-27 논리삭제). 상품링크는 둘 다 활성(del_yn=N)·dflt_yn=Y → 188은
    링크활성/마스터삭제 불일치(정직 관찰·SIZ_000293 A1 동형). → 자재 표에 master_del 컬럼 추가.
  - 공식 = PRF_POSTER_TYVEK(use_yn=Y) → COMP_POSTER_CANVAS_FABRIC(★[동형결합] 4소재 통합:
    캔버스패브릭포스터·레더아트프린트·메쉬프린트·타이벡프린트·use_dims=[siz_width,siz_height,min_qty]).
    구 per-tyvek COMP_POSTER_TYVEK_PRINT는 use_yn=N(레거시 은퇴·use_dims=[siz_width,siz_height])—배선 안 함.
  - 카테고리 CAT_000076(아트프린트·부모 CAT_000004 포스터·lvl2·main_cat_yn=N) 1행. 사이즈=A3/A2/A1 이산 preset.
  - ★비종이류(대형 롤): 판형 무의미 — plate 3행 전부 del_yn=Y(2026-06-30 정리)·output_paper_typ 공백.
...
사용:  python3 transcribe_product_127.py            # stdout markdown
       python3 transcribe_product_127.py --json     # cache/transcribed-127-260703.json
...
STAMP = "2026-07-03"
PRD = "PRD_000127"
SELF = "transcribe_product_127.py"
AREA_COMP = "COMP_POSTER_CANVAS_FABRIC"
LEGACY_COMP = "COMP_POSTER_TYVEK_PRINT"
...
def build():
    ...
    mats = by_prd_all("t_prd_product_materials.csv")         # del 표기 위해 전 행(링크)
    ...
    def mat_row(r):
        ...
        return {"mat_cd": r["mat_cd"], "mat_nm": m.get("mat_nm", "?"),
                "mat_typ_cd": m.get("mat_typ_cd", "?"), "usage_cd": r["usage_cd"],
                "dflt_yn": r["dflt_yn"], "link_del": r.get("del_yn", "N"),
...
def md(d):
    ...
    o.append(_tb("t_prd_products PRD_000127"))
    ...
    o.append("| mat_cd | 이름 | mat_typ | usage_cd | dflt | 링크 del | 마스터 del |")
    o.append("|---|---|---|---|---|---|---|")
    for r in d["materials"]:
        o.append(f'| {r["mat_cd"]} | {r["mat_nm"]} | {r["mat_typ_cd"]} | {r["usage_cd"]} | {r["dflt_yn"]} | {r["link_del"]} | {r["master_del"]} |')
    ...
    o.append(_tb("t_prc_component_prices (COMP_POSTER_CANVAS_FABRIC/TYVEK_PRINT 집계)"))
...
if __name__ == "__main__":
    ...
    if "--json" in sys.argv:
        ...
        pth = os.path.join(os.path.dirname(__file__), "cache", "transcribed-127-260703.json")
