...
### S4 — 아크릴 [신규 componentType NC-2]

- **(a) 어댑터:** ACNTHAP fixture 보유([SRC fixtures], `vTmpl_price`, custom W/H). 아크릴 옵션단가([PRC §8.5] 고리/자석/바디칼라)를 **OptionGroup(NC-2)** 으로. SizeMatrix2D는 S3 NC-1 재사용.
- **(b) componentType:** **신규 NC-2 `option-addon-picker`** — 가격기여 옵션(고리없음 0 / 은색구슬줄 300 / 금색고리 1200 등 [PRC §8.5])을 라벨+추가단가 함께 표시하는 선택형. option-button과 다른 점: **각 옵션 값에 가격 델타 표시**(투명성). 단 가격은 여전히 BFF 권위 — 표시값은 어댑터가 라벨에 병기(`priceColorCount`류 평면화 안 함, 라벨 텍스트). 사이즈=NC-1.
...
| **NC-2** | `option-addon-picker` | S4 (아크릴) | 가격기여 옵션(고리/자석/바디칼라) 라벨+추가단가 표시 선택형 | **변형 가능** (option-button + 값별 부가표시) — 단 가격델타 표시가 RULE-2 버튼과 시각 다르면 신규 | union 1개 추가 검토. 추가단가는 **라벨 텍스트로 병기**(서버권위 유지, 계약 가격필드 추가 금지) |
...
| **신규 componentType (flag)** | **S3=NC-1(dimension-matrix-input, 확정), S4=NC-2(option-addon-picker, 변형우선), S5=NC-3(image-option-selector, image-chip variant 우선)** |
...
- NC-2(option-addon-picker)·NC-3(image-option-selector) **신규 vs variant 최종 판정** — Figma 확인 필요(S4/S5 진입 시).
