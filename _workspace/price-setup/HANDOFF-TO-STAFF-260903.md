...
### 10-6. 기존 7블록 대조 결과 + 교정 실행 (2026-09-03 · 지니 승인)

전체 차이표는 `_workspace/price-setup/t34b/P4-DIFF-260903.md`.
**겹치는 모든 칸에서 값차이 0** 이었습니다 — 권위와 라이브가 다른 값을 든 칸은 없었습니다. 문제는 「값」이 아니라 「덮는 범위」였고, **지니 승인 후 아래 3그릇을 권위대로 채웠습니다.**

#### ⚠ 실무진 통지 — 오늘부터 아래 구간의 견적 금액이 오릅니다

| 대상 | 이전 | 지금 | 사유 |
|---|---|---|---|
| 귀돌이(둥근모서리) **101~299장** | 2,000 | **4,000** | 권위 `101~300` 구간의 하한이 라이브에 `300`으로 들어가 있었습니다 |
| 미싱(1줄) **301~499장** | 15,000 | **17,000** | 권위 `301~500` 구간의 하한이 `500`으로 들어가 있었습니다 |
| 귀돌이(둥근) **6,000~10,000장** | 51,000 | **61,000~101,000** | 권위 상단 5구간이 라이브에 없어 5,000 구간 값으로 계산되고 있었습니다 |
| 오시·미싱(1줄) **6,000~10,000장** | 105,000 | **125,000~205,000** | 동상 |

2·3줄도 같은 폭으로 함께 채웠습니다(오시·미싱 각 45행). **그 밖의 수량 구간 금액은 하나도 바뀌지 않았습니다** — 100·300·500·1,000·5,000장을 대조군으로 넣어 계산해 확인했습니다.

#### 무엇을 어떻게 바꿨나

| 그릇 | 이전 | 지금 | 내용 |
|---|---|---|---|
| `COMP_PP_CORNER_RIGHT` 귀돌이 | 18행(9구간) | **28행(14구간)** | 하한 `300`→`101` · 6,000~10,000 추가 |
| `COMP_PP_CREASE_1L` 오시 | 30행(10구간) | **45행(15구간)** | 6,000~10,000 추가(하한 수정 없음 — 권위가 `500` 단일 표기) |
| `COMP_PP_PERF_1L` 미싱 | 30행(10구간) | **45행(15구간)** | 하한 `500`→`301` · 6,000~10,000 추가 |

- 값은 권위 엑셀을 프로그램이 직접 읽어 넣었습니다. **적용일은 기존 `2026-06-01` 을 그대로 유지**했습니다(새 날짜로 갈리지 않게).
- 라이브 재검산: 3그릇 118행 **누락 0 · 잉여 0 · 값차이 0 · 적용일차이 0**.
- 실제 견적 재계산(골든) 81줄 대조: 변한 줄 24개가 **전부 위 표의 구간**이고, 의도 밖 변화 **0**.
- 되돌리기용 원본은 `_workspace/price-setup/rollback/t34b-p6/before-rows.csv` (교정 전 78행).

#### 이름 2건 변경

| 그릇 | 이전 이름 | 지금 이름 | 구 이름 보존 |
|---|---|---|---|
| `COMP_ENV_MAKING` | 봉투제작(합판) | **봉투제작 완제품가** | 비고에 「구 이름: 봉투제작(합판)」 |
| `COMP_CUT_FULL_DIECUT` | 커팅 완제품가 완칼(모양엽서·라벨택) | **커팅_국4절_완칼** | **비고 아님** — 비고가 이미 478자로 한도(500자)에 걸려, 권위 근거와 정정 이력을 지키려고 비고는 그대로 두었습니다. 구 이름은 이 문서와 `P4-DIFF-260903.md`·progress 에 남아 있습니다 |

코드·유형·차원·단가행은 둘 다 그대로입니다(72행·60행 불변).

#### 손대지 않은 것

- **가변텍스트·가변이미지** — 권위 메모가 「수정없음」이라 그대로 뒀습니다. 최종수정 시각이 `08-30 16:46` 로 남아 있어 확인 가능합니다.
- **완칼(국4절)·봉투제작** 단가표 — 값·행수 모두 권위와 이미 일치(수량구간까지 완전 일치)라 이름만 바꿨습니다. 완칼의 라이브 `SIZ_000535`(330x540) 36행은 권위 메모가 「국4절만 적용」이라 권위가 안 덮는 영역이며 그대로 뒀습니다.
- **10,000장 초과** — 권위 주석(「1,000장당 20,000원씩 올립니다」 등)은 증가식이라 단가표에 담을 자리가 없습니다. §10-5 그대로입니다.
- **가격유형 표기 3건** — 권위 「(합가)」 vs 라이브 고정금액(귀돌이·오시·미싱). 이번 교정 범위 밖이라 그대로 두었습니다.

#### 봉투 B15 — 실무진 변경분 귀속

라이브 60행 최종수정일 **07-07 40행 · 09-03 20행**. 오늘 레자크 2종을 자재별로 분리하신 것으로 보이고, **그 20행도 권위와 값차이 0** 입니다. 저희는 이 그릇의 단가표를 읽기만 했습니다(이름만 변경).

### 10-7. 저희가 확인하지 못한 것

- 신설 8그릇으로 **실제 견적을 돌려보지 않았습니다.** 아직 어느 공식도 물지 않아 계산할 대상이 없습니다. 배선 후 검산이 필요합니다.
- §10-6 교정 3그릇은 견적을 실제로 돌려 확인했습니다(골든 81줄 · 상품 4 · 그릇별 3상품 · 수량 9종). 다만 **21개 상품 전수가 아니라 대표 4상품**이며, 2·3줄은 단가표 대조로만 확인했습니다(골든은 1줄 기준).
- 완칼 그릇의 **구 이름이 비고에 들어가지 못했습니다**(비고 500자 한도). 문서에만 남아 있습니다.
