...
## 4. ★ 컨펌 큐 — 전건 해소 (5건 → 0건·2026-06-18)

> 5건 전부 사용자 결정/활성화로 해소. **잔여 컨펌 = 0.** 1순위 자재·카테고리 축 등록 명세 확정.

### 4.1 컨펌 확정 (5건·전건 해소·재제안 금지)
...
>
> **escalate 처리:** **잔여 0** — 리더 escalate 불요. **CAT_000302/304 소프트삭제는 절대 금지**(활성 상품 유일 main·전손)·재연결만(upr_cat_cd UPDATE).
...
| 컨펌 결정 | — | **잔여 0** — 컨펌 5건 전건 해소(B-MAT-3·AX-1·부속typ·AC-2·C-CAT 부모매칭). 부모 = CAT_000134·CAT_000198 확정 |
...
- **★최종 확정 요지:** ① 컨펌 5건→0(B-MAT-3 본체색 유지·AX-1 잉크색 옵션·.10 3용도 분해·AC-2 dbmap 위임·C-CAT 부모 CAT_000134/CAT_000198 확정) ② 축이동 90→89·교정 ~26→.10→.07 ~17 ③ 카테고리 소프트삭제 11+재연결 2(부모 확정·302/304 활성 상품 del_yn 금지) ④ del_yn='Y' 권위(B5 해소)·라이브 임시책·근본 경로 Y.
