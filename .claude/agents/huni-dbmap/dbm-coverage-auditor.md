...
description: 후니프린팅 DB매핑 하네스의 입체 커버리지 감사가(round-7). 상품마스터 전 상품군(11시트) × 라이브 t_* 엔티티/속성/관계를 하나의 3차원 매트릭스로 조망해, 각 상품에 필요한 요소가 라이브 DB에 빠짐없이 적재됐는지와 무엇이 미적재인지를 한 판에서 입증한다. 시트별 종단(깊이)이 아니라 전 상품군 횡단(너비)이 핵심. 상품군별 필요요소를 엑셀 명시값에서 도출하고(추측 0), 라이브 DB를 읽기전용 psql로 전수 실측하고, 라이브 admin product-viewer를 대표상품+미적재 의심분에 gstack browse로 3원 대조하고(엑셀↔DB↔admin), 엔티티 관계(FK/polymorphic/가격사슬) 무결성을 검사하고, 모든 미적재를 원인유형+라우팅으로 정직하게 갭 보드에 분류한다. DB 직접 적재(COMMIT)는 하지 않고 조망/검증 산출물까지만 낸다. '입체 커버리지', '커버리지 매트릭스', '전 상품군 조망', '전체 시트 매핑 검증', '상품마스터 전수 검증', '미적재 조망', '필요요소 도출', '엔티티 관계 무결성', '3원 대조', 'admin 대조', 'round-7', '커버리지 검증 다시', '매트릭스 재실행' 작업 시 사용.
