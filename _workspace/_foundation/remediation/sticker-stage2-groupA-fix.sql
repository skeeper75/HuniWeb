-- sticker-stage2-groupA-fix.sql — ★실 COMMIT (인간 승인 2026-06-28·사용자 '승인·COMMIT 진행')
-- dryrun(groupA) 전 어서션 PASS 검증 후 변환. 백업 선생성·verbatim·되돌리기 보유.
\set ON_ERROR_STOP on
BEGIN;

-- 물리 백업 (pre-state·undo용)
DROP TABLE IF EXISTS bak_sticker_s2_sizes_260628;
CREATE TABLE bak_sticker_s2_sizes_260628 AS
  SELECT * FROM t_prd_product_sizes WHERE prd_cd IN('PRD_000052','PRD_000053','PRD_000054','PRD_000055','PRD_000056');
DROP TABLE IF EXISTS bak_sticker_s2_mats_260628;
CREATE TABLE bak_sticker_s2_mats_260628 AS
  SELECT * FROM t_prd_product_materials WHERE prd_cd IN('PRD_000055','PRD_000056','PRD_000057');
\echo '== 백업 생성 완료(sizes·materials pre-state) =='
...
COMMIT;
\echo '== COMMIT 완료 — 그룹A 라이브 적재(undo=bak_sticker_s2_* + note LIKE %stage2%) =='
