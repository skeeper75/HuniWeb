...
\echo '== 구조 검증: 6상품 바인딩=PRF_CLR_ACRYL · 옵션그룹 0 · 템플릿/단가/링크 12 =='
SELECT prd_cd, string_agg(frm_cd,',') AS binds,
  (SELECT count(*) FROM t_prd_product_option_groups g WHERE g.prd_cd=pf.prd_cd) AS optgrp_remain
FROM t_prd_product_price_formulas pf
WHERE prd_cd IN('PRD_000146','PRD_000148','PRD_000149','PRD_000150','PRD_000152','PRD_000154')
GROUP BY prd_cd ORDER BY prd_cd;
SELECT 'tmpl' k, count(*)::text v FROM t_prd_templates WHERE tmpl_cd BETWEEN 'TMPL-000015' AND 'TMPL-000026'
UNION ALL SELECT 'tmpl_price', count(*)::text FROM t_prd_template_prices WHERE tmpl_cd BETWEEN 'TMPL-000015' AND 'TMPL-000026'
UNION ALL SELECT 'addon_link', count(*)::text FROM t_prd_product_addons WHERE tmpl_cd BETWEEN 'TMPL-000015' AND 'TMPL-000026';
\echo '== 단가 verbatim 확인 =='
SELECT t.base_prd_cd, t.tmpl_nm, tp.unit_price FROM t_prd_templates t JOIN t_prd_template_prices tp USING(tmpl_cd)
WHERE t.tmpl_cd BETWEEN 'TMPL-000015' AND 'TMPL-000026' ORDER BY t.tmpl_cd;
ROLLBACK;
