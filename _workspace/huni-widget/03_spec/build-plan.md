...
- 계약 테스트: 양 어댑터 출력이 정규화 타입 스키마 일치. [HARD] 가격 검증 의미 = "후니 어댑터가 후니 가격 API 응답을 `NormalizedPriceBreakdown`로 올바로 매핑하는가" — **Red 역공학 가격값과 후니 가격값을 비교/정합하는 것이 아니다**(별개 가격 공식). Red 가격 역공학은 Red 위젯 동작 설명용 참고로만 유효.
...
| # | 항목 | 출처 | 영향 | 폴백 |
|---|------|------|------|------|
| O1 | 비책자(굿즈/아크릴) 가격 요청 정확 필드 | [역공학 미검증] | 어댑터 분기 | 책자 계약+정적. 검증 = "후니 어댑터가 후니 가격 API를 정규화 계약으로 올바로 사상하는가"(Red값과의 정합 비교 아님) |
| O2 | 후니 할인/금액 산정 방식 | [후니 자체 정책] | 어댑터 finalPrice | 후니 가격 API가 최종금액 반환 → 어댑터가 finalPrice로 사상. Red 워터폴은 참고일 뿐 후니에 강제 안 함 |
...
| 2026-06-02 | 가격 정합 가정 제거 — Red price_gbn/ORD_INFO+PCS_INFO/3단 워터폴을 후니에 강제·정합하지 않도록 보정(Red 역공학은 참고로 보존). 서버 권위 + 클라이언트 캐싱(디바운스 300ms·옵션조합 캐시 TTL 30s·hit/miss 흐름) 명시. 대상: price-engine.md §0·2·3·5·7, data-adapter.md §4, data-contract.md §3, build-plan O1·O2·§3, price-engine-reversed.md §1·3·4 | 사용자 정정 — 후니는 자체 가격 공식 보유, Red→후니 가격 정합/포팅 금지 |
