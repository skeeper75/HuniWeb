...
## 재확인 (보정 검증 — 2026-06-06, 변경분만)

> 빌더가 F-2(MINOR)·F-3(INFO)를 정정(매니페스트 문서표기만, 적재본 CSV·집계 무변경). 본 재확인은 변경분만 대조하고 나머지 게이트의 기존 PASS를 승계한다. 적재본 데이터 결함은 1차에서 0건이었고 본 정정은 문서표기 수준이라 회귀 0.

| 발견 | 정정 위치 | 확인 | 결과 |
|------|----------|------|------|
| **F-2** mat_cd/clr_cd 카운트 | `load-manifest.md §2` | `mat_cd 10 \| 10/10 (insertable)` 정정 ✓ · `clr_cd` 판정에 "insertable distinct 0, CLR_000002/000005는 차단 행에만 출현" 단서 추가 ✓ (1차 라이브 확인값 mat 10·clr 0과 일치) | **RESOLVED** |
| **F-3** columns.csv stale | `load-manifest.md §4` | "G4 권위=라이브 DDL, columns.csv는 t_prc_* 미수록 stale, 차후 재추출 권장" 주석 추가 ✓ (파일 재추출은 별도 작업) | **RESOLVED** |

**회귀 0 확인:** 적재본 load CSV 무변경(component_prices 2,108행 불변) · 집계 2,108+2,697=4,805 불변 · `assemble_price_bundle.py` 재실행 후 load CSV md5 불변(스크립트엔 FK 카운트 문자열 없음 = 매니페스트 수기, 재실행 시 카운트 되살아나지 않음).

## 최종 종합 판정: **GO — MINOR RESOLVED**

G1~G9 전 게이트 PASS, BLOCKER·MAJOR 0, F-2/F-3 RESOLVED. 가격표 적재본은 적재 가능성을 통과한다(즉시 적재가능 2,319행 무결·차단 2,697·GAP 1 명시·침묵드롭 0·재현 멱등·독립 검증 G9). 잔여는 게이트 실패가 아닌 **인간 승인 에스컬레이션 5건**(위 §). 실제 INSERT·코드행/siz 등록·라이브 DRY-RUN은 본 트랙 종착점 너머.
