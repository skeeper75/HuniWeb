...
NEW = [
    ...
    ("5", "NEW-M12", "[NEW-M17 하위] 발행본 원천 DB 이전 대응(Railway → Lightsail webapp) — 읽는 쪽·쓰는 쪽 동시 전환",
     "migration-plan.md:52 F2-3(vc_ 2객체 재덤프 → webapp) · /Users/innojini/Dev/huni-skin-shopby/src/lib/printly/publications.ts:85(SELECT)·:14-17(PRINTLY_WORKSPACE_ID 범위 고정)",
     "F2-3 은 **읽는 쪽**(몰)만 옮긴다. **쓰는 쪽**(페이지빌더)의 발행 대상 DB 를 같이 바꾸지 않으면 오픈 후 상세탭 발행이 멈춘다 — 원장 행 0"),
    ...
    # ── 페이지빌더(vibe-canvas) 후니 맞춤 제작 → Lightsail 이전 (지니 확정 260919)
    ("5", "NEW-M17", "[상위] 페이지빌더(vibe-canvas) 후니 맞춤 제작 → Lightsail 이전",
     "_workspace/huni-page-compose/01_recon/pie-canvas-model.md:16-24(현 상태: Next.js App Router+Turbopack · Vercel icn1 · Auth.js v5+Google OIDC 단독 · Supabase Storage) · 지니 확정 260919(페이지빌더 = 김동학 대표 작업분 · Lightsail 통합)",
     "이전 대상 4개(webadmin·DB·huni-mall·페이지빌더) 중 페이지빌더만 migration-plan F1~F4 에 행이 0. 아래 NEW-M12·M18~M22 가 이 행의 하위"),
    ("5", "NEW-M18", "[NEW-M17 하위] 페이지빌더 컨테이너화·배포 파이프라인 — 저장소 소재부터",
     "pie-canvas-model.md:18-19(Next.js App Router + Turbopack · Vercel icn1) · 저장소 미발견(§ 돌린 패턴은 mall-todo.md ⑤)",
     "huni-mall 은 standalone 출력+Dockerfile 이 이미 있었다(next.config.ts:7). 페이지빌더도 같은지 **확인하지 못했다** — 소스 저장소를 로컬에서 찾지 못했다"),
    ("5", "NEW-M19", "[NEW-M17 하위] 페이지빌더 저장 스키마·DB 목적지 확정",
     "pie-canvas-model.md:97 G7(「페이지/블록 저장 스키마(실 DB 형태) — 서버 액션 기반, REST 미노출」)",
     "무엇을 옮겨야 하는지가 아직 문서로 없다. 발행 결과(vc_v_product_detail_tabs)는 알지만 **원본 페이지·블록이 어느 DB 에 어떤 형태로 사는지**가 미확인"),
    ("5", "NEW-M20", "[NEW-M17 하위] Supabase Storage 이미지 자산 이관 판정",
     "pie-canvas-model.md:22(「스토리지 = Supabase Storage」 · /help/media) · :23(PNG/JPEG→WebP 변환)",
     "Lightsail 통합이면 이미지 원천을 S3 로 옮길지 Supabase 를 남길지 결정해야 한다. 옮기면 발행본 HTML 안의 이미지 URL 이 전부 바뀐다(publications.ts 의 image_url)"),
    ("5", "NEW-M21", "[NEW-M17 하위] 페이지빌더 도메인·Google OIDC Redirect URI 전환",
     "pie-canvas-model.md:17(현 도메인 vibe-canvas-builder.vercel.app) · :21(Auth.js v5 + Google OIDC 단독 · /api/auth/providers)",
     "huni-mall 의 NEW-M16(소셜 3사)과 같은 종류의 일이 페이지빌더에도 따로 있다. 로그인 수단이 Google 단독이라 Redirect URI 하나가 틀리면 편집기 접근이 통째로 막힌다"),
    ("5", "NEW-M22", "[NEW-M17 선행] /sites·/assets 500 원인 해소",
     "pie-canvas-model.md:30(라우트 표면 실측: /sites·/assets = 500) · :98 G8(「서버측 — 파트너사 확인(C4)」)",
     "이전 전에 정상 상태를 만들어 두지 않으면 이전 후 500 이 이전 탓인지 원래 탓인지 가릴 수 없다"),
