...
description: >
  후니프린팅 가격엔진 이해·진단 하네스 오케스트레이터. 가격엔진 5개 장치(가격공식·가격구성요소·할인테이블·가격뷰어·
  가격시뮬레이터)의 역할·조합 메커니즘을 원리로 정의하고, 코드(pricing.py·price_views.py)가 DB 엔티티 속성에 맞게
  구현됐는지 진단하며, 결론 전에 "아는 것 vs 모르는 것"을 분리하는 이해·진단 전용 하네스. 3인 팀(mechanism-researcher
  장치 역할 정의 / code-schema-auditor 코드↔DB 속성 정합 / binding-validity-designer U-7 배선 유효성). 검증·결론이
  아니라 이해·정의·진단까지·라이브 읽기전용·DB 미적재. 트리거: 가격엔진 이해, 가격엔진 진단, 가격 장치 역할, 조합
  메커니즘, 코드 DB 정합, 속성 단위 구현 진단, 아는것 모르는것, 지식격차 리서치, U-7 배선 유효성, 오배선 적발,
  특정 장치만 진단, 진단 다시. 정합 검증·P1~P7 게이트·골든 재계산·교정은 huni-price-quote가 담당.
---
