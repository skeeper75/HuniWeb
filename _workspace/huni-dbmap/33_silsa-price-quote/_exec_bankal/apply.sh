#!/usr/bin/env bash
# ============================================================================
# 반칼 모양 062/063 가격 연결 로더 (apply.sh)
...
"${PSQL[@]}" -At -F, -c "\copy (SELECT prd_cd,frm_cd,apply_bgn_ymd FROM t_prd_product_price_formulas WHERE prd_cd IN ('PRD_000062','PRD_000063') OR frm_cd='PRF_STK_FIXED' ORDER BY prd_cd) TO '$BACKUP_DIR/pre_bindings.csv' CSV HEADER"
echo "[backup] 1개 CSV 저장 완료 (undo 근거)"
