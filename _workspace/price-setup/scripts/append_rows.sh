#!/bin/bash
# D-16 — 이미 행이 있는 그릇에 단가행을 「덧붙인다」.
# 기존 행은 화면이 들고 있는 값을 그대로 다시 얹으므로 한 칸도 바뀌지 않는다.
# 적용일 일괄 설정 체크박스는 건드리지 않는다(기존 행 적용일 보존).
...
CODE=$1; NEWROWS=$2   # NEWROWS = JSON 배열의 배열
...
sleep 5
$B screenshot --viewport "$WT/_workspace/price-setup/screens/d16/$CODE-before.png" >/dev/null 2>&1
...
(() => {
  ...
  const add = $NEWROWS;
  const have = new Set(cur.map(r => String(r[1])));
  for (const r of add) if (have.has(String(r[1]))) return 'ALREADY_PRESENT:' + r[1];
  inst.setData(cur.concat(add));
  return 'rows ' + cur.length + ' -> ' + inst.getData().length;
})()
JS
RES=$($B eval $SP/append.js 2>&1 | tail -1)
echo "[$CODE] 그리드: $RES"
case "$RES" in *NO_INSTANCE*|*MISMATCH*|*ALREADY_PRESENT*) echo "[$CODE] ★ 중단 — 저장 안 함"; exit 1;; esac
...
sleep 5
$B screenshot --viewport "$WT/_workspace/price-setup/screens/d16/$CODE-after.png" >/dev/null 2>&1
