...
| **C-09** | 배경지 043/044/045 (+상품권 041/042) | 카테고리 | MIS-LOADED | 043/044/045→CAT_000296 배경지(NULL orphan)·041/042→CAT_000295 상품권(NULL orphan) | 정상 포장 노드에 연결: 043→**CAT_000273**(인쇄배경지OPP, 012 하위 실재)·044→**CAT_000274**(투명케이스, 실재)·045→**CAT_000275**(인쇄헤더택, 실재) | 정체=카테고리 012 포장(MES 012·product-master:82). **정상 노드 273/274/275는 이미 upr=012로 실재**(F-GATE-1 재측정). load_master가 시트 `구분`→잉여 고아 296/295를 별도 생성하고 상품을 거기 연결(L-H). | **상품을 기존 정상 노드(273/274/275)로 재연결**(`t_prd_product_categories` UPDATE, search-before-mint) + 잉여 고아 296 논리정리. 296 upr=012 UPDATE는 차선(잉여 노드 잔존) | High | 컨펌(Q-ID-B)+load-execution |
...
| **C-14** | 라벨택 046 | 카테고리 | MIS-LOADED | CAT_000296 배경지(NULL orphan, 라이브 확인) | **CAT_000283 라벨/포장스티커**(upr=CAT_000012 포장, 실재) | 정체=포장 라벨(product-master:175·388). 잉여 고아 그룹(배경지 296)에 오연결(L-H). 정상 노드 283은 이미 012 하위로 실재(F-GATE-1) | **상품을 기존 정상 노드 CAT_000283으로 재연결**(`t_prd_product_categories` UPDATE, search-before-mint) | Med | load-execution |
...
- **Q-ID-B [🔴·신규] 배경지/상품권/라벨택 카테고리 재연결** — 상품 043/044/045/046은 잉여 고아 노드 CAT_000296(배경지)에, 041/042는 CAT_000295(상품권)에 연결됨(둘 다 upr_cat_cd=NULL). **정상 포장 하위 노드는 이미 실재**: CAT_000273 인쇄배경지OPP·274 투명케이스·275 인쇄헤더택·283 라벨/포장스티커(전부 upr=CAT_000012 포장). (a) 상품을 기존 정상 노드로 재연결 + 잉여 고아 296/295 논리정리(권장·search-before-mint) (b) 고아 296/295의 upr만 012로 UPDATE(잉여 노드 잔존) — 어느 쪽? 상품권 041/042의 정상 노드도 확인 필요(03 인쇄홍보물 하위 vs 별도).
