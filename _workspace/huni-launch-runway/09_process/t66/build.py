...
def build_decisions(gaps, tree_rows, ranked):
    ...
    L = []
    # 건수는 절대 상수로 쓰지 않는다 — 한 카드가 세는 법을 고치면 문구만 낡는다(260919 실제로 겪음).
    L.append('# t66 — 지니가 정해야 하는 것 %d건 (「라 결정 미정」 전수)\n' % len(items))
    ...
    L.append('**줄 세우는 기준.** ① `기다리는것` = %d건 중 **자기 `선행` 칸에 이 결정의 row_id 나 ' % len(gaps) +
...
def build_quickwins(gaps, tree_rows):
    ...
    L.append('네 카드의 빠진 곳 %d건 중 **선행이 비었거나 「없음 — 바로 착수 가능」이라고 적힌 것**만 골랐다. ' % len(gaps) +
