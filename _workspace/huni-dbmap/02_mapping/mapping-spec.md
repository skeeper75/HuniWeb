...
- **apply_ymd / apply_bgn_ymd — `2026-06-01`** (yyyy-MM-dd, **D-E 확정**). DESIGN DECISIONS D2 참고.
...
| `apply_ymd` | D-E 확정 | `2026-06-01` (varchar(10), yyyy-MM-dd) | `2026-06-01` |
...
| `dsc_tbl_cd` | group → 코드 | 범위 표대로 | `DSC_ACR_QTY` |
| `apply_bgn_ymd` | D-E 확정 | `2026-06-01` (varchar(10), yyyy-MM-dd) | `2026-06-01` |
| `note` | — | NULL | |
...
- **D2 (적용일) — CONFIRMED (D-E).** `apply_ymd` (details)와 `apply_bgn_ymd` (links)는
  `2026-06-01` (yyyy-MM-dd) 확정. 스키마상 둘 다 `varchar(10)`이며 `2026-06-01`(10자) 들어맞음.
  모든 행이 한 날짜를 공유 → 전역 치환 완료. (그룹별 상이 일자는 미적용 — 단일 go-live 가정.)
