...
## 스티커 절 (이산 siz_cd 단가형 + 세트 합가형 혼합 종단 · 라이브 실측 2026-06-20) — formula-map-sticker.md 참조

> **★스티커는 dbmap round-23 적재 완주로 6종단 중 라이브 완성도 최고** — 16/16 바인딩·7소재 전개·타투/팩 합가형 교정·SIZ_520 반칼 분리. **그러나 "구조 완성·정합 미완"** — 공식·comp·단가행·바인딩 4계층 모두 실재하나 **바인딩 siz/mat ↔ 단가행 siz/mat 키 정합이 4곳에서 깨져** evaluate_price가 no_match(가격 0/오류). dbmap 6문서(2026-06-17/18)의 "13 미배선·3 collapse·062/063만 GO"는 stale·해소됨(본 절이 권위·2026-06-20 재실측).

```
스티커 가격공식 baseline (라이브 실재·전건 바인딩):
🟢 PRF_STK_FIXED(이산 siz_cd 단가형·COMP_STK_PRINT 2,694행·7소재·siz 19종)
🟢 PRF_STK_TATTOO(.02 합가형·min_qty=3·4000) · PRF_STK_PACK(.02·min_qty=54·4000) · PRF_GANGPAN_FIXED(.01·370행)
바인딩: BOUND 16/16 (052~067 전건·use_yn=N 063/064 포함)
```

| ID | 갭 | 유형 | 심각도 | designer 작업 |
|----|----|------|--------|---------------|
| **G-STK-1** | **055/056/057 소재코드 불일치** — 상품 바인딩 mat=154(유포지)/243(투명전용지)이나 COMP_STK_PRINT 단가행 mat=153(유포스티커)/162(투명스티커)·154/243 단가행 0행 | 바인딩 정합 단절(WIRE형) | 🔴 High | 정합 교정 택1: ① 상품 055/057 mat 바인딩 154→153·056 243→162 재바인딩 / ② 단가행 mat 153/162에 154/243 동일가 추가행. **돈크리티컬·라이브 실측 no_match 확정**. 어느 mat 코드가 정답인지 권위(상품마스터 소재명) 대조 선결 |
| **G-STK-2** | **052/053/054 SIZ_172 오염** — 반칼인데 A4 바인딩이 SIZ_172(B02 낱장 4000)·반칼 B01 5000 아님. 058~061엔 SIZ_520(반칼 A4 분리) 적용됐으나 052~054엔 미적용 | siz 공유 오염 | 🟡 Med (돈크리티컬) | 052/053/054의 A4 바인딩 SIZ_172→SIZ_520(반칼 전용·이미 실재) 교체. dbmap Q-DC-2 058~061엔 적용·052~054 누락분 동형 처리 |
| **G-STK-3** | **바인딩 siz 단가행 부재** — 052/053/054 SIZ_196(A6 105x148)·062/063 SIZ_058(100x140) 단가행 0행 | 단가행 결손 | 🟡 Med | A6·100x140 단가 출처 컨펌(가격표 B01 6사이즈에 A6/100x140 부재) → 적재 or 바인딩 제거. 추측 적용 금지(돈크리티컬·라이브 price_rows=0 실측) |
| **G-STK-4** | **064 잠정 단가** — 소형반칼 7사이즈 전건 [잠정] B01 col1 규격가(6000/7000) 사이즈무관 복사·실측 미수령 + SIZ_036(94x94)/043(80x80)은 굿즈(인쇄배경지/해더택) siz 재사용 | 잠정가 + siz 의미혼선 | 🟡 Med | 실측 소형반칼 단가 수령 시 교체(소형은 판걸이 32 vs 4·단가 다를 개연). SIZ_036/043 굿즈 siz 재사용 = 064 전용 siz 분리 검토(의미 충돌). 사용자 "우선 적용·추후변경" 정책 |
| **G-STK-5** | **타투 base 2000(1~2장) 미반영** | 컨펌큐 | ⚪ Low | 상품 min_qty=3·qty_incr=3이라 정상주문 base 무발현. 1~2장 주문 허용·flat 2000 여부 컨펌(dbmap Q-TAT-1). 정상 3장 경로는 GO |
| **G-STK-6** | **팩 수량 환산단위** (장÷54 vs 세트) | 컨펌큐 | ⚪ Low | 위젯 수량 입력이 장(min_qty=54·.02·÷54)인가 세트(세트당 4000)인가 컨펌(dbmap Q-STK-3b). 현 라이브=장 단위(min_qty=54) |

**designer 우선순위**: ① G-STK-1(055/056/057 소재 불일치=no_match)=가격 0 직결 최우선 → ② G-STK-2(052~054 A4 오청구·돈크리티컬·이미 실재 SIZ_520 재바인딩만) → ③ G-STK-3(A6/100x140 단가 출처) → ④ G-STK-4(064 잠정·실측 수령 시)·G-STK-5/6(컨펌큐). **핵심: 단가값 결손 아님(단가행 3,066개 가격표 verbatim·값결함 0)·전부 바인딩 siz/mat 정합 교정**(WIRE형 동근).

**돈크리티컬 안전(동형결함 점검)**: ① ×qty 폭발 = 없음(단가형 단가가 개당/장당·합가형은 ÷min_qty 환산). ② silent 이중합산 = 없음(각 공식 comp 1개·합산대상 1). ③ min_qty NULL ValueError = 안전(타투 3·팩 54 명시·신규 .02 행 INSERT 시 min_qty 가드 필수). **유일 위험 = 정합 결함 4종(G-STK-1~4)의 오청구/no_match**(설계 선결).
