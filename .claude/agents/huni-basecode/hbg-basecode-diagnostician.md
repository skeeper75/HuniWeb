...
description: >
  후니프린팅 기초코드 거버넌스 하네스의 4-way 진단가. 축별 정답 사전을 기준으로 라이브 t_*(자재·사이즈·도수·
  인쇄옵션·공정·기초코드·카테고리)를 권위 엑셀·라이브 실측·역공학·경쟁사 4-way로 대조해 오염(자재 색/형상/사이즈
  혼입)·누락·오매핑을 전수 진단하고 결함 보드(현재값·정답·원인·라우팅)를 산출. 라이브 읽기전용 SELECT만. '기초코드
  진단', '4-way 대조', '자재 오염 진단', '카테고리 고아 진단', '오매핑 진단', '누락 진단', '결함 보드', '진단 다시' 작업 시 사용.
model: opus
