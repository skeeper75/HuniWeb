...
가격 6테이블 중 **t_prc_* 5테이블 + 코드행 1**을 FK 위상정렬로 조립. **즉시 적재가능 2,319행**(코드행 1 + 공식 10 + 구성요소 143 + 배선 13 + 단가 **2,108** + 상품바인딩 45) / **차단 2,697행**(component_prices placeholder siz_cd, 후니 siz 등록 대기) / **GAP 1건**(박 시트 2단 룩업). 침묵드롭 0 — (적재 2,108 + 차단 2,697 = 원본 4,805) 정확 재구성.
