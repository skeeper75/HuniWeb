---
id: product-265-linen-mini-eco-bag
type: product
anchor: t_prd_products/PRD_000265
badge: verified
sources:
  - {source_file: "live-snapshot/latest/t_prd_products.csv", source_locator: "키:PRD_000265 (린넨 미니에코백·prd_typ_cd=PRD_TYPE.01·nonspec_yn=N·use_yn=Y·del_yn=N·min1/max10000/incr1·QTY_UNIT.01)", captured_at: "live 20260702_1119", badge: verified, src_id: SR-5-livesnap}
  ...
  - {source_file: "live-snapshot/latest/t_prd_product_prices.csv", source_locator: "키:PRD_000265 unit_price(GP-1 base 단일고정가·260610 verbatim·전사표)", captured_at: "live 20260702_1119", badge: verified, src_id: SR-5-livesnap}
relations:
...
answers_cq: ["구체 상품 질의(린넨 미니에코백 구성·가격)", "백류/에코백 탐색"]
tags: ["#백류", "#봉제상품", "#비종이류판형없음", "#고정가룩업"]
updated: 2026-07-04
---

# 린넨 미니에코백 (product-265-linen-mini-eco-bag)

린넨 미니에코백(`PRD_000265`)는 **백류·필통(봉제) 완제품 단품**(`prd_typ_cd=PRD_TYPE.01`)이다. 셋트 아님
...
- **정체**: pack §1.3(FRESH) + live-snapshot(`PRD_000265` 실재·use_yn=Y·del_yn=N).
- **사이즈/도수**: 라이브 `t_prd_product_sizes`·`t_prd_product_print_options` **0행**(has_size·has_print_option 미배선·정직 표기).
- **substrate 자재**: 린넨(MAT_000184). ★비종이 → 판형 없음(도메인 [HARD]).
...
<!-- transcribed-by: awk t_prd_products.csv PRD_000265 from live-snapshot/latest (snap_20260702_1119) @ 2026-07-04 -->
...
<!-- transcribed-by: awk t_prd_product_materials.csv + t_mat_materials.csv PRD_000265 from live-snapshot/latest (snap_20260702_1119) @ 2026-07-04 -->
| mat_cd | 자재명 | mat_typ | usage_cd | 역할 판정 | KB 배선 |
|---|---|---|---|---|---|
| MAT_000184 | 린넨 | MAT_TYPE.05 | USAGE.07 | 실 substrate(원단/가죽) | uses_material 배선 |
...
<!-- transcribed-by: awk t_prd_product_prices.csv PRD_000265 from live-snapshot/latest (snap_20260702_1119) @ 2026-07-04 -->
| prd_cd | apply_ymd | unit_price | note |
|---|---|---|---|
| PRD_000265 | 2026-06-10 | 16500.00 | GP-1 base 단일고정가 §21 R-GP4-1 (260610 verbatim) |
