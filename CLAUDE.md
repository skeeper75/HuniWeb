...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-13 | **round-17(가격공식 정리 검증 트랙 신설) — 하네스 확장** — 사용자 `/harness:harness`("admin 가격관리 가격공식이 제대로 정리됐는지 확인·공식명/비고 실무진 가독성·가격구성요소 사용가능·가격뷰어 확인"). huni-dbmap round-17 확장: 신규 에이전트 `dbm-price-formula-auditor`(라이브 `t_prc_price_formulas`↔webadmin 소스 `price_views.py`/가격허브·뷰어 mockup↔admin 화면 **3중 대조**)·스킬 `dbm-price-formula-audit`·오케스트레이터 round-17. **4축 판정**(정리 상태·실무진 가독성·`formula_components` 배선 사용가능성·`price_viewer` 노출) + 실무진 정리표 + 결함 보드 + 공식명/비고 개선안(쉬운 한국어). 게이트 F1~F5(생성자≠검증자 `dbm-validator`). **즉시 포착 쟁점: round-16은 "frm_typ_cd 라이브 부재" 전제였으나 `price_views.py:234`가 frm_typ_cd 실사용 → 라이브 information_schema 결판이 검증 1순위. round-16 가격사슬 단절(광범위)이 "사용가능성·뷰어 노출" 결함 원천.** 산출 루트 `21_price-formula-audit/`. 비파괴(DB 미적재·실 교정 인간 승인). | `.claude/agents/huni-dbmap/dbm-price-formula-auditor`(신규)·`.claude/skills/dbm-price-formula-audit`(신규)·`huni-dbmap-orchestrator`·CLAUDE.md | 사용자(`/harness:harness` — 가격공식 정리 검증) |
