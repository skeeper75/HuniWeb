...
## ★이번 세션(260707 오후) = 떡메모지 de-set 진단·설계·검증 완료 · 실행 BLOCKER(사용자 결정 대기)
- **판정**: 떡메모지(PRD_000097)=**껍데기 셋트**(19셋트 중 유일 단일구성원·내지098 자재0/공식0/가격0 기여). 셋트일 이유 0.
  삼중 corroboration: ①사용자 직관 ②가격표 SOT("떡메=사이즈×장수/수량·**셋트로 안 쪼갬**") ③RED 라이브(단일 완제품·용지1택·표지없음·가격축=규격×부수×매수).
- **가격중립 실증**: 097 가격=`PRF_TTEOKME_FIXED`←`COMP_TTEOKME`(단가형·use_dims[siz_cd,bdl_qty,min_qty]·그리드 2siz×2매수×28부수 완비). **셋트경로=단일경로 동일가** 실측(43,200/39,600/66,000/19,200·제외0·matched). de-set=순수 구조정리·리그레션0.
- **전 사슬 검증**(지니 지시): 공식(set플래그 없음·셋트판정은 오직 t_prd_product_sets 행에서)←구성요소←상품구성요소(siz2 90x90/70x120·매수50/100·mat 백색모조120·공정 떡제본·단면). 098 전참조=sets(sub1)·cat·products **외 0**(고아). 상세=`batch-scan/DESET-SPEC-TTEOKME.md`·`NOMINAL-SET-SCAN.md`.
- **★실행 BLOCKER(미결정)**: 셋트행(t_prd_product_sets 097←098) **논리삭제 경로가 WebUI에 없음**. 복합PK 인라인=물리삭제만(`admin.py:911-914 qs.delete()`·del_yn 필드 미노출·clean 강제"N"). 098 상품 "삭제" 버튼=논리삭제지만 **098만으론 de-set 안됨**(엔진 `_set_members_meta`가 셋트행 del_yn만 보고 sub del_yn 무시·`price_views.py:1718` → 097 여전히 is_set + 삭제상품 가리키는 불정합).
  - **두 갈래(사용자 결정 대기)**: (a) 셋트행 **WebUI 물리삭제 수용**(관계행이라 기초마스터 무삭제 원칙 무관·재구성 가능·즉시) (b) **DEV-REQUEST**(개발자가 ProductSetsInline에 [W-04] del_yn 드롭다운 이식=이미 TPrdTemplateSelectionsInline 선례·배포 후 플래그삭제·재사용·감사이력보존).
  - **[HARD] 확정**: raw/webadmin **코드는 사용자 직접수정 금지**(개발자 아님)·DB 직접쓰기 금지. → 둘 다 이 제약 하에서만.
- **명목상-셋트 전수 스캔**(`batch-scan/NOMINAL-SET-SCAN.md`): 19셋트→진짜셋트8(원자합산·건드리지말것)·**떡메형1(097)**·고정가세트1(엽서북094)·문구류10(172~181·메모패드179). de-set 동형 후보=094·179 등(떡메 확정 후).

## ★미팅 결정(260707) 기록 → `_workspace/_foundation/MEETING-NOTES-260707.md`
6항: ①UV평판=인쇄옵션 등록 ②옵션그룹=사이즈/자재/공정에 미등록 옵션만 ③**카테고리·자재·공정 정본=DB관리자 입력(엑셀 아님·권위 예외·메모리화 [[authority-db-admin-input-cat-mat-proc-260707]])** ④포토북·디자인캘린더=1품목코드 N디자인 ⑤캘린더(인쇄)=페이지룰 1+ 가격매칭 ⑥상품정보=비규격 입력 상품 체크.
