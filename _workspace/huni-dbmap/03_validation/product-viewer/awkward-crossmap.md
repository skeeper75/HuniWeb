# 데이터 불일치 366건 × round-2 가격매핑 영향 교차분석

후니 "데이터 불일치 뷰어"(`product-viewer`) 전수 스캔 **366 findings**(4 dim · 10 rule)를 round-2 가격엔진(`t_prc_*`/`t_prd_*`) 매핑 영향과 기존 어색데이터 목록(AWK-1~9)에 교차한 분석. 권위=`dbm-price-formula` 규칙1~9 + `price-engine-ddl.md` + `price-awkward-data.md`. 식별자/컬럼/코드는 영어, 해석은 한국어. (v2; v1 599에서 SIZE_DIMS_NULL 233 해소)

> [v1→v2 변경이력] 후니가 모든 size의 작업·재단 치수(work/cut dims)를 등록 → **SIZE_DIMS_NULL 233건 RESOLVED**(zero size remains all-null). 그 결과 D-C/N-1 면적상품 가격차단이 해소되고 **DIRECT 246→160**으로 감소. set-difference=제거 233행·신규 0·수정 0(다른 9개 rule·분류 전부 동일).

> 핵심: 366건은 우리가 파일럿에서 손으로 발견한 AWK 항목의 **모집단(superset)**이다. 전수 스캔으로 AWK-3/5/6이 어디까지 일반화되는지, 그리고 AWK에 아직 없던 NEW 패턴이 무엇인지 확정한다. **DB 미적재·스키마 무변경** 원칙 하에 분류만 수행한다.

---

## 0. 한 줄 결론

366건 중 **DIRECT 160 · INDIRECT 125 · NONE 81**. 가격매핑을 실제로 막는(variant→차원 매핑 또는 공식 입력 결손) 항목은 **DIRECT 160건**이며, 핵심 차단은 **mat 차원 160건**(색상/형태/용량/속성조합이 자재 슬롯에 들어가 mat_cd 미해소)으로 단일화된다. (v1의 면적상품 작업·재단 치수 결손 SIZE_DIMS_NULL 86건 DIRECT는 v2에서 후니가 work/cut 치수를 전건 등록하여 **해소됨**.) **후니 결정이 필요한(needs_huni=Y) 항목은 242건**, 결정 유형은 5종으로 수렴한다.
...
|-----|------|------|--------------|--------------------------|
| siz | ~~SIZE_DIMS_NULL~~ | ~~233~~ → 0 | **RESOLVED(v2)** (was DIRECT 86 / INDIRECT 147 → 0) | v2에서 후니가 모든 size의 작업·재단 치수를 등록 → 결손 0건. v1 근거(작업·재단 치수가 비어 면적기반 가격 입력 부재 → 면적상품 DIRECT, 고정가형 INDIRECT)는 해소됨. 면적매트릭스형 매핑 착수 가능 |
...
**요약 카운트(v2)**: DIRECT 160 = MAT_COLOR_IN_NAME 68 + MAT_SIZE_ONLY 45 + MAT_COLOR_ONLY 27 + MAT_SHAPE_ONLY 10 + MAT_ATTR_COMBO 10 (mat only). INDIRECT 125 = PRINTSIDE_INVALID 63 + PLATE_FILETYPE_FREEFORM 43 + MAT_PRINTSIDE_ONLY 19. NONE 81 = SIZE_NAME_NOISE 81. (v1 대비: SIZE_DIMS_NULL 면적 86 DIRECT·고정가 147 INDIRECT가 v2에서 전건 해소 → DIRECT 246→160·INDIRECT 272→125.)
...
## 2. 366건 → AWK-1~9 커버리지 (전수 스캔이 무엇을 확증·확장하는가)
...
| **AWK-3** | 다중가격 variant 35종(손거울/머그/키캡 등) | mat dim 160건이 variant 가격축의 모집단(v1의 면적 siz 86건은 v2 치수 등록으로 해소). **35종은 (가격포함)100종 표본**이었고 전수 스캔은 variant 패턴이 **거울/머그/키캡을 넘어 파우치 14종·아크릴 30+종·책자/캘린더/의류로 확산**함을 보임 | **대폭 확장** |
| AWK-4 | 사이즈 descriptor 불일치 150건(시트 E열 ↔ siz_nm) | v1에서 SIZE_DIMS_NULL 고정가형 147건이 동일 본질(siz_nm은 있으나 work/cut 치수 결손)로 좌표화됐으나, **v2에서 후니가 전 size에 work/cut 치수를 등록하여 RESOLVED** → 더 이상 active findings 아님 | **해소됨(v2)** |
...
### NEW 어색 패턴 (AWK-1~9에 없던 것, v2 awk_ref=NEW 125건; v1은 211건이었으나 SIZE_DIMS_NULL 86 해소)

| NEW 패턴 | rule | 건수 | 내용 | round-2 처리 |
|----------|------|------|------|--------------|
| ~~N-1 면적상품 치수 전결손~~ | ~~SIZE_DIMS_NULL(면적)~~ | ~~86~~ → 0 | **RESOLVED(v2)**: v2에서 후니가 실사/아크릴/포스터/보드/액자/패브릭/매트 등 면적상품의 work/cut 치수를 전건 등록 → 면적매트릭스형 가격 입력 확보 | 해소됨. siz_cd 치수 등록 완료 → component_prices(siz_cd) 면적단가 착수 가능 |
...
|------|-------------|-------------|-----------|
| ~~SIZE_DIMS_NULL(면적)~~ | ~~86~~ → 0 | ~~29~~ | **해소됨(v2)** — work/cut 치수 전건 등록 |
...
### 3.3 SIZE_DIMS_NULL 면적상품 29종 — 해소됨(v2)

> [해소됨(v2)] 아래 29종은 v1에서 work/cut 치수 전결손으로 면적단가가 차단됐으나, **v2에서 후니가 모든 size에 work_width/work_height/cut_width/cut_height를 등록**하여 해소됐다(PRD_000001 SIZ_000078 ww/wh/cw/ch null→70/200/70/200 등, zero size remains all-null). 추적성을 위해 상품 목록만 보존한다 — 더 이상 DIRECT 차단이 아니며 면적매트릭스형 매핑 착수 가능.
...
이 29종은 v1에서 **work_width/work_height/cut_width/cut_height가 모두 NULL**이었으나 v2에서 후니가 각 siz_cd에 치수를 등록 완료(규칙⑤: 기존 t_siz_sizes 활용 + 부족분 추가) → `t_prc_component_prices.siz_cd` 면적 룩업 키 확보.
...
| 처리 | 대상 | 방법 |
|------|------|------|
| 후니 데이터 등록 후 가능 | mat 색상/마감 105건(AWK-6, ATTR_COMBO 색상축 포함) | t_mat_materials 색상자재 추가 (스키마 무변경, 데이터 추가) |
...
| **해소됨(v2)** | ~~면적 치수 86건(N-1)~~ | v2에서 후니가 t_siz_sizes에 work/cut 치수를 전건 등록 완료 → 면적매트릭스형 매핑 착수 가능 |
