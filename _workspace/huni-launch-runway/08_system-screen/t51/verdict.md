...
webadmin 의 남은 일이 1행뿐인 것은 「할 일이 없다」가 아니다 — t48 이 webadmin 으로 넘긴 62건이
webadmin 원장에서 같은 plan_row_id 로 확인되지 않았다(§4).
...
## 4. 미수록 83건 — plan_row_id 기준 (핵심 발견 · 260919 리드 지적 반영 정정)

t48 `out-of-scope.csv` 89건의 라우팅 대상을 t49·t50 원장에 교차검산한 결과:

- 받는 원장에 **같은 plan_row_id 로 실재 6건** (STD-ADC-014→t49 · STD-FIN-007 · STD-SYS-031 · STD-MFG-110 · STD-MFG-111 · STD-MFG-116→t50)
- **미수록 83건** — 넘겨받은 원장에서 그 id 를 찾을 수 없음

### [정정] 「id 없음」을 「그 일 없음」으로 쓰지 않는다

초판은 「어느 화면 원장에도 없다」로 적었다. **과하다.** 리드 lane-1 표본 지적(260919)을 직접 재확인한 결과:

- 「쿠폰 생성·발행 관리」(STD-ADC-005)·「공지사항 관리」(STD-ADC-007)는 webadmin 으로 넘겼으나,
  **t48 의 shopby 원장에 쿠폰 18행 · 공지 11행**이 셀러어드민 `config` 행으로 **다른 plan_row_id**(STD-ADC-006·STD-PRM-001·STD-B2B-014 등)에 실려 있다.
- 초판의 「두 원장 동시 키워드 0건」은 **t49·t50 한정으로는 여전히 참**이지만, t48 을 포함하면 거짓이다. 검산 범위를 좁게 적고 넓게 주장했다.

→ 83건은 **「빠진 일」과 「라우팅 대상 오기」가 섞인 집합**이다. 단정하지 않고 기계로 갈라 상·하한으로 싣는다.
제목에서 흔한 낱말(관리·등록·설정·조회 등 44종 stoplist)을 뺀 낱말을 뽑아 세 원장 전체를 훑었다:

| 구분 | 건수 | 뜻 |
|---|---|---|
| 낱말 흔적 전무 | **6** | 어느 원장에도 비슷한 낱말조차 없음 = **「빠진 일」의 하한** |
| 어딘가 흔적 있음 | **77** | 라우팅 오기 **후보의 상한**. 낱말 일치라 과다 계상(예: 「충전 입금 확인」이 세 카드 모두에 걸림) |

흔적 전무 6건: STD-B2B-006 월 마감 청구서 발행 · STD-ADC-003 탈퇴회원 관리 · STD-MFG-106 1차포장 처리 ·
STD-MFG-112 납품명세서 출력(퀵) · STD-MFG-113 출고명세서 출력(직접방문) · STD-MFG-117 재고상품 관리.

어느 쪽도 확정이 아니다. 가르려면 webadmin·셀러어드민 재조사 1건이 필요하다.
검증기 **V7b** 가 이 단정 금지를 게이트로 고정했다(단정 표현 부재 + 상·하한 표기 + t48 실재 재확인).
...
**지어내지 않았다** — 83건은 「넘겨받은 원장에서 그 plan_row_id 를 찾지 못했다」는 사실이다.
그 이상(필요 없는 일인지·넘긴 곳이 틀린 것인지·조사에서 빠진 것인지)은 t51 범위에서 판정하지 않았다.
판정에는 webadmin·셀러어드민을 다시 훑는 조사 1건이 필요하다(후속 카드 제안).
...
| 파일 | 내용 |
|---|---|
| `reports/launch-schedule-20260919.html` | 통합 문서(315KB) — 프로세스 구조도(mermaid+noscript) · 시스템별 현황(인라인 SVG) · 구현/연동 분리 뷰 · **그룹→화면 드릴다운**(필터·검색) · 일정표 · 관리 요소 · 미수록 · 한계 |
| `reports/launch-schedule-20260919.md` | 린 마크다운 트윈(19KB · 에이전트 컨텍스트용) |
...
`moai-domain-html-report` 의 basic 티어 크기 예산은 120KB 인데 산출은 **315KB** 다.
...
[PASS] V8 산출물·핵심 수치 실재 — 누락 파일 없음 · 문서에 없는 수치 없음 · HTML 322,566B
[PASS] V9 승계 의무 주석 — 실린 주석 6/6
[PASS] V7b 미수록 단정 금지 — t48 실재 {'쿠폰': 18, '공지': 11} · 단정 표현 없음 · 상·하한 표기 있음 (흔적 전무 6 / 흔적 있음 77)
[PASS] V10 verdict 표 = 계산값 — 일치

게이트 12종 · PASS 12 · FAIL 0
EXIT=0
```
...
- **V7b**(리드 판정 후 추가) — 미수록 83건을 「그 일이 없다」로 단정한 초판 표현을 차단한다(§4).
  t48 의 쿠폰·공지 실재를 매번 재확인하고, 문서에 단정 표현이 없고 상·하한 표기가 있어야 PASS 한다.

부가 검사: 임베드 JSON 파싱 OK(747행·16열) · 인라인 JS 파싱 OK(node) · 태그 균형 div 47/47 · table 10/10.

## 9. 판정

**GO** — 게이트 12/12 PASS. 카드 t51 요구 5개 절(프로세스 구조도 · 그룹→화면 드릴다운 · 구현/연동 분리 뷰 · 일정표 · 관리 요소) 모두 산출.

### 리드 회신 (lane-1 · 260919 · 수신 완료)

1. **병합 키 정정 승인** — `CONTRACT.md` 보충 3 에 「`path:line` + 이음매 무순 쌍」으로 기록됨(반영 확인).
2. **미수록 83건은 지니 결정으로 상신** — 새 카드 개설은 운영자 권한.
   리드 표본 지적(쿠폰·공지가 t48 shopby 원장에 실재)을 직접 재확인해 §4 를 정정하고 V7b 게이트를 추가했다.
3. **HTML 크기 예산 이탈 승인** — 드릴다운 요구 우선.

리드 판정: **PASS · 재작업 없음**(브랜치·커밋·병합 3건·verify 재실행·행수·xlsx 시트 직접 확인).
이후 본 세션이 한 일은 리드 ② 지적에 따른 §4 정정 1건뿐이며, 새 주장·새 범위는 없다.
