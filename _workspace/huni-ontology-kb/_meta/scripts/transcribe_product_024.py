...
# -*- coding: utf-8 -*-
"""
전사 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — 포토카드 PRD_000024 전용.
...
사용:  python3 transcribe_product_024.py            # stdout에 markdown 블록
       python3 transcribe_product_024.py --json     # cache/transcribed-024-260703.json 갱신
...
def build():
    ...
    return {
        "meta": {"snapshot": SNAP_ID, "captured_at": STAMP, "prd_cd": PRD,
                 "generated_by": "transcribe_product_024.py"},
...
def _tb(what):
    return f"<!-- transcribed-by: _meta/scripts/transcribe_product_024.py from {SNAP_ID} {what} @ {STAMP} -->"


def md(d):
    o = []
    o.append("### 상품 정체·수량 (전사)\n")
    o.append(_tb("t_prd_products PRD_000024"))
    p = d["product"]
...
if __name__ == "__main__":
    d = build()
    if "--json" in sys.argv:
        p = os.path.join(os.path.dirname(__file__), "cache", "transcribed-024-260703.json")
