...
# -*- coding: utf-8 -*-
"""
전사 보강 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — PRD_000045 인쇄헤더택.

배경지 형제(043·044·046)의 전용 스크립트와 동형이다. 헤더택(045)의 전용 사이즈·자재/도수/
공정/판형/공식 멤버십은 파일럿 공통 스크립트 범위 밖이라 이 보강 스크립트를 새로 추가한다
(기존 스크립트 무수정·transcribe_043.py에서 PRD만 교체). 노드 파일의 사이즈 치수·수량 스칼라·
축 멤버십은 이 스크립트가 라이브 스냅샷에서 결정론적으로 뽑은 것을 그대로 옮긴 것이며,
사람이 눈으로 읽어 손으로 옮기지 않는다.

사용:  python3 transcribe_045.py           # stdout에 markdown 블록(사이즈 전사표 + 멤버십 요약)
       python3 transcribe_045.py --json    # cache/transcribed-045-260703.json 갱신
...
def build():
    ...
    return {
        "meta": {"snapshot": SNAP_ID, "captured_at": STAMP, "prd_cd": PRD,
                 "generated_by": "transcribe_045.py"},
        ...
        "raw": {"materials": mats, "print_options": popts, "processes": procs,
                "plates": plates, "formulas": forms, "categories": cats,
                "option_groups": ogroups, "addons": addons},
    }


def md_sizes(d):
    out = [f"<!-- transcribed-by: _meta/scripts/transcribe_045.py from {SNAP_ID} t_siz_sizes (PRD_000045 sizes) @ {STAMP} -->",
...
def md_membership(d):
    p = d["product"]
    out = [f"<!-- transcribed-by: _meta/scripts/transcribe_045.py from {SNAP_ID} t_prd_product_* (PRD_000045) @ {STAMP} -->",
           "| 축 | 라이브 멤버십 |", "|---|---|",
           ...
           f'| 자재 | {", ".join(m["mat_cd"]+"("+str(m["usage_cd"])+")" for m in d["materials"])} |',
...
if __name__ == "__main__":
    d = build()
    if "--json" in sys.argv:
        p = os.path.join(os.path.dirname(__file__), "cache", "transcribed-045-260703.json")
        with open(p, "w", encoding="utf-8") as f:
            json.dump(d, f, ensure_ascii=False, indent=2, sort_keys=True)
        print("wrote", p)
    else:
        print("### 사이즈 전사표 (045 전용)\n")
        print(md_sizes(d))
        print("\n### 라이브 축 멤버십 (045)\n")
        print(md_membership(d))
        print("\n### RAW dump\n")
        print(json.dumps(d["raw"], ensure_ascii=False, indent=2))
