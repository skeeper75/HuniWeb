...
WAIT_BASIS = {
    ...
    'STD-MYP-007': ('외부(토스페이먼츠)', '_workspace/huni-shopby/17_prepaid-money/prepaid-money-design-260915.md:312 「9/16 재확정 — 토스페이먼츠 가상계좌 API 직접 발급 채택」 · :314 「14-0. 확정 흐름 (2026-09-16)」 — 원장 선행의존 「EXT-PG」(이니시스)는 9/15 이전 판이라 9/16 확정이 우선 · 단 :304 전제 V24·V26 미충족 시 무통장+PayAction 복귀 대안'),
...
BLOCKED_MAP = {
    ...
    'S2-3': ('T4', 'C5', 'ext', 'wait', '외부(MES 담당)'),
    'S2-4': ('T4', 'C4', 'huni', 'config', '대표(구매)'),  # S2 findings:48 D-P1 결정자 원문 · 실명 원천 미기재 · :39 조달 문제(리드 정정 260917)
    ...
    'S4-6': ('T1', 'F4', 'huni', 'config', '김동학·PM'),  # S4 런북:131 「쇼핑개발/PM」 · :84 F4-7 「PM·쇼핑개발」(리드 정정 260917)
...
TOP = [
    ...
    dict(id='T1-1', track='T1', steps=['F1'], title='관리서버(webadmin) Lightsail dev 배포·검증 — F1-1~F1-5',
         ...
         check='AWS Lightsail 콘솔(서울 리전 · apne2)을 열어 컨테이너 서비스 dev-huni-admin-apne2-cs 의 admin 이 healthy · cron 이 running 인지 조회하고, 그 서비스의 /healthz 페이지에 접속해 {"ok":true} 가 나오는지 본다 — 서비스 전체 호스트 주소는 원천 미기재 · 확인처: 인프라팀',
    ...
    dict(id='T1-2', track='T1', steps=['F2'], title='DB 반입(PG 18.6→17) 리허설·본반입·검증 4종·vc_ 분리 — F2-1~F2-4',
         ...
         check='리허설 실행 화면에서 검증 4종(audit_data_integrity --exposure --strict · audit_published_prices · audit_set_qty_mode · audit_engine_dims)이 모두 exit 0 인지 조회하고, 테이블별 행수 지문 목록이 Railway 와 같은지 본다 — 리허설 로그 보관 위치는 원천 미기재 · 확인처: 인프라팀',
    ...
    dict(id='T1-4', track='T1', steps=['F4', 'E2'], title='스킨 컨테이너화·도메인·소셜 콜백·env 이관(F4-8 Vercel 정지는 오픈 테스트 통과 전 금지) + 로그인 리다이렉트 재검증',
         ...
         ev=(RUNBOOK, r'^### F4 '), prereq='김동학·PM Vercel 프로젝트 접근 · 외부(소셜 3사 콘솔 권한자)',
    ...
    dict(id='T2-1', track='T2', steps=['A1', 'A2'], title='상품·가격 데이터 충전 — 기본사양가 없음 2건·0원 조합 교정',
         ...
         ev=(S28, r'^A2,'), prereq='—',
         check='webadmin 가격 시뮬레이터 화면을 열어 ① 아크릴키링(PRD_000146)에 사이즈 7종 중 하나 · 고리 「고리없음」 · 수량 1 을 넣고 ② 무선책자(PRD_000069)에 본품 사이즈 A5세로 · 내지종이 백색모조지 120g · 페이지 24 · 부수 2 를 넣어 두 상품 모두 제외 구성요소 0 · 합계 0원 아님을 조회한다. 이어 상품별 기본사양가 메뉴(/admin/start-price/) 목록을 열어 「없음」 2행이 0행이 됐는지 본다'),
    ...
    dict(id='T3-5', track='T3', steps=['D3'], title='취소·반품·문의 — 범위 결정 후 스킨 버튼·문의 라우트 구현',
         ...
         ev=(S28, r'^D3,'), prereq='결정 27 · 선행 입력 「클레임·문의 범위」',
    ...
    dict(id='T4-1', track='T4', steps=['C1'], title='주문 등록 다리 — 스킨이 결제 직후 order/register 를 부르게 배선(부르는 쪽 0건)',
         owner_name='김동학', owner='huni', work='dev', status='부분',
         ev=(S2P, r'^C1,order/register'), prereq='선행 입력 「order/register 계약」 · T3-3',
    ...
    dict(id='T4-3', track='T4', steps=['C4'], title='파일 검수 경로 결정·구축 — PitStop 조달 또는 사람 검수(D-P1·D-P2)',
         ...
         ev=(S2P, r'^C4,PitStop'), prereq='대표(구매) PitStop 구매·라이선스 결정(D-P1)',
    ...
    dict(id='T4-4', track='T4', steps=['C5'], title='접수 화면·주문 상태머신·MES 접수 전송(D-P5·D-P6·D-P7)',
         owner_name='서희항', owner='huni', work='dev', status='없음',
         ev=(S2P, r'^C5,접수'), prereq='T4-3 · 외부(MES 담당) WCF 스펙',
    ...
    dict(id='T6-1', track='T6', steps=['D5'], filt=REST, title='프린팅머니 잔액 이관 — 추출 주체·개발 주체 결정 후 6단계',
         ...
         ev=(S3M, r'^## B\. '), prereq='외부(구 사이트 운영사/IDC) DB 접속 · 결정 6',
