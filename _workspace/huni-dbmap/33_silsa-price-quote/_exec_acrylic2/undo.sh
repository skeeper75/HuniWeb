#!/usr/bin/env bash
# undo.sh — 아크릴 마무리(A5 + 코롯토 B2~B4) 실 COMMIT 원복 로더. 기본 = 롤백전용 검증. --commit 시에만 실 원복(인간 승인).
# 백업 권위: backup_a5_minqty_pre.csv(81·보정 전) + 코롯토 신설 전 부재(comp/formula 0행).
...
[ -f backup_a5_minqty_pre.csv ] || { echo "FATAL: backup missing: backup_a5_minqty_pre.csv (apply.sh dryrun 으로 먼저 생성)"; exit 1; }
