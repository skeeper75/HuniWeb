...
description: 후니프린팅 한 상품군을 "견적가능"까지 종단 완주시키는 방법론 스킬(round-19). 레이어별 라운드(분석·매핑·적재·CPQ·가격)가 흩어져 끝까지 닫힌 상품군이 0인 문제를 해결한다 — 한 상품군의 엑셀 전 의미컬럼을 목표 t_*에 매핑·적재됐는지 라이브 실측으로 점검(컬럼 readiness 매트릭스, booklet식)하고, 견적가능을 Q-게이트(① UI 옵션 선택 + ② 선택의 차원 환원=생산정보 + ③ 가격 계산)로 판정하고, 미달분을 기존 라운드(round-13/5/6/16/18)로 라우팅하는 종단 조립 순서와 RTM(상품군×견적요소 진척판)을 제공한다. DB 직접 적재는 하지 않는다(점검·판정·조율·라우팅 전용). '종단 파이프라인', '상품군 종단 완주', '견적가능 검증', '견적가능 판정', '컬럼 readiness', '컬럼 적재 준비 점검', 'readiness 매트릭스', '한 상품군 끝까지', 'RTM', '진척판', 'round-19', '종단 검증 다시', '견적가능 재점검', '상품군 완주 다시' 작업 시 반드시 이 스킬을 사용. 전 상품군 횡단 엔티티 조망(round-7)은 dbm-coverage-matrix, 단일 레이어(매핑/적재/CPQ/가격)는 각 round 스킬이 담당하므로 그 작업에는 트리거하지 않는다.
