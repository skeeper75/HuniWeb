...
# -*- coding: utf-8 -*-
"""
전사 보강 스크립트 — PRD_000056 낱장 자유형 투명스티커 (D-9·§4·[HARD] LLM 손전사 금지).
...
사용:  python3 transcribe_product_056.py            # stdout markdown 블록(사이즈·자재·공정·도수·판형·수량·공식)
       python3 transcribe_product_056.py --json     # cache/transcribed-056-260703.json 갱신
...
def build():
    ...
    return {
        "meta": {"snapshot": SNAP_ID, "captured_at": STAMP, "generated_by": "transcribe_product_056.py"},
...
def md(d):
    o = []
    tb = lambda tbl: o.append(f"<!-- transcribed-by: _meta/scripts/transcribe_product_056.py from {SNAP_ID} {tbl} PRD_000056 @ {STAMP} -->")
...
if __name__ == "__main__":
    d = build()
    if "--json" in sys.argv:
        p = os.path.join(os.path.dirname(__file__), "cache", "transcribed-056-260703.json")
