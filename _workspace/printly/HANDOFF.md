...
**PART 1(Step0~3) 완결 + PART 2 (1) 브릿지 + ★전반부 종단 파일럿 실동(자동).** 다음 세션 시작점 후보(지니 확정 대기·PART 4 게이트):
- **(a) NL 입력 진입** ★권장 — 파일럿(`pilot/`)이 지금 '업종 키' 직접 입력. 이를 말문장("카페 오픈…")→intent 원자 분해로 진입 단계 추가(진짜 자연어 데모). 시스템 Stage 2/3.
- **(b) 견적 0원 결함 교정** — 파일럿이 적발한 프리미엄명함(PRD_000031)·펄명함(034) 견적0(미배선). 가격 하네스(§13/§27) 위임.
- **(c) PART 2-2 런타임 루프** — 잡티켓 생성 + 엔진 호출 에이전트 오케스트레이션(단 Q5/Q7 지니 입력 일부 필요).
- **(d) 잡티켓 정형 스키마 / 엔진 인터페이스 스펙(Q7)**.
