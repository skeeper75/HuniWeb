...
| 2026-06-13 | **round-17 개선안 A 라이브 적용 — 가격공식 비고(note) 쉬운 한국어 반영 (huni-dbmap 첫 운영 DB 쓰기)** — 사용자 "핸드오프 개선안 쉬운 용어로 실제 반영"(범위=note 16건 확정). 라이브 `t_prc_price_formulas` 16공식 `note`를 내부 행번호("계산공식집초안 행N")·영문약어(comp/mat) → 비전문가가 바로 읽는 쉬운 한국어로 멱등 UPDATE·COMMIT(16×`UPDATE 1`·exit0). 절차=백업(`rollback-note.sql` 현재값 추출)→DRY-RUN(BEGIN…ROLLBACK 16행 무위반)→COMMIT→검증(라이브 재조회 개선안 값 일치·내부행번호0·영문약어0·빈note0). frm_nm은 한국어 양호라 불변. **B(떡메모지 바인딩 Q-1)·C(frm_typ_cd DDL Q-2)는 컨펌 미해소로 미적용.** 비파괴·롤백 가능. | `_workspace/huni-dbmap/21_price-formula-audit/_apply/`(신규 APPLIED·apply-note·rollback-note)·improvement-proposal·HANDOFF·메모리·CLAUDE.md | 사용자(개선안 쉬운 용어 실제 반영) |---
