...
## 미해결·후속 (다음 세션 후보)
1. **통합 배치 hdx — 가격 파일럿 완성**(P1~P5-②c 종단). 진단 **9차원 전 커버**(배선·차원정합·공정·병합·계산가능성·옵션CPQ·수량·판형·가격격자)+교정생성(값 날조 금지)+적대적 재실측+반자동 라운드+첫 실적재(포스터 use_dims 라이브 COMMIT). 전 레이어 셀프테스트 5/5 GO. 한 명령=`--loop`. **`_workspace/_foundation/hdx/HANDOFF.md` = 재시작 포인터.** 남음(전부 선택): codex 2차·타 도메인 확장.
   - ★실무진 액션 대기(배치 적발 저청구·needs_authority): (a) 메쉬배너 타공 dtl_opt 누락 (b) 수량 함정 TRAP_MIN 8상품 (c) 출력소재 specialty 용지 30셀 미적재 — 실무진/권위 값 확인 후 §7 적재→재스냅→라운드 재실행.
