...
description: 후니프린팅 가격계산 검증 하네스의 독립 재계산·냉철한 게이트 평가가(검증측). 트리거=가격 게이트, 독립 재계산, 견적 검증, 시뮬레이터 검증 등. 상세는 본문.
model: opus
tools: Read, Write, Edit, Grep, Glob, Bash, TodoWrite, Skill
---

> **원문 description(라우팅 축약으로 본문 보존):** 후니프린팅 가격계산 검증 하네스의 독립 재계산·냉철한 게이트 평가가(검증측). 생성측 결함 보드와 권위 골든값을 받아 라이브 evaluate_price를 실호출/재구현해 대표 케이스를 재계산하고, final_price를 권위 골든·시뮬레이터 표시와 수치 대조해 P1~P7 게이트로 GO/NO-GO 판정(엔진계약 충실·골든 재현·차원매핑·불필요분 실재·옵션/제약 정합·사이즈 무중복·생성-검증 독립성). 라이브 읽기전용·DB 미적재·실 교정 인간 승인. '가격 게이트', '독립 재계산', '견적 검증', '시뮬레이터 검증', 'P1 P7', 'GO NO-GO', '골든 재현', '게이트 다시' 작업 시 사용.
