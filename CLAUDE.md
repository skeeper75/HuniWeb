...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-10 | **round-11 확대 #1 — 스티커 시트 컬럼 도메인 + 상품 BOM + 적재명세** — 사용자 "스티커만 진행"(round-11 4산출 패턴을 나머지 시트로 확대, 첫 시트=스티커). 디지털인쇄 파일럿 패턴으로 **스티커(16상품·32컬럼·1090행)** 5산출(`15_domain-spec/sticker/` 4 + `_loadspec/` delta, 510줄·확정도 빈칸0): ① column-dictionary(32컬럼) ② product-bom(16상품·인쇄방식 5분기) ③ mapping-info(32컬럼→t_*) ④ domain-research-notes(컨펌 ST-1~5) ⑤ loadspec 스티커 delta. **스티커 고유 도출(디지털인쇄와 차이): ⓐ 코팅=자재 흡수(무광/유광코팅스티커·C23 빈값) ⓑ 커팅=핵심 공정(PROC_000053완칼/054반칼/055스티커완칼+prcs_dtl_opt 모양·조각수) ⓒ 조각수(C25)=묶음수+공정param 이중귀속(benchmark §3) ⓓ 화이트 별색만(투명/홀로그램 도메인필수 G-SK-1) ⓔ C13 폴더=인쇄방식 5분기(디지털/실사/화이트/합판/전사) ⓕ 합판도무송 형상37종이 C5 size로 흡수(C24 빈값)=G-SK-2 오모델 실증·round-10 escalate "커팅옵션 클리어 37셀" 동일대상.** 07_domain 스티커 KB(PROC_53/54/55·MAT_TYPE.11·benchmark §2형상/§3조각수·process-recipe §2-2) 재사용(재유도0). DB 미적재(분석/정리 전용)·컨펌 ST-1~5(ST-1 형상 size vs 공정=가격표 대조+schema-intent-map 의존) 해소는 인간/다음·다음=책자(확대 #2) | _workspace/huni-dbmap/15_domain-spec/{sticker(4),_loadspec(스티커 delta)}·HANDOFF·메모리1갱신·CLAUDE.md, 커밋 본커밋 | 사용자("스티커만 진행") — round-11 확대 |
