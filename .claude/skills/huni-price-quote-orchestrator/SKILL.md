...
description: >-
  후니프린팅 "상품군 옵션 선택→가격계산" 검증·뼈대 하네스 오케스트레이터. 라이브 가격엔진
  (raw/webadmin catalog/pricing.py `evaluate_price` 단일 권위)과 라이브 t_prc_* 가격 데이터(공식 48·
  구성요소 146·단가행 7,293·상품-공식 바인딩 76)가 권위 엑셀(상품마스터 260610·인쇄상품 가격표 260527)에
  맞게 적재됐는지 냉철하게 평가·검증하고, 옵션선택→가격구성요소→가격공식→시뮬레이터 검증→위젯 계약의
  단단한 뼈대를 명세한다. 5인 에이전트 팀(hpq-engine-cartographer 엔진 흐름·계약 / hpq-authority-curator
  권위 골든값 / hpq-price-chain-inspector 가격사슬 정합·불필요분·차원매핑·사이즈중복 / hpq-option-constraint-mapper
  옵션·템플릿·제약·공정json / hpq-quote-gate-validator 독립 재계산·P1~P7 게이트)으로 기준점 팬아웃 →
  생성 검사 팬아웃 → 냉철한 게이트 검증을 수행한다. 생성≠검증 분리·권위 엑셀이 절대 권위·라이브 읽기전용·
  DB 미적재(실 교정은 인간 승인 후 dbmap 위임)·대표 상품군 파일럿 우선. 기존 dbm-price-* 6종을 입력·도구로
  재사용. 트리거: '가격계산 하네스', '옵션 가격 검증', '가격엔진 검증', '가격공식 가격구성요소 검증', '가격
  정합 평가', '권위 엑셀 매핑 검증', '인쇄상품가격테이블 차원 검증', '시뮬레이터 검증', '옵션 템플릿 제약
  검증', '사이즈 중복 점검', '가격 뼈대', '가격계산 하네스 실행/재실행/업데이트/보완/다시', '특정 상품군만
  가격검증' 등 본 도메인 요청 시 반드시 이 스킬을 사용. 단순 질문은 직접 응답.
---
