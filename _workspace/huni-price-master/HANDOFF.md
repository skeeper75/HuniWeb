...
## 미해결·후속 (다음 세션 후보)
1. **[활성 빌드] 통합 배치 hdx P5-②c** — PriceGridDx 19시트 어댑터(§26 배치 브릿지) + codex 2차(선택). P2~P5-②b 완료 = **진단 8차원 커버**(배선·차원정합·공정·병합·계산가능성·옵션CPQ·수량·판형)·첫 실적재 완료·가격 파일럿 종단 실증. **`_workspace/_foundation/hdx/HANDOFF.md` 참조.**
   - ★실무진 액션 대기(배치 적발 저청구): (a) 메쉬배너 타공 dtl_opt 누락 (b) 수량 함정 TRAP_MIN 8상품 — 전부 실무진/권위 확인 후 값 채움.
