...
- **배정 뼈대 v0.2**: 1단계 능력 게이트(후니 못 만들면 자동 와우) → 2단계 경쟁력(후니 기본·와우 "확실히 나으면" 전환)·최종 선택=지니(균형).
- **부가세 확정**: 후니=별도(공급가·이전사이트 "공급가:N원(부가세 전)" 근거)·와우 ordcost_bill=포함(부가세 전=ordcost_sup). `printshops/huni.json` vat_included=false 반영.
- **★지니 입력 대기(빌드 게이트 2건)**: (a) "확실히 낫다" 잣대(가격/품질/납기·권장=상품 성격별) (b) 소비자 "한 가격" 매기는 법(원가+마진 / 소매가 그대로 / 후니·와우 다르게). → 정해지면 배정 검토표 파일럿(카페 7상품군)+와우 ordcost_sup 재수집.
