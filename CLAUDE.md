...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-13 | **round-16(가격표 → Phase11 가격엔진 그릇 import 준비) 하네스 신설 + 3시트 GO** — 사용자 "Wave-1 교정 전, 가격표를 webadmin 복붙용 엑셀 그릇 + 매핑절차 mermaid로 정리". `/harness:harness` 신설: 스킬 `dbm-price-import-prep`·에이전트 `dbm-price-import-builder`·오케스트레이터 round-16. **그릇 권위=webadmin Phase11 가격엔진 `evaluate_price` 4테이블**(`t_prc_price_formulas`[공식정의]→`t_prd_product_price_formulas`[상품바인딩·별 테이블]→`t_prc_formula_components`[배선]→`t_prc_price_components`[`prc_typ_cd` 단가/합가·`use_dims`]→`t_prc_component_prices`[10차원]). 산출=시트별 `20_price-import/<sheet>/` import.xlsx(복붙용·테이블 1:1·1행 DB컬럼·2행 한글)+mapping-flow(mermaid)+분해문서. **3시트=3구조 GO:** 스티커(MATRIX·**소재 7종 개별분해**[사용자 피드백·라이브 t_mat_materials 실측]·1차 NO-GO→3회 GO·round-2 BLOCKED 2건 해소)·디지털인쇄비(합산형·954행·**재사용 신규0**·1차 GO)·아크릴(면적매트릭스형·8블록·4매트릭스·🔴**가격사슬 단절 발견**[단가행 적재됐으나 배선0=엔진 조회불가]·1차 GO). **5대 교훈: ①그릇 권위=라이브 information_schema(개념설계 11-CONTEXT 아님·`frm_typ_cd` 라이브 부재) ②공식정의≠상품바인딩(별 테이블) ③헤더"/"=개별소재 나열(collapse 금지) ④BLOCKED=NULL강제 금지·별 시트 ⑤가격사슬 단절(단가행 적재≠공식배선).** round-14 stale 반영(8→10차원·단가/합가). 커밋 5ee79cb·89db932·0889aab·ff78213. DB 미적재 | .claude/skills/dbm-price-import-prep(신규)·.claude/agents/huni-dbmap/dbm-price-import-builder(신규)·huni-dbmap-orchestrator·20_price-import/{sticker,digital-print,acrylic,_gate}·HANDOFF·메모리·CLAUDE.md | 사용자(`/harness:harness` — 가격표 webadmin 복붙 그릇 + 매핑절차 준비) |
| 2026-06-13 | **round-15 후속(케이스 컨펌 치환표 + Wave-0 전건 확정)** — 핸드오프 파악→다음 작업 (가)(나) 진행. ① 치환표: round-13 14 BATCH + AMBIGUOUS 3 = **17건**을 round-15 §2 생산형태×그릇 매트릭스로 치환(원칙도출 9·경계 3·직교 5). dbm-validator 독립검증 S1~S6 GO·뒤집힘 0 — **개별 ~70 컨펌이 "생산형태 3×그릇 원칙"으로 수렴**(케이스별 재논의 소멸). ② Wave-0(roadmap §4 선결 게이트) 전건 확정: 확정 3(BATCH-2/3/4 round-15 P1) + 사용자 결정 5 — **D-1 DB에서 직접 교정**(정답 L1 보유·v03 재작업=webadmin 일정 의존→하류 즉시·멱등 UPSERT+재실행 가드 전제)·**D-2 생산형태 그릇 원칙 일괄 채택**(원칙도출 9 자동 수렴)·D-3 카테고리 113상품 이동+빈 임시함 숨김·D-4 경계2(size↔option·폴더 위치)는 round-6/2 트랙에서·D-5 운영 권장. **[HARD 사용자 directive] 모든 교정 산출물=쉬운 한국어**(비전문가 빠른 확인·실무진 라벨 §1.0-b 연장). **Wave-0 게이트 해제→Wave-1 진입 가능**(권장 착수=BATCH-4 레더 .06 MAT_000186 1행 UPDATE=6상품→BATCH-1 카테고리 113상품·round-10 델타·인간 승인). 커밋 f9c8197·92e4999. DB 미적재 | _workspace/huni-dbmap/19_grid-binding/{case-substitution-table·wave0-decision-package}.md·_gate/case-substitution-gate.md·HANDOFF·메모리·CLAUDE.md | 사용자(핸드오프 파악→다음 작업 진행) |
