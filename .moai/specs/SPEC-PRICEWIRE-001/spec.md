...
> `PRD_000165`(아크릴포카코롯토, 게시됐으나 `use_yn=N`)는 **분모에 포함하지 않고 별도 치명 항목**으로 원장에 단독 계상한다.
...
| **2026-08-27 (현행 정본)** | **192** | **192** | 라이브 재실측. `use_yn='Y'` 191 + `use_yn≠'Y'` 1(`PRD_000165`) |
...
> 아래 표의 건수는 **260822 스냅샷 관측치**다 — 종료 판정 시점에는 §1.2 대로 게시 분모를 재실측하고 렌즈 B 를 재실행해 그 시점 값으로 계상한다(M0-5).
> 아래 나머지 3개 층은 **동일 대상의 다른 층 관측치**이며 합산하지 않는다.

| 층 | 건수 | 분모 | 판정층 | 본 SPEC 취급 |
|---|---|---|---|---|
| **렌즈 B `E1 NO_SOURCE`** | **75** | **게시 194**(260822 실측) | **실호출** | **종료 기준 분모(정본)** |
...
- 렌즈 B(게시 194 실호출, **260822 실측**) 상품 verdict: `BROKEN 66 · OK 119 · WARN 2 · NOT_EVALUATED 6 · INFRA_FAIL 1`. 게시 분모가 192 로 이동했으므로 M0-5 재스윕 시 이 분포는 재산출된다.
