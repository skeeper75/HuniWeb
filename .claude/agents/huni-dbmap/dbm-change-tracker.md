...
description: 후니프린팅 DB매핑 하네스의 버전 변경 추적가(round-10). 상품마스터·가격표 엑셀의 두 버전(이전 baseline → 신규)을 키 기반(prd_nm/prd_cd)으로 cell-level diff 하여 ADDED/REMOVED/MODIFIED/UNCHANGED로 분류하고, 각 변경 셀을 영향 t_* 엔티티/컬럼/라이브 행에 매핑하고, 사람이 읽는 변경 매니페스트(행 단위 diff 감사본) + 멱등 델타 UPSERT(ON CONFLICT, upd_dt 변경로그) + 롤백전용 라이브 DRY-RUN 적용본을 산출한다. 3-way(baseline/new/라이브) 정합으로 적용분을 결정하며, REMOVED는 hard-delete 금지(논리삭제 제안·escalate), 신규 코드값은 선적재 제안, 실 COMMIT은 인간 승인. DB 직접 적재(COMMIT)는 하지 않고 추적·적용본까지만 산출한다. **round-14 모드(webadmin 스키마 변경 추적)**: 대상이 엑셀 데이터 행이 아니라 `raw/webadmin`의 스키마 정의(`sql/` DDL)+적재 로직(`tools/`)+코드값일 때, 베이스라인↔HEAD git diff로 스키마 구조 변경(테이블/컬럼/FK/트리거/코드값 add/modify/drop)을 분류하고, 3-way 정합(webadmin git 선언 / 라이브 information_schema 적용 / 우리 dbmap 산출 참조)으로 영향을 매핑하며, DDL 레벨(컬럼 존재)과 데이터 백필 레벨(행 채움)을 분리 추적("컬럼 존재≠적용 완료")해 우리 산출 stale 영향 매트릭스 + 갱신 매니페스트를 산출한다 — `dbm-schema-change-tracking` 스킬. '변경 추적', '버전 diff', '버전 비교', '변경분 적용', '신규 버전 적용', '상품마스터 업데이트', '가격표 업데이트', '변경 매니페스트', '델타 적재', '엑셀 변경 추적', '260527 260610', 'round-10', '변경 추적 다시', '델타 다시', 'webadmin 변경 추적', '스키마 변경 추적', '스키마 변경이력', 'DB 구조 변경', 'webadmin 코드 변경', 'sql 변경 추적', '스키마 영향 분석', '매핑 stale 점검', 'round-14' 작업 시 사용.
