...
대상 = 사내 4담당이 든 행 중 판정이 담당맞음이 아닌 **213행**.
...
| STD-SYS-017 | T7.E4 | 미착수 | 데이터 백업·복구 절차 | 김동학 | 서희항 | rule-track | 데이터 백업·복구 절차 = 관리서버·DB 영역(T1 인프라와 같은 손). PM 은 만들지 않는다 |
...
| STD-CAT-007 | T3.A5 | 부분 | 상품 상세페이지 본문(이미지·설명 블록) | 김동학 | 김동학+최숙진 | merged-owner_side | 상품 상세페이지 본문 — 이미지·설명 블록 화면=김동학 / 본문 콘텐츠=최숙진(역할②) |
| STD-CAT-009 | T3.A5 | 미착수 | 후가공 용어 설명 툴팁(도무송·오시·형압·귀돌이) | 김동학 | 김동학+최숙진 | merged-owner_side | 후가공 용어 툴팁 — 툴팁 구현=김동학 / 용어 설명 문구=최숙진(역할②) |
| STD-CAT-011 | T3.A5 | 미착수 | 상품별 제작 소요일/출고 기준 안내 | 김동학 | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,webadmin) — 행을 나눠 각자에게 |
| STD-CAT-012 | T3.A5 | 부분 | 재작업 불가 사항 고지(폰트 아웃라인 등) | 김동학 | 김동학+최숙진 | merged-owner_side | 재작업 불가 고지 — guide-data.ts 스텁 채우기=김동학 / 고지 문구=최숙진(역할①) |
| STD-CAT-014 | T3.A5 | 작동 | 상품 리뷰 목록·평점 노출 | 김동학 | 김동학+최숙진 | merged-owner_side | 두 시스템에 걸침(huni-mall,shopby) — 행을 나눠 각자에게 |
| STD-CAT-034 | T3.A5 | 부분 | 상세페이지 탭 6종 자동 등록(디자인가이드·디자인보기 포함) | 김동학 | 김동학+서희항+최숙진 | merged-owner_side | 상세페이지 탭 6종 자동 등록 — 자동등록 도구=서희항(webadmin)·표시=김동학(스킨·페이지빌더) / 탭 콘텐츠=최숙진(역할②) |
| STD-ADC-014 | T3.A5 | 구현-미검증 | 인쇄 가이드 콘텐츠 관리 | 김동학 | 서희항+최숙진 | merged-owner_side | 인쇄 가이드 콘텐츠 관리 — 관리 화면·도구=서희항 / 표시명·태그·비고 채우기=최숙진(역할①). [직접 확인 260919] raw/webadmin/webadmin/config/urls.py:73- |
...
| T5-3 | T5.E3 | 부분 | 법정 표기 단일화(사업자번호 206-29-88022 우세)·증빙 화면 목업 제거(D-5) | 김동학 | 김동학+최숙진 | rule-track | 법정 표기 단일화(표기 내용=최숙진) + 증빙 화면 목업 제거(스킨 구현=김동학). PM 은 만들지 않는다 |
| STD-B2B-005 | T6.D5 | 미착수 | 후불 주문 승인·미결제 관리 | 김동학 | 김동학+신우진 | rule-track | 결정 안건이 실무 담당에게 걸려 있다 — 결정(PM)/실행(담당) 분리 |
| STD-OPT-057 | T3.A5 | 미착수 | 규격 가이드 모달(사이즈 안내 팝업) | 서희항 | 김동학+최숙진 | merged-owner_side | 규격 가이드 모달 — 모달 구현=김동학 / 사이즈 안내 내용=최숙진(역할①) |
| STD-OPT-058 | T3.A5 | 미착수 | 주문가능 자재(용지) 목록 모달 | 서희항 | 김동학+최숙진 | merged-owner_side | 주문가능 자재 목록 모달 — 모달 구현=김동학 / 자재 안내 내용=최숙진(역할①) |
...
| STD-INF-005 | T3.A5 | 부분 | 공지사항 고객 화면(목록·상세) | 최숙진 | 김동학+최숙진 | merged-owner_side | 공지사항 고객 화면 — 화면=김동학 / 공지 내용 작성·게시=최숙진 |
