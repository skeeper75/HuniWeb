 ...
 */
var globalReference = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
...
function extractDefaultExport(moduleObj) {
  return moduleObj && moduleObj.__esModule && Object.prototype.hasOwnProperty.call(moduleObj, "default") ? moduleObj.default : moduleObj
}
...
function getFreeGlobal() {
  ...
  var freeGlobal = typeof globalReference == "object" && globalReference && globalReference.Object === Object && globalReference;
...
function getBaseGetTag() {
  ...
  function baseGetTag(value) {
    return value == null ? value === void 0 ? UNDEFINED_TAG : NULL_TAG : symToStringTag && symToStringTag in Object(value) ? getRawTag(value) : objectToString(value)
  }
...
function initToNumber() {
  ...
  function toNumber(value) {
    ...
    return isBinary || OCTAL_REGEX.test(value) ? nativeParseInt(value.slice(2), isBinary ? 2 : 8) : HEX_REGEX.test(value) ? NAN_VALUE : +value
  }
...
function initDebounce() {
  ...
  function debounce(func, waitMs, options) {
    var lastArgs, lastThis, maxWaitMs, lastResult, timerId, lastCallTime, lastInvokeTime = 0,
      isLeading = !1,
      hasMaxWait = !1,
      isTrailing = !0;
    if (typeof func != "function") throw new TypeError(FUNC_ERROR_TEXT);
    waitMs = toNumber(waitMs) || 0, isObject(options) && (isLeading = !!options.leading, hasMaxWait = "maxWait" in options, maxWaitMs = hasMaxWait ? mathMax(toNumber(options.maxWait) || 0, waitMs) : maxWaitMs, isTrailing = "trailing" in options ? !!options.trailing : isTrailing);
    ...
    function invokeFunc(time) {
      var args = lastArgs,
        thisArg = lastThis;
      return lastArgs = lastThis = void 0, lastInvokeTime = time, lastResult = func.apply(thisArg, args), lastResult
    }

    /** leading edge에서 실행 시작 */
    function leadingEdge(time) {
      lastInvokeTime = time, timerId = setTimeout(timerExpired, waitMs), isLeading ? invokeFunc(time) : lastResult
    }
    ...
    function shouldInvoke(time) {
      var timeSinceLastCall = time - lastCallTime,
        timeSinceLastInvoke = time - lastInvokeTime;
      return lastCallTime === void 0 || timeSinceLastCall >= waitMs || timeSinceLastCall < 0 || hasMaxWait && timeSinceLastInvoke >= maxWaitMs
    }
    ...
    function trailingEdge(time) {
      return timerId = void 0, isTrailing && lastArgs ? invokeFunc(time) : (lastArgs = lastThis = void 0, lastResult)
    }
    ...
    function debounced() {
      ...
      if (lastArgs = arguments, lastThis = this, lastCallTime = time, canInvoke) {
        ...
        if (hasMaxWait) return clearTimeout(timerId), timerId = setTimeout(timerExpired, waitMs), invokeFunc(lastCallTime)
      }
...
function initGetTag() {
  ...
  var DataView = initDataView(),
    ...
    getTag = baseGetTag;
  return (DataView && getTag(new DataView(new ArrayBuffer(1))) != DATAVIEW_TAG || MapRef && getTag(new MapRef) != MAP_TAG || PromiseRef && getTag(PromiseRef.resolve()) != PROMISE_TAG || SetRef && getTag(new SetRef) != SET_TAG || WeakMapRef && getTag(new WeakMapRef) != WEAKMAP_TAG) && (getTag = function(value) {
    ...
    return result
  }), getTagResult = getTag, getTagResult
}
...
var bufferModule = {
  ...
  stubFalseResult, stubFalseInitialized;
...
function initIsBuffer() {
  return bufferModuleInitialized || (bufferModuleInitialized = 1, (function(module, exports) {
    var rootObj = getRootObject(),
  ...
  })(bufferModule, bufferModule.exports)), bufferModule.exports
}
...
function initBaseIsTypedArray() {
  ...
  var baseGetTag = getBaseGetTag(),
    ...
    typedArrayTags = {};
  typedArrayTags[FLOAT32_TAG] = typedArrayTags[FLOAT64_TAG] = typedArrayTags[INT8_TAG] = typedArrayTags[INT16_TAG] = typedArrayTags[INT32_TAG] = typedArrayTags[UINT8_TAG] = typedArrayTags[UINT8C_TAG] = typedArrayTags[UINT16_TAG] = typedArrayTags[UINT32_TAG] = !0, typedArrayTags[ARGS_TAG] = typedArrayTags[ARRAY_TAG] = typedArrayTags[ARRAYBUF_TAG] = typedArrayTags[BOOL_TAG] = typedArrayTags[DATAVIEW_TAG] = typedArrayTags[DATE_TAG] = typedArrayTags[ERROR_TAG] = typedArrayTags[FUNC_TAG] = typedArrayTags[MAP_TAG] = typedArrayTags[NUMBER_TAG] = typedArrayTags[OBJECT_TAG] = typedArrayTags[REGEXP_TAG] = typedArrayTags[SET_TAG] = typedArrayTags[STRING_TAG] = typedArrayTags[WEAKMAP_TAG] = !1;
...
function initNodeUtil() {
  return nodeUtilInitialized || (nodeUtilInitialized = 1, (function(module, exports) {
    var freeGlobal = getFreeGlobal(),
  ...
  })(nodeUtilModule, nodeUtilModule.exports)), nodeUtilModule.exports
}
...
async function fetchProductInfo(locale = "ko", productCode, patternCode) {
  try {
    const queryParams = new URLSearchParams(patternCode ? {
        pdt_cod: productCode,
        ptt_cod: patternCode
      } : {
        pdt_cod: productCode
    ...
    return error instanceof Error && (console.error("[RedWidgetSDK/ERROR] 제품 정보 가져오기 실패 > ", error), error.message && (errorMessage = error.message)), {
      result: null,
...
async function fetchPriceCalculation(priceRequestPayload, locale = "ko") {
  ...
  try {
    ...
    if (responseData = await (await fetch(apiUrl, {
        method: "POST",
        headers: {
      ...
      })).json(), responseData.retCode !== 200) throw new Error(responseData.msg);
    return {
      result: responseData,
      errorMessage: null
    }
...
async function fetchS3FileInfo(params) {
  try {
    const {
      lang: locale,
      file_name: fileName
    } = params, apiUrl = `${REDPRINTING_BASE_URL}/${locale}/product/s3GetObjectJson`, responseData = await (await fetch(apiUrl, {
      method: "POST",
      body: JSON.stringify({
        file_name: fileName
      })
    ...
    let errorMessage = "";
    return error instanceof Error && (errorMessage = error.message), console.error("[RedWidgetSDK/ERROR] s3 파일 정보 가져오기 실패 >. ", errorMessage || error), null
  }
}
...
async function fetchAvailableMaterials(params) {
  try {
    const {
      lang: locale,
      pdt_cod: productCode
    } = params, apiUrl = `${REDPRINTING_BASE_URL}/${locale}/product/guide_product_paper`, paperList = await (await fetch(apiUrl, {
      method: "POST",
      body: JSON.stringify({
        pdt_cod: productCode
      })
    ...
    for (const paper of paperList) {
      ...
      seenKeys.has(uniqueKey) || (seenKeys.add(uniqueKey), uniquePapers.push({
        ...paper,
    ...
    return console.error("[RedWidgetSDK/ERROR] 주문 가능 용지(자재) 정보 가져오기 실패 > ", error), null
  }
}
...
async function downloadTemplate(params) {
  try {
    ...
    const apiUrl = `${REDPRINTING_BASE_URL}/${locale}/product/get_download`,
      blobResponse = await (await fetch(apiUrl, {
        method: "POST",
        body: formData
      })).blob();
    if (blobResponse.type !== "application/zip") throw new Error("템플릿 파일(.zip)이 존재하지 않습니다.");
    const blobUrl = URL.createObjectURL(blobResponse),
      downloadLink = document.createElement("a");
    return downloadLink.href = blobUrl, downloadLink.download = `${downloadParams.file_nm.replace(/\./g,"_")}`, document.body.appendChild(downloadLink), downloadLink.click(), downloadLink.remove(), URL.revokeObjectURL(blobUrl), !0
  } catch (error) {
...
async function downloadCoverTemplatePdf(params) {
  try {
    ...
    const apiUrl = `${REDPRINTING_BASE_URL}/${locale}/product/get_pdf_download`,
      responseData = await (await fetch(apiUrl, {
        method: "POST",
        body: formData
      })).json();
    if (!responseData.success || !responseData.url) throw new Error(responseData.msg);
    const downloadLink = document.createElement("a");
    return downloadLink.href = responseData.url, document.body.appendChild(downloadLink), downloadLink.click(), downloadLink.remove(), !0
  } catch (error) {
