...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-17 | **round-23 후속 — 린넨 가공옵션·아크릴 동형/마무리·스티커 라이브 COMMIT** — 사용자 "린넨페브릭포스터 가공옵션 등록+가격, 아크릴 동형 확장, 아크릴 마무리하고 스티커로". 두 트랙 생성≠검증·실 COMMIT(사용자 최종승인). **린넨(PRD_000124) 마감가공 5택1 INSERT 10**(오버로크0·+리본끈800·말아박기1000·+면끈2000·봉미싱7cm2000·단가 verbatim·복합 OPV-000024 재사용/OPV_000424 신규·add-on comp `COMP_POSTEROPT_LINEN_FINISH` use_dims[opt_cd]+단가행5+`PRF_POSTER_LINEN` 배선·트리거 fn_chk_opt_item_ref 통과·골든 본체32,400+가산). **아크릴 동형 217셀**(A1 siz_cd→siz_width/height 전환121·A2 GAP verbatim96·use_dims2·배선1·**★work금지 siz_nm파싱 W앞·두께 mat_cd 직교**·골든30×30 3T3100/1.5T2480[×0.8]·채번0·off-grid ceiling). 사후검증 GO(단가verbatim·골든·채번0·FK고아0). **BLOCKED(미COMMIT)**: 아크릴 .02 엔진계약(Q-ACR-7)·미러 바인딩·코롯토/카라비너 신설. undo round-trip diff0 보유. 교훈: option_items에 dflt_yn 부재(L5 no-op). **[아크릴 마무리] ★A5 긴급보정(이미 COMMIT된 결함): CLEAR3T(.02 합가형) 81행 min_qty NULL→엔진 ÷min_qty ValueError 견적불가→min_qty=1(골든 불변)·교훈 .02는 min_qty 필수**+코롯토 comp신설(B06 verbatim·채번0)·미러/카라비너 BLOCKED. **[스티커 항목7] 405행: 소재4미적재288(그룹단가→mat_cd 개별전개)·투명 오매핑 170→162 교정90(과교정0)·판형 B3/B4 24·바인딩3. ★스티커 판형=이산 siz_cd(명시규격·신안 가로세로구간 제외 반례)**. 사후검증 GO(견적불가0·170잔존0·소재288·코롯토21). BLOCKED 잔여=미러·카라비너·타투·팩prc_typ·반칼변형7·A4/A3·채번분·별색dedup(U5') | `_workspace/huni-dbmap/33_silsa-price-quote/`{`_exec_linen`·`_exec_acrylic`·`_exec_acrylic2`·`_exec_sticker`·정립md·_gate}·CLAUDE.md §7·[[dbmap-area-matrix-wh-dimension]] | 사용자(린넨·아크릴·마무리·스티커) |
| 2026-06-17 | **round-23 실사 가격 견적화 — 가로/세로 구간 차원 모델 + 후가공 배선 라이브 COMMIT(1,080행)** — 사용자 `/harness:harness`(실사시트 가격 구성요소 등록→견적가능 7항목). 신규 자산 0(기존 가격 트랙 재사용). **항목3 결정=A안(기초코드 t_siz_sizes·siz_cd FK only·가로세로 수치컬럼 부재)·좌표 112종 중 6재사용+106신규(SIZ_000518~623)**. **★그룹핑 재검증: 우리 "별색 424행 오염삭제·제본 통합비대상" 판정 전면 오독 적발→철회**(통합 키검사에서 종류축 proc_cd 누락→정상통합을 5중복 오판. 별색 530행=proc_cd 5색×53밴드 키유일·전면보존·`별색인쇄-금색` 조회 정상). **후니 통합=단일 comp+종류축(공정→proc_cd트리/수량→dim_vals/색→proc_cd/도수→clr_cd→print_opt_cd/재질→mat_cd)·comp_grp 불요·통합=use_yn=N+배선축소+단가행보존(DELETE 아님)**. **★webadmin 병행진화(change-tracker)**: 단가 6→11차원(plt_siz_cd·print_opt_cd 전역마스터·proc_grp·prcs_dtl_opt 상속) 라이브 적용·**Phase11 evaluate_price 구현완료(38 passed)→엔진 막힌게 아니라 단가행+후가공 배선만 채우면 견적작동**. 차원→기초코드 전건표현가능·가격영향 차원 CPQ-1~5(거치대 택1·부속 BUNDLE·가공 다중·타이벡·수량구간)=L2 옵션3층(현수막/PET배너 라이브 실증)·선택→가산 간접사슬(add_price 부재). **공정상세옵션 누락 2건(🔴 G-D1 미싱 prcs_dtl_opt EMPTY·🔴 G-D2 포스터본체 use_dims 후가공 미배선 formula_components 0행)**. **★A안(좌표 siz 채번) 폐기→신안(siz_width/siz_height 구간 차원·webadmin 신설·엔진 '이하' 상한 ceiling 내장) 전환**(좌표 채번 0·가격표 셀 verbatim 언피벗). G-D2 후가공 배선(엔진 addtn_yn 무참조·매칭합산·U6 공식분리 선행 필수). 6전문가·생성≠검증. **★실 라이브 COMMIT(사용자 최종승인·2026-06-17): INSERT975·UPDATE77·DELETE28**(신안 가로세로구간+G-D2 후가공 통합·X-1 use_dims 단일화·undo 안전망 보강). 사후검증 GO(siz_width 684행[이전0]·면적13 use_dims 전환·바인딩28·FK고아0·골든 600×1800=21,600+오시2줄6,000=**27,600** 라이브실측). 미적재 잔여=아크릴 동형(Q-PR-3)·별색 dedup(U5')·고정가15 siz_cd유지·스티커(항목7). undo 보유(`_exec_integrated/backup_*`) | `_workspace/huni-dbmap/33_silsa-price-quote/`{component-inventory·silsa-dimension-analysis·grouping-model-design·grouping-recheck-components·dimension-basecode-verification·webadmin-change-mapping·cpq-option-design·silsa-quote-design·_exec·_gate}·CLAUDE.md §7·[[dbmap-price-component-grouping]] | 사용자(`/harness:harness` 실사 가격 견적화) |
