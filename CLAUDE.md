...
| 2026-06-17 | **TP(디자인템플릿) 종단 완주 + codex 데드락 해소** — 사용자 "카테고리 확대"→TP. TP 23상품 대표3(디자인명함·티켓·탁상캘린더) 역공학→메타모델 **15→16축**(#16 "디자인 입력 채널" distinct 승격·BN/GS 미발굴·★증거=TPCLSTD vs HLCLSTD 트윈 본체동일·입력채널만 차이→**TP가 추가하는 단 하나=디자인입력레이어**, 자재/사이즈/후가공/가격은 동종 100% 공유)→갭(#16=**vessel-GAP**·`t_prd_products`에 editor_yn/file_upload_yn 불리언2개뿐·item_gbn/에디터종류/koi_template_resource_id/VDP 그릇0건·dbmap 미터치 순신규·충돌0)→그릇 V-10(컬럼·채널1:1·신규테이블0)+V-11(TemplateAsset 신규테이블2·**디자인시안≠t_prd_templates 완제SKU 이중의미분리[HARD]**·오염차단)→validator M1~M6 전건 GO·DDL DRY-RUN 0 leaked. **★codex 데드락 해소: 근본원인=버전(0.38.0 오진단) 아닌 모델선택**(바이너리 이미 0.140.0·gpt-5-codex/gpt-5 ChatGPT계정 400미지원·gpt-5.5 지원)→`~/.codex/config.toml` model=gpt-5.5 영구설정(백업 보유). TP deepcheck(53후보→HIGH9)→**HIGH 3건 검증 전부 기각·모델 robustness 입증**(H-6 VDP=RedEditorSDK 메서드3·facet유지·H-8 포토카드 set="20장"=같은디자인 판매단위·수량모델#10충분·H-1 STATE=주문런타임 out-of-scope·dbmap MES영역). **★교훈: codex "라이브 인용 보유→즉시 checkable" 신뢰금지**(네트워크 격리 샌드박스 `mcp_servers='{}'` confabulation·검증트리거로만·실측전 unverified). 실 적용=인간 승인+dbmap 트랙 | `_workspace/huni-rpmeta/categories/TP/`·02~05 extend·`04_vessel/vessel-{design-input-channel,template-asset}.md`·`_workspace/huni-dbmap/11_ddl_proposals/`·`~/.codex/config.toml`·CLAUDE.md §11·[[huni-rpmeta-harness]] | 사용자(카테고리 확대→TP + codex 점검) |
| 2026-06-17 | **하네스 강화(운영/유지보수) — 카테고리별 폴더 구조 + codex 시각화·심층보강 2 에이전트 신설** — 사용자 요청. ① **산출 구조 하이브리드 재편**: `categories/{CAT}/`(reverse·deepcheck·summary·viz) 집약 + 02~05 횡단 누적 유지·`_index.md` 커버리지. 기존 BN/GS reverse를 `categories/{BN,GS}/reverse.md`로 git mv 마이그레이션(내용 보존). ② **rpm-visualizer** 신설(codex-image·`codex exec` 병렬 N≤5·옵션트리/축맵/갭히트맵/BOM 4종 도해·gpt-image2 폴백·분석 충실 도해[없는 사실 금지]). ③ **rpm-deepcheck** 신설(codex-cli·`codex exec` read-only·분석 외 누락 옵션/자재/공정/축/제약/도메인 second-opinion 발굴·**환각경계 HARD**[codex=unverified 가설·라이브/엑셀 검증 전 사실 아님]·후보 triage→메타모델/갭 재진입). 스킬 `rpm-visualize`·`rpm-deep-augment` 추가. 오케스트레이터 **7인 팀**·Phase 4.5(deepcheck)·5.5(visualize) 추가·트리거 키워드 확장. 에이전트≠스킬명 충돌 정리(deepcheck 스킬→deep-augment). codex login(ChatGPT) 전제·codex off 시 해당 Phase pending. 에이전트 5→7·스킬 5→7 | `.claude/agents/huni-rpmeta/{rpm-visualizer,rpm-deepcheck}`·`.claude/skills/{rpm-visualize,rpm-deep-augment}`·오케스트레이터·기존 4 에이전트/3 스킬 경로 갱신·`_workspace/huni-rpmeta/categories/`·CLAUDE.md §11·[[huni-rpmeta-harness]] | 사용자(`/harness:harness` — 카테고리별 폴더·codex 시각화·codex 심층보강) |
---
