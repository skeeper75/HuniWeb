...
description: >
  후니프린팅 기초코드 거버넌스 하네스의 검증 게이트 방법론 스킬 — B1~B6 게이트 정의·라이브 재실측 패턴·verdict
  포맷. B1 권위 충실성(정답 사전 ↔ 상품마스터/가격표 명시값 셀 대조·날조 0·역공학/경쟁사 권위 덮어쓰기 적발),
  B2 진단 정확성(4-way 결함 ↔ 라이브 information_schema 재실측·dbmap round-13/22 정합·중복/stale 오판),
  B3 라우팅 타당성(원인유형·라우팅 정합), B4 search-before-mint 준수(신규 그릇 남발 적발), B5 등록 명세
  실행가능성(FK 위상·적재경로 실재·채번 정합·영향분석·dry walk-through), B6 생성-검증 독립성(라이브 직접 재실측·
  dodge-hunt). '기초코드 검증', '거버넌스 게이트', 'B1 B6', '등록명세 검증', '권위 충실성', 'search-before-mint
  검증', '교차검증' 작업 시 반드시 이 스킬을 사용. 생성(큐레이션/진단/명세)은 각 생성 스킬이 담당하므로 본 스킬은
  검증 전용이다.
---
