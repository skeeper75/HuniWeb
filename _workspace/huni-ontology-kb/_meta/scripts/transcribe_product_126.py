...
# -*- coding: utf-8 -*-
"""
전사 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — 레더아트프린트 PRD_000126 전용.
...
transcribe_product_051.py(썬캡) 패턴을 계승하되 실사(silsa) 면적매트릭스 특성에 맞춘다:
  ...
  - 자재 = MAT_000186(레더·MAT_TYPE.05 교정됨 2026-06-27·USAGE.07 단일 슬롯).
  - 공식 = PRF_POSTER_LEATHER_AP → COMP_POSTER_CANVAS_FABRIC(★[동형결합] 4소재 통합:
    캔버스패브릭포스터·레더아트프린트·메쉬프린트·타이벡프린트·use_dims=[siz_width,siz_height,min_qty]).
    구 per-leather COMP_POSTER_LEATHER_ARTPRINT는 use_yn=N(레거시 은퇴) — 배선 안 함(현재값 실측).
  - 면적매트릭스 셀 = D-22 접기(노드 미전개) → comp별 행수·가로/세로축·단가범위 요약만 전사.

사용:  python3 transcribe_product_126.py            # stdout markdown
       python3 transcribe_product_126.py --json     # cache/transcribed-126-260703.json
...
STAMP = "2026-07-03"
PRD = "PRD_000126"
SELF = "transcribe_product_126.py"
AREA_COMP = "COMP_POSTER_CANVAS_FABRIC"
LEGACY_COMP = "COMP_POSTER_LEATHER_ARTPRINT"
...
def md(d):
    ...
    o.append(_tb("t_prd_products PRD_000126"))
    ...
    o.append(_tb("t_prc_component_prices (COMP_POSTER_CANVAS_FABRIC/LEATHER_ARTPRINT 집계)"))
...
if __name__ == "__main__":
    ...
    if "--json" in sys.argv:
        ...
        pth = os.path.join(os.path.dirname(__file__), "cache", "transcribed-126-260703.json")
