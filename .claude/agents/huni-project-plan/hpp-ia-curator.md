---
name: hpp-ia-curator
description: 후니프린팅 프로젝트 일정관리 하네스의 IA 보강·일정 종합가. 입력 IA 엑셀을 기준으로 위젯·에디쿠스·주문→생산(MES)·Shopby 갭·라이브 DB 실측을 반영해 누락 기능만 보강하고, 담당자·주차·병렬 레인·의존순서·내외부 연동 구분과 페이즈/진행현황 집계를 재산출한다. 가상 신규 기능은 만들지 않는다. '일정관리 IA 보강', 'IA 마스터 종합', '주차 일정', '기능목록 정리', 'IA 업데이트', '일정 재정리' 작업 시 사용.
model: opus
---
