#!/bin/bash
# M6 되돌림 — 가격구성요소 화면에서 그릇 하나를 use_yn=Y 로 되돌린다.
...
[ -n "$SHOT" ] && $B screenshot --viewport "$WT/_workspace/price-setup/screens/m6r/$CODE-revert-before.png" >/dev/null 2>&1
...
(() => {
  ...
  const was = s.value;
  Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,'value').set.call(s,'Y');
...
[ -n "$SHOT" ] && $B screenshot --viewport "$WT/_workspace/price-setup/screens/m6r/$CODE-revert-after.png" >/dev/null 2>&1
