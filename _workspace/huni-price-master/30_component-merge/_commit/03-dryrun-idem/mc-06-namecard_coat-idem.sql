...
-- 멱등 실증(롤백전용): mc-06-namecard_coat 본문 2회 적용 후 ROLLBACK. 게이트 RAISE 시 즉시 abort(비0 종료).
BEGIN;
\echo '=== mc-06-namecard_coat PASS 1 ==='
...
BEGIN
  ...
  SELECT count(*) INTO v_canon_pre  FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_COAT';
  SELECT count(*) INTO v_member_price FROM t_prc_component_prices WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
  IF v_canon_pre>0 AND v_member_price>0 THEN
    RAISE EXCEPTION '게이트① 혼재중단: 정본 COMP_NAMECARD_COAT 단가행 %건 + 멤버 단가행 %건 동시존재(반쯤적용 의심).', v_canon_pre, v_member_price;
  END IF;
  ...
  SELECT count(*)-count(DISTINCT (apply_ymd,siz_cd,plt_siz_cd,clr_cd,mat_cd,proc_cd,opt_cd,print_opt_cd,coat_side_cnt,bdl_qty,siz_width,siz_height,min_qty,COALESCE(dim_vals,'{}'::jsonb)::text)) INTO v_collisions
    FROM t_prc_component_prices WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
...
VALUES ('COMP_NAMECARD_COAT', '코팅명함 완제품가 단면(용지포함)', 'PRC_COMPONENT_TYPE.06', '명함 완제품가(용지 포함). 소재·인쇄면·수량을 합산한 1건당 단가표. [차원통합] 통합축=print_opt_cd(면)', 'Y', 'PRICE_TYPE.02', '["mat_cd", "min_qty", "print_opt_cd"]', 'N', now())
...
UPDATE t_prc_component_prices SET comp_cd='COMP_NAMECARD_COAT', upd_dt=now() WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');

-- [3] formula_components 재배선: 멤버 배선 DELETE + 정본 seq1 UPSERT(공식별)
DELETE FROM t_prc_formula_components WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
INSERT INTO t_prc_formula_components (frm_cd, comp_cd, disp_seq, addtn_yn, reg_dt) VALUES
  ('PRF_NAMECARD_COAT', 'COMP_NAMECARD_COAT', 1, 'Y', now())
...
UPDATE t_prc_price_components SET use_yn='N', del_yn='Y', del_dt=now(),
  note='[차원통합→COMP_NAMECARD_COAT]', upd_dt=now() WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
...
BEGIN
  ...
  SELECT count(*) INTO v_member_fc FROM t_prc_formula_components WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
  IF v_member_fc<>0 THEN RAISE EXCEPTION '게이트② 멤버 fc참조 %건(0 기대) → 중단.', v_member_fc; END IF;
  -- 구조적 골든 등가①: 정본 단가행수 == 이관 기대(4)
  SELECT count(*) INTO v_canon_rows FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_COAT';
  ...
  SELECT count(*)-count(DISTINCT (apply_ymd,siz_cd,plt_siz_cd,clr_cd,mat_cd,proc_cd,opt_cd,print_opt_cd,coat_side_cnt,bdl_qty,siz_width,siz_height,min_qty,COALESCE(dim_vals,'{}'::jsonb)::text)) INTO v_dup
    FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_COAT';
  IF v_dup<>0 THEN RAISE EXCEPTION '정본 내부 nat_key 중복 %건(0 기대) → 중단.', v_dup; END IF;
END $post$;

\echo '=== mc-06-namecard_coat PASS 2 (idempotency) ==='
SET LOCAL statement_timeout = '120s';
...
\echo '=== mc-06-namecard_coat 2PASS 통과 → 멱등·게이트OK ==='
ROLLBACK;
