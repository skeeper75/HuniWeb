...
- **088 레더링바인더 = 유일 진짜 결함(PRICE=0)**: 부모공식 바인딩 전무 + 구성원(표지+면지4) 전부 무공식 + **가격 낼 내지 member 부재**(082/077과 결정적 차이·빈 바인더). 데이터 dryrun만으로 교정 불가 — 실무진 제본방식(HOLD)+권위단가+cover_mult ×2 C트랙 확인 후 §23 SOP로 구성.
  ...
  - **★교정(2026-07-02): use_yn Y→N 숨김 COMMIT**(라이브 0원 노출 위험 차단·사장님 승인·del_yn 불변=삭제 아님). undo=`_foundation/batch/wiring/088-hide-260702-undo.sql`. D링 제본단가 실무진 확정 후 082 동형(표지+D링제본)으로 구성→재노출.
