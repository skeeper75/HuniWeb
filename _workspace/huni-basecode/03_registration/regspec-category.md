...
>
> **★round-22 ⑥ 미완분 [HARD·정정]:** round-22 ⑥ COMMIT은 빈 노드를 **use_yn='N'으로만 토글**(del_yn 미수행). 조회 차단 권위 = del_yn이라, **고아 14노드 중 진짜 소프트삭제(del_yn='Y') = CAT_000297 1노드뿐**, 나머지 13(294·293·295·296·298~301·303·305·306 등)은 **del_yn='N' = 여전히 조회 노출**(use_yn만 'N'). → round-22 ⑥은 use_yn 토글까지·del_yn 미완. 본 명세가 권위 정정(중복 아님·round-22 COMMIT 자체는 재제안 금지·del_yn 미수행분만 신규).

---

## 1. 신규 교정 — 빈 고아 노드 소프트삭제 del_yn='Y' (즉시·확정)

> 진단 §del_yn 카테고리 동형 보정: 조회 차단 권위 = **del_yn**. round-22 ⑥은 빈 노드 use_yn='N'까지만(del_yn 미수행) → 13노드(294 포함·297 제외)가 del_yn='N' = 조회 노출 잔존.
...
### 1.2 round-22 ⑥ del_yn 미완 빈 고아 ~12노드 — 소프트삭제 보정

| 항목 | 내용 |
|------|------|
| **대상** | round-22 ⑥이 use_yn='N'으로만 토글하고 del_yn='N' 잔존한 빈 고아 노드(CAT_000293/295/296/298/299/300/301/303/305/306 등 ~12·CAT_000297은 del_yn='Y' 완료라 제외) |
| **올바른 의미** | 조회 차단 권위 = del_yn → use_yn='N'만으론 고객/admin 조회에서 안 사라짐 = round-22 ⑥ 미완분. |
...
| **권위 근거** | 진단 §del_yn 카테고리 동형 보정(진짜 소프트삭제=CAT_000297 1노드뿐) · category §B 재판정 |
| **채번** | 없음(del_yn UPDATE). |
...
> **즉시 가능분(컨펌 무관).** 라우팅 = 카테고리 소프트삭제(del_yn='Y'·신규 1 + round-22 미완 보정 ~12).

---

## 2. BLOCKED 보존 — CAT_000302 · CAT_000304 (컨펌 C-CAT-1)

> 진단 §2.2: 각 1상품(PRD_000218 타이벡북커버·PRD_000229 이미지피켓·둘 다 use_yn='N' 비활성)·main='Y'가 이 고아에만. round-22가 의도적 보존(BLOCKED).

| 항목 | 내용 |
|------|------|
| **대상** | CAT_000302(데스크/사무용품)·CAT_000304(말랑 PVC고주파) |
| **올바른 의미** | 비활성 상품의 유일 main 카테고리 — 삭제 시 비활성 상품 카테고리 전손. |
| **목적지/조치** | **보존(BLOCKED)** — 재제안 금지. PRD_000218/229 활성/폐기 결정 후 재연결 또는 삭제. |
| **search-before-mint** | 신규 0. 조치 = 컨펌 대기(보존). |
| **권위 근거** | 정답 사전 §3(BLOCKED 보존) · 진단 §2.2 · round-22 apply-log(BLOCKED 미적용) |
| **채번** | 없음. |
| **적재경로** | 컨펌 후 분기 — 활성화 시 `pvEdit(categories)` 재연결 / 폐기 시 `tcatcategories change` 삭제. |
| **FK 위상** | 보존 중 — 변경 0. |
| **영향분석** | 삭제 시 비활성 상품 유일 카테고리 전손 → 보존 정당. |

> **컨펌 큐 → C-CAT-1.** PRD_000218/229 비활성 상품 활성/폐기 결정 후 재연결/삭제 분기.
...
| 카테고리 소프트삭제(round-22 del_yn 미완 보정) | 빈 고아 ~12(297 제외) | ~12 | 0 | **즉시** | tcatcategories change(**del_yn='Y'**+del_dt) |
| BLOCKED 보존(컨펌) | CAT_000302·304 | 2 | 0 | 컨펌 C-CAT-1 | (분기) |
| 재연결 | — | 0 | 0 | — | — |
| 신설 | — | 0 | 0 | — | — |
| 교정완료(인용·재제안 금지) | 고아 페어 DELETE 111·use_yn 토글 12 | — | — | round-22 COMMIT | — |

**카테고리 소프트삭제(del_yn='Y') 명세 = ~13노드**(CAT_000294 신규 1 + round-22 ⑥ del_yn 미완 보정 ~12). 재연결/신설 = 0(정상 leaf 전부 실재). BLOCKED 2(컨펌). **★삭제 권위 정정:** use_yn='N' → del_yn='Y'(조회 차단 권위). round-22 ⑥ DELETE/use_yn 토글은 재제안 금지이나 del_yn 미수행분은 신규(중복 아님).
