...
### [HARD] 비밀값 redaction — 직렬화 출력 "전체"에 적용

가격/정보 API의 **respBody가 Edicus 세션 JWT(`refreshToken`·`customerCode`·`userId`·`companyCode`)를 echo**한다. reqBody만 redact하면 respBody로 누출된다 — 실제로 이 결함이 S2~S6 캡처에서 반복돼 12개 커밋 파일에 토큰이 새어나갔다(사후 sanitize). 그래서 규칙은 단순하다: **파일에 쓰기 직전, `JSON.stringify(out)` 결과 문자열 전체를 redact한 뒤 기록한다.** 특정 필드만 redact하지 말 것 — 어디서 echo될지 예측 불가하다.

- 공유 redact + 캡처 패턴은 `scripts/capture-scaffold.cjs`에 고정돼 있다(아래 Step 2). 새 캡처 스크립트는 이 scaffold를 복사해 시작하라 — redact를 손으로 재구현하지 말 것(누락의 원천).
- 커밋 전 확인: `grep -rlE 'eyJ[A-Za-z0-9_-]{15,}\.[A-Za-z0-9_-]{15,}\.[A-Za-z0-9_-]{10,}' _workspace/huni-widget/` 가 0건이어야 한다.
...
기동 확인: `http://localhost:3001` → `/rp-api/* /widget-api/* /makers-api/*` 프록시 로그 출력.

### Step 1.5 — 토큰/쿠키 수명 (가격 캡처 전 필수 확인)

widget_monitor는 **두 종류의 자격**을 관리하며 수명·갱신 경로가 다르다. 가격 캡처는 둘 다 신선해야 한다.
...
**확대 스테이지(신규 상품 캡처)는 공유 scaffold로 시작한다.** `scripts/capture-scaffold.cjs`가 selectProduct → Shadow DOM 옵션스키마 덤프 → `get_ajax_price_vTmpl`/`get_digital_product_info` 인터셉트 → 첫 규격+수량100으로 PRICE>0 강제 → **전체출력 redact** 까지를 한곳에 고정한다. playwright가 설치된 `raw/widget_monitor/local/`에 복사해 실행:

```bash
cp .claude/skills/huni-widget-live-capture/scripts/capture-scaffold.cjs raw/widget_monitor/local/
cd raw/widget_monitor/local && PREFIX=s7_xxx PRODUCTS=CODE1,CODE2 node capture-scaffold.cjs
```

스테이지 고유 상호작용(자유 W/H 2D 단가, 수량 스윕 등)이 필요하면 scaffold를 복사해 step 블록만 추가한다 — redact·마운트 프로브·스키마 덤프는 그대로 재사용.

수동/세밀 제어가 필요할 때만 직접 구동: `http://localhost:3001?pdt={상품코드}` 접속(예: `GSTGMIC`, `PRBKORD`). Playwright/claude-in-chrome으로:
