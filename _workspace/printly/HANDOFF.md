...
**PART 1(온톨로지 설계 Step0~3) 완결 + PART 2 (1) 브릿지 설계 완료 + 라이브 실증.** 다음 세션 시작점 후보(지니 확정 대기·PART 4 게이트):
- **(a) PART 2-2 런타임 루프** ★권장 — 잡티켓 생성 + 엔진 호출 에이전트 오케스트레이션(맥락→제안→선택→견적→매칭→파일변환→생산). ANA 런타임 스타일·온톨로지 경계 유지.
- **(b) 잡티켓 정형 스키마** — 덱 다이어그램→실 필드 계약(JSON shape·G-BR-1).
- **(c) 엔진 인터페이스 스펙(Q7)** — preflight/imposition/production_time 경계 노드 4종 입출력 계약.
- **(d) 전반부 파일럿 인스턴스화** — 명함·엽서·쿠폰·스티커 실 노드(INTENT_FN·constraint) candidate로 채워 시스템 Stage 2(시뮬레이터) 재료화.
