...
2. **[✅ 적재본 빌드 완료 2026-06-07] ENV 봉투 — `09_load/_exec_env/`** — 봉투 4종 작업사이즈(상품마스터 봉투제작 003-0015: 티켓225×193/소238×262/자켓262×238/대510×387)=라이브 siz `SIZ_000191~194` EXACT 재사용. 매핑 검증 **GO**(`03_validation/price-foil-envelope-gate.md`) + **적재본 빌드 완료**(멱등 INSERT…ON CONFLICT comp_price_id 40행[1713~1752]·siz 실치환·단일트랜잭션 `apply.sh` 기본 ROLLBACK). **siz등록·바인딩·코드행 0(전부 라이브 선존재)=최단 GO 트랙**. **다음: dbm-validator R1~R6 + (리드 승인 시)라이브 DRY-RUN → `apply.sh --commit`(인간 승인)**.
