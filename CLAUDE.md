...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-14 | **round-18 신설 — 가격엔진 실증검토 하네스(`/harness:harness`)** — 사용자 "가격공식이 상품에 제대로 매핑돼 가격을 계산하는지 실증검토 + 하네스 구성"(아크릴·문구·굿즈/파우치·가격뷰어). **라이브 gstack 실증: `evaluate_price` 엔진 미구현 확인**(가격뷰어 ⑤ "가격엔진 다음 단계 완성 후 활성화"·`pricing.py` 부재·ROADMAP Phase 11 미완·아크릴명찰 "가격 미구성"·PUR책자 공식 bound이나 할인 미연결·등급 0행). → 호출할 엔진 없음 → **가격사슬 완전성 실측 + 명세(11-CONTEXT/prcx01) 기반 검증용 재계산**으로 확정(생성자≠검증자). 신규 에이전트 `dbm-price-engine-verifier`·스킬 `dbm-price-engine-verify`(단가형/합가형·NULL 와일드카드·동시매칭 오류·수량구간·시계열·할인 순차곱·우선순위 재구현 + PE1~PE6 게이트)·오케스트레이터 round-18 추가(라운드/팀/파이프라인/시나리오/트리거). round-16/17 산출 재사용·중복0. **하네스 구성만(실증 실행은 다음 세션)·비파괴.** | `.claude/agents/huni-dbmap/dbm-price-engine-verifier.md`·`.claude/skills/dbm-price-engine-verify/SKILL.md`·`huni-dbmap-orchestrator/SKILL.md`·CLAUDE.md §7·HANDOFF·메모리 | 사용자(`/harness:harness` 가격엔진 실증검토 하네스 구성) |
