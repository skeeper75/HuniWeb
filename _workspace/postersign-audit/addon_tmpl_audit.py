...
def fuzzy(live_nm, auth):
    ...
    for a in auth:
        text = a['name'] + ' ' + a['spec']
        # 숫자 시퀀스는 **엄격 일치**로 둔다.
        # 부분집합 허용(「사각 100mm」 ↔ 「사각(100x100mm)」을 붙이려던 완화)을 시도했으나
        # 트레싱지봉투 70x100 30장이 권위 32,000 행에 붙는 등 서로 다른 사양이 엮여
        # OK 46→45 / 미판정 10→12 로 판정이 나빠졌다. 되돌렸다.
        # 엄격 매칭이 놓치는 건은 수동 확인으로 문서에 남긴다 — 도구 한계이지 미검증이 아니다.
        if nums(text) != ln:
