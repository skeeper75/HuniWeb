...
- 판정 분포(735행 전체): 담당맞음 453 · 재배정 142 · 분할 39 · provided 37 · 불필요 1 · 미정 63
- 근거 등급: merged-owner_side 366 · evidence-path 177 · rule-track 95 · rule-step 89 · manual-read 8
...
재판정 뒤 이 사람이 실제로 질 남은 일(제안 기준) = 113 + 넘어옴 62 = **175행** (+ 나눠야 하는 9행의 자기 몫)
...
| STD-OPT-057 | T3.A5 | 미착수 | 규격 가이드 모달(사이즈 안내 팝업) | 김동학 | merged-owner_side | 그 일이 사는 곳=huni-mall |
| STD-OPT-058 | T3.A5 | 미착수 | 주문가능 자재(용지) 목록 모달 | 김동학 | merged-owner_side | 그 일이 사는 곳=huni-mall |
...
| STD-ADP-015 | T2.A1 | 부분 | 상품 상세페이지 콘텐츠 편집 | 김동학 | 분할 | manual-read |
...
| STD-ADC-014 | T3.A5 | 구현-미검증 | 인쇄 가이드 콘텐츠 관리 | 김동학 | 재배정 | merged-owner_side |
...
| STD-SYS-021 | T7.E4 | 미착수 | 페이지 빌더(운영자 화면 편집) | 김동학 | 재배정 | merged-owner_side |
...
재판정 뒤 이 사람이 실제로 질 남은 일(제안 기준) = 124 + 넘어옴 43 = **167행** (+ 나눠야 하는 15행의 자기 몫)
...
| T2 상품·가격·위젯 준비 | 11 | 5 | 4 | 2 | 0 | 0 |
| T3 쇼핑몰 주문·결제·회원 | 102 | 81 | 11 | 10 | 0 | 0 |
| T4 주문 수신·원고·생산 연동 | 19 | 7 | 10 | 2 | 0 | 0 |
| T5 운영 설정·알림·CS·증빙 | 32 | 12 | 19 | 0 | 1 | 0 |
| T6 프린팅머니 | 7 | 2 | 1 | 1 | 0 | 3 |
| T7 테스트·리허설·컷오버 | 13 | 8 | 5 | 0 | 0 | 0 |
...
| STD-CAT-009 | T3.A5 | 미착수 | 후가공 용어 설명 툴팁(도무송·오시·형압·귀돌이) | merged-owner_side |
...
| STD-ADC-014 | T3.A5 | 구현-미검증 | 인쇄 가이드 콘텐츠 관리 | 서희항 | merged-owner_side | 그 일이 사는 곳=webadmin |
...
| STD-SYS-008 | T4.C3 | 부분 | 인쇄 도메인 DB ↔ 커머스 상품 동기화 | 서희항 | manual-read | 동기화 코드가 webadmin 안에 산다(모듈 docstring 「우리 DB 가 원본이고 샵바이는 밀어넣는 대상」). 스킨 코드 0. merged t48:35 o |
...
| T5-3 | T5.E3 | 부분 | 법정 표기 단일화(사업자번호 206-29-88022 우세)·증빙 화면 목업 제거(D-5) | 신우진·최숙진 | rule-step | 사는 시스템 미확인 — 단계 E3 기본 담당과 불일치(약한 근거·실측 필요) |
...
| STD-SYS-017 | T7.E4 | 미착수 | 데이터 백업·복구 절차 | 신우진 | rule-step | 사는 시스템 미확인 — 단계 E4 기본 담당과 불일치(약한 근거·실측 필요) |
| STD-SYS-019 | T7.E4 | 미착수 | 반응형/모바일 대응 | 신우진 | rule-step | 사는 시스템 미확인 — 단계 E4 기본 담당과 불일치(약한 근거·실측 필요) |
| STD-SYS-021 | T7.E4 | 미착수 | 페이지 빌더(운영자 화면 편집) | 서희항 | merged-owner_side | 그 일이 사는 곳=pagebuilder |
| STD-SYS-022 | T7.E4 | 미착수 | 구 사이트 → 신규 사이트 URL 리다이렉트 | 신우진 | rule-step | 사는 시스템 미확인 — 단계 E4 기본 담당과 불일치(약한 근거·실측 필요) |

### ③ 행을 나눠야 함 — 15행

| row_id | 트랙.단계 | status | 일 | 제안 담당 | 근거등급 | 근거 |
|---|---|---|---|---|---|---|
| STD-ADP-015 | T2.A1 | 부분 | 상품 상세페이지 콘텐츠 편집 | 서희항+김동학 | manual-read | 스킨은 fetchPublishedDetailTabs 로 **발행된 탭을 읽기만** 한다. 「편집」 화면은 pagebuilder(vibe-canvas) 쪽 — 조회 |
...
| STD-CAT-007 | T3.A5 | 부분 | 상품 상세페이지 본문(이미지·설명 블록) | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,pagebuilder) — 행을 나눠 각자에게 |
| STD-CAT-011 | T3.A5 | 미착수 | 상품별 제작 소요일/출고 기준 안내 | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,webadmin) — 행을 나눠 각자에게 |
| STD-CAT-012 | T3.A5 | 부분 | 재작업 불가 사항 고지(폰트 아웃라인 등) | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,pagebuilder) — 행을 나눠 각자에게 |
| STD-CAT-034 | T3.A5 | 부분 | 상세페이지 탭 6종 자동 등록(디자인가이드·디자인보기 포함) | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,pagebuilder,webadmin) — 행을 나눠 각자에게 |
...
| STD-CAT-019 | T3.A5 | 부분 | 인쇄 가이드 콘텐츠(11종) | 최숙진 | 분할 | merged-owner_side |
| STD-INF-005 | T3.A5 | 부분 | 공지사항 고객 화면(목록·상세) | 최숙진 | 재배정 | merged-owner_side |
| STD-INF-007 | T3.A5 | 부분 | 가이드북 고객 화면(작업 유의사항 11종) | 최숙진 | 분할 | merged-owner_side |
| STD-INF-012 | T3.A5 | 미착수 | 뉴스·소식 공간 신설(갤러리형/게시판형/커뮤니티형 중 택1 + 메뉴명) | 채훈희 | 재배정 | merged-owner_side |
| STD-OPT-057 | T3.A5 | 미착수 | 규격 가이드 모달(사이즈 안내 팝업) | 서희항 | 재배정 | merged-owner_side |
| STD-OPT-058 | T3.A5 | 미착수 | 주문가능 자재(용지) 목록 모달 | 서희항 | 재배정 | merged-owner_side |
...
재판정 뒤 이 사람이 실제로 질 남은 일(제안 기준) = 104 + 넘어옴 58 = **162행** (+ 나눠야 하는 9행의 자기 몫)
...
| STD-INF-005 | T3.A5 | 부분 | 공지사항 고객 화면(목록·상세) | 김동학 | merged-owner_side | 그 일이 사는 곳=huni-mall |
...
| STD-CAT-019 | T3.A5 | 부분 | 인쇄 가이드 콘텐츠(11종) | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,pagebuilder) — 행을 나눠 각자에게 |
| STD-INF-007 | T3.A5 | 부분 | 가이드북 고객 화면(작업 유의사항 11종) | 김동학+서희항 | merged-owner_side | 두 시스템에 걸침(huni-mall,pagebuilder) — 행을 나눠 각자에게 |
...
| T5-3 | T5.E3 | 부분 | 법정 표기 단일화(사업자번호 206-29-88022 우세)·증빙 화면 목업 제거(D-5) | 김동학 | 재배정 | rule-step |
...
재판정 뒤 이 사람이 실제로 질 남은 일(제안 기준) = 19 + 넘어옴 20 = **39행** (+ 나눠야 하는 4행의 자기 몫)
...
| STD-SYS-017 | T7.E4 | 미착수 | 데이터 백업·복구 절차 | 김동학 | 재배정 | rule-step |
| STD-SYS-019 | T7.E4 | 미착수 | 반응형/모바일 대응 | 김동학 | 재배정 | rule-step |
| STD-SYS-022 | T7.E4 | 미착수 | 구 사이트 → 신규 사이트 URL 리다이렉트 | 김동학 | 재배정 | rule-step |
