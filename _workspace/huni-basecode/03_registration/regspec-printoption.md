...
| **★화이트/클리어 underbase 확정 [B-PO-1 해소·2026-06-18 사용자]** | 화이트/클리어 언더베이스 = **UV 인쇄 옵션(PROC_000002 공정 param·dtl_opt)** 확정. **별색공정 PROC_000008 아님.** PO-1a 14 실연결 상품의 dtl_opt param으로 이관(`{"언더베이스":"화이트"}`·`{"언더베이스":"클리어"}` 또는 변형값 `{"변형":"풀빼다"}`와 동일 슬롯). **UV 14 실연결 한정** — PO-1b 7 무연결은 PROC_000002 연결 선결 후 동일 처리. |
| **권위 근거** | `diagnosis-printoption.md §6`(B4 dtl_opt 재실측·B2 14/7 분기 라이브 실측) · `diagnosis-process.md §6.1`(dtl_opt 실재) · round-13(print_side UV 오적재 CONFIRMED) · **B-PO-1 사용자 확정(2026-06-18·underbase=UV param)**. |
...
| ID | 확정 결과 | 반영 |
|----|-----------|------|
| **B-PO-1** ✅해소 | 화이트/클리어 언더베이스 = **UV 인쇄 옵션(PROC_000002 공정 param·dtl_opt)** 확정·**별색공정 PROC_000008 아님**·UV 14 실연결 한정 | §1.1 underbase 확정 행(dtl_opt param 이관)·PO-1a 처리 |

### 6.2 잔여 컨펌 (후속 실측 — escalate)

| ID | 막힌 결정 | 평이한 한국어 질문 |
|----|-----------|--------------------|
| **PO-1b 정체(7 무연결)** | UV 변형값은 있으나 공정 연결이 없는 7상품(코롯토164·카라비너166 등)의 정체·공정 연결 여부 | "코롯토·카라비너 같은 7개 상품은 화면엔 'UV 변형'이 적혀 있는데 실제 UV 공정 연결이 없습니다. 정말 UV 가공인가요, 비활성/정체인가요? 공정 연결을 먼저 정해야 변형값(언더베이스 포함)을 옮길 수 있습니다." |

> **escalate:** **B-PO-1 해소** — underbase=UV param 확정(별색공정 아님·14 한정). **잔여 = PO-1b 정체 1건**(7 무연결분 PROC_000002 공정 연결 선결·연결 없이 underbase/변형 이관 불가). AX-5(이관 범위)는 공정 §6과 합류(신규 그릇 아님·DDL 불요).
...
- **컨펌 확정:** **B-PO-1 해소** — underbase=UV 인쇄 옵션(PROC_000002 param·dtl_opt) 확정·별색공정 아님(§1.1·§6.1). **잔여 escalate = PO-1b 정체 1건**(7 무연결 PROC_000002 공정 연결 선결). AX-5(이관 범위·신규 그릇 아님)는 공정 §6 합류.
