...
|---|---|---|---|---|---|
| 아크릴 | `acrylic-authority-grid.csv` | `acrylic-authority-dims.md` | **433** | area·qty_discount·addon·fixed | 추출완료(파일럿·1호) |
| 스티커 | `sticker-authority-grid.csv` | `sticker-authority-dims.md` | **1089** | siz_cd×mat×qty(이산단가형)·세트단가(합가형) | 추출완료(2호) |

(이후 동형 전파 시 시트 추가)

## 스티커 블록별 셀 수 (1089) — 상품마스터 16종·라이브 PRD_052~067

| 블록 | 차원유형 | 셀 | 비고 |
|---|---|---|---|
| B01 반칼 자유형(공유) | siz_cd6×mat3그룹×qty36 | 648 | grp1/2/3 각 216·반칼 12상품 공유(원형/정사각/직사각/띠지/팬시/소량 포함) |
| B02 낱장 자유형 | siz_cd5×qty6 | 30 | 유포지+엠보 |
| B03 낱장 투명 | siz_cd5×qty6 | 30 | 투명전용지(Q-STK-5) |
| B04 대형 자유형 | siz_cd1×qty6 | 6 | 400x600 |
| B05/B06 타투 | 세트단가(합가형.02) | 3 | 기본가2000+3장4000(Q-STK-1) |
| B07 스티커팩 | 세트단가(합가형.02) | 2 | 54장1세트4000(Q-STK-6) |
| GANGPAN 원형 | 형상(siz)×mat2×qty5 | 110 | 합판도무송 별 단가표 |
| GANGPAN 정사각 | 형상×mat2×qty5 | 120 | 〃 |
| GANGPAN 직사각 | 형상×mat2×qty5 | 140 | 〃 |

가격모델 2종: 고정가by-siz_cd(1084셀)·팩세트고정 합가형(5셀). 면적·addon·확인필요시그널 없음.
★스티커 특이점: addon 가격블록 0(상품마스터 추가상품 컬럼 전부 공란)·코팅=자재variant(공정 아님)·소재가 핵심 가격축(7소재 전개).
