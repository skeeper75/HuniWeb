...
> - **v7.0 (AC·2026-06-17): AC 카테고리 = 신규 그릇 0건(7번째 distinct 0·17축 재포화), V-3 3차원 확장.** AC(아크릴·키링·코롯토·명찰·등신대) distinct 신축 0(가공방식 그룹핑 #18 부결·PR·CL 패턴 반복) → search-before-mint 통과·기존 V-1~V-12 충분. AC facet 6항(PASS 2: #3 받침=`t_prd_product_addons` addon→tmpl_cd·#6 화이트=`PROC_000008` / WEAK 4) 흡수: **#1 두께(measure_type)·#2 surface-finish(글리터/거울/자개·★ST S-4 adhesion/weather와 통합)·#4 단일 부자재 마스터(고리/받침/자석 MAT_TYPE.04/.07/.10/.02 4버킷 분산·D링 3중복·★round-22 B-3 조율) → V-3**(`vessel-material-axis.md §10` 3차원 확장)·**#5 acrylic2025 가격엔진 prc_typ .02(Q-ACR-7 라이브 확증·CLEAR3T 84행) → V-7**(`vessel-quantity-size-pricing.md §C5`·가격 트랙 round-16/17 위임)·**(부)ACTPKEY 키링 템플릿 → V-10/V-11**(`vessel-template-asset.md §8`·완제SKU 적재 금지·이중의미 분리). **신규 테이블/컬럼/DDL/V-번호 0**(dbm-ddl-proposer 미호출). ★dbmap 31_acrylic 라이브 산출과 구조 동형 확증(CLEAR3T mat_cd 통합·MIRROR3T 별 comp·화이트=공정)·Q-ACR-7(.02)/미러 GAP은 dbmap 가격 트랙 범위 외. **17축 재포화의 vessel-side 검증** — 7번째 카테고리(가장 강한 새 후보 A-8 가공방식 그룹핑 포함)가 새 그릇을 요구하지 않음. 인벤토리·Wave·정비권고 무수정·v1.0~v6.0 무수정.
> - **v8.0 (PD·2026-06-17): PD 카테고리 = 신규 그릇 0건(8번째 distinct 0·17축 재포화).** PD(스툴·슬리퍼·강아지계단=봉제 구조물/3D 조립 완제품) distinct 신축 0(완제 구조물 내재BOM #18 부결·PR·CL·AC 패턴 반복) → search-before-mint 통과·기존 V-1~V-12 충분. PD facet 5항(PASS 1: 단수/형상=#13 사이즈 1:1 흡수[★ST 형상#17 1:多 미충족이라 V-12 강제 불요] / WEAK 2 / GAP 1 / ★data-gap 1) 흡수: **PD-1(봉제/제품가공) → V-8**(`vessel-form-assembly.md §7`·SEW_LTR/PDT_WRK = GS PDT_WRK/FLX_ZIP 동일 #14 GAP family·PROC_CLASS 분류축뿐·PD 추가 0·봉제/조립/솜충전 공정 행=data)·**PD-2(직물/PU 원단)·PD-5(밑창색×사이즈 SUB_MTR) → V-3**(`vessel-material-axis.md §11`·직물 물성 차원=★ST 점착/AC surface_finish 동근 합성차원·밑창=★별색 아님 정정[D-PD-1]·usage_cd .07 sub_mtrl+ref_key1/ref_key2 2D[§9 CL 동형])·**★PD-4 완제 구조물 내재BOM(다리/받침/논슬립·솜/지퍼) → data-gap not vessel-gap**(`_data-gaps-noted.md §9`·`t_prd_product_addons` addon→tmpl_cd 그릇·usage_cd .07 639행 슬롯 실재·후니 KB 1급 모델링·ST 형상 G-SK-2와 정반대·dbmap round-22 GPM-4 적재). **신규 테이블/컬럼/DDL/V-번호 0**(dbm-ddl-proposer 미호출). **17축 재포화의 vessel-side 검증** — 8번째 카테고리(가장 이질적 봉제 구조물 완제품·directive 최대 관전 조립/구조/3D폼/완제 내재BOM 포함)가 새 그릇을 요구하지 않음. 인벤토리·Wave·정비권고 무수정·v1.0~v7.0 무수정.
> - **v9.0 (PH·2026-06-17): PH 카테고리 = 신규 그릇 0건(9번째 distinct 0·17축 재포화·★directive 최대 관전).** PH(포토보드·액자·사진인화·포토북·포토굿즈 = "사진을 어떤 물성으로 출력하느냐" 출력매체 카테고리·5개 이질 상품군 공존) distinct 신축 0(완제 액자 그릇/마운팅 #18 부결·PR·CL·AC·PD 패턴 반복) → search-before-mint 통과·기존 V-1~V-12 충분. §0.5 client-render 재캡처(gstack browse)가 PH 고유 블로커(액자/사진인화 SSR-negative)를 거치(탁상용/벽걸이) OBSERVED로 해소했음에도, 거치가 **옵션#3 캐스케이드 상위 차원 + 완제SKU#4 variant**로 무왜곡 구현. PH facet 6항(PASS 3: 거치 캐스케이드=옵션#3 option_groups 134/constraints.logic 10·형태 비율=#13 사이즈 1:1·다중분류=#7 main_cat_yn 2카테고리 8상품 / WEAK 3) 흡수: **PH-2 완제 액자 SKU(거치+마감+사이즈 인코딩) → V-11**(`vessel-template-asset.md §9`·완제 액자 SKU[주문 단위]↔에디터 디자인 자산 분리·AC 두께/소재 variant·GS 완제SKU 동형)·**PH-3 인화지×마감 surface-finish → V-3**(`vessel-material-axis.md §12`·유광/반광/스노우 마감이 `mat_nm` 융합·★AC A-2 surface_finish·ST S-4 점착/내후와 *동근*)·**PH-6 set/sheets 고정 단위 → #10 수량모델**(`vessel-quantity-size-pricing.md`·600매/4sheets=`bundle_qtys` 28행·GS 텀블러 base_quant 동형·data)·**★PH-1/PH-2 거치 마운팅(directive 최대 관전) → data-gap not vessel-gap**(`_data-gaps-noted.md §10`·옵션#3 option_groups 134/items 469·제약#5 constraints.logic 10·완제SKU#4 templates 12/selections 14 그릇 실재·거치 §0.5 OBSERVED했으나 옵션 일반 cascade 무왜곡 표현·후니 KB 결함 없음·ST 형상 G-SK-2와 정반대·dbmap round-6 CPQ/round-22 적재). **신규 테이블/컬럼/DDL/V-번호 0**(dbm-ddl-proposer 미호출). **★deepcheck carry-forward(codex gpt-5.5 second-opinion·전부 `unverified`·mint 0): H-1 content-container 합성역할 축(#18 최강 적대 후보·codex가 우리 data-gap 판정 기준에 수렴)·H-2 매팅/패스파르투(mat board·우리 분석 전무)·H-3 aperture/창 기하·M-4 glazing 다형성·M-6 포토북 페이지 모델 = 전부 §0.5 미캡처 영역(멀티/매팅/aperture 액자)·라이브 실측 대상(OBSERVED되면 그때 그릇 검토)·현재 mint 0(unobserved-pending).** **17축 재포화의 vessel-side 검증** — 9번째 카테고리(directive 최대 관전: 완제 액자 프레임[인쇄물 사후 끼우는 2-파트 빈 그릇]·마운팅/거치·전면 보호재)가 새 그릇을 요구하지 않음. 인벤토리·Wave·정비권고 무수정·v1.0~v8.0 무수정.
...
### v7.0 (AC) — facet 흡수만 (신규 그릇 0·V-3 3차원 확장·17축 재포화)
| 축 | gap 판정 | vessel 결과 | 사다리 | 신규 그릇 |
|---|---|---|---|---|
| **AC #1 두께(measure_type)** | WEAK → V-3 흡수 | **신규 그릇 0** — 아크릴 두께=WGT 슬롯 다의(평량 g/두께 mm/용량)·`weight`/`depth` NULL·`mat_nm` 융합 → `MAT_FACET` measure_type 세분(`vessel-material-axis.md §10.1`·dbmap CLEAR3T mat_cd 통합 동형) | (V-3 공유·코드행) | 0 (V-3 흡수) |
| **AC #2 surface-finish(글리터/거울/자개/홀로)** | WEAK → V-3 흡수 | **신규 그릇 0** — surface/finish 컬럼 전역 0건·`mat_nm` 융합 → `MAT_FACET` 표면마감 코드(★ST S-4 adhesion/weather와 **동근·통합**·`vessel-material-axis.md §10.2`) | (V-3 공유·코드행) | 0 (V-3 흡수) |
| **AC #4 단일 부자재 마스터(고리/받침/자석)** | WEAK → V-3 버킷 재정의 | **신규 그릇 0** — 고리/받침/자석/와이어링 MAT_TYPE.04/.07/.10/.02 4버킷 분산·D링 3중복·RP KR/CN/CR 횡단 단일카탈로그 미달 → V-3 §6 버킷 재정의(`vessel-material-axis.md §10.3`·★round-22 ④자재 B-3 조율·행 영향 큼·우선순위 중·designer 단독 mint 금지) | (V-3·B-3 공유·주 data) | 0 (V-3 흡수·일부 data) |
| **AC #5 acrylic2025 가격엔진(prc_typ .02)** | WEAK → V-7 위임 | **신규 그릇 0** — `PRF_CLR_ACRYL` 1행·frm_typ_cd 부재·★Q-ACR-7(.02 미확정·CLEAR3T 84행) 라이브 확증 = V-7 frm_typ_cd 결손(`vessel-quantity-size-pricing.md §C5`)·dbmap 31_acrylic 가격 트랙 위임 | (V-7 공유·위임) | 0 (V-7 흡수) |
| **AC #3 입체/받침 부속물** | PASS | **신규 그릇 0** — `t_prd_product_addons`(addon→tmpl_cd)=받침/스탠드 부속물 표현(BN 거치대 D-1 동형)·입체성 분산(받침=#8·두께=자재·양면=옵션·조형=공정) 전부 기존 축 | — | 0 (그릇 보유) |
| **AC #6 인쇄면+화이트(투명 종속)** | PASS | **신규 그릇 0** — `PROC_000008 화이트` 별색 family 실재·인쇄면=옵션#3·투명소재→가용=제약#5(JSONLogic)(ST S-7·TP T-E 동형) | — | 0 (그릇 보유) |
| **AC (부) ACTPKEY 키링 템플릿** | GAP → V-10/V-11 흡수 | **신규 그릇 0** — 키링 디자인 자산=#16 TemplateAsset(T-A 동형)·`t_prd_templates` 완제SKU 적재 금지(`vessel-template-asset.md §8`) | (V-10/V-11 공유) | 0 (V-11 흡수) |

### v8.0 (PD) — facet 흡수만 (신규 그릇 0·8번째 distinct 0·17축 재포화)
| 축 | gap 판정 | vessel 결과 | 사다리 | 신규 그릇 |
|---|---|---|---|---|
| **PD-1 봉제/제품가공** | GAP → V-8 흡수 | **신규 그릇 0** — SEW_LTR/PDT_WRK = GS PDT_WRK/FLX_ZIP 동일 #14 GAP family(`vessel-form-assembly.md §7`)·PROC_CLASS 분류축뿐(GS와 동일·PD 추가 0)·봉제/조립/솜충전 공정 행=data | (V-8 공유·코드행) | 0 (V-8 흡수) |
| **PD-2 직물/PU 원단(물성 차원)** | WEAK → V-3 흡수 | **신규 그릇 0** — 번수(면10수)·신축성·원단종류=`MAT_FACET` 합성차원(★ST 점착/AC surface_finish와 *동근*·`vessel-material-axis.md §11.1`)·미적재 본체소재 행=data | (V-3 공유·코드행) | 0 (V-3 흡수) |
| **PD-5 밑창색×사이즈 SUB_MTR variant** | WEAK → V-3 흡수 | **신규 그릇 0** — ★별색 아님 정정(D-PD-1·`six_clr` 별색 아닌 부자재 variant)·밑창=usage_cd .07 sub_mtrl+addon#8·색×사이즈 12-variant=`ref_key1/ref_key2` 2D(§9 CL 동형·라이브 255/469)·`vessel-material-axis.md §11.2` | (V-3 공유·기존 그릇) | 0 (V-3 흡수) |
| **PD-3 단수(2/3단)·스툴 형상** | PASS | **신규 그릇 0** — `t_siz_sizes`(work/cut width·height) 1:1 프리셋 흡수(2단=495×320·원형=305×305)·★ST 형상#17의 1:多 미충족이라 V-12 형상축 강제 *불요*(BN/GS/TP/PR 1:1 흡수 카테고리 동형·size 유지) | — | 0 (그릇 보유·#13 사이즈) |
| **★PD-4 완제 구조물 내재BOM(다리/받침/논슬립·솜/지퍼)** | ★data-gap (not vessel-gap) | **신규 그릇 0·vessel 아님** — `t_prd_product_addons`(addon→tmpl_cd·#8 부속물·BN 거치대 D-1·AC 받침 동형) + `t_prd_product_materials.usage_cd .07`(639행 슬롯) **그릇 3-레벨 실재**·후니 KB addl_product/usage/생산방식 1급 모델링·★ST 형상 G-SK-2("어느 축에도 없음" vessel-gap)와 정반대 → 적재만 안 됨(`_data-gaps-noted.md §9`·dbmap round-22 GPM-4) | — | 0 (그릇 보유·data-gap) |

### v9.0 (PH) — facet 흡수만 (신규 그릇 0·9번째 distinct 0·17축 재포화·★directive 최대 관전)
| 축 | gap 판정 | vessel 결과 | 사다리 | 신규 그릇 |
|---|---|---|---|---|
| **PH-2 완제 액자 SKU(거치+마감+사이즈 인코딩)** | WEAK → V-11/#4 흡수 | **신규 그릇 0** — `t_prd_templates`(12행)+`template_selections`(14행)로 거치+마감+사이즈 combobox 완제SKU 표현 그릇 보유(AC 두께/소재 variant·GS 완제SKU 동형)·★완제 액자 SKU[주문 단위]↔에디터 디자인 자산 이중의미 분리=`vessel-template-asset.md §9`(TP T-A·V-11 결함과 동근)·"인쇄물+별도 프레임 조립" 2-파트 완제도 template으로 담음·SKU breadth=data | (V-11 공유·기존 #4 테이블) | 0 (V-11 흡수) |
| **PH-3 인화지×마감 surface-finish** | WEAK → V-3 흡수 | **신규 그릇 0** — surface/finish/glitter 컬럼 전역 0건·마감(유광/반광/스노우)이 `mat_nm` 융합("유광(Glossy)_캐논전용지") → V-3 `surface_finish` 합성차원(★AC A-2·ST S-4 점착/내후와 *동근*·`vessel-material-axis.md §12`)·인화지 매체행=data | (V-3 공유·코드행) | 0 (V-3 흡수) |
| **PH-6 set/sheets 고정 단위(600매·4/5sheets)** | WEAK → #10 수량모델 흡수 | **신규 그릇 0** — `t_prd_product_bundle_qtys`(28행)+`qty_unit_typ_cd` 고정 set 단위 그릇 보유(GS 텀블러 base_quant 동형)·★set-as-base-unit vs 자유 수량 구분 약함=기존 #10 수량모델 WEAK(V-5 보류 동일)·set 단위=data | (기존 #10·보류) | 0 (기존 그릇·#10) |
| **PH-1 거치 캐스케이드(탁상용/벽걸이)** | PASS | **신규 그릇 0** — 거치=택1 `option_groups`(SEL_TYPE.01·134행)→`option_items`(469·polymorphic)→cascade `constraints.logic`(10행·JSONLogic) 그릇 보유(AC 명찰 cascade·ST disable·BN 어깨띠 동형)·§0.5 거치 OBSERVED했으나 옵션 일반 cascade 무왜곡·후니 KB 결함 없음→#18 부결·cascade 적재=data | — | 0 (그릇 보유·옵션#3/제약#5) |
| **PH-4 형태(일반/정사각/파노라마) 비율 프리셋** | PASS | **신규 그릇 0** — `t_siz_sizes`(work/cut width·height) 1:1 프리셋 흡수(정사각=W:H 동일·파노라마=W≫H)·★ST 형상#17의 1:多 미충족(형태 1↔width·height 1:1·PD-3 단수/형상 동형)이라 V-12 형상축 강제 *금지*(size#13 유지·BN/GS/TP/PR/PD 1:1 흡수 동형) | — | 0 (그릇 보유·#13 사이즈) |
| **PH-5 PHMG/PHPO 다중분류(출력매체 PH ⊥ 컵&홀더)** | PASS (정책) | **신규 그릇 0** — `t_prd_product_categories`(PK=(prd_cd,cat_cd)·`main_cat_yn`)·2카테고리 8상품 **라이브 실재** N:M 다중분류 그릇 보유·작동(GS 코스터 동형)·"PH로 묶을지 컵&홀더로 묶을지"=정책 결정(메타모델/vessel 판정 아님) | — | 0 (그릇 보유·#7 다중분류·정책) |
| **★PH-1/PH-2 거치 마운팅·완제 액자(directive 최대 관전)** | ★data-gap (not vessel-gap) | **신규 그릇 0·vessel 아님** — 거치=옵션#3 `option_groups`(134)/`option_items`(469)·제약#5 `constraints.logic`(10행)·완제SKU#4 `templates`(12)/`selections`(14)·자재#1 프레임재질 variant **그릇 4-레벨 실재**·거치 §0.5 OBSERVED·후니 KB 옵션/완제SKU/자재 1급 모델링·★ST 형상 G-SK-2("어느 축에도 없음" vessel-gap)와 정반대 → 적재만 안 됨(`_data-gaps-noted.md §10`·dbmap round-6 CPQ/round-22) | — | 0 (그릇 보유·data-gap) |
| **★deepcheck H-1/H-2/H-3·M-4/M-6 (codex carry-forward)** | ★unobserved-pending (mint 0) | **신규 그릇 0·미관측·검증 전 그릇 제안 금지** — H-1 content-container 합성역할(#18 최강 적대 후보·codex가 우리 data-gap 기준 수렴)·H-2 매팅/패스파르투(mat board·우리 분석 전무)·H-3 aperture/창 기하·M-4 glazing 다형성·M-6 포토북 페이지 모델 = 전부 §0.5 미캡처(멀티/매팅/aperture 액자) 영역·**라이브 실측 대상**(mat board/aperture/glazing 옵션이 별 슬롯으로 OBSERVED되면 그때 그릇 검토)·전부 `unverified` | — | 0 (★HARD: 미관측·mint 금지) |
...
- **"신규 그릇 불요" 재분류: 8** — V-2·#4·V-5·V-6·V-7 + **V-9(prd_typ_cd+semi_role_cd PASS)** + MAT_TYPE(신규 0·use_yn만) + **TP facet(VDP본문보류·페이지/형태/특수인쇄 기존 흡수)** + **ST facet 5항(재단입자 PASS·점착→V-3·인쇄방식→V-2·disable→V-4·칼선→V-12/V-1 게이팅)**. + essential(V-4 내부 PASS). **+ PR/CL/AC/PD/PH facet 전건 흡수(신규 0)** — PR 6항·CL 5항·AC 6항·PD 5항·**PH 6항(완제 액자 SKU→V-11·인화지×마감→V-3·set 단위→#10·거치 캐스케이드=옵션#3 PASS·형태 비율=#13 PASS·다중분류=#7 PASS·★PH-1/PH-2 거치 마운팅=data-gap·★deepcheck H-1/H-2/H-3=unobserved-pending mint 0)** 전부 기존 V-항목/판정에 귀속·새 V-번호 0.
...
- **★PD 교훈(v8.0): 8번째 카테고리가 신규 그릇 0건(17축 재포화·PR·CL·AC 패턴 반복)** — PD facet 5항이 전부 기존 17축(V-8 봉제 family·V-3 직물물성/밑창 sub_mtr)으로 흡수, **가장 이질적 봉제 구조물 완제품(directive 최대 관전: 조립/구조/3D폼/완제 내재BOM)조차 새 그릇 0.** ★핵심 = **PD-4 완제 구조물 내재BOM = data-gap not vessel-gap 라이브 확정** — 다리/받침/논슬립=`t_prd_product_addons`(addon→tmpl_cd·#8 부속물)·솜/지퍼=`usage_cd .07`(639행 슬롯) 그릇 3-레벨 실재·후니 KB가 addl_product/usage/생산방식를 *이미 1급 모델링*. ★대조 정직성: 같은 PD 안에서 PD-4 내재BOM은 그릇 실재(data-gap·dbmap)인데 ST 형상(#17 KB G-SK-2 "어느 축에도 없음")은 그릇 부재(vessel-gap·V-12) — **vessel-gap vs data-gap 판정은 라이브 그릇 실재 여부가 결정하지 "완제품이라 복잡해 보임"이 아님**(directive 핵심 #18 완제 구조물 내재BOM 부결의 근거). ★봉제(PD-1)도 GS PDT_WRK/FLX_ZIP과 동일 #14 family로 합류 — V-8 PROC_CLASS 분류축 1개가 GS 파우치~PD 봉제 구조물까지 커버(family 멤버만 늘고 그릇은 안 늘음). ★직물물성(PD-2)이 BN→GS→ST→AC→PD 5카테고리째 V-3 합성-분해축에 차원을 더했으나 여전히 MAT_FACET 코드행으로 닫힘(테이블 mint로 번지지 않음·합성-분해축 사다리 정직성 5연속 확증).
- **★PH 교훈(v9.0): 9번째 카테고리가 신규 그릇 0건(17축 재포화·PR·CL·AC·PD 패턴 반복·★directive 최대 관전 부결)** — PH facet 6항이 전부 기존 17축(V-11 완제 액자 SKU·V-3 인화지×마감 surface_finish·#10 set 단위·옵션#3 거치·#13 형태·#7 다중분류)으로 흡수, **directive 최대 관전(완제 액자 프레임[인쇄물 사후 끼우는 2-파트 빈 그릇]·마운팅/거치[탁상용/벽걸이]·전면 보호재)조차 새 그릇 0.** ★핵심 = **PH-1/PH-2 거치 마운팅 = data-gap not vessel-gap 라이브 확정** — 거치=옵션#3 `option_groups`(134)/`constraints.logic`(10)·완제 액자=완제SKU#4 `templates`(12)/`selections`(14)·프레임재질=자재#1 variant 그릇 4-레벨 실재·후니 KB가 옵션/완제SKU/자재를 *이미 1급 모델링*. ★결정적 분기 정직성: §0.5 client-render 재캡처가 PH 고유 블로커(거치 SSR-negative)를 거치 OBSERVED로 *해소했음에도* 거치가 옵션 일반 cascade로 무왜곡 구현됨이 실측 확인 → ST 형상#17(전용 슬롯 OBSERVED ✅ + KB G-SK-2 "어느 축에도 없음" 결함 ❌→❌ 둘 다 충족→승격)과 달리 거치=전용 슬롯 OBSERVED(✅)+KB 결함 없음(❌)=②만 충족→#18 부결. **vessel-gap 승격은 OBSERVED만으로 부족·KB가 "기존 축이 못 담음" 결함을 명시해야 함**(과잉승격 회피의 정직성). ★완제 액자 SKU(PH-2)가 BN→GS→AC→PH 동형 완제SKU#4/V-11 흡수 — "인쇄물+별도 프레임 조립" 2-파트 완제 구조조차 거치+마감+사이즈 인코딩 완제SKU combobox로 무손실(완제SKU#4 flat이 견딤). ★인화지×마감(PH-3)이 BN→GS→ST→AC→PD→PH 6카테고리째 V-3 합성-분해축에 surface_finish 차원을 더했으나 여전히 코드행으로 닫힘(테이블 mint 0·합성-분해축 사다리 정직성 6연속 확증). ★**deepcheck carry-forward 정직성[HARD]: codex가 #18 부결에 독립 동의(conditionally)하며 H-1 content-container를 #18 최강 적대 후보로 제기했으나, codex 결정 기준("재사용 라이브 슬롯 증거 없으면 승격 안 함")이 우리 data-gap 판정과 정확히 수렴.** H-1(content-container)·H-2(매팅/mat board·우리 분석 전무)·H-3(aperture/창 기하)·M-4(glazing 다형성)·M-6(포토북 페이지) = 전부 §0.5 미캡처(멀티/매팅/aperture 액자) 영역·전부 `unverified` → **search-before-mint HARD: 미관측 후보에 그릇 mint 금지·라이브 실측 대상으로만 carry-forward**(mat board/aperture/glazing 옵션이 별 슬롯으로 OBSERVED되면 그때 그릇 검토). AC layer-stack·PD construction-spec REFUTED 교훈(검증 전 distinct 신뢰 금지) 적용 — 가설을 그릇으로 굳히지 않음.
- **★AC 교훈(v7.0): 7번째 카테고리가 신규 그릇 0건(17축 재포화·V-3 3차원 확장)** — AC facet 6항이 전부 기존 17축(V-3/V-7/V-10/V-11)으로 흡수, **가장 강한 새 후보(A-8 가공방식 그룹핑 #18)조차 부결.** ★핵심 = **V-3 자재 분해축이 AC로 3차원 확장되면서도 새 그릇 0** — ① 두께 measure_type(평량 g/두께 mm/용량을 한 WGT 슬롯이 다의로 씀·`MAT_FACET` 세분) ② surface_finish(글리터/거울/자개·★ST adhesion/weather와 *동근 통합*=한 합성-분해축이 점착·내후·표면마감을 다 담음) ③ 단일 부자재 마스터(고리/받침/자석 MAT_TYPE 4버킷 분산·D링 3중복·버킷 재정의·★round-22 B-3 조율·주 data). 이는 BN→GS→ST→AC 4카테고리 누적이 *합성-분해축은 차원을 계속 더해도 facet 코드행/버킷으로 닫힌다*는 사다리 정직성을 입증 — 차원이 늘어도 테이블 mint로 번지지 않음. ★단일 부자재 마스터는 vessel(버킷 재정의)+data(D링 행 통합) **양면이라 round-22 B-3과 조율 필수**(행 영향 큼·designer 단독 mint 금지). ★dbmap 31_acrylic 라이브 산출과 구조 동형 확증(CLEAR3T mat_cd 통합·MIRROR3T 별 comp·화이트=공정)이 V-3/V-7 흡수의 외부 근거.
...
- **PH 검증 요청(v9.0):** ⑧ **PH 신규 그릇 0건 정당성**(9번째 distinct 0·완제 액자 그릇/마운팅 #18 부결·PH facet 6항 전부 기존 V-11/V-3/#10/옵션#3/#13/#7 흡수·새 V-번호 0) ⑨ **★PH-1/PH-2 거치 마운팅(directive 최대 관전) = data-gap not vessel-gap 판정 정당성**(거치=옵션#3 option_groups 134/items 469·제약#5 constraints.logic 10·완제SKU#4 templates 12/selections 14 그릇 4-레벨 라이브 실재·거치 §0.5 OBSERVED했으나 옵션 일반 cascade 무왜곡·후니 KB 결함 없음·ST 형상 G-SK-2와 정반대 = vessel-gap 승격 ②기준 불충족) ⑩ **★[HARD] deepcheck H-1/H-2/H-3·M-4/M-6 = unobserved-pending·mint 0 확인**(content-container/mat board/aperture/glazing/포토북 페이지 전부 `unverified`·§0.5 미캡처 영역·검증 전 그릇 제안 silently 채택 0·search-before-mint 준수·라이브 실측 대상으로만 carry-forward — AC layer-stack/PD construction REFUTED 교훈 적용) ⑪ **V-11 TemplateAsset 분리가 PH 완제 액자 SKU↔디자인시안 이중의미를 무손실 담는지**(TP T-A 동근·완제SKU#4 오염 방지) ⑫ **V-3 surface_finish 합성차원이 PH 인화지×마감을 AC A-2·ST S-4와 동근 흡수하는지**(6카테고리째 합성-분해축에 차원 추가·테이블 mint 0 유지).
