...
| 2026-06-14 | **라이브 정정 방법론 전환 + 최신 260610 전시트 추출·검증 GO + 레더 교정 라이브 적용** — 사용자 `/harness:harness`("상품마스터·가격표 엑셀 추출로 라이브 오류 정정하려는데 왜 v03 토대로 검증하나·올바르게 적재"). **데이터 3층 규명**: ① 원본 엑셀(정답)=`docs/huni/후니프린팅_상품마스터_260610.xlsx`+`_가격표_260527.xlsx` ② v03 마이그레이션(오염원)=`raw/webadmin` 클론에 파일 없음·원본→v03 정규화 코드도 부재(외부 블랙박스) ③ 라이브=`load_master.py`가 v03을 `TRUNCATE`+재적재한 거울. **[HARD 방법론 전환] 라이브 정정 권위=원본 엑셀 L1·v03/load_master 권위 배제·라이브 직접 정정(멱등 UPSERT·v03 우회).** `06_extract`=260527 stale 적발 → `dbm-excel-analyst` 최신 260610 전13시트 재추출(`24_master-extract-260610/`·1,231행 L1) → `dbm-validator` 독립검증(추출 스크립트 무관 엑셀 raw 3만셀 대조) **E1~E5 GO·뒤집힘0**(MINOR 1 보정). 260527→260610 차이=4시트만(아크릴 −16·굿즈 size→option·스티커37·실사1·round-10 527셀 byte정합)·나머지 8시트 동일→round-13 교정 유효. **라이브 COMMIT(원본 L1 정답·멱등·검증): 레더 BATCH-4(`t_mat_materials` MAT_000186 .08→.06·MAT_000006 .01→.06=가죽·6상품·photobook F-PB-2 정답).** 교훈: load_master `TRUNCATE` 재적재→DB직접교정 재실행 회귀(v03 레포없어 직접정정 안전·소스가드 보류). DB 정정 일부 적용·다음=260610 L1 기준 라이브 정정 매니페스트(dbm-correctness-auditor)→검증→적재. | `24_master-extract-260610/`(신규)·`23_remediation-apply/leather/`(신규)·`22_price-load-prereq/`(신규)·HANDOFF·메모리·CLAUDE.md | 사용자(`/harness:harness` — 라이브 오류 올바른 적재) |
---
