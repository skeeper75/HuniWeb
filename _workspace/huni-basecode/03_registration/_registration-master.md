...
| 인쇄옵션 🔴 | t_prd_product_print_options(166) | **PO-1 UV 변형 63행 축이동**(print_side→**기존 dtl_opt**·PO-1a 14/PO-1b 7 분기) | **0**(dtl_opt 재사용·B4) | B-PO-1(14 한정)·PO-1b 정체 |
| ⑤ 공정 🟡 | t_proc_processes(102) | **기존 dtl_opt 재사용**(param 선택값 채움·data-gap·신규 컬럼 철회) | **0**(B4·신설 철회) | AX-5(이관 범위)·AX-6 |

**4축 = 즉시 교정 2(SZ-1) + 축이동 63(PO-1a 42/PO-1b 21·선결 의존) + dtl_opt 재사용 채움(신규 컬럼 0) + 판정불가 31(SZ-2 30·PR-2 1). 신규 mint·컬럼·코드행·테이블 = 0.**
...
| **★4축: 인쇄옵션 UV 축이동(PO-1)** | **63행** (PO-1a 14 실연결 ~42 + PO-1b 7 무연결 ~21·print_side→**기존 dtl_opt**·선결=option_items 행/PROC 링크·돈 크리티컬 formula 간접) |
...
| **★4축: 판정불가(컨펌 보류)** | **31** (SZ-2 형상+EA 30·PR-2 레이플랫 1) |
...
## 4. ★ 컨펌 큐 — 자재·카테고리 0건 해소 + 4축 신규 escalate 4건

> **1순위(자재·카테고리) = 전건 해소(잔여 0)** · **4축(2차 회차) = 신규 escalate 4건**(SZ-2·AX-5·AX-6·B-PO-1).
...
### 4.2 ★ 4축 컨펌 큐 — escalate 5건 (평이한 한국어 질문 첨부·B4/B2 정정)

> 진단 escalate(AX-5·B-PO-1·PO-1b 정체·AX-6·SZ-2). 명세는 만들되 인간 결정 전 등록/삭제 보류. 평이한 한국어 질문 = 비전문가 운영자/실무진용. **[B4] AX-5 = 신규 칸 신설 아님·이관 범위만(dtl_opt 기존 실재).**

| ID | 축 | 막힌 결정 | 평이한 한국어 질문 | 명세 위치 |
|----|----|-----------|--------------------|-----------|
| **AX-5** [B4 정정] | 공정/인쇄옵션 | 공정 파라미터(줄수·조각수·UV 변형) 선택값을 **기존 dtl_opt 칸에 채우는 범위**(신규 칸 아님) | "오시 2줄, 반칼 4조각, 아크릴 풀빼다 같은 '공정 세부 선택값'을 담는 칸은 이미 있습니다(봉제 옵션에서 쓰는 중). 대부분 비어 있을 뿐이에요. 어느 상품·옵션부터 채울지만 정하면 됩니다(새 칸 만들 필요 없음)." | `regspec-process.md §1·§6` |
| **B-PO-1** [B2 보정] | 인쇄옵션 | 화이트/클리어 underbase가 별색공정(PROC_000008)인지 UV 변형 param(풀빼다)인지 — **UV 14 실연결 한정** | "UV로 인쇄하는 14개 아크릴 상품에서 흰색을 먼저 깔아 색이 잘 보이게 하는 작업(풀빼다)을 'UV 변형'으로 보면 될까요? (디지털/실사는 '별색 화이트', UV는 '풀빼다')" | `regspec-printoption.md §6` |
| **PO-1b 정체** [B2 신규] | 인쇄옵션 | UV 변형값은 있으나 공정 연결 0인 7상품(코롯토164·카라비너166 등)의 정체·연결 여부 | "코롯토·카라비너 같은 7개 상품은 화면엔 'UV 변형'이 적혀 있는데 실제 UV 공정 연결이 없습니다. 정말 UV 가공인가요, 비활성/정체인가요? 공정 연결을 먼저 정해야 변형값을 옮길 수 있습니다." | `regspec-printoption.md §1·§6` |
| **AX-6** | 공정 | 레이플랫(PROC_000025)이 실제 운영 공정인지 안 쓰는 잔존 코드인지 | "제본 중 '레이플랫'이 지금 살아있는데(활성), 실제로 주문에서 쓰는 가공인가요, 아니면 예전에 만들어두고 안 쓰는 항목인가요? 안 쓰면 숨기고(소프트삭제), 쓰면 그대로 둡니다." | `regspec-process.md §3·§6` |
...
> **★4축 escalate 처리 [B4·B2 정정]:** **AX-5 = 신규 칸 신설 아님·이관 범위 결정만**(dtl_opt 기존 실재·DDL/인간 승인 불요·dbmap 적재 트랙). B-PO-1은 **UV 14 실연결 한정**(풀빼다 유력·실무진 확정). PO-1b 정체는 7 무연결분 **공정 연결 선결이 underbase 판정보다 먼저**. AX-6·SZ-2는 라이브 상품 연결/칼틀 실측 전 등록·삭제 0(판정불가·보류). **★신규 그릇 = 0**(ref_param_json 철회·전부 교정/축이동/data-gap/판정불가·신규 그릇 모호 0).
...
| 트랙 | 위임처 | 내용 |
|------|--------|------|
| 실 적재(축이동·교정) | `dbm-axis-staged-load`(경로 Y 교정엑셀 우선) / `dbm-load-execution`(멱등 UPSERT·`WHERE del_yn='N'` 가드) | 자재 89행 축이동·BOM link 177 제거(GPM-4)·소프트삭제 del_yn='Y'·.10→.07 ~17 교정·봉투/헤더 정리 11·카테고리 소프트삭제 11·재연결 2 · **4축: SZ-1 색오염 2행 정규화·PO-1 UV 63행 축이동(PO-1a 14: option_items 행 선적재 후 / PO-1b 7: PROC_000002 링크 선행·기존 dtl_opt로 이관)** |
...
| **컨펌 결정(4축·신규)** | 리더 escalate / 실무진 | **escalate 5건** — AX-5(dtl_opt 이관 범위·DDL 불요·B4)·B-PO-1(underbase 귀속·UV 14 한정)·PO-1b 정체(7 무연결 공정 연결)·AX-6(레이플랫 운영)·SZ-2(형상 칼틀·EA 의미). 평이한 한국어 질문 §4.2 |
...
- **4축 escalate [B4·B2]:** AX-5(dtl_opt 이관 범위·**DDL 불요**·신규 그릇 아님)·B-PO-1(underbase 귀속·**UV 14 한정**)·PO-1b 정체(7 무연결 공정 연결 선결)·AX-6(레이플랫 운영)·SZ-2(형상 칼틀·EA). 평이한 한국어 질문 §4.2. **★신규 그릇 정당성 모호 = 0**(ref_param_json 철회·전부 교정/축이동/data-gap/판정불가).
