> ★★ SUPERSEDED / 폐기 (2026-07-01 라이브 재진단) — 본 설계의 핵심 전제("페이지=print동형 opt_cd 차원, 옵션그룹 신설")는
> webadmin 실제 메커니즘과 불일치. 라이브 실호출 결과 094=셋트(내지 095)이고 페이지는 page_rule(전용 차원)로 처리됨.
> opt_cd 옵션그룹은 page_rule과 이중 입력 충돌 → 폐기. 진짜 해법=셋트 내지 가격공식(§18/§23)+pansu(§26).
> 미적용·COMMIT 0. 재진단 상세: `diag-pcb30p-set-live-260701.md`. 아래 원문은 이력 보존용.
