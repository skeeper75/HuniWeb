...
| 판정축 | 실측 | 판단 |
|---|---|---|
| 영향 단위 | 게시 위젯/상품 전량(판정 시점 실측 — **260827 재스윕 기준 192/192**, 260822 승계 194/193), 렌즈 B `BROKEN` **21**(260822 승계 66) | 15 파일-등가 초과 → L |
...
| 게시 전수 점검(분모=실행 시점 실측) | `raw/webadmin/tools/audit_published_prices.py` | `--products/--limit/--workers` |
...
### M1 — 가격소스·할인 배선 [우선순위 High]

> **[v0.1.6 전면 재산정]** 제목이 「E1 가격소스 배선」에서 바뀌었다. M0-5 재스윕과 M0-7 라이브 실측 결과 **게시 분모 안 E1 잔량이 0** 이 되어 v0.1.4 대상 6계열 중 4계열이 소멸했고, 남은 대상의 과반이 할인 배선이기 때문이다. 근거 정본: `progress.md §E.2 M0-7 B·C`.

**대상 재산정 (2026-08-27 라이브 실측)**

| 계열 | v0.1.4 기재 | 260827 실측 | 판정 |
|---|---|---|---|
| 렌즈 B `E1 NO_SOURCE` ∩ 게시 | 75건 | **0** | 소멸 |
| `wiring-health-index` 치명 E1 | 15상품 | 게시중 무소스 0 | 소멸 |
| 백로그 t6 게시중 무가격 | 15상품 | 〃 | 소멸 |
| `PRD_TYPE.01` 가격소스 전무 | 2건 | `PRD_000038`·`PRD_000220` 둘 다 `FORMULA` 확보·게시 Y | 소멸 |
| 백로그 t7 | 2건 | `PRD_000010`·`PRD_000218` 둘 다 소스 `NONE`·**미게시** | 분모 밖 |
| **고아 공식 바인딩** | 10건 | **10** (`t_prc_price_formulas` 중 바인딩 0) | **유효** |

**게시 분모 안 M1 실질 대상 = 16건**

| # | 대상 | 건 | 화면 |
|---|---|---|---|
| 1 | **할인 적용 범위 교정**(REQ-PW-023) — `PRD_000147` 아크릴마그넷 · `PRD_000149` 아크릴집게 · `PRD_000154` 아크릴 머리끈 | **3** | 가격 뷰어 「적용 대상」 |
| 2 | **할인 미연결** — `PRD_000097` 떡메모지 · `PRD_000217` 만년스탬프 · `PRD_000280` 레더라벨제작 (셋 다 소스 `FORMULA` 보유·할인만 없음) | **3** | 가격 뷰어 할인표 연결 |
| 3 | **고아 공식 바인딩 → 결함 S 교정** — `PRF_ACRYL_BADGE`→`PRD_000148` 아크릴뱃지 · `PRF_ACRYL_NAMETAG`→`PRD_000152` 아크릴명찰 · `PRF_ACRYL_SMARTTOK`→`PRD_000150` 아크릴스마트톡 | **3** | 가격 뷰어 → `price_source_save` |

- 화면: 가격 뷰어 → `price_source_save`(`kind:"formula"` 우선, 직접단가 상품은 `kind:"price"`) / 할인 연결은 `t_prd_product_discount_tables`(`dsc_tbl_cd` + **`comp_cd` = 적용 대상**).
- **선행 확인 [HARD]**: 각 건마다 직접단가 보유 여부를 먼저 확인한 뒤에만 소스 배선으로 올린다(REQ-PW-007).
- blast_radius: 상품 단위 바인딩이므로 원칙적으로 1. 공식 공유 시 해당 공식의 바인딩 상품 수 표기.
- **분모 밖으로 이동한 8건** — t7 2건 · 할인 미연결 미게시 5건(`PRD_000199`·`204`·`218`·`227`·`312`) · `PRD_000222` 말랑증사홀더. 기존 `Out of Scope — 미게시 상품` 조항이 커버하며 **신규 예외 조항을 두지 않는다**.
- 별항: `PRD_000218` 은 권위 엑셀 굿즈파우치 row88 가격 셀 **공란** → `needs_authority`, 실무진 문항으로 분리(미게시이므로 게시 분모 밖).
**고아 공식 10건 판정 확정 [M0-8]** — 권위 대조 완료. 근거 정본: `progress.md §E.2 M0-8`.

| 판정 | 건 | 대상 |
|---|---|---|
| **바인딩 필요** | **3** | 위 표 #3 — 결함 S 교정 |
| 바인딩 불요(구버전·부분 공식) | 4 | `PRF_PHOTOCARD_FIXED` · `PRF_NAMECARD_FIXED_FOIL` · `PRF_POSTER_FIXED` · `PRF_FOLD_SUM` |
| 설계 미완(`_TBD` · 단가행 0) | 3 | `PRF_ACRYL_SHCOROTTO_TBD` · `PRF_ACRYL_ZIBITZ2_TBD` · `PRF_ACRYL_ZIBITZ_TBD` |

> **🔴 결함 S [돈 크리티컬] — 아크릴 3상품 부속비 전액 미청구.** `PRD_000148`·`150`·`152` 는 부속이 빠진 공용 공식 `PRF_CLR_ACRYL`(구성요소 `COMP_ACRYL_CLEAR3T` 단독)을 쓰는데, 형제 4상품(`146` 키링 · `147` 마그넷 · `149` 집게 · `154` 머리끈)은 전부 「인쇄가공비 + 부속」 2구성요소다. 부속 comp(`COMP_ACRYL_BADGE` 600·1,000 / `COMP_ACRYL_NAMETAG_PIN` 700·1,700 / `COMP_ACRYL_SMARTTOK` 2,600·3,000)는 **고아 공식에만 존재**해 어떤 계산에도 도달하지 못한다. 3상품 전부 **게시중**이다.
>
> REQ-PW-023(부속에 할인이 잘못 먹는 저청구)보다 **한 층 아래**의 결함이다 — 부속이 공식에 없어 청구 자체가 안 된다. 값이 0원이 아니라 덜 나오므로 `verify_zero_quote.py` 로는 잡히지 않는다(review-3 D2 사각).

- **교정 방식 [HARD] — 추가가 아니라 교체다.** `pricing.py:637` `_latest_ymd(frm_rows,'apply_bgn_ymd',as_of)` 가 바인딩 중 **최신 1건만** 고르므로, 기존 `PRF_CLR_ACRYL` 바인딩을 남긴 채 새 공식을 추가하면 어느 쪽이 선택될지 `apply_bgn_ymd` 에 좌우된다. 기존 바인딩 정리를 포함해 설계하고 **건별 인간 승인**을 받는다.
- **선행 확인 [HARD]**: 저장 전 시뮬레이터로 교정 전/후 최종가를 산출해 **차액이 부속 단가와 일치**하는지 확인한다(§C.3 절차 5).
**흡수 대상 [v0.1.2 — M0 산출]** (근거 정본: `progress.md §E.2 M0-1c`·`M0-1d`·`M0-1e`)

1. **할인 적용 범위 교정 3건**(REQ-PW-023) — `PRD_000147` 아크릴마그넷 · `PRD_000149` 아크릴집게 · `PRD_000154` 아크릴 머리끈. 전부 `PRF_ACRYL_*` = `COMP_ACRYL_CLEAR3T`(인쇄가공비) + 부속[가산] 2구성요소인데 할인이 **총액 스코프**로 걸려 있다. 교정 = 가격 뷰어 「적용 대상」에 `COMP_ACRYL_CLEAR3T` 지정 후 재연결. **기준 형태 = `PRD_000146` 아크릴키링**(4형제 중 유일하게 이미 정상). 권위 = 「계산공식집초안」 row 109 `(2) 수량별할인 = 인쇄가공비에만 적용`. **[M0-2 주의]** `PRD_000146` 은 2026-08-24 에 게시중단(`WGT_000301` → `WGT_STS_TYPE.03`)되어 **현재 게시 분모 밖**이다. 라이브 데이터는 그대로 남아 있어 **참조 형태로는 계속 유효**하지만, 교정 결과를 「게시 상품과 같은 모양」이라고 서술하지 않는다(`progress.md §E.2 M0-2 B` 파생 발견 J).
...
- 3군 판매가능 = 상품마스터 아크릴 21 + 굿즈파우치 96 + 문구 **10** = **127상품**(그레이배경 미출시·품절 제외). 「라이브 없음」 3건 · 라이브有미게시 34 · **게시 90**. 조인은 `prd_nm` 공백 정규화 기준. **[정정]** v0.1.2 의 «문구 5 / 합계 122» 는 범례 6줄을 이미 제외된 수에서 다시 뺀 중복 차감이었다(`progress.md §E.2 M0-1f` 분모 집계 오류 정정). **이 127 은 3군 필터일 뿐 SPEC 분모가 아니다** — 분모는 `spec.md §1.2` 의 게시 전량이며, 3군은 그 부분집합(게시 90)이다.

- **층별 중복 관계 [HARD · C6]**: 위 대상 목록의 앞 3개 항목은 **같은 "가격 소스 없음" 을 서로 다른 분모·판정층에서 본 관측치**이며 서로 중복된다 — 렌즈 B `E1 NO_SOURCE` 75건(게시 194 / 실호출 — 260822 실측) ⊇ `wiring-health-index` 치명 E1 15상품(게시 / 오프라인 집계) ⊇ t6 게시중 무가격 15상품(같은 15상품의 카드 표현). PRD_TYPE.01 2건은 `PRICE-DB-STATE` `d_none` 50건(분모 266)의 부분집합이다. **대상 목록은 4개 층의 합집합으로 구성하되, 종료 시 E1 잔량 계상은 렌즈 B 층 단일 기준으로만 센다**(`spec.md §1.2.1`). 건수를 합산하지 않는다.
...
**흡수 대상 [v0.1.6 — M0-4 산출] 인쇄후가공 권위 갱신분 4건** (근거: `progress.md §E.2 M0-4 D`)

권위를 260705 → **260822_1** 로 갱신하자 19시트 배치 diff 에서 신규로 드러난 결함이다. 나머지 11개 DIFFED 시트는 무변동이었고 **사라진 결함은 0건**이다(순증만).

| ID | defect | key | 권위 | 라이브 | 돈영향 |
|---|---|---|---|---|---|
| **N-1** | `missing_cell` | 인쇄후가공 `가변(이미지)/1개/10000` | **168,000** | 없음 | 견적불가/최소가 |
| **N-2** | `missing_cell` | 인쇄후가공 `가변(이미지)/2개/10000` | **252,000** | 없음 | 〃 |
| **N-3** | `missing_cell` | 인쇄후가공 `가변(이미지)/3개/10000` | **285,600** | 없음 | 〃 |
| **N-4** | `mismatch` | 인쇄후가공 `오시/3줄/5000` | **113,000** | 103,000 | **저청구 10,000원** — `comp_price_id=8228` |

- N-1~N-3 은 `REQ-PW-024` 계열(단가 미적재 → 견적 차단)과 동형, N-4 는 `REQ-PW-023` 계열과 같은 **저청구**다.
- 처리 = **권위 반영 등록**이며, M2 의 「값 무변경·코드 이관만」 원칙의 예외가 아니다(그 원칙은 재키 드리프트 교정에 걸린다). N-4 는 값 변경이므로 **권위 행 인용 + 인간 승인**을 건별로 남긴다.
- 재현: `python3 _batch/scripts/run_all_260822.py` → `ALL-SHEETS-defects.csv`(기준선 `ALL-SHEETS-defects.260705-baseline.csv`).
...
- 도구: `verify_zero_quote.py` + `lens_b_runner.py` + `audit_published_prices.py`. 백로그 **t8**(WB-DIAG 근거 게시본 전수 진단) 흡수. 분모는 실행 직전 라이브 재실측(`spec.md §1.2`).
...
| **t8** | WB-DIAG 근거 게시 전수 진단 + 제외 3분류 | **M5** | 본 SPEC 의 직계 모체 |
