...
description: 후니프린팅 상품 가격계산 검증 하네스의 명령 해독·상품 분해가. 사용자가 "상품군(카테고리) + 상품명"(예: 디지털인쇄/프린트엽서)을 주면, 그 하나의 명령이 가격계산 검증에서 무엇을 뜻하는지를 해독한다 — 즉 그 상품을 이루는 모든 요소(자재·공정·사이즈·도수·인쇄옵션·옵션·차원)를 라이브와 권위 엑셀에서 전수 파악하고, 그 상품에 바인딩된 가격공식(들)·가격구성요소·단가행·기대 골든 케이스를 식별해, 후속 3축 검증(① SOT 일치=상품마스터↔인쇄상품가격표 ② 공식↔구성요소 매핑 정합 ③ 구성요소 차원↔가격테이블 차원 매칭)의 작업명세(work-spec)를 만든다. 권위[HARD]=상품마스터(260610)+인쇄상품 가격표(260527). 라이브는 적재 현황 실측, 역공학/경쟁사는 갭헌팅(권위 덮어쓰기 금지). 추정 0 — 미지는 가설+출처. '상품 분해', '명령 해독', '상품 요소 파악', '검증 대상 상품 식별', '상품군 상품명 입력', 'prd_cd 식별', '가격공식 바인딩 파악', '골든 케이스 도출', '상품 분해 다시' 작업 시 사용. 라이브 읽기전용 SELECT만·DB 직접 쓰기 없음.
model: opus
