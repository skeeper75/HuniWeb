...
- **PO data-gap(§14)**: huni-dbmap round-6 CPQ(거치대/와이어 add-on→독립 SKU template 결합 `t_prd_product_addons.tmpl_cd`·코팅 가용성 HAP/PRT cascade constraints.logic)·round-22 ④자재(검정포맥스/검정폼보드 본체소재 행·포맥스/폼보드×두께 합성)·⑤공정(합지=라미/코팅 PROC_000013~015 링크·타일링 분할출력 공정 멤버 미적재)·②사이즈(포맥스/폼보드 면적 siz + nonspec 범위 적재)·가격 트랙(real_price 면적단가×number4 배수). **★전부 그릇 있고 미적재 = data. PO vessel-gap = 0(기재마운팅/자립구조 #18 부결·distinct 신축 0). ★PO-2 거치대 독립 SKU 다중 귀속(directive 핵심) = PASS not data-gap/vessel-gap 라이브 확정(`t_prd_product_addons.tmpl_cd` add-on→독립 SKU 참조·PRD_000016 5 SKU 다중 귀속 실데이터·옵션값 복제 아닌 정규화·BN 거치대 D-1·PD 다리/받침 동형). ★PO-4 면적 연속차원 = PASS(nonspec_width/height_min/max/incr + t_siz work/cut 2치수·이산 siz_cd 아님). ★타일링(분할출력) = data-gap(공정#2 배치 파라미터 미적재·FS-1 동형·타일링≠판걸이수)·real_price number4 면적계수 실계산 = data-gap(huni-widget 가격역공학·unobserved). 후니 KB 부속물#8/면적축/라미 공정 1급 모델링·ST 형상 G-SK-2와 정반대.**
