...
## 1.5 ✅GREEN 36 적재본 분리 (사용자 확정 2 · 08_load-green/)

이번 적재 대상 = **✅정상등록가능 36건만**. 🟡197·❌11은 본 명세 유지·**적재 제외**.
- 적재본 = `08_load-green/`(`product-cat-green.csv`·`apply-green.sql`·`backup-green.sql`·`dryrun-green.sql`·`manifest-green.md`).
- ✅36 본체 36행(별칭 0 — 18 별칭은 전부 🟡 본체 소속). 가변깊이(사용자 확정 1) 11행 L2 재지정.
- 라이브 DRY-RUN(롤백): DELETE 26(del='Y' orphan 정리)·UPDATE 4(대표 강등)·UPSERT 36·제약/FK/main위반 0.
- 상세 = `manifest-green.md`. 실 COMMIT은 dbm-validator GO 후 별도 실행자.
