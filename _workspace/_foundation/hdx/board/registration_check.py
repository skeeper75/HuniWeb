...
SHEETS = {
    ...
    # ── 형식가격 6시트(전 상품시트 확장) ──
    "디지털인쇄":             dict(pid=1, nm=3, opt_price=[(37, 38)]),          # 추가상품+추가가격(봉투류=addon 원천)
    "스티커":                 dict(pid=1, nm=3, opt_price=[(30, 31)]),          # 추가상품+추가가격
    "책자":                   dict(pid=1, nm=3, opt_price=[]),                  # 격자/제본옵션 가격·인라인 유료열 없음
    "포토북(가격포함)":        dict(pid=1, nm=3, opt_price=[]),                  # 기본+추가페이지 격자(§26 축B)
    "문구(가격포함)":          dict(pid=1, nm=3, opt_price=[]),                  # 격자·제본옵션 base price
    "상품악세사리(가격포함)":  dict(pid=1, nm=3, opt_price=[]),                  # 정가표=addon 템플릿 원천(타 상품이 참조)
}
...
# ── 조인 다리: prd_nm 브리지(스텝1b·신뢰도 등급) — 인라인 nm2cd 대체 ─────
def bridge_join():
    """prd_nm → {prd_cd, tier, needs_human, cardinality}. 잘못된 매칭=엉뚱한 가격=no-match보다 나쁨
    → EXACT(needs_human=FALSE)만 자동 진단, 나머지(AMBIGUOUS/UNMATCHED/…)는 needs_human 플래그로 표기.
    브리지 부재 시 빈 dict(호출측이 인라인 nm2cd 로 graceful 폴백)."""
    m = {}
    if not BRIDGE.exists():
        return m
    for r in csv.DictReader(open(BRIDGE, encoding="utf-8")):
        nm = r.get("excel_prd_nm", "")
        cds = [c.strip() for c in (r.get("matched_prd_cd") or "").split(";") if c.strip()]
        m[nm] = {"prd_cd": (cds[0] if len(cds) == 1 else (cds[0] if cds else None)),
                 "tier": r.get("tier", ""), "needs_human": (r.get("needs_human") == "TRUE"),
                 "cardinality": r.get("cardinality", "")}
    return m


def conf(gaps: str) -> str:
    """신뢰도: 끊김/addon삭제=HIGH(옵션 실재·경로0=키링류 저청구) / 그 외=REVIEW(이름차이·미연결 가능)."""
    return "HIGH" if ("끊김" in gaps or "addon 삭제" in gaps) else "REVIEW"


# ── 상품별 대조 ─────────────────────────────────────────────────────────
def check(join=None):
    """엑셀 의도 ↔ DB 등록 대조. 반환 = 레코드 dict 리스트.
    join = {prd_nm: {prd_cd, tier, needs_human, …}}(브리지). None → bridge_join() 로드(폴백 nm2cd).

    [HARD] 브리지 AMBIGUOUS/UNMATCHED/needs_human 상품은 도달성 결론 금지 — needs_human 레코드로만 표기.
    """
    ...
    for sheet, cfg in SHEETS.items():
        ...
        for nm, exp in sorted(intent.items()):
            ...
            paid_disp = "; ".join(f"{o}({d})" for o, d, _ in exp["paid"])

            # ── 조인 해소: 브리지 우선(신뢰도 등급), 폴백 인라인 nm2cd ──
            bj = join.get(nm)
            if bj is not None:
                prd, tier, nh = bj["prd_cd"], bj["tier"], bj["needs_human"]
            else:
                prd, tier, nh = nm2cd.get(nm), "NO_BRIDGE", (nm2cd.get(nm) is None)

            # (A) 조인 불확실(AMBIGUOUS/UNMATCHED/미등록) → 자동 결론 금지·needs_human 표기
            if nh or prd is None:
                recs.append(dict(
                    sheet=sheet, prd_nm=nm, prd_cd=prd, tier=tier, needs_human=True,
                    status="⏳ 조인 불확실(진단 보류)",
                    gaps=[f"[조인 불확실:{tier}] 유료옵션 {paid_disp} — prd_cd 매핑 확정 전 도달성 진단 보류"]))
                continue

            # (B) EXACT 조인 → 도달성 로직(verbatim: 옵션 opt_cd·option_items·addon 3경로)
            ...
            if gaps:
                recs.append(dict(
                    sheet=sheet, prd_nm=nm, prd_cd=prd, tier=tier, needs_human=False,
                    status="🔴 저청구/누락", gaps=gaps))
    wb.close()
    return recs
...
def main():
    ...
    with open(OUT, "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["시트", "상품명", "prd_cd", "조인등급", "needs_human", "신뢰도", "상태", "갭"])
        for r in recs:
            gaps = " | ".join(r["gaps"])
            c = "NEEDS_HUMAN" if r["needs_human"] else conf(gaps)
            w.writerow([r["sheet"], r["prd_nm"], r["prd_cd"] or "", r["tier"],
                        "TRUE" if r["needs_human"] else "FALSE", c, r["status"], gaps])
    by_sheet = {}
    for r in recs:
        by_sheet.setdefault(r["sheet"], []).append(r)
    n_high = sum(1 for r in recs if (not r["needs_human"]) and conf(" | ".join(r["gaps"])) == "HIGH")
    ...
    for sheet, items in by_sheet.items():
        ...
        for r in items:
            gaps = " | ".join(r["gaps"])
            c = "NEEDS_HUMAN" if r["needs_human"] else conf(gaps)
            print(f"  [{c}] {r['status']}  {r['prd_nm']} ({r['prd_cd'] or '-'}·{r['tier']})")
            for g in r["gaps"]:
                print(f"       {g}")
    print("\n" + "=" * 78)
    print(f"총 후보 {len(recs)}건(상품×시트) · HIGH(가격경로 끊김·옵션 실재)={n_high}건 · "
          f"REVIEW={n_review}건 · NEEDS_HUMAN(조인 불확실)={n_human}건")
    print(f"CSV → {OUT}")
    print("─" * 78)
    print("조인: prd_nm 브리지(스텝1b·신뢰도 등급). EXACT(needs_human=FALSE)만 도달성 진단.")
    print(" HIGH   = 옵션/addon 실재하나 가격 도달 0 → 키링류 저청구 강신호(끊김/addon 삭제).")
    print(" REVIEW = 옵션·addon 모두 없음 → ① 진짜 미연결 ② 이름차이 오탐 ③ 별도상품.")
    print(" NEEDS_HUMAN = 브리지 AMBIGUOUS/UNMATCHED — prd_cd 매핑 확정 전 자동 결론 금지.")
    print("커버: 전 상품시트(인라인 유료 5시트 + 형식가격 6시트). 형식가격 격자=§26 축B(PriceGridDx) 소관.")
