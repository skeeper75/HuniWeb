...
**변경이력:** 최신: 2026-07-02(후속) **088 실무진 답변 반영 — 4항 확정·Q2 CONFLICT 재질문** — 권위 0702 신규 엑셀 수신·대조: 제본=싸바리(수량 단일축 1부30,000→100부9,000→1000부7,000)·두께/링 무차등·면지 포함 확정. ★라이브 `COMP_BIND_SSABARI`(PROC_000098) 6밴드 권위 verbatim 일치·formula_components 바인딩 0=고아→088 배선만 남음(082 동형 shape·member 현행 유지·PROC_000098 단독 선언=옵션오염 가드). ★Q2 "레더링바인더 A4 소재+인쇄비" 행=0702 전시트+라이브 단가/자재 3중 확인 **부재 CONFLICT**(2안 돈크리티컬: 커버소재 7,000 vs 레더아트 19,000·100부 최대 1.75M 차)→재질문서 `03_design/088-Q2-재질문-260702.md`·재설계 준비본 `03_design/088-redesign-260702/`(apply.sql 미생성·값 확정 후 게이트→승인→적재). 직전: 2026-07-02 **셋트상품 19개 가격레벨 전수 진단(18 PRICE≠0·088만 결함)→088 구성 COMMIT(0→796,900)→재점검·실무진 질문서** — 라이브 simulate-set 전수(독립 재현): 18/19 GO·094/097/100 오진단정정(옵션차원 set_selections 채우면 정상 450k/135k/1.5M)·문구9 all-in 정상·088만 진짜 결함. 088=권위 '(보류중)' 빈 레더 D링바인더(내지없음)→077 COVERBIND 동형 임시 COMMIT(use_yn N→Y·1부34,100/100부796,900)·★재점검: 레더프리미엄=계산공식집 커버분해(표지인쇄+코팅+용지+제본)이지 +3,900 정액 아님·도메인+국내외경쟁사 리서치=링두께는 수용량제약(가격축 아님·3자수렴)·싸바리(하드케이스)≠링바인더(D링)·상품마스터/가격표 전수=D링 제작비 없음(보류중 정당)·표지레더인쇄=COMP_POSTER_CANVAS_FABRIC(레더아트프린트 611×374→19,000 재사용가능)·레더프리미엄 참고 다이어리+3,000. **실무진 질문서 5항 산출**(쉬운 인쇄용어·D링제작비/표지레더인쇄/D링두께/면지유무료/O·D링). 신규 C트랙=coat_side_cnt 셋트경로 드롭(068/069/070 표지코팅 저평가·개발팀). 다음=실무진 답(D링제작비) 후 088 커버분해 정확모델 재설계 → `_workspace/huni-set-product/HANDOFF.md`·[[set-product-full-diagnosis-260702]]. 직전: 2026-07-01 셋트 동작화 5건 COMMIT(068~070·077·082)+[[leather-hardcover-077-live-commit-260701]]
