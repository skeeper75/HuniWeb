...
- **디자인캘린더 5상품 별도 prd_cd 신규 등록**(Q-DC-0, master INSERT=DB 쓰기+후니 출시 승인) = `09_load/design-calendar/`(placeholder PRD_NEW_DCAL_*, 실번호 후니 부여). **라이브 정정**: 디자인캘린더는 calendar(108~112)와 다른 별도 상품(후니 (N,Y) 패턴=포토북[디자인명]). 추출본 ref-products.csv의 113~117은 가짜(라이브 부재). calendar editor_yn UPDATE는 철회됨(_deferred/WITHDRAWN). Q-DC-1=디자인엽서(L1 디자인보유● 비표시) 멤버 확정.
