...
from crossbrand_query import (  # 후니 견적·basis·게이트 판정 재사용
    huni_repr_price, family_verdict, WOW_DIR, DATA)
...
def assign(fam, huni_can, huni_sup, status, wow_sup, verdict_kind):
    ...
    if status == "below_min":
        return "후니(기본)·ℹ", "후니 최소수량 미달(비교 수량↑ 필요·엔진 정상) → 유효 수량 재견적"
    if status != "ok" or huni_sup is None:
        return "후니(기본)·⚠", "후니 견적 0/불가(대표사양·수량 재확인) → 오라클 대조 후 재판정"
    # 둘 다 신뢰 가능 → 후니 기본, 와우 원가 유의미 저가면 검토 신호
    if wow_sup and wow_sup < huni_sup * 0.7:
        gap = round((1 - wow_sup / huni_sup) * 100)
        if huni_sup > wow_sup * ARTIFACT_RATIO:  # 극단차 = 사양 불일치 의심
            return "후니(기본)·⚠사양", f"후니≫와우 {gap}% — 사양 불일치 의심(후니 대표상품/사이즈 다름·표의 후니 사양 확인)"
        return "후니(기본)+와우 검토", f"와우 원가 {gap}% 저(방향 신호·사양/수량 정합 확인 전) → 마진 관점 와우 검토(최종 지니)"
    return "후니(기본)", "후니 공동사업 기본·원가차 유의미하지 않음(사양/수량 정합 확인 전)"
...
def main():
    ...
    for fam, r in recipes.items():
        ...
        hp = huni_repr_price(huni_list[0]["prd_cd"], wqty) if (huni_can and wqty) else {"price": None, "siz_nm": "", "status": "n/a"}
        hsup, hspec, hstat = hp["price"], hp["siz_nm"], hp["status"]
        배정, 사유 = assign(fam, huni_can, hsup, hstat, wsup, vkind)
        rows.append({
            "fam": sfa["family"], "qty": wqty, "unit": r["product"].get("unit", "매"),
            "hname": hname, "hsup": hsup, "hspec": hspec, "hstat": hstat,
    ...
    def fmt(v, status):
        if status == "below_min":
            return "최소미달(0원)ℹ"
        if v is None or status in ("zero", "error", "no_product", "n/a"):
            return "견적불가"
        return f"{v:,}원"

    md = ["# 카페 상품군 인쇄소 배정 검토표 (v1.1 · 2026-07-05)",
          "",
          ...
          "|---|---|---|---|---|---|---|---|---|---|"]
    print("=" * 100)
    print("카페 상품군 인쇄소 배정 검토표 (v1.1) — 지니 내부 소싱 도구 · 원가=부가세 전 공급가 · 후니 사양=기본값")
    print("=" * 100)
    for r in rows:
        huni_cap = "✓" if not r["hname"].startswith("(") else "✗"
        wow_cap = "⚠재앵커" if r["vkind"] == "NO_GO" else "✓"
        h = fmt(r["hsup"], r["hstat"])
        w = fmt(r["wsup"], "ok") + (f' {r["wderiv"]}' if r["wderiv"] else "")
        hspec = r["hspec"] or "-"
        md.append(f'| {r["fam"]} | {r["qty"]}{r["unit"]} | {huni_cap}/{wow_cap} | {h} | {hspec} | {w} | {r["wspec"]} | 수집예정 | **{r["배정"]}** | {r["사유"]} |')
        print(f'\n▶ [{r["fam"]}] {r["qty"]}{r["unit"]}  능력 후니{huni_cap}/와우{wow_cap}')
        print(f'    후니 {r["hname"]:<16} {h:>16}   [{hspec}]')
        print(f'    와우 {r["wname"]:<16} {w:>16}   [{r["wspec"]}]')
