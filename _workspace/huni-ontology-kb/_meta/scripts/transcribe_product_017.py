...
# -*- coding: utf-8 -*-
"""
전사 스크립트 (D-9·§4·[HARD] LLM 손전사 금지) — product-017-coated-postcard 전용.

기존 transcribe_product_016.py(형제 엽서)를 수정하지 않고, 코팅엽서(PRD_000017)의
상품별 차원·BOM·연결 수치를 라이브 스냅샷에서 결정론적으로 전사한다.
- 사이즈(+사이즈별 수량규칙) · 자재 BOM(활성) · 공정 라우트(mand/opt) · 인쇄옵션(도수)
- 판형 · 추가상품(템플릿) · 옵션그룹(+item→ref 다형참조) · 제약 · 가격공식 바인딩
노드 본문의 표는 이 스크립트가 뽑은 것을 그대로 옮긴 것(사람 손전사 아님).

사용:  python3 transcribe_product_017.py           # stdout markdown 블록
...
# 공유 axis(03_kb/axis/*)에 민팅된 노드 화이트리스트(017 관련 축만)
MINTED_PROC = {"PROC_000004", "PROC_000014", "PROC_000015", "PROC_000027", "PROC_000028"}
...
def marker(tbl):
    return f"<!-- transcribed-by: _meta/scripts/transcribe_product_017.py from {SNAP_ID} {tbl} PRD_000017 @ {STAMP} -->"
...
def materials():
    ...
    minted_mat = {"MAT_000074", "MAT_000081", "MAT_000082", "MAT_000091", "MAT_000092",
                  "MAT_000101", "MAT_000109"}
    for r in rows:
        c = r["mat_cd"]
        out.append(f'| {c} | {mm.get(c, {}).get("mat_nm", "?")} | {blank(r.get("usage_cd"))} | '
                   f'{"O" if c in minted_mat else "X(미민팅)"} |')
    return "\n".join(out)


def print_options():
    pom = {r["print_opt_cd"]: r for r in rd("t_prt_print_options.csv")} \
        if os.path.exists(os.path.join(SNAP, "t_prt_print_options.csv")) else {}
    ...
    out = [marker("t_prd_product_print_options+t_prt_print_options"),
           ...
           "|---|---|---|---|---|---|"]
    for r in rows:
        out.append(f'| {blank(r.get("opt_id"))} | {blank(r.get("print_side"))} | '
                   f'{blank(r.get("print_opt_cd"))} | {blank(r.get("front_colrcnt_cd"))} | '
                   f'{blank(r.get("back_colrcnt_cd"))} | {blank(r.get("dflt_yn"))} |')
    return "\n".join(out)
...
def addons():
    ...
    out = [marker("t_prd_product_addons"),
           f"추가상품 행수 {len([r for r in rows if r.get('del_yn') != 'Y'])} (없으면 has_addon 없음)"]
    if rows:
        tm = {r["tmpl_cd"]: r for r in rd("t_prd_templates.csv")}
        out += ["", "| disp | tmpl_cd | 템플릿명 | base_prd_cd |", "|---|---|---|---|"]
        for r in sorted(rows, key=lambda r: (r.get("disp_seq") or "", r["tmpl_cd"])):
            t = tm.get(r["tmpl_cd"], {})
...
def optgroups():
    grps = [r for r in by_prd("t_prd_product_option_groups.csv")]
    grps.sort(key=lambda r: (r.get("disp_seq") or "", r["opt_grp_cd"]))
    opts = by_prd("t_prd_product_options.csv")
    items = by_prd("t_prd_product_option_items.csv")
    # opt_cd -> items(ref)
    items_by_opt = {}
    for it in items:
        items_by_opt.setdefault(it.get("opt_cd"), []).append(it)
    out = [marker("t_prd_product_option_groups+t_prd_product_options+t_prd_product_option_items"),
           "| opt_grp_cd | 그룹명 | sel_typ | use | del | option(opt_cd) | ref_dim | ref_key1 |",
           "|---|---|---|---|---|---|---|---|"]
    for g in grps:
        gc = g["opt_grp_cd"]
        gopts = [o for o in opts if o.get("opt_grp_cd") == gc]
        gopts.sort(key=lambda o: (o.get("disp_seq") or "", o.get("opt_cd") or ""))
        if not gopts:
            out.append(f'| {gc} | {blank(g.get("opt_grp_nm"))} | {blank(g.get("sel_typ_cd"))} | '
                       f'{blank(g.get("use_yn"))} | {blank(g.get("del_yn"))} | — | — | — |')
        for o in gopts:
            its = [it for it in items_by_opt.get(o.get("opt_cd"), []) if it.get("del_yn") != "Y"]
            if not its:
                out.append(f'| {gc} | {blank(g.get("opt_grp_nm"))} | {blank(g.get("sel_typ_cd"))} | '
                           f'{blank(g.get("use_yn"))} | {blank(g.get("del_yn"))} | '
                           f'{o.get("opt_cd")} ({blank(o.get("opt_nm"))}) | — | — (센티넬/미참조) |')
            for it in its:
                out.append(f'| {gc} | {blank(g.get("opt_grp_nm"))} | {blank(g.get("sel_typ_cd"))} | '
                           f'{blank(g.get("use_yn"))} | {blank(g.get("del_yn"))} | '
                           f'{o.get("opt_cd")} ({blank(o.get("opt_nm"))}) | {blank(it.get("ref_dim_cd"))} | '
                           f'{blank(it.get("ref_key1"))} |')
    return "\n".join(out)
...
def constraints():
    ...
    out = [marker("t_prd_product_constraints"),
           f"제약 행수 {len(rows)} (없으면 constraint 노드 없음·GAP 아님)"]
    if rows:
...
def categories():
    ...
    rows = [r for r in by_prd("t_prd_product_categories.csv")]
    out = [marker("t_prd_product_categories+t_cat_categories"),
           "| cat_cd | 분류명 | main_cat_yn |", "|---|---|---|"]
    for r in rows:
        c = r["cat_cd"]
        out.append(f'| {c} | {cm.get(c, {}).get("cat_nm", "?")} | {blank(r.get("main_cat_yn"))} |')
    return "\n".join(out)
...
if __name__ == "__main__":
    for title, fn in [("사이즈+수량규칙", sizes), ("수량 규칙(상품/묶음)", qty),
                      ...
                      ("추가상품(템플릿)", addons), ("옵션그룹(item→ref)", optgroups),
