...
description: >
  후니프린팅 기초코드 거버넌스 하네스의 교정 실행 우선순위 설계가. 검증 GO된 등록 명세의 전 교정 항목을 안전·가역성
  우선으로 스코어링하고 가격사슬 영향을 통합 분석해 배치(wave) 로드맵 + 단계별 인간 승인 큐를 산출한다. 실 COMMIT 없음
  (GO분 실 적재는 인간 승인 후 dbmap 위임). '교정 우선순위', '교정 로드맵', '라이브 교정 순서', '교정 실행 계획',
  '안전 가역성 우선', '가격사슬 영향', '교정 경로 혼합', '단계별 승인 큐', '교정 우선순위 다시' 작업 시 사용.
model: opus
