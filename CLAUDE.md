...
| 2026-06-10 | **schema-design-intent-map 작성 — 1회성 권위 스키마 설계의도 지도(HANDOFF #1 해소)** — round-9 "기계적 매핑 금지" directive 해소: round-11(엑셀 측 11시트)을 t_* 측 설계의도와 결합해 매핑을 도출로 전환. `expert-backend` 위임+메인 QA로 `00_schema/schema-design-intent-map.md`(493줄·4부): (1) t_* 도메인 클러스터 ERD(Mermaid 34엔티티·6클러스터+polymorphic ref_dim·트리거·가격사슬 4단) (2) 각 핵심 t_* WHY(담는것/anti-pattern·라이브 행수) (3) 인쇄 구성요소 11종→t_* 귀속+삼중바인딩(UI componentType / 생산 BOM·MES / 가격엔진) decision table+가격공식 4종 (4) 라이브 진단+오모델 OM-1~7(카드봉투 색=siz·굿즈파우치 size→option·실사 입력UX≠가격격자·아크릴 두께소실·UV/별색·CPQ 미적재·GAP-PARAM 각 근거+해소경로)+횡단컨펌(확정5+미회신5). 매핑=도출(추정0·기계적매핑0). 다음=미회신 5시트 회신→각 시트 L1/L2 매핑 확정→적재본→인간 승인 COMMIT. | _workspace/huni-dbmap/00_schema/schema-design-intent-map.md·HANDOFF·메모리1갱신·CLAUDE.md, 커밋 본커밋 | 사용자("schema-design-intent-map 작성 진행하자") |
