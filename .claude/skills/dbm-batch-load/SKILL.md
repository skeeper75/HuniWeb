...
name: dbm-batch-load
description: >
  후니프린팅 270 상품을 동형 클래스(동일 옵션구성×동일 가격계산방식=frm_cd 집합) 단위로 묶어 결정적
  스크립트로 배치 적재하는 방법론. 상태 분류(출시/미출시/신규예정)·선결 게이트(컬럼 완정성)·배치 멱등
  UPSERT·SQL 집계 전수 검증·백업은 위험 변경만. 트리거: 배치 적재, 배치 처리, 동형 상품 묶음 적재, 대량 적재,
  270 상품 일괄, 스크립트 배치, 효율 적재, 집계 검증, 배치 다시. 단건 적재본 조립은 dbm-load-execution,
  한 시트 매핑 설계는 dbm-mapping/dbm-price-formula 담당.
---
...
> **도메인 전제 [HARD]**: 인쇄상품 = 출력소재+색+부속물+가공의 **레시피**. 맛간장처럼 **공유 comp/공식이 여러 상품에 재사용**되어 한 시트에 여러 공식이 혼재하고 한 공식을 여러 상품이 쓴다(`formula_components`가 frm_cd 단위인 이유). 여러 가격산정방식(규격가·합가매트릭스·고정·면적)은 결함이 아니라 **시스템을 처음 만드는 회사의 도메인 현실** — 실무진이 상품마스터·가격표로 고심해 정리한 결과다. 배치는 이를 평탄화·획일화하는 게 아니라 **안전·효율적으로 시스템화**한다. 비효율로 단정 금지. [[dbmap-print-domain-recipe-philosophy]]
...
2. **동형축**: **동일 옵션구성**(같은 option_groups/items 패턴) × **동일 가격계산방식**(상품에 바인딩된 가격공식 `frm_cd` **집합**). 같은 (옵션패턴, 가격클래스) = 한 동형 클래스 = 한 배치.
...
**가격 동형 = 공식(frm_cd) 기준 — "한 시트 1공식 vs 여러 공식" 자동 처리**(라이브 실측 2026-06-15):
- **한 시트에 여러 공식**이면 상품마다 frm_cd가 달라 **자동으로 다른 클래스로 분리**(디지털인쇄 = PRF_DGP_A~F + NAMECARD/PHOTOCARD 등 여러 클래스). **한 시트에 1공식**이면 1클래스 큰 배치(PRF_POSTER_FIXED 28상품·PRF_DGP_A 9). 시트는 분류 단위가 아니다.
- **`formula_components`는 `frm_cd` 기준(prd_cd 컬럼 없음)** → 같은 공식에 바인딩된 모든 상품은 **같은 comp 세트 공유**. "같은 공식 다른 comp" 적재 분기는 없음 — comp 차이는 런타임 option 선택(option_items)이 결정. 그래서 frm_cd가 진짜 가격 동형 단위.
- **상품 다중 공식(1:N)**: 라이브 현재 0건(상품당 1공식)이나 스키마 허용 → `classify.py`는 frm_cd **집합** 시그니처(`string_agg`)로 묶어 다중 공식도 안전 처리(중복 행 방지).
...
- **가격 클래스 배치 = 공식(frm_cd) = 5테이블 한 묶음**: `price_formulas`(공식 정의 1행) → `formula_components`(comp 배선 N행·frm_cd 기준) → `price_components`(comp 정의) → `component_prices`(단가행·comp×차원) → `product_price_formulas`(상품 바인딩 N행). 한 공식 클래스는 이 5테이블 행을 FK 위상 순서로 함께 멱등 UPSERT. 단가행만 채우는 GAP(종이비 등)은 `component_prices` 1테이블만(공식·배선 기존 재사용).
