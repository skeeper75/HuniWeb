...
class RedWidgetSDK {
  ...
  constructor(clientKeyValue) {
    if (!ALLOWED_CLIENT_KEYS.includes(clientKeyValue)) throw new Error("존재하지 않는 사용자입니다");
  ...
  init(initConfig, callbacks) {
    ...
    } = initConfig, targetElement = document.querySelector(targetSelector);
    ...
    if (vueApp.use(createVueQueryPlugin()), vueApp.use(createPiniaPlugin), vueApp.use(createDomPurifyPlugin), useConfigStore().setLocale(localeCode), vueApp.provide("deviceType", deviceTypeValue), vueApp.provide("productCode", {
        pdtCode: productCode,
        pttCode: patternCode
      }), vueApp.provide("callbacks", callbacks), vueApp.provide("member", memberInfo), isCommonProduct) {
    ...
    return appendStylesheetToShadow(shadowRoot, "https://d2vgy67dgpwzce.cloudfront.net/RedWidgetSDK/prod/widget.css"), vueApp.mount(rootDiv), isCommonProduct ? new CommonWidgetInstance(initConfig) : new AccWidgetInstance(initConfig)
  }
}
...
const dynamicImportHelper = (importMap, modulePath, expectedDepth) => {
    ...
    return moduleEntry ? typeof moduleEntry == "function" ? moduleEntry() : Promise.resolve(moduleEntry) : new Promise((resolve, reject) => {
      (typeof queueMicrotask == "function" ? queueMicrotask : setTimeout)(reject.bind(null, new Error("Unknown variable dynamic import: " + modulePath + (modulePath.split("/").length !== expectedDepth ? ". Note that variables only represent file names one level deep." : ""))))
    })
  };
 ...
 */
const OptionRow = withScopeId(defineComponent({
  __name: "OptionRow",
  props: {
    title: {},
    extra: {},
  ...
  setup(props) {
    return (instance, cache) => (openBlock(), createBlock("fieldset", {
      class: normalizeClass(["option-row", instance.rowClass]),
      style: normalizeStyle(instance.priority ? {
        order: instance.priority
      } : null)
    }, [instance.title ? (openBlock(), createBlock("legend", {
      key: 0,
      class: normalizeClass(["title", {
        underline: instance.underline
      }])
    }, [createElement("span", OPTION_ROW_SUBJECT_CLASS, toDisplayString(unref(translate)(instance.title)), 1), instance.extra ? (openBlock(), createBlock("button", {
      key: 0,
      type: "button",
      class: normalizeClass(["extra-btn", instance.extra.style]),
      onClick: cache[0] || (cache[0] = (...args) => instance.extra.callback && instance.extra.callback(...args))
    }, toDisplayString(unref(translate)(instance.extra.name)), 3)) : createComment("", !0)], 2)) : createComment("", !0), renderSlot(instance.$slots, "default", {}, void 0, !0)], 6))
  }
}), [
 ...
 */
const ImageButton = withScopeId(defineComponent({
  __name: "ImageButton",
  props: {
    data: {},
    active: {
  ...
  setup(props, {
    ...
    const componentProps = props,
      ...
      /** 아이콘 이미지 경로 계산 */
      iconImageSrc = computed(() => componentProps.data.forcedImg ? componentProps.data.imgPath : `${ASSETS_IMAGE_CDN_URL}/ordericon/${componentProps.data.imgPath}.png`),

      /** 대체(서브) 이미지 경로 */
      fallbackImageSrc = computed(() => `${ASSETS_IMAGE_CDN_URL}/ordericon/${componentProps.data.subImgPath}.png`);
    ...
    function handleImageError(event) {
      ...
      imgElement && (imgElement.src = fallbackImageSrc.value, imgElement.onerror = () => {
        imgElement.src = `${ASSETS_IMAGE_CDN_URL}/ordericon/order_icon1-3.png`
      })
    }

    return (instance, cache) => withDirectives((openBlock(), createBlock("div", {
      onClick: handleClick,
      class: normalizeClass(["icon-checkbox", {
        disabled: instance.disabledStyling && instance.disabled
      }])
    }, [createElement("div", {
      class: normalizeClass(["icon-label", {
        active: instance.active
      }])
    }, [createElement("div", IMAGE_BUTTON_ICON_WRAP_CLASS, [createElement("img", {
      src: iconImageSrc.value,
      alt: instance.data.name,
      onError: handleImageError
    }, null, 40, IMAGE_BUTTON_IMG_ATTRS)])], 2), createElement("span", IMAGE_BUTTON_NAME_CLASS, toDisplayString(unref(translate)(instance.data.name)), 1), (!unref(deviceType) || unref(deviceType) === "pc") && instance.tip ? (openBlock(), createBlock("div", IMAGE_BUTTON_PC_TIP_CLASS, [(openBlock(), createBlock("img", {
      src: instance.tip.IMG_URL,
      alt: instance.tip.IMG_ALT,
      key: instance.tip.IDX,
      "data-idx": instance.tip.IDX
    }, null, 8, IMAGE_BUTTON_TIP_IMG_ATTRS))])) : createComment("", !0), renderSlot(instance.$slots, "input", {}, void 0, !0)], 2)), [
...
const PageDirectionComponent = defineComponent({
  ...
  setup(props, {
    ...
    watch(() => selectedDirection.value, directionValue => {
      exteriorStore.isAfterEdit() && resetDirectionCallback(), emit("update", {
    ...
    const autoDetectedDirection = computed(() => {
      ...
      const {
        DIV_NM: sizeName,
        cutSize: cutSizeData
      } = componentProps.relatedData.sizeInfo;
    ...
    return watch(() => autoDetectedDirection.value, detectedValue => {
      ...
      default: withCtx(() => [createElement("div", PAGE_DIRECTION_FLEX_ROW, [(openBlock(), createBlock(Fragment, null, renderList(directionOptions, optionItem => createVNode(ImageButton, {
        key: optionItem.value,
        data: optionItem,
        active: selectedDirection.value === optionItem.value,
        disabled: autoDetectedDirection.value && autoDetectedDirection.value !== optionItem.value,
        "disabled-styling": !!autoDetectedDirection.value,
        onSelect: cache[0] || (cache[0] = selectedItem => selectedDirection.value = selectedItem.value)
...
const translate = (key, params) => {
  ...
  } = storeToRefs(useConfigStore()), translatedText = (currentLocale.value === "ko" ? TRANSLATIONS_KO : TRANSLATIONS_EN)[key] || key;
  if (!params) return translatedText;
  let result = translatedText;
  return Object.entries(params).forEach(([paramKey, paramValue]) => {
...
const useExteriorStore = defineStore("exterior", () => {
  ...
  return watch(() => editorData, data => {
    ...
    isAfterEdit: key => editorData[key || "default"] ? uploadType[key || "default"] === "editor" && editorData[key || "default"].editingYn === "Y" : !1,
...
const useOrderStore = defineStore("order", () => {
  ...
  function setOrderData(data, summary) {
    orderData.value = data, callbacks?.onOptionChange && callbacks.onOptionChange({
      type: "COMMON",
      data: data,
      summary: summary
    })
...
const RESET_MATERIAL_AFTER_EDIT_CODES = new Set(["GSPNJLY", "GSPNBAL", "GSPNCLP", "GSPNDFT", "GSPNFLT", "GSTTCRK", "GSCAEPB", "GSCAGBP", "GSCAGBM", "GSCAGBR", "GSCAGBH", "GSCATPP", "GSCATPG", "GSCATCP", "GSCACDP", "GSCAPHN", "GSWLMAG", "GSCATIN", "GSKYHOT", "GSHDMGT", "GSTGMIC", "GSCAPDF", "GSCACAP", "PHFRDIA", "GSSKSHH", "GSFBSTK"]);
...
/** 달력 제품 코드 */
const CALENDAR_PRODUCT_CODES = new Set(["TPCLECO", "TPCLWLB", "PRCLSTD", "PRCLHOL", "PRCLWAL", "TPCLSTD", "TPCLHOL", "TPCLWAL"]);
...
class CommonWidgetInstance {
  ...
  getSummary() {
    ...
    const clothesSummary = clothesData ? {
      ...
      ...clothesData.sizeInfo ? {
        size: {
          label: "사이즈",
          value: clothesData.sizeInfo.map(item => `${item.size.COD_NME}(${item.quantity}장)`).join(", ")
        }
      } : {},
      material: {
        label: "제품명",
    ...
    /* 책자 제품 요약 */
    const bookSummary = itemGroup === "book2025_item" ? {
      innerInfo: {
        label: "내지 정보",
        children: [{
          label: "내지 인쇄 옵션",
          ...
          label: "내지 용지",
          value: orderData?.inner_meterialInfo.PTT_NM + `${orderData?.inner_meterialInfo.WGT_CD?`${+orderData?.inner_meterialInfo.WGT_CD}g`:""}`
        }, {
          label: "내지 장수",
      ...
      coverInfo: {
        ...
        children: [{
          label: "표지 인쇄 옵션",
          value: orderData?.dosuInfo.COD_NME + (orderData?.dosuInfo.BNC_GB === "BNC_COL" ? "컬러" : "흑백")
        }, {
          label: "표지 용지",
    ...
    const widgetSkinInfo = productBaseInfo?.product_option.option.skinInfo,
      isCustomSize = orderData?.sizeInfo.DIV_NM === "사이즈직접입력" || orderData?.sizeInfo.DIV_NM === "Input Size",
      sizeLabel = orderData?.sizeInfo.DIV_SEQ ? isCustomSize ? `${orderData.sizeInfo.cutSize.width}mm X ${orderData.sizeInfo.cutSize.height}mm` : orderData?.sizeInfo.DIV_NM : null,
      displayOrdCnt = widgetSkinInfo?.quantityGroup.view_yn === "Y" ? orderData?.quantityInfo.ordCnt : null,
      displayPrnCnt = widgetSkinInfo?.quantityGroup.view_yn === "Y" ? orderData?.quantityInfo.prnCnt : null;

    /* 후가공 요약 (그룹별 정리) */
    const postPcsGrouped = orderData?.pcsInfo.reduce((grouped, pcsItem) => {
        const {
          VIEW_YN: viewYn,
          PCS_GRP_NM: groupName,
          selectedOptions: selectedOpts
        } = pcsItem;
        if (viewYn === "Y" && groupName && selectedOpts[0].PCS_DTL_NM) {
          const existing = grouped[groupName];
          existing ? existing.push(selectedOpts[0].PCS_DTL_NM) : grouped[groupName] = [selectedOpts[0].PCS_DTL_NM]
        }
        return grouped
      }, {}),
      postPcsSummaryItems = postPcsGrouped ? Object.entries(postPcsGrouped).map(([label, values]) => ({
        label: label,
        value: values.join(", ")
      })) : null;
    ...
    return {
      ...
      ...widgetSkinInfo?.sizeSelect.view_yn === "Y" && sizeLabel && orderData?.dosuInfo.PRN_CLR_CNT !== 0 ? {
        size: {
          label: "사이즈",
          value: sizeLabel
        }
  ...
  setEditorData(editorResultData) {
    ...
    const enrichedData = {
      ...
      processedData = enrichedData.type === "KOI" ? processKoiEditorData(enrichedData, productBaseInfo) : processRpEditorData(enrichedData);
    processedData ? this.editorStore.setEditorData(processedData) : console.error(`[RedWidgetSDK/ERROR] 에디터에서 온 데이터가 없습니다 > 받은 데이터: ${editorResultData}`)
  }
  ...
  canOrder() {
    try {
      ...
      /* 제품 주문 가능 상태 확인 */
      if (productBaseInfo?.product_option.option.order_yn === "N") throw new Error(translate("주문불가상태"));

      /* 사이즈 유효성 검증 */
      if (orderData?.validation && orderData.validation.length > 0) throw new Error(translate("주문불가-사이즈"));

      /* 가격 계산 결과 확인 */
      if (orderData?.priceCalc.result.retCode !== 200 || !orderData.priceCalc.result.result_sum.PRICE) throw new Error("주문불가-가격");
      ...
      if (itemGroup === "book2025_item") {
        for (const [uploadKey, uploadValue] of Object.entries(uploadTypeState)) {
          ...
          if (uploadValue === "editor" && !this.editorStore.isAfterEdit(uploadKey)) throw new Error(`[${sectionLabel}] ${translate("주문불가-에디터")}`);
          if (uploadValue === "pdf") {
            if (!fileUploadInfo) throw new Error(`[${sectionLabel}] ${translate("주문불가-파일")}`);
            if (uploadKey === "inner" && !fileUploadInfo[0]) throw new Error(`[${sectionLabel}] ${translate("주문불가-파일")}`);
            if (uploadKey === "default" && !fileUploadInfo[1]) throw new Error(`[${sectionLabel}] ${translate("주문불가-파일")}`)
          }
        }
        /* 내지/표지 파일명 중복 검사 */
        if (fileUploadInfo && fileUploadInfo[0] && fileUploadInfo[1] && fileUploadInfo[0].org_file_nm === fileUploadInfo[1].org_file_nm) throw new Error(translate("주문불가-파일명중복"));
        return {
          success: !0
        }
      }
      ...
      if (uploadTypeState.default === "pdf") {
        ...
        if (itemGroup === "clothes2025_item" && orderData.clothesSelectData.printType.COD === "PTP_SLK" && !orderData?.clothesSelectData.pantoneInfo) throw new Error(translate("주문불가-인쇄컬러미선택"))
      }

      /* 의류 제품: 인쇄 없음이면 통과 */
      if (itemGroup === "clothes2025_item" && !orderData?.clothesSelectData.printType.COD) return {
        success: !0
      };

      /* 에디터 모드: 편집 완료 확인 (인쇄 없음 제외) */
      if (uploadTypeState.default === "editor" && !this.editorStore.isAfterEdit() && orderData.dosuInfo.COD !== "SID_X") throw new Error(translate("주문불가-에디터"));
      ...
      return error instanceof Error && (errorMessage = error.message), {
        success: !1,
    ...
    try {
      ...
      if (apparelInfo) {
        ...
        const printAreaPcsList = tabData.PAGES.map(pageName => ({
            PCS_COD: "PDT_WRK",
            PCS_DTL_COD: apparelInfo.print_area.find(area => area.KOI_NME === pageName)?.COD || ""
          })),
        ...
        const updatedOrderInfo = [{
          ...
          updatedPcsInfo = [...previousPriceParams.PCS_INFO.filter(item => item.PCS_COD !== "DIR_MTR" && item.PCS_COD !== "PDT_WRK"), ...printAreaPcsList, materialPcsEntry],
          priceRequestPayload = {
            ...previousPriceParams,
        ...
        if (priceResult.errorMessage || !priceResult.result) throw new Error(priceResult.errorMessage);
        ...
        return {
          type: "PRICE",
          data: discountPrice !== mallPrice ? mallPrice + mallPriceVat : orgPrice !== discountPrice ? discountPrice + discountPriceVat : orgPrice + orgPriceVat
        }
      }
    } catch (error) {
      return console.error("[RedWidgetSDK/ERROR] 코이에디터 데이터 산정 실패 > ", error), {
...
class AccWidgetInstance {
  ...
  getSummary() {
    const productBaseInfo = this.getProductBaseInfo(),
      orderData = this.getOrderData();
    if (!orderData?.subMtrlInfo) return {
      errorMessage: translate("주문불가-옵션미선택")
    };
    const itemList = [];
    let totalQuantity = 0;
    for (const subMaterial of orderData.subMtrlInfo) itemList?.push({
    ...
    const priceSum = orderData.priceCalc.result.result_sum,
      ...
      totalAmount = hasMallDiscount ? priceSum.PRICE_MALL + priceSum.PRICE_MALL_VAT : hasDiscount ? priceSum.PRICE + priceSum.PRICE_VAT : priceSum.ORG_PRICE + priceSum.ORG_PRICE_VAT;
    return {
      product: {
        label: "제품명",
  ...
  canOrder() {
    try {
      if (this.getProductBaseInfo()?.product_option.option.order_yn === "N") throw new Error(translate("주문불가상태"));
      const orderData = this.getOrderData();
      if (!orderData?.subMtrlInfo) throw new Error(translate("주문불가-옵션미선택"));
      if (!orderData?.priceCalc.result.result_sum.PRICE) throw new Error(translate("주문불가-가격"));
      return {
        success: !0
      }
    } catch (error) {
...
function appendStylesheetToShadow(shadowRoot, cssUrl) {
  ...
  linkElement.rel = "stylesheet", linkElement.href = cssUrl, shadowRoot.appendChild(linkElement)
}
