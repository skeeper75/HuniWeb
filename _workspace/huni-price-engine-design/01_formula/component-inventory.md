...
## 스티커 절 (이산 siz_cd 단가형 + 세트 합가형 · 라이브 실측 2026-06-20) — formula-map-sticker.md 참조

### 스티커 라이브 실재 comp (4 comp·전건 바인딩·round-23 완주)

| comp_cd | comp_nm | 공식 | prc_typ | use_dims | 단가행 | 차원 분포 | 비고 |
|---------|---------|------|---------|----------|:--:|----------|------|
| **COMP_STK_PRINT** | 스티커 완제품가(소재·규격) | PRF_STK_FIXED | .01 단가형 | `["siz_cd","mat_cd","min_qty"]` | **2,694** | siz 19종·mat 7종·min_qty 36단 | B01~B04 + 058~064 전부 흡수(단일 comp)·가격표 verbatim |
| **COMP_STK_TATTOO** | 타투 완제품가(3장세트) | PRF_STK_TATTOO | **.02 합가형** | `["siz_cd","mat_cd","min_qty"]` | 1 | siz=060·mat=167·min_qty=3·4000 | 4000÷3 환산·min_qty=3 필수(NULL→ValueError) |
| **COMP_STK_PACK** | 스티커 완제품가 팩(54장1세트) | PRF_STK_PACK | **.02 합가형** | `["siz_cd","min_qty"]` | 1 | siz=068·min_qty=54·4000 | mat_cd 미사용·4000÷54 환산·.01→.02 교정 완료 |
| **COMP_GANGPAN_PRINT** | 합판도무송 완제품가(형상·소재별) | PRF_GANGPAN_FIXED | .01 단가형 | `["siz_cd","mat_cd","min_qty"]` | 370 | siz 37종·mat 2종(084/153)·min_qty 5단 | 별 시트(형상=siz_cd 고정사이즈)·18,000~189,900 |

### 스티커 차원 패턴 (재사용 후보·대조)

| 패턴 | 스티커 적용 | 차원 | 6종단 대조 |
|------|------------|------|-----------|
| **이산 siz_cd 단가형** | COMP_STK_PRINT·COMP_GANGPAN | `[siz_cd, mat_cd, min_qty]` | 면적매트릭스(아크릴/실사 siz_width/height)와 직교·문구 단가형과 부분 동형 |
| **세트 합가형(.02 ÷min_qty)** | COMP_STK_TATTOO·COMP_STK_PACK | `[siz_cd, (mat_cd), min_qty]` | 책자 세트와 달리 **단일 본체의 묶음단위**(부품 합산 아님)·디지털 명함 .02 합가형과 동형(÷min_qty) |
| **소재 7종 개별 단가행** | COMP_STK_PRINT mat 7종 | mat_cd | 가격표 그룹(유포/비코/미색 동가)도 소재별 개별 행(엔진 mat_cd 매칭·collapse 금지) |
| **형상=칼틀(가격직교)** | 058~063 형상 | (가격축 아님) | 같은 siz/mat 동일가·CPQ/siz 형상코드 별 관리·가격 무관 |

### 스티커 단가 소스 셀 (가격표 매핑·골든 라이브 재현)

| 블록 | 단가 소스 | 라이브 siz_cd | 골든(라이브 재현) |
|------|----------|---------------|-------------------|
| B01 반칼국4절 | 가격표 [수량]×[6규격]×[3소재그룹] | 059·060·518·519·170(A5)·520(A4반칼) | 124x186 유포 qty3 = **5,900**·qty1=6,000 / A4반칼 5,000 |
| B02/B03 낱장 | A4·B4·A3·B3·A2 | 172·515·174·514·197 | A4 유포 = 4,000(낱장)·A3 = 8,000·A2 = 16,000 |
| B04 대형 | 400x600 | 199 | 16,000(유포) |
| B05 타투 | 90x190 ÷3 | 060 + mat 167 | qty9 = 4000÷3×9 = **12,000** |
| B06 팩 | 75x110 ÷54 | 068 | qty54 = 4000÷54×54 = 4,000 |

**★스티커 comp 4개 모두 단일 본체 공식(합산 아님)·디지털/아크릴/실사/책자의 다-comp 합산 모델과 다름.** 재사용 패턴 = ① 이산 siz_cd 단가형(규격 드롭다운 상품 전반)·② .02 세트 합가형(÷min_qty 묶음단위). **결함은 comp 그릇이 아니라 바인딩 정합**(상품 siz/mat ↔ 단가행 키 불일치 4건·G-STK-1~4·formula-map-sticker §6).
