...
## 9. PH facet 메모 (v9.0 — 완제 액자 SKU 동형·신규 그릇 0·★directive 최대 관전)
> PH 갭(`categories/PH/reverse.md` §0.5·`02_metamodel/_resolved-fragments.md` PH-1/PH-2·`gap-matrix §XXI`·`vessel-needs.md PH 흡수 매핑 ②`). **PH distinct 신축 0(완제 액자 그릇/마운팅 #18 부결·17축 재포화·9번째 카테고리). 본 V-11/완제SKU#4에 새 그릇 수요 추가 없음(기존 그릇 흡수 확증).**
- **PH-2 완제 액자 SKU(거치+마감+사이즈 인코딩)가 완제SKU#4/V-11 이중의미 분리와 동근:** §0.5 client-render 재캡처 실측 — PHFRDIA 디아섹 액자에서 거치(탁상용/벽걸이)+마감+사이즈가 **완제 SKU combobox 1개에 인코딩**(AC 두께/소재 variant·GS 완제SKU G-1 동형). 완제 액자(주문 단위 SKU·`price_gbn=tmpl_price`)는 `t_prd_templates`(12행)+`template_selections`(14행) 완제SKU 그릇이 무손실 표현 — "인쇄물+별도 프레임 조립" 2-파트 완제 구조조차 거치+마감+사이즈 인코딩 완제SKU로 담음. **신규 테이블/컬럼 0 — 완제SKU#4가 이미 담음.**
- **★이중의미 분리 [HARD] 적용:** 완제 액자 SKU(주문 단위·`t_prd_templates`)는 본 V-11 디자인 시안 자산(`t_prd_template_assets`·가격0·에디터 리소스)과 **다른 엔티티** — TP T-A·§4 완제SKU 오염 방지 동일. 액자 디자인 입력(PDF/에디터·§0.5 PHFRDIA 편집하기 OBSERVED)은 V-10 채널·V-11 시안 자산, 완제 액자 주문 SKU는 완제SKU#4. **둘 다 그릇 실재·적재만 안 됨(data-gap·`_data-gaps-noted §10`).** 이것이 17축 재포화의 TemplateAsset-side 증거: 9번째 카테고리(PH·directive 최대 관전 완제 액자 프레임)가 디자인 자산/완제SKU 그릇에 새 수요를 더하지 않고 흡수.
- **★deepcheck carry-forward 주의 [HARD]:** codex H-1(content-container 합성역할·"빈 프레임 단독 주문·프레임 사이즈 ≠ 삽입 인쇄물 사이즈")·H-2(mat board)·H-3(aperture/창 기하) = #18 최강 적대 후보였으나 **전부 `unverified`·§0.5 미캡처(멀티/매팅/aperture 액자) 영역.** codex 결정 기준("content part vs container part 구별 *재사용 라이브 슬롯* 증거 없으면 승격 안 함")이 우리 PH-1 data-gap 판정과 수렴 → **현재 완제SKU#4/V-11이 무손실 표현·새 그릇 mint 0.** 멀티/매팅 액자에서 "프레임 사이즈 ≠ 이미지 사이즈" 별 option_group·content↔container price-component 분리가 OBSERVED되면 그때 완제SKU#4 확장 검토(라이브 실측 대상·search-before-mint HARD·AC layer-stack/PD construction REFUTED 교훈 적용).
