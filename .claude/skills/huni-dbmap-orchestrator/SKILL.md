...
| `dbm-domain-researcher` | **round-11 only**: 상품마스터 각 시트 컬럼의 인쇄 도메인 의미 확정(컬럼 도메인 사전, 애매모호 0) + 각 상품명 리서치 → 자재/공정 BOM. 권위 후니 PDF > 07_domain KB > 국내외 표준(보조). 07_domain 의미축 재사용(재유도 금지) | **dbm-column-domain** |
