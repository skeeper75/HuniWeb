...
description: >
  후니프린팅 기초데이터 표시중복 정리 하네스의 4축 검수·정리 매핑 설계 방법론. 축별 인덱스 캐시를 입력으로
  표시값↔내부값을 4축(표시↔실제 정합·표시 중복·내부값 중복·의미구분 보존)으로 검수해 진짜 중복·오적재만
  적발하고 정당한 구분(작업/재단/판형/단위/상품전용)은 보존. canonical 도출→라이브 환원→충돌 검출, 가격종속
  BLOCKED 분류, 무손실 정리(정본+논리삭제+재배선), NO-OP 인정. 트리거: 표시중복 판정, 4축 검수, 중복 정리 매핑,
  canonical, 오적재 적발, 정리 매핑 다시.
license: Apache-2.0
