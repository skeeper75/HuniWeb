...
## ═══ OT 모호 fragment 판정 (상자/패키징 — 전개도/dieline #18 적대 프로브) ═══

> `categories/OT/reverse.md` Ambiguous fragments 1~4 해소. **선별 모드 핵심 프로브 = "전개도/dieline(접지 구조)이 distinct #18 관리축인가" — 부결(흡수).** 도메인 정초 = `entity-semantic-model.md:22,27`(★size=재단치수 + plate_size=작업/전지 판형[임포지션·판걸이] *2 별개 1급 축* — 후니가 재단/작업 2치수를 이미 분리 모델링) + `process-recipe-tree.md`(도무송/오시=공정 멤버) + ST 칼선/PR 접지 reuse + 메모리 `dbmap-platesize-is-output-paper`·`dbmap-compute-in-app-db-stores-lookup`. domain-researcher 신규 호출 불요(박스 차원 후니 KB+17축 확정·dieline 템플릿 내부 스키마만 validator 실측·O-2).

## O-1. 3D 제품사이즈 "표시 텍스트"(130×105 높이80) 관리 위치 — 사이즈#13 미저장 파생(앱 계산) [facet·미저장 파생]

- **판정:** **사이즈#13의 미저장 파생값(앱 계산)** — size 프리셋(작업사이즈=전개도 평면) 선택 시 *파생 표시*되는 3D 환산치이지 *선택 옵션도 저장 차원도 아님*.
- **근거:**
  - 박스 표시 사양은 "제품사이즈(3D) / 재단사이즈 / 작업사이즈" 3종을 *텍스트 병기*하나, 사용자가 **선택**하는 옵션은 size 프리셋("케익상자(소) 495×280") 1개뿐(`[live:detail]` 5박스 일치). 3D 치수는 그 프리셋에서 *파생 표시*.
  - 작업사이즈(평면 전개도)↔제품사이즈(3D 조립 후)는 **비선형 변환**(전개도 접기·풀칠탭/뚜껑/날개 접합으로 평면 영역이 입체 외곽으로 환산). 이 변환은 *앱 계산 파생*(메모리 `dbmap-compute-in-app-db-stores-lookup` 판수=앱계산·off-grid ceiling 동형) — DB는 size 프리셋(작업/재단치수)만 룩업, 3D 환산은 앱.
  - **가설 (가) size 프리셋 부가 표시 컬럼 vs (나) 미저장 파생:** 후니 t_siz에 박스 3D 표시 컬럼 부재 추정(평면 인쇄물엔 3D 개념 없음)·박스 진열 표시용이라면 size_nm 라벨에 묶거나 앱 파생이 자연 — **(나) 미저장 파생 채택**(불확실분=갭분석가/validator 라이브 확인). 어느 쪽이든 *새 "전개도 축" 증거 아님*(파생 표시값).
- **결론:** size#13 파생(미저장 또는 size 부가 표시). 3D 입체치수가 박스를 distinct로 만들지 *않음* — 선택축이 아닌 파생값. → facet(거부).

## O-2. dieline 에디터 템플릿(`makers.../templates/{code}` 접지선·풀칠탭 좌표) — 디자인 입력 채널#16 TemplateAsset(D-11) [facet·#16·sub-type unobserved]

- **판정:** **디자인 입력 채널#16(D-11) TemplateAsset** — 박스 dieline=koi 에디터가 로드하는 *디자인 자산*(주문옵션 슬롯 아님). "구조 dieline(structural)" sub-type 필요 여부는 응답 스키마 unobserved(validator 실측).
- **근거:**
  - 박스 전개도 도안(`makers.redprinting.net/v1/templates/OTPKCAK`)은 *에디터가 로드하는 디자인 시안*(가격0·#16 TemplateAsset)이지 *주문옵션*이 아님(reverse §0). 일반 명함 템플릿과 같은 입력채널 리소스 계열.
  - **단, 구조 정보 풍부 트레이드오프:** 박스 dieline은 명함 시안보다 *구조 정보 풍부*(접지선·오시선·풀칠탭 좌표·칼선) — #16 TemplateAsset(평면 시안 좌표)으로 *충분히 담기는가*, 아니면 "구조 dieline" sub-type(접지/접합 메타)이 필요한가? `makers.../templates/{code}` 응답 스키마가 **unobserved**.
  - 어느 쪽이든 **디자인 자산 카테고리 자체는 #16으로 흡수**(별 distinct 축 아님) — sub-type이 필요해도 #16 *내부* 변별이지 새 1급 관리축 아님.
- **결론:** #16 TemplateAsset(D-11). 구조 sub-type 여부=validator가 응답 스키마 실측(O-2 gap). → facet(거부·#16).

## O-3. 재단사이즈(485×270) vs 작업사이즈(495×280) 2치수 동시 표시 — 사이즈#13(재단) + plate_size(작업판형) 2축 [facet·★data-gap 결정적 분기]

- **판정:** **사이즈#13(size=재단치수) + plate_size(작업/전지 판형) 2 별개 1급 축의 박스 프리셋 묶음.** ★결정적 분기: 전개도 #18 축 증거가 아니라 **사이즈 2치수 표현력 data-gap**(vessel-gap 아님).
- **근거:**
  - 박스 size 1개 선택에 *재단(485×270)·작업(495×280) 2치수*가 동시 표시(여백 10mm). 이는 **후니가 *이미 2 별개 1급 축*으로 분리 모델링한 두 차원**(`entity-semantic-model.md:22` size=재단치수[고객 선택 완성품]·`:27` plate_size=작업/전지 판형[임포지션·판걸이·돔보]).
  - **3 의미 분해:** ① 재단치수(완성품 외곽)=사이즈#13(size·`_sizes`) ② 작업사이즈(전개도 펼친 평면 칼틀)=plate_size(작업/전지 판형·메모리 `dbmap-platesize-is-output-paper`·t_siz 이중등록 impos_yn 동형) ③ 박스 프리셋이 두 치수를 한 라벨로 묶음=size↔plate_size *cascade/연계*.
  - **(a) 전개도 축 증거 가설 = 기각.** 재단/작업 2치수는 전개도 고유 슬롯이 아니라 *평면 인쇄물도 가진 일반 차원*(재단치수↔작업사이즈[plate]·여백·돔보). 박스 10mm 여백=그 일반 관계의 박스 인스턴스. **(b) data-gap = 채택** — 후니가 박스류를 취급하려면 size#13(재단)+plate_size(작업/전개도 평면)를 한 박스 프리셋으로 묶어 적재해야(두 기존 축의 표현력/연계 적재 문제)이지 새 "전개도 축" 부재 아님.
- **결론: data-gap(vessel-gap 아님).** ★NC 이산tier·PD-4 내재BOM·FS-1 타일링·PH-2 거치 data-gap 동일 계열(축 충분·표현력 미적재). 갭분석가: size#13↔plate_size가 박스 1프리셋에 재단/작업 2치수 묶음을 수용하는지(t_siz 이중등록 패턴)·3D 제품치수 파생(O-1) 라이브 확인. → facet(거부).

## O-4. OTPKENV "커스텀 제품" 라벨(봉투상자만) — 카테고리#7 운영 라벨 / 생산형태#15(소량 unobserved) [facet]

- **판정:** **카테고리#7 주문 운영 분류(마케팅/용도 라벨)** — 봉투상자만 "커스텀 제품" 표기하나 옵션 구조는 타 박스와 동일·생산형태#15(C 완제품)는 박스 공통이라 변별 안 됨.
- **근거:**
  - OTPKENV 옵션 모델(paper BV·sodu 단면·size 프리셋·건수·수량)이 타 박스(CAK/FLT/HMN/ARP)와 **동일 슬롯**(`[live:detail]`) — "커스텀"이 *별 옵션 구조나 차원*을 만들지 않음.
  - "커스텀 제품"=주문 운영/마케팅 라벨(PR P-8 용도=카테고리#7 태그/마케팅 라벨 동형). 생산형태#15(D-9)로 보려면 박스 간 생산형태 차이가 있어야 하나, 박스는 전부 C(완제 평면지 출고)라 *변별 안 됨* — 소량 주문 정책 차이는 unobserved.
- **결론:** 카테고리#7 운영 라벨(생산형태#15는 박스 공통이라 distinct 변별 없음·소량 정책 unobserved). → facet(거부).
