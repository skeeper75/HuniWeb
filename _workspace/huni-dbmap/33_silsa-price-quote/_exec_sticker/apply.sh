...
"${PSQL[@]}" -At -F, -c "\copy (SELECT comp_price_id,comp_cd,apply_ymd,siz_cd,mat_cd,min_qty,unit_price,note FROM t_prc_component_prices WHERE comp_cd='COMP_STK_PRINT' ORDER BY siz_cd,mat_cd,min_qty) TO '$BACKUP_DIR/pre_comp_stk_print.csv' CSV HEADER"
"${PSQL[@]}" -At -F, -c "\copy (SELECT prd_cd,frm_cd,apply_bgn_ymd FROM t_prd_product_price_formulas WHERE frm_cd='PRF_STK_FIXED' ORDER BY prd_cd) TO '$BACKUP_DIR/pre_bindings.csv' CSV HEADER"
