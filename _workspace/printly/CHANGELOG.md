...
- **★층 구분 정립**: ①온톨로지 노드 등록(§35·완료) ≠ ②프린틀리 서비스 제공 상품군(레시피/커버리지·기존 7뿐). 지니 의도=②로 전개.
- **`service_coverage.py`→`data/sourcing/wow-service-coverage-260705.md`**: 와우 24군을 family-alignment 16쌍+family-verdicts에 매핑. **프린틀리 확보 상품군=24개**(양벤더 정렬 16 · 와우단독/미정렬 8). 각 군=[상품수·와우 대표·후니 정렬·판정(GO/DOWNGRADE/NO_GO)·배정후보]. 값 지어내기0(상품군·수=등록데이터·정렬=산출물).
- **커버리지 결과**: 양벤더(후니 기본 배정) 16군=명함·스티커·사인·책자·전단·홍보물·자석·어패럴·굿즈·판촉·행택쿠폰·캘린더·봉투·부자재·홀더·폰. 와우단독/미정렬 8군=확정 와우단독 4(카페용품·선거홍보물·와우기획·시스템)+**정렬 확장 후보 4**(팬시·서식·디지털인쇄·포토액자→후니 매칭 조사 시 양벤더 승격).
- **후니 스티커 2백만원 규명(참고·지니 교정)**: 엔진 정상·데이터 문제(반칼원형 기본=A5·이전사이트 goods.asp 실측 원형32mm=333,000원·새 DB 작은 사이즈 미등록+dflt_yn 3중복). webadmin 시뮬=evaluate_price 래퍼(독립검증 아님).
- 변경: 신규 `data/_scripts/service_coverage.py`·`data/sourcing/wow-service-coverage-260705.md`·`HANDOFF.md`.

## 2026-07-05 (5세션·스펙 정규화) — 후니 대표사양 고정(v1.1·dflt_yn)

- **지니 "1번 진행"**(후니 스펙 정규화). 오라클=이전사이트 goods.asp(공급가·부가세 전) 선정(관리자 시뮬=비대표·golden_fetch 자동화 有).
- **crossbrand_query.py `huni_repr_price()` 신설**: `.first()`→**기본값(dflt_yn='Y')** 사양 + 상태 반환(ok/below_min/zero). `sourcing_table.py`가 후니·와우 **사양을 나란히 표시**(불일치=비교 주의 신호).
- **★규명(후니 엔진 정상·"비교 설정" 문제)**: 스티커 2백만원=후니 기본 **A5(148x210) 반칼원형**≠와우 60x40 사각도무송(다른 물건) / 합판도무송(진짜 짝) 0원=**최소수량 1000매 규칙**(500매 미달→합산 제외·1000매=20,000원 정상) / 현수막 견적불가=후니 기본 **5000x900mm off-grid**. 헛값 전부 사양/수량 불일치 탓·가격 버그 아님.
- **★진짜 비교가능=명함만**(후니·와우 둘 다 90x50 동일)→와우 82% 저=실 신호. 나머지(스티커·엽서·전단·현수막)=상품군별 대표상품·사이즈 큐레이션 필요.
- 다음: 상품군별 후니 대표상품 큐레이션(와우 정합)·goods.asp 오라클 대조(값 확정=지니)·표준 리드타임·쿠폰 재앵커.
- 변경: `data/_scripts/crossbrand_query.py`(huni_repr_price)·`sourcing_table.py`·`data/sourcing/cafe-sourcing-table-260705.md`·`HANDOFF.md`.
