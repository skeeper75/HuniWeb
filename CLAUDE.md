...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-13 | **round-15(생산형태 × 그릇 × 선택→견적 binding) 신설 — 보정·재게이트 GO** — 사용자 "엑셀→DB 적재 분석 후 이후작업 정리". 생산형태 3분류(완제품/반제품/기성상품·디자인=직교 editor·prd_cd 단위) × 구성요소 11 그릇 × 선택→견적 end-to-end를 전 11시트 binding(A~E)로 조망(`19_grid-binding/`). dbm-domain-researcher 팬아웃(binding+경쟁사 생산형태별 확장[완제품·반제품 흡수·GAP 0])+dbm-validator 독립검증. **3대 발견: ① 가격=두 엑셀 다 존재(인쇄상품 가격표 6시트+상품마스터 가격포함 5시트)·라이브 ❌=미적재 ② [신규 오모델] 라이브 prd_typ_cd≠생산형태(굿즈/문구=.03기성·디지털/실사/아크릴/스티커/캘린더=.04디자인·완제품.01은 3건·악세사리만 일치)→사용자 "디자인≠생산형태" 역증 ③ [HARD] 실무진 표시 필드(note/tags)=쉬운 한국어(PRD_TYPE.04→"디자인상품(에디터)")·각 binding 쉬운 라벨 열.** 독립검증 NO-GO(prd_typ_cd 오귀속·"라이브 0" 과장)→보정→재게이트 G1·G5·G6 GO(생성자≠검증자). 라이브 실측 option_items 25·comp_prices 3,481·product_prices 0·formulas 63(coverage ❌=상품군 충족률). Wave-0 P1 확정(BATCH-2 색분리/3 코팅혼합/4 레더 .08→.06 가죽). 본커밋. DB 미적재 | _workspace/huni-dbmap/19_grid-binding/(매트릭스·10 binding·경쟁사·index·게이트)·HANDOFF·메모리·CLAUDE.md | 사용자(엑셀→DB 적재 이후작업 정리) |
| 2026-06-11 | **round-14(webadmin 스키마 변경 추적) 하네스 신설 + Phase10/11 영향 진단** — webadmin(라이브 DB 스키마+적재 소스 오브 트루스)이 진화할 때 추적하는 트랙. 신규 스킬 `dbm-schema-change-tracking` + `dbm-change-tracker`에 round-14 스키마 모드 추가(신규 에이전트 0)·오케스트레이터 round-14. **3-way 정합(webadmin git 선언 / 라이브 information_schema 적용 / 우리 dbmap 산출 참조) + DDL·데이터백필 레벨 분리("컬럼 존재≠적용 완료").** 영향 진단(`18_schema-change/impact-diagnosis.md`, 베이스라인 d6026be↔HEAD bd12d03·sql 15→23·t_* 34→35): Phase10/11이 constraint_json·dep_proc_cd **삭제**·template_prices·use_dims·pricing_dims 신설·가격엔진 차원 8→10·PRICE_TYPE 코드값 신설. **라이브 DDL 갭 0(전부 반영)·단 백필 미완(proc_cd/opt_cd 0행·prc_typ 전부 단가형·template_prices 0행).** 우리 산출 round-2/6/11/12/13 전부 MAJOR stale(CRITICAL 0·적재값 무손상·문서만 stale, 가격엔진 Phase11 전면 미인지). Top3 stale=constraint_json 삭제·가격차원+단가유형·dep_proc_cd 삭제. 커밋 5f00203 + 본커밋. 추적·영향·갱신 제안 전용(webadmin/DB/산출 수정 없음) | .claude/skills/dbm-schema-change-tracking(신규)·dbm-change-tracker·huni-dbmap-orchestrator·18_schema-change·HANDOFF·메모리·CLAUDE.md | 사용자(`/harness:harness` — webadmin 코드 변경→스키마 변경이력+매핑 정확 처리) |
| 2026-06-11 | **round-13(라이브 정합 교정) 하네스 신설 + 전 11시트 GO(디지털인쇄 파일럿 + 1차 5 family[굿즈파우치·상품악세사리·실사·아크릴·문구] + 잔여 4 family[스티커·책자·포토북·캘린더+디자인캘린더])** — 라이브=교정대상 역전(round-10/12 권위 뒤집음). 신규 에이전트 `dbm-correctness-auditor`·스킬 `dbm-correctness-audit`·오케스트레이터 round-13. 정답 oracle=0.상품정체(실제 사이트 huniprinting.com)>1.엑셀 L1>2.webadmin 적재(`sql/`·`tools/load_master.py`)>3.스키마의도>4.도메인>5.라이브(피고). `models.py`=inspectdb 거울(적재로직 아님). **[HARD 사용자 directive] v03 마이그레이션(`prdmaster_full_migration_v03`)=상품마스터 미분석 오류 多 → 정답 참조 금지·load_master=순수 전파기·진원=상류 v03 정규화·정답=상품마스터 L1.** 디지털인쇄: "7상품"=실은 36상품·인쇄배경지=포장재 오분류·카테고리 고아(043~046→CAT_000296, 정상노드 273/274/275/283)·적재결함 8건·교정 18건. 확대 5 family: 굿즈파우치 정체오분류0(의심 반증·결함=속성축)·상품악세사리 이중등록=의도(sql/09 삭제제외 입증·Q-ID-A=봉투세트 sets+CPQ)·실사 size 면적매트릭스 CORRECT(의심 반증)·아크릴 print_side에 UV변형 20상품 오적재(정답 PROC_000002)·문구 미싱제본 MISSING(030/074는 제본 family 아님). 횡단=카테고리 고아·색상/형상=자재 오염(MAT_TYPE.08~10). `dbm-validator` 2-pass가 분류 정반대·카운트 과대·재연결 오매핑 적발·보정. **잔여 4 family 전건 GO로 round-13 전 11/11 시트 완료(2026-06-11):** 스티커 코팅 8상품 자재 오적재(Q9 공정 CONFLICT)·책자 PRD_000078 sub_prd 몽블랑130g(레더여야)+BK-CAT 전용노드 6개(CAT_000100~103/106/107) 상품0 고아·포토북 레더 mat_typ→.06(MAT_000186 .08 6상품 횡단오염)·캘린더 삼각대/링 자재 오적재+plate .01↔load_master .03 재적재 퇴행. **validator 2-pass가 또 실결함 적발(포토북 F-PB-1 oracle 날조[소프트 page 4~14 엑셀 공란→MISSING 오분류]·F-PB-2 재연결 오매핑·책자 정체표 라이브 불일치 digital-print 동형)·보정 후 GO.** 분류 합 67·횡단 확증(카테고리 고아·MAT_TYPE.07~10 자재 오염·v03 진원). **+횡단 결함 종합 GO(`_crosscut/` 3종·전 10 family 171 finding[결함성 113·CORRECT 58] 전수 1:1·개별 ~70 컨펌→14 결정점 BATCH-1~14 비전문가용·MAT_000186 레더 1행→6상품·4 family Top·X1~X6 게이트, 🔴 미결=BATCH-3 코팅 CONFLICT·BATCH-12 v03 상류 vs DB 직접·BATCH-6 size↔option).** 커밋 5906831·1893b3a·60e5062·7916cb1·656dcaf·b7d5cec(잔여)+본커밋(횡단종합). DB 미적재(실 교정=round-5/6/10+인간승인)·잔존 결정 14 | .claude/agents/huni-dbmap/dbm-correctness-auditor·.claude/skills/dbm-correctness-audit·huni-dbmap-orchestrator·17_correctness/{digital-print,goods-pouch,product-accessory,silsa,acrylic,stationery,sticker,booklet,photobook,calendar,_gate}·HANDOFF·메모리·CLAUDE.md | 사용자(`/harness:harness` — 라이브 적재 정확성 교정 + 전 시트 완료) |---
