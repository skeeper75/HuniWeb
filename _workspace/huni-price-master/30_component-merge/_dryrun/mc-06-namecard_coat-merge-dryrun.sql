-- ============================================================
-- MC-06  병합 DRY-RUN (ROLLBACK 전용 · COMMIT 아님 · 멱등 실증)
-- 정본 COMP_NAMECARD_COAT ← 2 멤버.  단가행 이관 4 · 영향공식 ['PRF_NAMECARD_COAT']
...
UPDATE t_prc_component_prices SET comp_cd='COMP_NAMECARD_COAT', upd_dt=now()
...
SELECT 'PRF_NAMECARD_COAT 배선 comp수' lbl, string_agg(comp_cd||':'||disp_seq, ', ' ORDER BY disp_seq) FROM t_prc_formula_components WHERE frm_cd='PRF_NAMECARD_COAT';
SELECT '멤버 잔존배선(0기대)' lbl, COUNT(*) FROM t_prc_formula_components WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
SELECT '정본 단가행수(4기대)' lbl, COUNT(*) FROM t_prc_component_prices WHERE comp_cd='COMP_NAMECARD_COAT';
SELECT '멤버 잔존 단가행(0기대)' lbl, COUNT(*) FROM t_prc_component_prices WHERE comp_cd IN ('COMP_NAMECARD_COAT_S1', 'COMP_NAMECARD_COAT_S2');
