...
**진행 상태:** round-1(구간할인, GO·미적재) DONE · round-2(가격 t_prc_*) **15시트 평면화 DONE**(베스트프랙티스 ETL·component_prices 4805행·검증 GO·후니 siz등록 대기) · round-3(매핑 audit→처리적재 설계) **DONE·DB 미적재**(master 신설·적재 별도 승인) · **CPQ 트랙: 라이브 구현 확인 DONE**(옵션/템플릿/제약 7테이블·트리거 7종·excl_groups 흡수 — 재문서화 `00_schema/cpq-schema.md`·하네스 강화).
...
| 2026-06-06 | CPQ 라이브 구현 검증·재문서화·하네스 강화 — 라이브 railway DB 직접 추출로 **CPQ 설계가 실제 구현됨 확인**(44테이블=t_* 34+Django 10). 신규 6테이블(option_groups[13행]/options/option_items[polymorphic+트리거 fn_chk_opt_item_ref 7종 OPT_REF_DIM.01~07]·templates[11행]/template_selections/constraints[logic jsonb])+categories(274행)+addons(addon_prd_cd→tmpl_cd)+products.constraint_json. **excl_groups 제거**(option_groups 흡수=제본 GRP-BOOK 10·캘린더 GRP-CAL 3, 설계 GAP-2 해소)·전도메인 del_yn/del_dt. **검증 문서↔라이브 정합 판정**: 사실인용 정확(MISMATCH0), 차이=🔴ref_param_json 미구현(공정파라미터 보존 GAP-6)·🟡templates.price 미구현·⚠️ref_dim 8→7종(addon 제외=tmpl 분리)·rule_typ→rule_typ_cd·✅color-count=opt_id 정정반영. 적재=스키마+택일그룹마이그/템플릿헤더/카테고리(옵션항목·선택·제약 0행). **하네스 강화**(harness 도구): dbm-schema-extract·orchestrator·dbm-mapping-audit 스킬 44테이블·CPQ 인지 동기화 | _workspace/huni-dbmap/00_schema/{cpq-schema.md,schema-overview.md,_live-schema-dump-260606.txt}, 10_configurator/{HANDOFF,cpq-design}, .claude/skills/{dbm-schema-extract,huni-dbmap-orchestrator,dbm-mapping-audit} | 사용자 — CPQ 검증문서 라이브 정합 보강+재문서화+하네스 강화 |
