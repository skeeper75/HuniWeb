...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-14 | **round-18+ 확장 — 가격공식 클래스 전수 + 경쟁사 벤치마크 + 정합 정립(`/harness:harness`)** — 사용자 "가격공식 동일 적용 상품군 분류·가격계산 실증 + 와우프레스·레드프린팅 동일 상품군 가격대조로 우리 계산방식 검증 + 미진 요건 정립 전문에이전트 심도 논의·전체 상품군·권위 엑셀". **수용 기준(directive): 같은 옵션인데 터무니없이 가격차 = 우리측 결함**(경쟁사=합리성 오라클·정답 아님). **경쟁사 접근(사용자 확정): 와우프레스 공개 API·레드프린팅 본인 계정 → 승인됨.** Phase 0/1: 라이브 역인덱스로 **가격공식 15클래스**(PRF_POSTER_FIXED 28·PRF_DGP_A 9·PRF_BIND_SUM 4 등·직접단가0·**184상품 가격 미구성**) 분류·클래스맵 산출. 경쟁사 가용성: 레드 캡처(책자·굿즈/파우치·캘린더 등)·명함/포스터/배너는 라이브 필요·와우프레스 가격0(공개 API 필요). 신규 에이전트 2(`dbm-competitor-benchmark` 합리성 벤치마크·B-gate / `dbm-price-arbiter` 돈-크리티컬 정합 정립 심의·권위 엑셀)·스킬 2·오케스트레이터 round-18+(클래스 분류→재계산→벤치마크→정립→PE/B/A 게이트). round-16/17/18 산출 재사용. **하네스 구성 + 클래스맵까지(클래스별 실증 실행은 다음)·비파괴.** | `.claude/agents/huni-dbmap/{dbm-competitor-benchmark,dbm-price-arbiter}.md`·`.claude/skills/{dbm-competitor-benchmark,dbm-price-arbiter}/SKILL.md`·`huni-dbmap-orchestrator/SKILL.md`·`26_price-engine-verify/_class-map.md`·CLAUDE.md §7·HANDOFF·메모리 | 사용자(`/harness:harness` 가격공식 클래스 전수 실증 + 경쟁사 벤치마크 + 정립) |
