...
**다음 시작점**:
1. ~~`contribution_sim_scan.py` 재실행해 110/020 HIGH 소멸 공식 확인~~ **완료(2026-07-02 8세션)**:
   110=HIGH 0(GO·qty100 16,357.40 골든 재현), 020=잔여 HIGH 1은 **스캐너 구조적 FP**(디지털인쇄비 기여0 —
   020은 별색 전용 상품·PROC_000004 없음·별색인쇄비 정상 발현 화이트 31,710/+클리어 48,510·이중과금 가드).
   스캐너 개선 후보: 상품 proc 목록에 매칭 proc 없는 코어 비목은 CORE_ZERO 제외(변형그룹 가드처럼).
2. 실무진 답변 수신 시: 아크릴 *_TBD 6건(단가/구성) + §23 088 질문서 5항 해소 → wiring 결함 0 완성.
   ★088 답변 진행(2026-07-02·사용자 전달): [확정] Q3 D링 두께 무관(②제본비 포함)·Q2 방향=레더 특수인쇄
   전용지 결합 가격테이블 신설(①·디지털인쇄비 동형)·Q4 면지=제본비 포함 무료(하드커버 무선 시 커버에 면지
   필요·가격은 제본비에 포함 — 기존 "면지=제본비포함 0원" 원칙과 일치).
   [대기] Q1 제작비=싸바리(7,000/개·100개)와 같은지 **실무진 확인 중** · Q2 레더 전용 테이블 **단가(수량구간별)**
   미수신 · ④ "수량구간 매칭…" 문장 끊김 · O링=D링 동일 추가금없음 확인(가벼움).
   → **Q1 확정+Q2 단가 수신 시 088 커버분해 재설계 착수**(뼈대: 표지 레더인쇄 테이블+제작비(싸바리?)+면지0).
3. 컨펌큐(report-only·conv21-design §4): 포맥스 A1 3종 미적재(§26)·포맥스 3mm 권위충돌(마스터 8,300/11,500 vs
   ...
   ★교차조합 0원 건은 **제약조건데모 등록으로 서버측 차단 수단 확보(아래 4)** — 위젯이 validate 호출하면 막힘.
4. **제약조건데모 등록(2026-07-02 8세션·사용자 지시 COMMIT)**: 129 폼보드·130 포맥스보드에
   t_prd_product_constraints (rule_cd=R_DEMO_MATSIZ·rule_nm=제약조건데모·RULE_TYPE.03 필수동반) —
   자재별 필수동반 사이즈 AND-4 implication(JSONLogic·var=mat_cd__usage_cd/siz_cd)으로 mat×siz 엇갈림(0원) 차단.
   검증: validate 엔드포인트 9/9 PASS + webadmin 제약빌더 실화면(130·129 각 엇갈림 막힘/정상 통과).
   fix/undo/backup=`constraint-demo-260702-{fix,undo}.sql`·`constraint-demo-backup-260702.txt`(사전 0행).
   ★한계: evaluate_price는 제약 미참조(엇갈림 simulate=0원 그대로) — 강제는 위젯/주문 경로가 validate 호출해야.
